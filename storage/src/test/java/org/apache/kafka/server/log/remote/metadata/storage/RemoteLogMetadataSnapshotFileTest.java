...
public class RemoteLogMetadataSnapshotFileTest {
    ...
    public void testWriteReadCommittedLogMetadataFile() throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            remoteLogSegmentMetadatas.add(new RemoteLogSegmentMetadataSnapshot(Uuid.randomUuid(), startOffset, endOffset, System.currentTimeMillis(), 1, 100, 1024, RemoteLogSegmentState.COPY_SEGMENT_FINISHED, Collections.singletonMap(i, startOffset)));
            startOffset = endOffset + 1;
        }

        RemoteLogMetadataSnapshotFile.Snapshot snapshot = new RemoteLogMetadataSnapshotFile.Snapshot(0, 120, remoteLogSegmentMetadatas);
        ...
        Assertions.assertEquals(new HashSet<>(snapshot.remoteLogSegmentMetadataSnapshots()), new HashSet<>(maybeReadSnapshot.get().remoteLogSegmentMetadataSnapshots()));
    }
}
