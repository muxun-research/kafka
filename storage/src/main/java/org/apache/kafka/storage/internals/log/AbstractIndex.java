...
public abstract class AbstractIndex implements Closeable {
        ...
        try {
            ...
            if (length == roundedNewSize) {
                ...
                try {
                    ...
                    log.debug("Resized {} to {}, position is {} and limit is {}", file.getAbsolutePath(), roundedNewSize, mmap.position(), mmap.limit());
                    return true;
                } finally {
    ...
    public int relativeOffset(long offset) {
        ...
        return relativeOffset.orElseThrow(() -> new IndexOffsetOverflowException("Integer overflow for offset: " + offset + " (" + file.getAbsoluteFile() + ")"));
    }
