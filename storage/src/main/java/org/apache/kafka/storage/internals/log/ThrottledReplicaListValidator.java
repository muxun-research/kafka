...
public class ThrottledReplicaListValidator implements Validator {
    ...
    public void ensureValid(String name, Object value) {
        if (value instanceof java.util.List<?>) {
            ...
            if (!(proposed.stream().allMatch(s -> s.matches("([0-9]+:[0-9]+)?")) || String.join("", proposed).equals("*")))
                throw new ConfigException(name, value, name + " must be the literal '*' or a list of replicas in the following format: [partitionId]:[brokerId],[partitionId]:[brokerId],...");
        } else
