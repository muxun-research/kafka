    ...
    public void append(AbortedTxn abortedTxn) throws IOException {
        lastOffset.ifPresent(offset -> {
            if (offset >= abortedTxn.lastOffset())
                throw new IllegalArgumentException("The last offset of appended transactions must increase sequentially, but " + abortedTxn.lastOffset() + " is not greater than current last offset " + offset + " of index " + file.getAbsolutePath());
        });
    ...
    public void sanityCheck() {
        ...
        for (AbortedTxnWithPosition txnWithPosition : iterable(() -> buffer)) {
            ...
            if (abortedTxn.lastOffset() < startOffset)
                throw new CorruptIndexException("Last offset of aborted transaction " + abortedTxn + " in index " + file.getAbsolutePath() + " is less than start offset " + startOffset);
        }
    }
            ...
            public AbortedTxnWithPosition next() {
                try {
                    ...
                    if (abortedTxn.version() > AbortedTxn.CURRENT_VERSION)
                        throw new KafkaException("Unexpected aborted transaction version " + abortedTxn.version() + " in transaction index " + file.getAbsolutePath() + ", current version is " + AbortedTxn.CURRENT_VERSION);
