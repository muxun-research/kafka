...
public class OffsetIndex extends AbstractIndex {
    ...
    public OffsetIndex(File file, long baseOffset, int maxIndexSize, boolean writable) throws IOException {
        ...
        log.debug("Loaded index file {} with maxEntries = {}, maxIndexSize = {}, entries = {}, lastOffset = {}, file position = {}", file.getAbsolutePath(), maxEntries(), maxIndexSize, entries(), lastOffset, mmap().position());
    }

    @Override
    public void sanityCheck() {
        if (entries() != 0 && lastOffset < baseOffset())
            throw new CorruptIndexException("Corrupt index found, index file " + file().getAbsolutePath() + " has non-zero size " + "but the last offset is " + lastOffset + " which is less than the base offset " + baseOffset());
        if (length() % entrySize() != 0)
            throw new CorruptIndexException("Index file " + file().getAbsolutePath() + " is corrupt, found " + length() + " bytes which is neither positive nor a multiple of " + ENTRY_SIZE);
    }
    ...
    public OffsetPosition entry(int n) {
        return maybeLock(lock, () -> {
            if (n >= entries())
                throw new IllegalArgumentException("Attempt to fetch the " + n + "th entry from index " + file().getAbsolutePath() + ", which has size " + entries());
    ...
    public void append(long offset, int position) {
        ...
        try {
            ...
            if (entries() == 0 || offset > lastOffset) {
                ...
                throw new InvalidOffsetException("Attempt to append an offset " + offset + " to position " + entries() + " no larger than the last offset appended (" + lastOffset + ") to " + file().getAbsolutePath());
        } finally {
            lock.unlock();
        }
    }
    ...
    private void truncateToEntries(int entries) {
        ...
        try {
            ...
            log.debug("Truncated index {} to {} entries; position is now {} and last offset is now {}", file().getAbsolutePath(), entries, mmap().position(), lastOffset);
        } finally {
            lock.unlock();
        }
    }
