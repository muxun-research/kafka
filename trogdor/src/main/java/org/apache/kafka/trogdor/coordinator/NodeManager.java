...
public final class NodeManager {
    ...
    void rescheduleNextHeartbeat(long initialDelayMs) {
        ...
        this.heartbeatFuture = this.executor.scheduleAtFixedRate(heartbeat, initialDelayMs, HEARTBEAT_DELAY_MS, TimeUnit.MILLISECONDS);
    }
        ...
        private void handlePresentWorkers(AgentStatusResponse agentStatus) {
            for (Map.Entry<Long, WorkerState> entry : agentStatus.workers().entrySet()) {
                ...
                if (worker == null) {
                    ...
                    workers.put(workerId, new ManagedWorker(workerId, state.taskId(), state.spec(), false, state));
                } else {
    ...
    class CreateWorker implements Callable<Void> {
        ...
        public Void call() throws Exception {
            ManagedWorker worker = workers.get(workerId);
            if (worker != null) {
                log.error("{}: there is already a worker {} with ID {}.", node.name(), worker, workerId);
                return null;
            }
