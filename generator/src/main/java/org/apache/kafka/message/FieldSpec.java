...
public final class FieldSpec {
    ...
    private void checkTagInvariants() {
        if (this.tag.isPresent()) {
            if (this.tag.get() < 0) {
                throw new RuntimeException("Field " + name + " specifies a tag of " + this.tag.get() + ".  Tags cannot be negative.");
            }
            if (this.taggedVersions.empty()) {
                throw new RuntimeException("Field " + name + " specifies a tag of " + this.tag.get() + ", but has no tagged versions.  If a tag is specified, taggedVersions must " + "be specified as well.");
            }
            ...
            if (!(nullableTaggedVersions.empty() || nullableTaggedVersions.equals(this.taggedVersions))) {
                throw new RuntimeException("Field " + name + " specifies nullableVersions " + this.nullableVersions + " and taggedVersions " + this.taggedVersions + ".  " + "Either all tagged versions must be nullable, or none must be.");
            }
            if (this.taggedVersions.highest() < Short.MAX_VALUE) {
                throw new RuntimeException("Field " + name + " specifies taggedVersions " + this.taggedVersions + ", which is not open-ended.  taggedVersions must " + "be either none, or an open-ended range (that ends with a plus sign).");
            }
            if (!this.taggedVersions.intersect(this.versions).equals(this.taggedVersions)) {
                throw new RuntimeException("Field " + name + " specifies taggedVersions " + this.taggedVersions + ", and versions " + this.versions + ".  " + "taggedVersions must be a subset of versions.");
            }
        } else if (!this.taggedVersions.empty()) {
            throw new RuntimeException("Field " + name + " does not specify a tag, " + "but specifies tagged versions of " + this.taggedVersions + ".  " + "Please specify a tag, or remove the taggedVersions.");
        }
    }
    ...
    String fieldDefault(HeaderGenerator headerGenerator, StructRegistry structRegistry) {
        if (type instanceof FieldType.BoolFieldType) {
            if (fieldDefault.isEmpty()) {
                ...
                throw new RuntimeException("Invalid default for boolean field " + name + ": " + fieldDefault);
            }
        } else if ((type instanceof FieldType.Int8FieldType) || (type instanceof FieldType.Int16FieldType) || (type instanceof FieldType.Uint16FieldType) || (type instanceof FieldType.Uint32FieldType) || (type instanceof FieldType.Int32FieldType) || (type instanceof FieldType.Int64FieldType)) {
            int base = 10;
            ...
            if (type instanceof FieldType.Int8FieldType) {
                if (defaultString.isEmpty()) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("Invalid default for int8 field " + name + ": " + defaultString, e);
                    }
                ...
                if (defaultString.isEmpty()) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("Invalid default for int16 field " + name + ": " + defaultString, e);
                    }
                ...
                if (defaultString.isEmpty()) {
                    ...
                    try {
                        ...
                        if (value < 0 || value > MessageGenerator.UNSIGNED_SHORT_MAX) {
                            throw new RuntimeException("Invalid default for uint16 field " + name + ": out of range.");
                        }
                    } catch (NumberFormatException e) {
                        throw new RuntimeException("Invalid default for uint16 field " + name + ": " + defaultString, e);
                    }
                    return fieldDefault;
                }
                ...
                if (defaultString.isEmpty()) {
                    ...
                    try {
                        ...
                        if (value < 0 || value > MessageGenerator.UNSIGNED_INT_MAX) {
                            throw new RuntimeException("Invalid default for uint32 field " + name + ": out of range.");
                        }
                    } catch (NumberFormatException e) {
                        throw new RuntimeException("Invalid default for uint32 field " + name + ": " + defaultString, e);
                    }
                    return fieldDefault;
                }
                ...
                if (defaultString.isEmpty()) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("Invalid default for int32 field " + name + ": " + defaultString, e);
                    }
                    return fieldDefault;
                }
                ...
                if (defaultString.isEmpty()) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("Invalid default for int64 field " + name + ": " + defaultString, e);
                    }
            ...
            if (fieldDefault.isEmpty()) {
                ...
                try {
                    ...
                    throw new RuntimeException("Invalid default for uuid field " + name + ": " + fieldDefault, e);
                }
                headerGenerator.addImport(MessageGenerator.UUID_CLASS);
            ...
            if (fieldDefault.isEmpty()) {
                ...
                try {
                    ...
                    throw new RuntimeException("Invalid default for float64 field " + name + ": " + fieldDefault, e);
                }
            ...
            if (fieldDefault.equals("null")) {
                ...
                throw new RuntimeException("Invalid default for bytes field " + name + ".  The only valid default for a bytes field " + "is empty or null.");
            }
            if (zeroCopy) {
            ...
            if (fieldDefault.equals("null")) {
                ...
                throw new RuntimeException("Invalid default for struct field " + name + ".  The only valid default for a struct field " + "is the empty struct or null.");
            }
            return "new " + type + "()";
            ...
            if (fieldDefault.equals("null")) {
                ...
                throw new RuntimeException("Invalid default for array field " + name + ".  The only valid default for an array field " + "is the empty array or null.");
            }
    ...
    private void validateNullDefault() {
        if (!(nullableVersions().contains(versions))) {
            throw new RuntimeException("null cannot be the default for field " + name + ", because not all versions of this field are " + "nullable.");
        }
    }
    ...
    void generateNonDefaultValueCheck(HeaderGenerator headerGenerator, StructRegistry structRegistry, CodeBuffer buffer, String fieldPrefix, Versions nullableVersions) {
        ...
        if (type().isArray()) {
            if (fieldDefault.equals("null")) {
                ...
                buffer.printf("if (%s%s == null || !%s%s.isEmpty()) {%n", fieldPrefix, camelCaseName(), fieldPrefix, camelCaseName());
            }
        } else if (type().isBytes()) {
            if (fieldDefault.equals("null")) {
                buffer.printf("if (%s%s != null) {%n", fieldPrefix, camelCaseName());
                ...
                if (zeroCopy()) {
                    buffer.printf("if (%s%s.hasRemaining()) {%n", fieldPrefix, camelCaseName());
                } else {
                    buffer.printf("if (%s%s.length != 0) {%n", fieldPrefix, camelCaseName());
                }
            } else {
                if (zeroCopy()) {
                    buffer.printf("if (%s%s == null || %s%s.remaining() > 0) {%n", fieldPrefix, camelCaseName(), fieldPrefix, camelCaseName());
                } else {
                    buffer.printf("if (%s%s == null || %s%s.length != 0) {%n", fieldPrefix, camelCaseName(), fieldPrefix, camelCaseName());
                }
            }
            ...
            if (fieldDefault.equals("null")) {
                ...
                buffer.printf("if (!%s%s.equals(%s)) {%n", fieldPrefix, camelCaseName(), fieldDefault);
            } else {
                buffer.printf("if (%s%s == null || !%s%s.equals(%s)) {%n", fieldPrefix, camelCaseName(), fieldPrefix, camelCaseName(), fieldDefault);
            }
        } else if (type() instanceof FieldType.BoolFieldType) {
            buffer.printf("if (%s%s%s) {%n", fieldDefault.equals("true") ? "!" : "", fieldPrefix, camelCaseName());
        } else {
            buffer.printf("if (%s%s != %s) {%n", fieldPrefix, camelCaseName(), fieldDefault);
        }
    }
    ...
    void generateNonIgnorableFieldCheck(HeaderGenerator headerGenerator, StructRegistry structRegistry, String fieldPrefix, CodeBuffer buffer) {
        generateNonDefaultValueCheck(headerGenerator, structRegistry, buffer, fieldPrefix, nullableVersions());
