...
public final class MessageDataGenerator implements MessageClassGenerator {
    ...
    void generate(MessageSpec message) throws Exception {
        ...
        generateClass(Optional.of(message), message.dataClassName(), message.struct(), message.struct().versions());
    ...
    private void generateClass(Optional<MessageSpec> topLevelMessageSpec, String className, StructSpec struct, Versions parentVersions) throws Exception {
        ...
        if (isTopLevel) {
            for (Iterator<StructSpec> iter = structRegistry.commonStructs(); iter.hasNext(); ) {
                ...
                generateClass(Optional.empty(), commonStruct.name(), commonStruct, commonStruct.versions());
            }
            buffer.decrementIndent();
            buffer.printf("}%n");
        }
        ...
        for (FieldSpec field : struct.fields()) {
            if (field.type().isStructArray()) {
                ...
                if (!structRegistry.commonStructNames().contains(arrayType.elementName())) {
                    generateClass(Optional.empty(), arrayType.elementType().toString(), structRegistry.findStruct(field), parentVersions.intersect(struct.versions()));
                }
            } else if (field.type().isStruct()) {
                if (!structRegistry.commonStructNames().contains(field.typeString())) {
                    generateClass(Optional.empty(), field.typeString(), structRegistry.findStruct(field), parentVersions.intersect(struct.versions()));
                }
            }
        }
        if (isSetElement) {
    ...
    private void generateHashSet(String className, StructSpec struct) {
        buffer.printf("%n");
        headerGenerator.addImport(MessageGenerator.IMPLICIT_LINKED_HASH_MULTI_COLLECTION_CLASS);
        buffer.printf("public static class %s extends ImplicitLinkedHashMultiCollection<%s> {%n", FieldSpec.collectionType(className), className);
        buffer.incrementIndent();
    ...
    private void generateHashSetSizeArgConstructor(String className) {
        buffer.printf("public %s(int expectedNumElements) {%n", FieldSpec.collectionType(className));
        buffer.incrementIndent();
    ...
    private void generateHashSetIteratorConstructor(String className) {
        ...
        buffer.printf("public %s(Iterator<%s> iterator) {%n", FieldSpec.collectionType(className), className);
        buffer.incrementIndent();
    ...
    private void generateHashSetFindMethod(String className, StructSpec struct) {
        ...
        buffer.printf("public %s find(%s) {%n", className, commaSeparatedHashSetFieldAndTypes(struct));
        buffer.incrementIndent();
    ...
    private void generateHashSetFindAllMethod(String className, StructSpec struct) {
        headerGenerator.addImport(MessageGenerator.LIST_CLASS);
        buffer.printf("public List<%s> findAll(%s) {%n", className, commaSeparatedHashSetFieldAndTypes(struct));
        buffer.incrementIndent();
        generateKeyElement(className, struct);
        headerGenerator.addImport(MessageGenerator.IMPLICIT_LINKED_HASH_MULTI_COLLECTION_CLASS);
    ...
    private void generateKeyElement(String className, StructSpec struct) {
        ...
        for (FieldSpec field : struct.fields()) {
            if (field.mapKey()) {
                buffer.printf("_key.set%s(%s);%n", field.capitalizedCamelCaseName(), field.camelCaseName());
            }
        }
    }

    private String commaSeparatedHashSetFieldAndTypes(StructSpec struct) {
        return struct.fields().stream().filter(f -> f.mapKey()).map(f -> String.format("%s %s", f.concreteJavaType(headerGenerator, structRegistry), f.camelCaseName())).collect(Collectors.joining(", "));
    }
    ...
    private void generateCollectionDuplicateMethod(String className) {
        ...
        buffer.incrementIndent();
        buffer.printf("%s _duplicate = new %s(size());%n", FieldSpec.collectionType(className), FieldSpec.collectionType(className));
    ...
    private void generateFieldDeclaration(FieldSpec field) {
        buffer.printf("%s %s;%n", field.fieldAbstractJavaType(headerGenerator, structRegistry), field.camelCaseName());
    }
    ...
    private void generateStructReader(FieldSpec field, Versions supportedVersions, boolean tagged) {
    ...
    private void generateCheckForUnsupportedNumTaggedFields(String conditional) {
        ...
        buffer.incrementIndent();
        headerGenerator.addImport(MessageGenerator.UNSUPPORTED_VERSION_EXCEPTION_CLASS);
        buffer.printf("throw new UnsupportedVersionException(\"Tagged fields were set, " + "but version \" + _version + \" of this message does not support them.\");%n");
        buffer.decrementIndent();
        buffer.printf("}%n");
    }
    ...
    private void generateVariableLengthWriter(Versions fieldFlexibleVersions, String name, FieldType type, Versions possibleVersions, Versions nullableVersions, boolean zeroCopy) {
        IsNullConditional.forName(name).possibleVersions(possibleVersions).nullableVersions(nullableVersions).alwaysEmitBlockScope(type.isString()).ifNull(() -> {
            VersionConditional.forVersions(nullableVersions, possibleVersions).ifMember(presentVersions -> {
                VersionConditional.forVersions(fieldFlexibleVersions, presentVersions).ifMember(___ -> {
                    buffer.printf("_writable.writeUnsignedVarint(0);%n");
                }).ifNotMember(___ -> {
                    if (type.isString()) {
            ...
            VersionConditional.forVersions(fieldFlexibleVersions, possibleVersions).ifMember(ifMemberVersions -> {
            ...
            if (type.isString()) {
                ...
                buffer.printf("for (%s %s : %s) {%n", elementType.getBoxedJavaType(headerGenerator), elementName, name);
                buffer.incrementIndent();
                if (elementType.isArray()) {
                    throw new RuntimeException("Nested arrays are not supported.  " + "Use an array of structures containing another array.");
                } else if (elementType.isBytes() || elementType.isString()) {
                    generateVariableLengthWriter(fieldFlexibleVersions, elementName, elementType, possibleVersions, Versions.NONE, false);
                } else {
    ...
    private void generateClassMessageSize(String className, StructSpec struct, Versions parentVersions) {
        ...
        VersionConditional.forVersions(parentVersions, struct.versions()).allowMembershipCheckAlwaysFalse(false).ifNotMember(__ -> {
            headerGenerator.addImport(MessageGenerator.UNSUPPORTED_VERSION_EXCEPTION_CLASS);
            buffer.printf("throw new UnsupportedVersionException(\"Can't size " + "version \" + _version + \" of %s\");%n", className);
        }).generate(buffer);
        Versions curVersions = parentVersions.intersect(struct.versions());
        for (FieldSpec field : struct.fields()) {
            VersionConditional.forVersions(field.versions(), curVersions).ifMember(presentVersions -> {
                VersionConditional.forVersions(field.taggedVersions(), presentVersions).ifMember(presentAndTaggedVersions -> {
    ...
    private void generateVariableLengthArrayElementSize(Versions flexibleVersions, String fieldName, FieldType type, Versions versions) {
        if (type instanceof FieldType.StringFieldType) {
            ...
            VersionConditional.forVersions(flexibleVersions, versions).ifNotMember(__ -> {
            ...
            VersionConditional.forVersions(flexibleVersions, versions).ifNotMember(__ -> {
                ...
                headerGenerator.addImport(MessageGenerator.BYTE_UTILS_CLASS);
                buffer.printf("_size.addBytes(" + "ByteUtils.sizeOfUnsignedVarint(%s.length + 1));%n", fieldName);
            }).generate(buffer);
        } else if (type instanceof FieldType.StructType) {
    ...
    private void generateFixedLengthFieldSize(FieldSpec field, boolean tagged) {
        if (tagged) {
            ...
            buffer.printf("_size.addBytes(%d);%n", MessageGenerator.sizeOfUnsignedVarint(field.type().fixedLength().get()));
    ...
    private void generateStringToBytes(String name) {
        ...
        buffer.incrementIndent();
        buffer.printf("throw new RuntimeException(\"'%s' field is too long to " + "be serialized\");%n", name);
        buffer.decrementIndent();
        buffer.printf("}%n");
    ...
    private void generateClassEquals(String className, StructSpec struct, boolean elementKeysAreEqual) {
        buffer.printf("@Override%n");
        buffer.printf("public boolean %s(Object obj) {%n", elementKeysAreEqual ? "elementKeysAreEqual" : "equals");
        buffer.incrementIndent();
        ...
        if (elementKeysAreEqual) {
            ...
            buffer.printf("return MessageUtil.compareRawTaggedFields(_unknownTaggedFields, " + "other._unknownTaggedFields);%n");
        }
        buffer.decrementIndent();
        buffer.printf("}%n");
    }
    ...
    private void generateFieldEquals(FieldSpec field) {
        if (field.type() instanceof FieldType.UUIDFieldType) {
            buffer.printf("if (!this.%s.equals(other.%s)) return false;%n", field.camelCaseName(), field.camelCaseName());
            ...
            if (field.zeroCopy()) {
                ...
                buffer.printf("if (!Objects.equals(this.%s, other.%s)) return false;%n", field.camelCaseName(), field.camelCaseName());
            } else {
                // Arrays#equals handles nulls.
                headerGenerator.addImport(MessageGenerator.ARRAYS_CLASS);
                buffer.printf("if (!Arrays.equals(this.%s, other.%s)) return false;%n", field.camelCaseName(), field.camelCaseName());
            }
        } else if (field.type().isRecords()) {
            headerGenerator.addImport(MessageGenerator.OBJECTS_CLASS);
            buffer.printf("if (!Objects.equals(this.%s, other.%s)) return false;%n", field.camelCaseName(), field.camelCaseName());
        } else {
            buffer.printf("if (%s != other.%s) return false;%n", field.camelCaseName(), field.camelCaseName());
        }
    }
    ...
    private void generateFieldHashCode(FieldSpec field) {
        if (field.type() instanceof FieldType.BoolFieldType) {
        ...
        } else if (field.type() instanceof FieldType.Int64FieldType || (field.type() instanceof FieldType.Uint32FieldType)) {
            buffer.printf("hashCode = 31 * hashCode + ((int) (%s >> 32) ^ (int) %s);%n", field.camelCaseName(), field.camelCaseName());
            ...
            buffer.printf("hashCode = 31 * hashCode + Double.hashCode(%s);%n", field.camelCaseName(), field.camelCaseName());
        } else if (field.type().isBytes()) {
            if (field.zeroCopy()) {
                headerGenerator.addImport(MessageGenerator.OBJECTS_CLASS);
                buffer.printf("hashCode = 31 * hashCode + Objects.hashCode(%s);%n", field.camelCaseName());
            } else {
                headerGenerator.addImport(MessageGenerator.ARRAYS_CLASS);
                buffer.printf("hashCode = 31 * hashCode + Arrays.hashCode(%s);%n", field.camelCaseName());
            }
        } else if (field.type().isRecords()) {
            headerGenerator.addImport(MessageGenerator.OBJECTS_CLASS);
            buffer.printf("hashCode = 31 * hashCode + Objects.hashCode(%s);%n", field.camelCaseName());
        } else if (field.type().isStruct() || field.type().isArray() || field.type().isString()) {
            buffer.printf("hashCode = 31 * hashCode + (%s == null ? 0 : %s.hashCode());%n", field.camelCaseName(), field.camelCaseName());
        } else {
    ...
    private void generateClassDuplicate(String className, StructSpec struct) {
        ...
        for (FieldSpec field : struct.fields()) {
            generateFieldDuplicate(new Target(field, field.camelCaseName(), field.camelCaseName(), input -> String.format("_duplicate.%s = %s", field.camelCaseName(), input)));
        }
        buffer.printf("return _duplicate;%n");
        buffer.decrementIndent();
        buffer.printf("}%n");
    ...
    private void generateFieldDuplicate(Target target) {
        ...
        if ((field.type() instanceof FieldType.BoolFieldType) || (field.type() instanceof FieldType.Int8FieldType) || (field.type() instanceof FieldType.Int16FieldType) || (field.type() instanceof FieldType.Uint16FieldType) || (field.type() instanceof FieldType.Uint32FieldType) || (field.type() instanceof FieldType.Int32FieldType) || (field.type() instanceof FieldType.Int64FieldType) || (field.type() instanceof FieldType.Float64FieldType) || (field.type() instanceof FieldType.UUIDFieldType)) {
            buffer.printf("%s;%n", target.assignmentStatement(target.sourceVariable()));
        } else {
            IsNullConditional cond = IsNullConditional.forName(target.sourceVariable()).nullableVersions(target.field().nullableVersions()).ifNull(() -> buffer.printf("%s;%n", target.assignmentStatement("null")));
            if (field.type().isBytes()) {
                if (field.zeroCopy()) {
                    cond.ifShouldNotBeNull(() -> buffer.printf("%s;%n", target.assignmentStatement(String.format("%s.duplicate()", target.sourceVariable()))));
                } else {
                    cond.ifShouldNotBeNull(() -> {
                        headerGenerator.addImport(MessageGenerator.MESSAGE_UTIL_CLASS);
                        buffer.printf("%s;%n", target.assignmentStatement(String.format("MessageUtil.duplicate(%s)", target.sourceVariable())));
                    });
                }
            } else if (field.type().isRecords()) {
                cond.ifShouldNotBeNull(() -> {
                    headerGenerator.addImport(MessageGenerator.MEMORY_RECORDS_CLASS);
                    buffer.printf("%s;%n", target.assignmentStatement(String.format("MemoryRecords.readableRecords(((MemoryRecords) %s).buffer().duplicate())", target.sourceVariable())));
                });
            } else if (field.type().isStruct()) {
                cond.ifShouldNotBeNull(() -> buffer.printf("%s;%n", target.assignmentStatement(String.format("%s.duplicate()", target.sourceVariable()))));
            } else if (field.type().isString()) {
                // Strings are immutable, so we don't need to duplicate them.
                cond.ifShouldNotBeNull(() -> buffer.printf("%s;%n", target.assignmentStatement(target.sourceVariable())));
            } else if (field.type().isArray()) {
                cond.ifShouldNotBeNull(() -> {
                    ...
                    buffer.printf("%s %s = new %s(%s.size());%n", type, newArrayName, type, target.sourceVariable());
                    FieldType.ArrayType arrayType = (FieldType.ArrayType) field.type();
                    buffer.printf("for (%s _element : %s) {%n", arrayType.elementType().getBoxedJavaType(headerGenerator), target.sourceVariable());
                    buffer.incrementIndent();
                    generateFieldDuplicate(target.arrayElementTarget(input -> String.format("%s.add(%s)", newArrayName, input)));
                    buffer.decrementIndent();
                    buffer.printf("}%n");
                    buffer.printf("%s;%n", target.assignmentStatement(String.format("new%s", field.capitalizedCamelCaseName())));
                });
            } else {
    ...
    private void generateFieldToString(String prefix, FieldSpec field) {
        if (field.type() instanceof FieldType.BoolFieldType) {
            ...
            buffer.printf("+ \"%s%s=\" + %s%n", prefix, field.camelCaseName(), field.camelCaseName());
        } else if (field.type().isString()) {
            buffer.printf("+ \"%s%s=\" + ((%s == null) ? \"null\" : \"'\" + %s.toString() + \"'\")%n", prefix, field.camelCaseName(), field.camelCaseName(), field.camelCaseName());
        } else if (field.type().isBytes()) {
            if (field.zeroCopy()) {
                buffer.printf("+ \"%s%s=\" + %s%n", prefix, field.camelCaseName(), field.camelCaseName());
            } else {
                headerGenerator.addImport(MessageGenerator.ARRAYS_CLASS);
                buffer.printf("+ \"%s%s=\" + Arrays.toString(%s)%n", prefix, field.camelCaseName(), field.camelCaseName());
            }
        } else if (field.type().isRecords()) {
            buffer.printf("+ \"%s%s=\" + %s%n", prefix, field.camelCaseName(), field.camelCaseName());
        } else if (field.type() instanceof FieldType.UUIDFieldType) {
            ...
            headerGenerator.addImport(MessageGenerator.MESSAGE_UTIL_CLASS);
            if (field.nullableVersions().empty()) {
                buffer.printf("+ \"%s%s=\" + MessageUtil.deepToString(%s.iterator())%n", prefix, field.camelCaseName(), field.camelCaseName());
            } else {
                buffer.printf("+ \"%s%s=\" + ((%s == null) ? \"null\" : " + "MessageUtil.deepToString(%s.iterator()))%n", prefix, field.camelCaseName(), field.camelCaseName(), field.camelCaseName());
            }
        } else {
            throw new RuntimeException("Unsupported field type " + field.type());
        }
    ...
    private void generateFieldAccessor(FieldSpec field) {
        buffer.printf("%n");
        generateAccessor(field.fieldAbstractJavaType(headerGenerator, structRegistry), field.camelCaseName(), field.camelCaseName());
    }
    ...
    private void generateFieldMutator(String className, FieldSpec field) {
        buffer.printf("%n");
        buffer.printf("public %s set%s(%s v) {%n", className, field.capitalizedCamelCaseName(), field.fieldAbstractJavaType(headerGenerator, structRegistry));
        buffer.incrementIndent();
        if (field.type() instanceof FieldType.Uint16FieldType) {
            buffer.printf("if (v < 0 || v > %d) {%n", MessageGenerator.UNSIGNED_SHORT_MAX);
            buffer.incrementIndent();
            buffer.printf("throw new RuntimeException(\"Invalid value \" + v + " + "\" for unsigned short field.\");%n");
            buffer.decrementIndent();
            buffer.printf("}%n");
        }
        if (field.type() instanceof FieldType.Uint32FieldType) {
            ...
            buffer.incrementIndent();
            buffer.printf("throw new RuntimeException(\"Invalid value \" + v + " + "\" for unsigned int field.\");%n");
            buffer.decrementIndent();
            buffer.printf("}%n");
        }
    ...
    private Versions fieldFlexibleVersions(FieldSpec field) {
        if (field.flexibleVersions().isPresent()) {
            if (!messageFlexibleVersions.intersect(field.flexibleVersions().get()).equals(field.flexibleVersions().get())) {
                throw new RuntimeException("The flexible versions for field " + field.name() + " are " + field.flexibleVersions().get() + ", which are not a subset of the flexible versions for the " + "message as a whole, which are " + messageFlexibleVersions);
            }
