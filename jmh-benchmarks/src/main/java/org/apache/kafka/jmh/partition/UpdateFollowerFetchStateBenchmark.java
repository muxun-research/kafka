...
public class UpdateFollowerFetchStateBenchmark {
    ...
    public void setUp() {
        ...
        replicas.add(1);
        replicas.add(2);
        LeaderAndIsrPartitionState partitionState = new LeaderAndIsrPartitionState().setControllerEpoch(0).setLeader(0).setLeaderEpoch(0).setIsr(replicas).setPartitionEpoch(1).setReplicas(replicas).setIsNew(true);
        ...
        partition = new Partition(topicPartition, 100, MetadataVersion.latest(), 0, () -> -1, Time.SYSTEM, alterPartitionListener, delayedOperations, Mockito.mock(MetadataCache.class), logManager, alterPartitionManager);
    ...
    public void updateFollowerFetchStateBench() {
        ...
        partition.updateFollowerFetchState(replica1, new LogOffsetMetadata(nextOffset, nextOffset, 0), 0, 1, nextOffset, -1);
        partition.updateFollowerFetchState(replica2, new LogOffsetMetadata(nextOffset, nextOffset, 0), 0, 1, nextOffset, -1);
        nextOffset++;
    }
    ...
    public void updateFollowerFetchStateBenchNoChange() {
        ...
        partition.updateFollowerFetchState(replica1, new LogOffsetMetadata(nextOffset, nextOffset, 0), 0, 1, 100, -1);
        partition.updateFollowerFetchState(replica2, new LogOffsetMetadata(nextOffset, nextOffset, 0), 0, 1, 100, -1);
    }
}
