...
public class KafkaClusterTestKit implements AutoCloseable {
    ...
    private static class ControllerQuorumVotersFutureManager implements AutoCloseable {
        ...
        synchronized void registerPort(int nodeId, int port) {
            ...
            if (controllerPorts.size() >= expectedControllers) {
                future.complete(controllerPorts.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> new RaftConfig.InetAddressSpec(new InetSocketAddress("localhost", entry.getValue())))));
            }
        }
    ...
    public static class Builder {
        ...
        private KafkaConfig createNodeConfig(TestKitNode node) {
            ...
            props.put(KafkaConfig$.MODULE$.ServerMaxStartupTimeMsProp(), Long.toString(TimeUnit.MINUTES.toMillis(10)));
            ...
            if (brokerNode != null) {
                props.put(KafkaConfig$.MODULE$.LogDirsProp(), String.join(",", brokerNode.logDataDirectories()));
            }
            ...
            props.put(KafkaConfig$.MODULE$.InterBrokerListenerNameProp(), nodes.interBrokerListenerName().value());
            ...
            String uninitializedQuorumVotersString = nodes.controllerNodes().keySet().stream().map(n -> String.format("%d@0.0.0.0:0", n)).collect(Collectors.joining(","));
        ...
        public KafkaClusterTestKit build() throws Exception {
            ...
            ControllerQuorumVotersFutureManager connectFutureManager = new ControllerQuorumVotersFutureManager(nodes.controllerNodes().size());
            File baseDirectory = null;

            try {
                baseDirectory = TestUtils.tempDirectory();
                nodes = nodes.copyWithAbsolutePaths(baseDirectory.getAbsolutePath());
                executorService = Executors.newFixedThreadPool(numOfExecutorThreads, ThreadUtils.createThreadFactory("kafka-cluster-test-kit-executor-%d", false));
                for (ControllerNode node : nodes.controllerNodes().values()) {
                    setupNodeDirectories(baseDirectory, node.metadataDirectory(), Collections.emptyList());
                    BootstrapMetadata bootstrapMetadata = BootstrapMetadata.fromVersion(nodes.bootstrapMetadataVersion(), "testkit");
                    SharedServer sharedServer = new SharedServer(createNodeConfig(node), MetaProperties.apply(nodes.clusterId().toString(), node.id()), Time.SYSTEM, new Metrics(), connectFutureManager.future, faultHandlerFactory);
                ...
                for (BrokerNode node : nodes.brokerNodes().values()) {
                    SharedServer sharedServer = jointServers.computeIfAbsent(node.id(), id -> new SharedServer(createNodeConfig(node), MetaProperties.apply(nodes.clusterId().toString(), id), Time.SYSTEM, new Metrics(), connectFutureManager.future, faultHandlerFactory));
                    BrokerServer broker = null;
                    try {
                        broker = new BrokerServer(sharedServer, JavaConverters.asScalaBuffer(Collections.<String>emptyList()).toSeq());
                    } catch (Throwable e) {
            ...
            return new KafkaClusterTestKit(executorService, nodes, controllers, brokers, connectFutureManager, baseDirectory, faultHandlerFactory);
        }
        ...
        static private void setupNodeDirectories(File baseDirectory, String metadataDirectory, Collection<String> logDataDirectories) throws Exception {
        ...
        futureConsumer.accept(executorService.submit(() -> {
            try (ByteArrayOutputStream stream = new ByteArrayOutputStream()) {
                try (PrintStream out = new PrintStream(stream)) {
                    StorageTool.formatCommand(out, JavaConverters.asScalaBuffer(Collections.singletonList(metadataLogDir)).toSeq(), properties, MetadataVersion.MINIMUM_BOOTSTRAP_VERSION, false);
                } finally {
    ...
    public void waitForReadyBrokers() throws ExecutionException, InterruptedException {
        ...
        TestUtils.waitForCondition(() -> brokers().values().stream().allMatch(brokerServer -> brokerServer.metadataCache().getAliveBrokers().size() == brokers.size()), "Failed to wait for publisher to publish the metadata update to each broker.");
    }

    public Properties controllerClientProperties() throws ExecutionException, InterruptedException {
        Properties properties = new Properties();
        if (!controllers.isEmpty()) {
            Collection<Node> controllerNodes = RaftConfig.voterConnectionsToNodes(controllerQuorumVotersFutureManager.future.get());
            ...
            properties.setProperty(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, controllerNodes.stream().map(n -> n.host() + ":" + n.port()).collect(Collectors.joining(",")));
        }
        return properties;
    }
