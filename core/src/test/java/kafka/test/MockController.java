...
public class MockController implements Controller {
    private final static NotControllerException NOT_CONTROLLER_EXCEPTION = new NotControllerException("This is not the correct controller for this cluster.");
    ...
    @Override
    synchronized public CompletableFuture<CreateTopicsResponseData> createTopics(ControllerRequestContext context, CreateTopicsRequestData request, Set<String> describable) {
        ...
        for (CreatableTopic topic : request.topics()) {
            if (topicNameToId.containsKey(topic.name())) {
                ...
                CreatableTopicResult creatableTopicResult = new CreatableTopicResult().setName(topic.name()).setErrorCode(Errors.NONE.code());
                try {
                    ...
                    if (describable.contains(topic.name())) {
                        ...
                        if (topic.assignments() != null && !topic.assignments().isEmpty()) {
                            creatableTopicResult.setTopicConfigErrorCode(Errors.NONE.code()).setReplicationFactor((short) topic.assignments().iterator().next().brokerIds().size()).setNumPartitions(topic.assignments().size());
                        } else {
    ...
    @Override
    synchronized public CompletableFuture<Map<String, ResultOrError<Uuid>>> findTopicIds(ControllerRequestContext context, Collection<String> topicNames) {
    ...
    @Override
    synchronized public CompletableFuture<Map<String, Uuid>> findAllTopicIds(ControllerRequestContext context) {
    ...
    @Override
    synchronized public CompletableFuture<Map<Uuid, ResultOrError<String>>> findTopicNames(ControllerRequestContext context, Collection<Uuid> topicIds) {
    ...
    @Override
    synchronized public CompletableFuture<Map<Uuid, ApiError>> deleteTopics(ControllerRequestContext context, Collection<Uuid> topicIds) {
        if (!active) {
    ...
    @Override
    public CompletableFuture<Map<ConfigResource, ApiError>> incrementalAlterConfigs(ControllerRequestContext context, Map<ConfigResource, Map<String, Entry<AlterConfigOp.OpType, String>>> configChanges, boolean validateOnly) {
    ...
    private ApiError incrementalAlterResource(ConfigResource resource, Map<String, Entry<AlterConfigOp.OpType, String>> ops, boolean validateOnly) {
        for (Entry<String, Entry<AlterConfigOp.OpType, String>> entry : ops.entrySet()) {
            ...
            if (opType != SET && opType != DELETE) {
                return new ApiError(INVALID_REQUEST, "This mock does not " + "support the " + opType + " config operation.");
            }
        }
        if (!validateOnly) {
            for (Entry<String, Entry<AlterConfigOp.OpType, String>> entry : ops.entrySet()) {
    ...
    @Override
    synchronized public CompletableFuture<List<CreatePartitionsTopicResult>> createPartitions(ControllerRequestContext context, List<CreatePartitionsTopic> topicList, boolean validateOnly) {
        if (!active) {
