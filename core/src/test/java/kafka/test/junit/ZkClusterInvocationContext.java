...
public class ZkClusterInvocationContext implements TestTemplateInvocationContext {
    ...
    public List<Extension> getAdditionalExtensions() {
        ...
        return Arrays.asList((BeforeTestExecutionCallback) context -> {
            ...
            IntegrationTestHarness cluster = new IntegrationTestHarness() {
                ...
                public ListenerName listenerName() {
                    return clusterConfig.listenerName().map(ListenerName::normalised).orElseGet(() -> ListenerName.forSecurityProtocol(securityProtocol()));
                }
        ...
        }, (AfterTestExecutionCallback) context -> clusterShim.stop(), new ClusterInstanceParameterResolver(clusterShim), new GenericParameterResolver<>(clusterConfig, ClusterConfig.class));
    }
    ...
    public static class ZkClusterInstance implements ClusterInstance {
        ...
        public Collection<SocketServer> brokerSocketServers() {
            return servers().map(KafkaServer::socketServer).collect(Collectors.toList());
        }
        ...
        public Collection<SocketServer> controllerSocketServers() {
            return servers().filter(broker -> broker.kafkaController().isActive()).map(KafkaServer::socketServer).collect(Collectors.toList());
        }

        @Override
        public SocketServer anyBrokerSocketServer() {
            return servers().map(KafkaServer::socketServer).findFirst().orElseThrow(() -> new RuntimeException("No broker SocketServers found"));
        }

        @Override
        public SocketServer anyControllerSocketServer() {
            return servers().filter(broker -> broker.kafkaController().isActive()).map(KafkaServer::socketServer).findFirst().orElseThrow(() -> new RuntimeException("No controller SocketServers found"));
        }

        @Override
        public Map<Integer, BrokerFeatures> brokerFeatures() {
            return servers().collect(Collectors.toMap(brokerServer -> brokerServer.config().nodeId(), KafkaServer::brokerFeatures));
        }

        @Override
        public String clusterId() {
            return servers().findFirst().map(KafkaServer::clusterId).orElseThrow(() -> new RuntimeException("No broker instances found"));
        }
        ...
        public Set<Integer> brokerIds() {
            return servers().map(brokerServer -> brokerServer.config().nodeId()).collect(Collectors.toSet());
        }
        ...
        private KafkaServer findBrokerOrThrow(int brokerId) {
            return servers().filter(server -> server.config().brokerId() == brokerId).findFirst().orElseThrow(() -> new IllegalArgumentException("Unknown brokerId " + brokerId));
        }
