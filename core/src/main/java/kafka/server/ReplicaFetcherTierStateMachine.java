...
public class ReplicaFetcherTierStateMachine implements TierStateMachine {
     ...
     * tier state machine
     */
    public PartitionFetchState start(TopicPartition topicPartition, PartitionFetchState currentFetchState, PartitionData fetchPartitionData) throws Exception {
        ...
        return PartitionFetchState.apply(currentFetchState.topicId(), offsetToFetch, Option.apply(initialLag), currentFetchState.currentLeaderEpoch(), Fetching$.MODULE$, replicaMgr.localLogOrException(topicPartition).latestEpoch());
    }
    ...
    private EpochEndOffset fetchEarlierEpochEndOffset(Integer epoch, TopicPartition partition, Integer currentLeaderEpoch) {
    ...
    private Long buildRemoteLogAuxState(TopicPartition topicPartition, Integer currentLeaderEpoch, Long leaderLocalLogStartOffset, Integer epochForLeaderLocalLogStartOffset, Long leaderLogStartOffset) throws IOException, RemoteStorageException {
        ...
        if (unifiedLog.remoteStorageSystemEnable() && unifiedLog.config().remoteLogConfig.remoteStorageEnable) {
            ...
            if (maybeRlsm.isPresent()) {
                ...
                log.debug("Built the leader epoch cache and producer snapshots from remote tier for {}, " + "with active producers size: {}, leaderLogStartOffset: {}, and logEndOffset: {}", partition, unifiedLog.producerStateManager().activeProducers().size(), leaderLogStartOffset, nextOffset);
            } else {
