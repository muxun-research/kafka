...
public class ControllerRequestContextUtil {
    ...
    public static ControllerRequestContext anonymousContextFor(ApiKeys apiKeys, short version, Consumer<Integer> partitionChangeQuotaApplier) {
        return new ControllerRequestContext(new RequestHeaderData().setRequestApiKey(apiKeys.id).setRequestApiVersion(version), KafkaPrincipal.ANONYMOUS, OptionalLong.empty(), partitionChangeQuotaApplier);
    }
}
