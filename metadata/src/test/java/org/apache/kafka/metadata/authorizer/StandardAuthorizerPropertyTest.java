...
public class StandardAuthorizerPropertyTest {
    ...
    @Property(tries = 5000)
    public void matchingPrefixDenyOverridesAllAllowRules(@ForAll Random random, @ForAll @ValidTopicChars String topic, @ForAll @Size(max = 10) Set<@ValidTopicChars String> randomSuffixes) throws Exception {
        ...
        assertAuthorizationResult(authorizer, AuthorizationResult.DENIED, AclOperation.WRITE, new ResourcePattern(ResourceType.TOPIC, topic, PatternType.LITERAL));
    }

    @Property(tries = 5000)
    public void matchingLiteralDenyOverridesAllAllowRules(@ForAll @ValidTopicChars String topic, @ForAll @Size(max = 10) Set<@ValidTopicChars String> randomSuffixes) throws Exception {
        Assume.that(Topic.isValid(topic));
        StandardAuthorizer authorizer = buildAuthorizer();
    ...
    @Property(tries = 5000)
    public void matchingPrefixAllowWithNoMatchingDenyRules(@ForAll Random random, @ForAll @ValidTopicChars String topic, @ForAll @Size(max = 10) Set<@ValidTopicChars String> randomSuffixes) throws Exception {
        Assume.that(Topic.isValid(topic));
        StandardAuthorizer authorizer = buildAuthorizer();
        ...
        assertAuthorizationResult(authorizer, AuthorizationResult.ALLOWED, AclOperation.WRITE, new ResourcePattern(ResourceType.TOPIC, topic, PatternType.LITERAL));
    }

    @Property(tries = 5000)
    public void matchingLiteralAllowWithNoMatchingDenyRules(@ForAll @ValidTopicChars String topic, @ForAll @Size(max = 10) Set<@ValidTopicChars String> randomSuffixes) throws Exception {
        Assume.that(Topic.isValid(topic));
        StandardAuthorizer authorizer = buildAuthorizer();
    ...
    private void assertAuthorizationResult(StandardAuthorizer authorizer, AuthorizationResult expectedResult, AclOperation operation, ResourcePattern pattern) throws Exception {
        ...
        List<AuthorizationResult> results = authorizer.authorize(newRequestContext(), Collections.singletonList(action));
    ...
    private void printCounterExample(StandardAuthorizer authorizer, AclOperation operation, ResourcePattern resourcePattern, AuthorizationResult result) {
        ...
        Iterable<AclBinding> allAcls = authorizer.acls(new AclBindingFilter(new ResourcePatternFilter(ResourceType.ANY, null, PatternType.ANY), new AccessControlEntryFilter(null, null, AclOperation.ANY, AclPermissionType.ANY)));

        allAcls.forEach(System.out::println);
    }

    private static AuthorizableRequestContext newRequestContext() throws Exception {
        return new MockAuthorizableRequestContext.Builder().setPrincipal(new KafkaPrincipal(USER_TYPE, "user")).build();
    }

    private static StandardAcl buildTopicWriteAcl(String resourceName, PatternType patternType, AclPermissionType permissionType) {
        return new StandardAcl(ResourceType.TOPIC, resourceName, patternType, "User:*", "*", AclOperation.WRITE, permissionType);
    }
    ...
    private void addRandomNonMatchingPrefixDenyAcls(StandardAuthorizer authorizer, String topic, Set<String> randomSuffixes) {
        addRandomPrefixRules(authorizer, topic, randomSuffixes, AclPermissionType.DENY, pattern -> !pattern.isEmpty() && !isPrefix(topic, pattern));
    }

    private void addRandomPrefixAllowAcls(StandardAuthorizer authorizer, String topic, Set<String> randomSuffixes) {
        addRandomPrefixRules(authorizer, topic, randomSuffixes, AclPermissionType.ALLOW, pattern -> !pattern.isEmpty());
    }
    ...
    private void addRandomPrefixRules(StandardAuthorizer authorizer, String topic, Set<String> randomSuffixes, AclPermissionType permissionType, Predicate<String> patternFilter) {
        ...
        for (String randomResourcePattern : prefixPatterns) {
            authorizer.addAcl(Uuid.randomUuid(), buildTopicWriteAcl(randomResourcePattern, PatternType.PREFIXED, permissionType));
        }
    }
