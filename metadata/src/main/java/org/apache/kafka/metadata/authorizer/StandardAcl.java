...
final public class StandardAcl implements Comparable<StandardAcl> {
    public static StandardAcl fromRecord(AccessControlEntryRecord record) {
        return new StandardAcl(ResourceType.fromCode(record.resourceType()), record.resourceName(), PatternType.fromCode(record.patternType()), record.principal(), record.host(), AclOperation.fromCode(record.operation()), AclPermissionType.fromCode(record.permissionType()));
    }
    ...
    public KafkaPrincipal kafkaPrincipal() {
        ...
        if (colonIndex == -1) {
            throw new IllegalStateException("Could not parse principal from `" + principal + "` " + "(no colon is present separating the principal type from the principal name)");
        }
    ...
    public boolean equals(Object o) {
        ...
        return resourceType.equals(other.resourceType) && resourceName.equals(other.resourceName) && patternType.equals(other.patternType) && principal.equals(other.principal) && host.equals(other.host) && operation.equals(other.operation) && permissionType.equals(other.permissionType);
    }

    @Override
    public int hashCode() {
        return Objects.hash(resourceType, resourceName, patternType, principal, host, operation, permissionType);
    }
    ...
    public String toString() {
        return "StandardAcl(" + "resourceType=" + resourceType + ", resourceName=" + resourceName + ", patternType=" + patternType + ", principal=" + principal + ", host=" + host + ", operation=" + operation + ", permissionType=" + permissionType + ")";
    }
}
