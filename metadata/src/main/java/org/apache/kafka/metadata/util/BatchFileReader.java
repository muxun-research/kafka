...
public final class BatchFileReader implements Iterator<BatchFileReader.BatchAndType>, AutoCloseable {
        ...
        for (Iterator<Record> iter = input.iterator(); iter.hasNext(); ) {
            ...
            try {
                ...
                switch (type) {
                    ...
                    default:
                        throw new RuntimeException("Unsupported control record type " + type + " at offset " + record.offset());
                }
            } catch (Throwable e) {
                throw new RuntimeException("Unable to read control record at offset " + record.offset(), e);
            }
        }
        return new BatchAndType(Batch.data(input.baseOffset(), input.partitionLeaderEpoch(), input.maxTimestamp(), input.sizeInBytes(), messages), true);
    }
    ...
    private BatchAndType nextDataBatch(FileChannelRecordBatch input) {
        ...
        return new BatchAndType(Batch.data(input.baseOffset(), input.partitionLeaderEpoch(), input.maxTimestamp(), input.sizeInBytes(), messages), false);
    }
