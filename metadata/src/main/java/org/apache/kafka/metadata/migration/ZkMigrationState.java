...
public enum ZkMigrationState {
    ...
    public ApiMessageAndVersion toRecord() {
        return new ApiMessageAndVersion(new ZkMigrationStateRecord().setZkMigrationState(value()), (short) 0);
    }

    public static ZkMigrationState of(byte value) {
        return optionalOf(value).orElseThrow(() -> new IllegalArgumentException(String.format("Value %s is not a valid Zk migration state", value)));
    }
