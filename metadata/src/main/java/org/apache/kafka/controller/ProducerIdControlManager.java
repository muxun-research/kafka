...
public class ProducerIdControlManager {
    static class Builder {
        ...
        ProducerIdControlManager build() {
            ...
            return new ProducerIdControlManager(logContext, clusterControlManager, snapshotRegistry);
        }
    }
    ...
    void replay(ProducerIdsRecord record) {
        ...
        if (nextBlock != ProducerIdsBlock.EMPTY && record.nextProducerId() <= nextBlock.firstProducerId()) {
            throw new RuntimeException("Next Producer ID from replayed record (" + record.nextProducerId() + ")" + " is not greater than current next Producer ID in block (" + nextBlock + ")");
        } else {
