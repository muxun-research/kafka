...
public class NetworkClientDelegate implements AutoCloseable {
    ...
    public NetworkClientDelegate(final Time time, final ConsumerConfig config, final LogContext logContext, final KafkaClient client) {
        this.time = time;
        this.client = client;
    ...
    private boolean doSend(final UnsentRequest r, final long currentTimeMs) {
    ...
    private ClientRequest makeClientRequest(final UnsentRequest unsent, final Node node, final long currentTimeMs) {
        return client.newClientRequest(node.idString(), unsent.requestBuilder, currentTimeMs, true, (int) unsent.timer.remainingMs(), unsent.handler);
    }
    ...
    public static class UnsentRequest {
        ...
        public UnsentRequest(final AbstractRequest.Builder<?> requestBuilder, final Optional<Node> node, final FutureCompletionHandler handler) {
