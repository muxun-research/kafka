...
class RequestState {
    ...
    public RequestState(long retryBackoffMs) {
        this.exponentialBackoff = new ExponentialBackoff(retryBackoffMs, RETRY_BACKOFF_EXP_BASE, retryBackoffMs, RETRY_BACKOFF_JITTER);
    }

    // Visible for testing
    RequestState(final long retryBackoffMs, final int retryBackoffExpBase, final long retryBackoffMaxMs, final double jitter) {
