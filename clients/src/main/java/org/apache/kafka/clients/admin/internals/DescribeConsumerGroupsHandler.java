...
public class DescribeConsumerGroupsHandler extends AdminApiHandler.Batched<CoordinatorKey, ConsumerGroupDescription> {
    ...
    public DescribeGroupsRequest.Builder buildBatchedRequest(int coordinatorId, Set<CoordinatorKey> keys) {
        ...
        DescribeGroupsRequestData data = new DescribeGroupsRequestData().setGroups(groupIds).setIncludeAuthorizedOperations(includeAuthorizedOperations);
    ...
    private void handleError(CoordinatorKey groupId, Errors error, Map<CoordinatorKey, Throwable> failed, Set<CoordinatorKey> groupsToUnmap) {
        switch (error) {
            ...
            case COORDINATOR_LOAD_IN_PROGRESS:
                ...
                log.debug("`DescribeGroups` request for group id {} failed because the coordinator " + "is still in the process of loading state. Will retry", groupId.idValue);
                break;
            ...
            case NOT_COORDINATOR:
                ...
                log.debug("`DescribeGroups` request for group id {} returned error {}. " + "Will attempt to find the coordinator again and retry", groupId.idValue, error);
