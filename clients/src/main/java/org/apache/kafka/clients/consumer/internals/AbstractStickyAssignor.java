...
public abstract class AbstractStickyAssignor extends AbstractPartitionAssignor {
     ...
     */
    private boolean allSubscriptionsEqual(Set<String> allTopics, Map<String, Subscription> subscriptions, Map<String, List<TopicPartition>> consumerToOwnedPartitions, Set<TopicPartition> partitionsWithMultiplePreviousOwners) {
    ...
    private class RackInfo {
        ...
        public RackInfo(List<PartitionInfo> partitionInfos, Map<String, Subscription> subscriptions) {
            ...
            subscriptions.forEach((memberId, subscription) -> subscription.rackId().filter(r -> !r.isEmpty()).ifPresent(rackId -> put(consumersByRack, rackId, memberId)));
            ...
            numConsumersByPartition = partitionRacks.entrySet().stream().collect(Collectors.toMap(Entry::getKey, e -> e.getValue().stream().map(r -> consumersByRack.getOrDefault(r, Collections.emptyList()).size()).reduce(0, Integer::sum)));
        }
        ...
        @Override
        public String toString() {
            return "RackInfo(" + "consumerRacks=" + consumerRacks + ", partitionRacks=" + partitionRacks + ")";
        }
    }
    ...
    private class ConstrainedAssignmentBuilder extends AbstractAssignmentBuilder {
        ...
        ConstrainedAssignmentBuilder(Map<String, List<PartitionInfo>> partitionsPerTopic, RackInfo rackInfo, Map<String, List<TopicPartition>> consumerToOwnedPartitions, Set<TopicPartition> partitionsWithMultiplePreviousOwners) {
            ...
            assignment = new HashMap<>(consumerToOwnedPartitions.keySet().stream().collect(Collectors.toMap(c -> c, c -> new ArrayList<>(maxQuota))));
        ...
        Map<String, List<TopicPartition>> build() {
            if (log.isDebugEnabled()) {
                log.debug("Performing constrained assign with partitionsPerTopic: {}, currentAssignment: {}, rackInfo {}.", partitionsPerTopic, currentAssignment, rackInfo);
            }
        ...
        private void assignOwnedPartitions() {
            ...
            for (Map.Entry<String, List<TopicPartition>> consumerEntry : currentAssignment.entrySet()) {
                ...
                List<TopicPartition> ownedPartitions = consumerEntry.getValue().stream().filter(tp -> !rackInfo.racksMismatch(consumer, tp)).collect(Collectors.toList());
                ...
                for (TopicPartition doublyClaimedPartition : partitionsWithMultiplePreviousOwners) {
                    if (ownedPartitions.contains(doublyClaimedPartition)) {
                        log.error("Found partition {} still claimed as owned by consumer {}, despite being claimed by multiple " + "consumers already in the same generation. Removing it from the ownedPartitions", doublyClaimedPartition, consumer);
            ...
            for (TopicPartition unassignedPartition : unassignedPartitions) {
                ...
                if (unfilledConsumerIter.hasNext()) {
                    ...
                    if (unfilledMembersWithUnderMinQuotaPartitions.isEmpty() && unfilledMembersWithExactlyMinQuotaPartitions.isEmpty()) {
                        ...
                        log.error("No more unfilled consumers to be assigned. The remaining unassigned partitions are: {}", unassignedPartitions.subList(currentPartitionIndex, unassignedPartitions.size()));
                ...
                if (currentNumMembersWithOverMinQuotaPartitions != expectedNumMembersWithOverMinQuotaPartitions) {
                    ...
                    for (String unfilledMember : unfilledMembersWithUnderMinQuotaPartitions) {
                        ...
                        if (assignedPartitionsCount != minQuota) {
                            ...
                            log.trace("skip over this unfilled member: [{}] because we've reached the expected number of " + "members with more than the minQuota partitions, and this member already has minQuota partitions", unfilledMember);
                        }
                    }
                }
            }
    ...
    private class GeneralAssignmentBuilder extends AbstractAssignmentBuilder {
         ...
         */
        GeneralAssignmentBuilder(Map<String, List<PartitionInfo>> partitionsPerTopic, RackInfo rackInfo, Map<String, List<TopicPartition>> currentAssignment, Map<String, Subscription> subscriptions) {
            ...
            partitionsPerTopic.keySet().forEach(topicName -> topic2AllPotentialConsumers.put(topicName, new ArrayList<>()));
        ...
        Map<String, List<TopicPartition>> build() {
            if (log.isDebugEnabled()) {
                log.debug("performing general assign. partitionsPerTopic: {}, subscriptions: {}, currentAssignment: {}, rackInfo: {}", partitionsPerTopic, subscriptions, currentAssignment, rackInfo);
            }
