...
public class SenderTest {
        ...
        RecordAccumulator.AppendCallbacks callbacks = new RecordAccumulator.AppendCallbacks() {
            ...
            public void onCompletion(RecordMetadata metadata, Exception exception) {
                if (exception instanceof TimeoutException) {
                    ...
                    try {
                        accumulator.append(tp1.topic(), tp1.partition(), 0L, key, value, Record.EMPTY_HEADERS, null, maxBlockTimeMs, false, time.milliseconds(), cluster);
    ...
    public void testNodeLatencyStats() throws Exception {
        try (Metrics m = new Metrics()) {
            ...
            accumulator = new RecordAccumulator(logContext, batchSize, CompressionType.NONE, 0, 0L, DELIVERY_TIMEOUT_MS, config, m, "producer-metrics", time, apiVersions, null, new BufferPool(totalSize, batchSize, m, time, "producer-internal-metrics"));

            SenderMetricsRegistry senderMetrics = new SenderMetricsRegistry(m);
            Sender sender = new Sender(logContext, client, metadata, this.accumulator, false, MAX_REQUEST_SIZE, ACKS_ALL, 1, senderMetrics, time, REQUEST_TIMEOUT, 1000L, null, new ApiVersions());
    ...
    public void testClusterAuthorizationExceptionInProduceRequest() throws Exception {
        ...
        Future<RecordMetadata> future = appendToAccumulator(tp0);
        client.prepareResponse(body -> body instanceof ProduceRequest && RequestTestUtils.hasIdempotentRecords((ProduceRequest) body), produceResponse(tp0, -1, Errors.CLUSTER_AUTHORIZATION_FAILED, 0));
    ...
    public void testCancelInFlightRequestAfterFatalError() throws Exception {
        ...
        client.respond(body -> body instanceof ProduceRequest && RequestTestUtils.hasIdempotentRecords((ProduceRequest) body), produceResponse(tp0, -1, Errors.CLUSTER_AUTHORIZATION_FAILED, 0));
        ...
        client.respond(body -> body instanceof ProduceRequest && RequestTestUtils.hasIdempotentRecords((ProduceRequest) body), produceResponse(tp1, 0, Errors.NONE, 0));
        sender.runOnce();
    }
    ...
    public void testUnsupportedForMessageFormatInProduceRequest() throws Exception {
        ...
        Future<RecordMetadata> future = appendToAccumulator(tp0);
        client.prepareResponse(body -> body instanceof ProduceRequest && RequestTestUtils.hasIdempotentRecords((ProduceRequest) body), produceResponse(tp0, -1, Errors.UNSUPPORTED_FOR_MESSAGE_FORMAT, 0));
    ...
    public void testUnsupportedVersionInProduceRequest() throws Exception {
        ...
        Future<RecordMetadata> future = appendToAccumulator(tp0);
        client.prepareUnsupportedVersionResponse(body -> body instanceof ProduceRequest && RequestTestUtils.hasIdempotentRecords((ProduceRequest) body));
    ...
    private void testSplitBatchAndSend(TransactionManager txnManager, ProducerIdAndEpoch producerIdAndEpoch, TopicPartition tp) throws Exception {
        ...
        try (Metrics m = new Metrics()) {
            ...
            Cluster cluster = TestUtils.singletonCluster();
            Future<RecordMetadata> f1 = accumulator.append(tp.topic(), tp.partition(), 0L, "key1".getBytes(), new byte[batchSize / 2], null, null, MAX_BLOCK_TIMEOUT, false, nowMs, cluster).future;
            Future<RecordMetadata> f2 = accumulator.append(tp.topic(), tp.partition(), 0L, "key2".getBytes(), new byte[batchSize / 2], null, null, MAX_BLOCK_TIMEOUT, false, nowMs, cluster).future;
            sender.runOnce(); // connect
            sender.runOnce(); // send produce request
    ...
    private void respondToProduce(TopicPartition tp, Errors error, long offset) {
        client.respond(request -> request instanceof ProduceRequest, produceResponse(tp, offset, error, 0));

    }

    private void respondToEndTxn(Errors error) {
        client.respond(request -> request instanceof EndTxnRequest, new EndTxnResponse(new EndTxnResponseData().setErrorCode(error.code()).setThrottleTimeMs(0)));
    }
    ...
    private static class OffsetAndError {
        ...
        OffsetAndError(long offset, Errors error, List<BatchIndexAndErrorMessage> recordErrors) {
            this.offset = offset;
            this.error = error;
    ...
    private FutureRecordMetadata appendToAccumulator(TopicPartition tp, long timestamp, String key, String value) throws InterruptedException {
        return accumulator.append(tp.topic(), tp.partition(), timestamp, key.getBytes(), value.getBytes(), Record.EMPTY_HEADERS, null, MAX_BLOCK_TIMEOUT, false, time.milliseconds(), TestUtils.singletonCluster()).future;
    }
    ...
    private AddPartitionsToTxnResponse buildAddPartitionsToTxnResponseData(int throttleMs, Map<TopicPartition, Errors> errors) {
        AddPartitionsToTxnResponseData.AddPartitionsToTxnResult result = AddPartitionsToTxnResponse.resultForTransaction(AddPartitionsToTxnResponse.V3_AND_BELOW_TXN_ID, errors);
