...
public class KafkaConsumerTest {
    ...
    private Map<String, Uuid> topicIds = Stream.of(new AbstractMap.SimpleEntry<>(topic, topicId), new AbstractMap.SimpleEntry<>(topic2, topicId2), new AbstractMap.SimpleEntry<>(topic3, topicId3)).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    private Map<Uuid, String> topicNames = Stream.of(new AbstractMap.SimpleEntry<>(topicId, topic), new AbstractMap.SimpleEntry<>(topicId2, topic2), new AbstractMap.SimpleEntry<>(topicId3, topic3)).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    ...
    private KafkaConsumer<?, ?> setUpConsumerWithRecordsToPoll(TopicPartition tp, int recordCount, Deserializer<String> deserializer) {
        ...
        consumer = newConsumer(time, client, subscription, metadata, assignor, true, groupId, groupInstanceId, Optional.of(deserializer), false);
    ...
    public void testInvalidSocketSendBufferSize() {
        ...
        assertThrows(KafkaException.class, () -> new KafkaConsumer<>(config, new ByteArrayDeserializer(), new ByteArrayDeserializer()));
    }
    ...
    public void testSubscriptionOnEmptyPattern() {
        consumer = newConsumer(groupId);
        assertThrows(IllegalArgumentException.class, () -> consumer.subscribe(Pattern.compile("")));
    }
    ...
    public void testSubscriptionWithEmptyPartitionAssignment() {
        ...
        assertThrows(IllegalStateException.class, () -> consumer.subscribe(singletonList(topic)));
    }
    ...
    public void testSeekNegative() {
        ...
        assertThrows(IllegalArgumentException.class, () -> consumer.seek(new TopicPartition("nonExistTopic", 0), -1));
    }
    ...
    public void testInterceptorConstructorConfigurationWithExceptionShouldCloseRemainingInstances() {
        ...
        try {
            Properties props = new Properties();
            props.setProperty(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9999");
            props.setProperty(ConsumerConfig.INTERCEPTOR_CLASSES_CONFIG, MockConsumerInterceptor.class.getName() + ", " + MockConsumerInterceptor.class.getName() + ", " + MockConsumerInterceptor.class.getName());
    ...
    public void verifyNoCoordinatorLookupForManualAssignmentWithOffsetCommit() {
        ...
        @SuppressWarnings("unchecked") ConsumerRecords<String, String> records = (ConsumerRecords<String, String>) consumer.poll(Duration.ofMillis(0));
        assertEquals(5, records.count());
        assertEquals(55L, consumer.position(tp0));
    ...
    public void testFetchProgressWithMissingPartitionPosition() {
        ...
        @SuppressWarnings("unchecked") ConsumerRecords<String, String> records = (ConsumerRecords<String, String>) consumer.poll(Duration.ofMillis(1));
        assertEquals(5, records.count());
    ...
    public void testMissingOffsetNoResetPolicy() {
        ...
        consumer = newConsumer(time, client, subscription, metadata, assignor, true, groupId, groupInstanceId, false);
        consumer.assign(singletonList(tp0));
    ...
    public void testOffsetIsValidAfterSeek() {
        ...
        consumer = newConsumer(time, client, subscription, metadata, assignor, true, groupId, Optional.empty(), false);
        consumer.assign(singletonList(tp0));
    ...
    private KafkaConsumer<?, ?> setupThrowableConsumer() {
        ...
        consumer = newConsumer(time, client, subscription, metadata, assignor, true, groupId, groupInstanceId, true);
        consumer.assign(singletonList(tp0));
    ...
    public void testWakeupWithFetchDataAvailable() throws Exception {
        ...
        @SuppressWarnings("unchecked") ConsumerRecords<String, String> records = (ConsumerRecords<String, String>) consumer.poll(Duration.ZERO);
        assertEquals(5, records.count());
    ...
    private OffsetFetchResponse offsetResponse(Map<TopicPartition, Long> offsets, Errors error) {
        ...
        return new OffsetFetchResponse(throttleMs, Collections.singletonMap(groupId, Errors.NONE), Collections.singletonMap(groupId, partitionData));
    }
    ...
    private KafkaConsumer<String, String> newConsumer(Time time, KafkaClient client, SubscriptionState subscription, ConsumerMetadata metadata, ConsumerPartitionAssignor assignor, boolean autoCommitEnabled, String groupId, Optional<String> groupInstanceId, boolean throwOnStableOffsetNotSupported) {
        return newConsumer(time, client, subscription, metadata, assignor, autoCommitEnabled, groupId, groupInstanceId, Optional.of(new StringDeserializer()), throwOnStableOffsetNotSupported);
    }
    ...
    private KafkaConsumer<String, String> newConsumer(Time time, KafkaClient client, SubscriptionState subscription, ConsumerMetadata metadata, ConsumerPartitionAssignor assignor, boolean autoCommitEnabled, String groupId, Optional<String> groupInstanceId, Optional<Deserializer<String>> valueDeserializer, boolean throwOnStableOffsetNotSupported) {
        ...
        if (groupId != null) {
            ...
            consumerCoordinator = new ConsumerCoordinator(rebalanceConfig, loggerFactory, consumerClient, assignors, metadata, subscription, metrics, metricGroupPrefix, time, autoCommitEnabled, autoCommitIntervalMs, interceptors, throwOnStableOffsetNotSupported, null);
        }
        IsolationLevel isolationLevel = IsolationLevel.READ_UNCOMMITTED;
        FetchMetricsManager metricsManager = new FetchMetricsManager(metrics, metricsRegistry.fetcherMetrics);
        FetchConfig<String, String> fetchConfig = new FetchConfig<>(minBytes, maxBytes, maxWaitMs, fetchSize, maxPollRecords, checkCrcs, CommonClientConfigs.DEFAULT_CLIENT_RACK, keyDeserializer, deserializer, isolationLevel);
        Fetcher<String, String> fetcher = new Fetcher<>(loggerFactory, consumerClient, metadata, subscription, fetchConfig, metricsManager, time);
        OffsetFetcher offsetFetcher = new OffsetFetcher(loggerFactory, consumerClient, metadata, subscription, time, retryBackoffMs, requestTimeoutMs, isolationLevel, new ApiVersions());
        TopicMetadataFetcher topicMetadataFetcher = new TopicMetadataFetcher(loggerFactory, consumerClient, requestTimeoutMs);

        return new KafkaConsumer<>(loggerFactory, clientId, consumerCoordinator, keyDeserializer, deserializer, fetcher, offsetFetcher, topicMetadataFetcher, interceptors, time, consumerClient, metrics, subscription, metadata, retryBackoffMs, requestTimeoutMs, defaultApiTimeoutMs, assignors, groupId);
    }
    ...
    private void prepareJoinGroupAndVerifyReason(MockClient client, Node node, String expectedReason) {
        ...
        }, joinGroupFollowerResponse(assignor, 1, memberId, leaderId, Errors.NONE), node);
    }
    ...
    public void testAssignorNameConflict() {
        ...
        configs.put(ConsumerConfig.PARTITION_ASSIGNMENT_STRATEGY_CONFIG, Arrays.asList(RangeAssignor.class.getName(), ConsumerPartitionAssignorTest.TestConsumerPartitionAssignor.class.getName()));

        assertThrows(KafkaException.class, () -> new KafkaConsumer<>(configs, new StringDeserializer(), new StringDeserializer()));
    }

    @Test
    public void testOffsetsForTimesTimeout() {
        final KafkaConsumer<String, String> consumer = consumerForCheckingTimeoutException();
        assertEquals("Failed to get offsets by times in 60000ms", assertThrows(org.apache.kafka.common.errors.TimeoutException.class, () -> consumer.offsetsForTimes(singletonMap(tp0, 0L))).getMessage());
    }

    @Test
    public void testBeginningOffsetsTimeout() {
        final KafkaConsumer<String, String> consumer = consumerForCheckingTimeoutException();
        assertEquals("Failed to get offsets by times in 60000ms", assertThrows(org.apache.kafka.common.errors.TimeoutException.class, () -> consumer.beginningOffsets(singletonList(tp0))).getMessage());
    }

    @Test
    public void testEndOffsetsTimeout() {
        final KafkaConsumer<String, String> consumer = consumerForCheckingTimeoutException();
        assertEquals("Failed to get offsets by times in 60000ms", assertThrows(org.apache.kafka.common.errors.TimeoutException.class, () -> consumer.endOffsets(singletonList(tp0))).getMessage());
    }
