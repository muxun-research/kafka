...
class AdminApiDriverTest {
    ...
    public void testCoalescedLookup() {
        TestContext ctx = TestContext.dynamicMapped(map("foo", "c1", "bar", "c1"));

        Map<Set<String>, LookupResult<String>> lookupRequests = map(mkSet("foo", "bar"), mapped("foo", 1, "bar", 2));

        ctx.poll(lookupRequests, emptyMap());

        Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("foo"), completed("foo", 15L), mkSet("bar"), completed("bar", 30L));
    ...
    public void testCoalescedFulfillment() {
        TestContext ctx = TestContext.dynamicMapped(map("foo", "c1", "bar", "c2"));

        Map<Set<String>, LookupResult<String>> lookupRequests = map(mkSet("foo"), mapped("foo", 1), mkSet("bar"), mapped("bar", 1));

        ctx.poll(lookupRequests, emptyMap());

        Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("foo", "bar"), completed("foo", 15L, "bar", 30L));
    ...
    public void testKeyLookupFailure() {
        ...
        Exception[] keyLookupExceptions = new Exception[]{new UnknownServerException(), new UnsupportedVersionException("")};
        for (Exception keyLookupException : keyLookupExceptions) {
            TestContext ctx = TestContext.dynamicMapped(map("foo", "c1", "bar", "c2"));

            Map<Set<String>, LookupResult<String>> lookupRequests = map(mkSet("foo"), failedLookup("foo", keyLookupException), mkSet("bar"), mapped("bar", 1));

            ctx.poll(lookupRequests, emptyMap());

            Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("bar"), completed("bar", 30L));
    ...
    public void testKeyLookupRetry() {
        ...
        Map<Set<String>, LookupResult<String>> lookupRequests = map(mkSet("foo"), emptyLookup(), mkSet("bar"), mapped("bar", 1));

        ctx.poll(lookupRequests, emptyMap());

        Map<Set<String>, LookupResult<String>> fooRetry = map(mkSet("foo"), mapped("foo", 1));

        Map<Set<String>, ApiResult<String, Long>> barFulfillment = map(mkSet("bar"), completed("bar", 30L));

        ctx.poll(fooRetry, barFulfillment);

        Map<Set<String>, ApiResult<String, Long>> fooFulfillment = map(mkSet("foo"), completed("foo", 15L));
    ...
    public void testStaticMapping() {
        TestContext ctx = TestContext.staticMapped(map("foo", 0, "bar", 1, "baz", 1));

        Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("foo"), completed("foo", 15L), mkSet("bar", "baz"), completed("bar", 30L, "baz", 45L));
    ...
    public void testFulfillmentFailure() {
        ...
        Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("foo"), failed("foo", new UnknownServerException()), mkSet("bar", "baz"), completed("bar", 30L, "baz", 45L));
    ...
    public void testFulfillmentRetry() {
        ...
        Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("foo"), completed("foo", 15L), mkSet("bar", "baz"), completed("bar", 30L));

        ctx.poll(emptyMap(), fulfillmentResults);

        Map<Set<String>, ApiResult<String, Long>> bazRetry = map(mkSet("baz"), completed("baz", 45L));
    ...
    public void testFulfillmentUnmapping() {
        ...
        Map<Set<String>, LookupResult<String>> lookupRequests = map(mkSet("foo"), mapped("foo", 0), mkSet("bar"), mapped("bar", 1));

        ctx.poll(lookupRequests, emptyMap());

        Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("foo"), completed("foo", 15L), mkSet("bar"), unmapped("bar"));

        ctx.poll(emptyMap(), fulfillmentResults);

        Map<Set<String>, LookupResult<String>> barLookupRetry = map(mkSet("bar"), mapped("bar", 1));

        ctx.poll(barLookupRetry, emptyMap());

        Map<Set<String>, ApiResult<String, Long>> barFulfillRetry = map(mkSet("bar"), completed("bar", 30L));
    ...
    public void testFulfillmentFailureUnsupportedVersion() {
        ...
        Map<Set<String>, ApiResult<String, Long>> fulfillmentResults = map(mkSet("foo"), failed("foo", new UnsupportedVersionException("")), mkSet("bar", "baz"), completed("bar", 30L, "baz", 45L));
    ...
    public void testFulfillmentRetriableUnsupportedVersion() {
        TestContext ctx = TestContext.staticMapped(map("foo", 0, "bar", 1, "baz", 2));
        ...
        requestSpecs.forEach(requestSpec -> {
            if (requestSpec.keys.contains("foo") || requestSpec.keys.contains("bar")) {
                ...
                ctx.driver.onResponse(ctx.time.milliseconds(), requestSpec, new MetadataResponse(new MetadataResponseData(), ApiKeys.METADATA.latestVersion()), Node.noNode());
            }
        });
        // Verify retry for "foo" but not for "bar" or "baz"
        ctx.poll(emptyMap(), map(mkSet("foo"), failed("foo", new UnsupportedVersionException(""))));
        ctx.poll(emptyMap(), emptyMap());
    }
    ...
    public void testRecoalescedLookup() {
        ...
        Map<Set<String>, ApiResult<String, Long>> fulfillment = map(mkSet("foo"), unmapped("foo"), mkSet("bar"), unmapped("bar"));

        ctx.poll(emptyMap(), fulfillment);

        Map<Set<String>, LookupResult<String>> retryLookupRequests = map(mkSet("foo", "bar"), mapped("foo", 3, "bar", 3));

        ctx.poll(retryLookupRequests, emptyMap());

        Map<Set<String>, ApiResult<String, Long>> retryFulfillment = map(mkSet("foo", "bar"), completed("foo", 15L, "bar", 30L));
    ...
    private static class MockRequestScope implements ApiRequestScope {
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(destinationBrokerId, that.destinationBrokerId) && Objects.equals(id, that.id);
        }
    ...
    private static class TestContext {
        ...
        public TestContext(Map<String, Integer> staticKeys, Map<String, String> dynamicKeys) {
            ...
            this.driver = new AdminApiDriver<>(handler, future, time.milliseconds() + API_TIMEOUT_MS, RETRY_BACKOFF_MS, new LogContext());
        ...
        private void assertLookupResponse(RequestSpec<String> requestSpec, LookupResult<String> result) {
        ...
        private void assertResponse(RequestSpec<String> requestSpec, ApiResult<String, Long> result, Node node) {
            int brokerId = requestSpec.scope.destinationBrokerId().orElseThrow(() -> new AssertionError("Fulfillment requests must specify a target brokerId"));
        ...
        public void poll(Map<Set<String>, LookupResult<String>> expectedLookups, Map<Set<String>, ApiResult<String, Long>> expectedRequests) {
            ...
            assertEquals(expectedLookups.size() + expectedRequests.size(), requestSpecs.size(), "Driver generated an unexpected number of requests");
    ...
    private static class MockLookupStrategy<K> implements AdminApiLookupStrategy<K> {
        ...
        public LookupResult<K> handleResponse(Set<K> keys, AbstractResponse response) {
            return Optional.ofNullable(expectedLookups.get(keys)).orElseThrow(() -> new AssertionError("Unexpected fulfillment request for keys " + keys));
        }
    ...
    private static class MockAdminApiHandler<K, V> extends AdminApiHandler.Batched<K, V> {
        ...
        public ApiResult<K, V> handleResponse(Node broker, Set<K> keys, AbstractResponse response) {
            return Optional.ofNullable(expectedRequests.get(keys)).orElseThrow(() -> new AssertionError("Unexpected fulfillment request for keys " + keys));
        }

        @Override
        public Map<K, Throwable> handleUnsupportedVersionException(int brokerId, UnsupportedVersionException exception, Set<K> keys) {
            return keys.stream().filter(k -> !retriableUnsupportedVersionKeys.containsKey(k)).collect(Collectors.toMap(k -> k, k -> exception));
        }
