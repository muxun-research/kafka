...
public class PrototypeAsyncConsumerTest {
    ...
    private PrototypeAsyncConsumer<?, ?> newConsumer(final Time time, final Deserializer<?> keyDeserializer, final Deserializer<?> valueDeserializer) {
        ...
        return new PrototypeAsyncConsumer<>(time, logContext, config, subscriptions, eventHandler, metrics, clusterResourceListeners, Optional.ofNullable(this.groupId), clientId, config.getInt(DEFAULT_API_TIMEOUT_MS_CONFIG));
    }
}
