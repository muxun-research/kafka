...
public class KafkaAdminClientTest {
    ...
    public static DeletableTopicResult deletableTopicResultWithId(Uuid topicId, Errors error) {
        return new DeletableTopicResult().setTopicId(topicId).setErrorCode(error.code());
    }
    ...
    private static FindCoordinatorResponse prepareBatchedFindCoordinatorResponse(Errors error, Node node, Collection<String> groups) {
        ...
        List<FindCoordinatorResponseData.Coordinator> coordinators = groups.stream().map(group -> new FindCoordinatorResponseData.Coordinator().setErrorCode(error.code()).setErrorMessage(error.message()).setKey(group).setHost(node.host()).setPort(node.port()).setNodeId(node.id())).collect(Collectors.toList());
    ...
    private static FeatureMetadata defaultFeatureMetadata() {
        return new FeatureMetadata(Utils.mkMap(Utils.mkEntry("test_feature_1", new FinalizedVersionRange((short) 2, (short) 2))), Optional.of(1L), Utils.mkMap(Utils.mkEntry("test_feature_1", new SupportedVersionRange((short) 1, (short) 5))));
    }
    ...
    private static ApiVersionsResponse prepareApiVersionsResponseForDescribeFeatures(Errors error) {
        if (error == Errors.NONE) {
            return ApiVersionsResponse.createApiVersionsResponse(0, ApiVersionsResponse.filterApis(RecordVersion.current(), ApiMessageType.ListenerType.ZK_BROKER), convertSupportedFeaturesMap(defaultFeatureMetadata().supportedFeatures()), Collections.singletonMap("test_feature_1", (short) 2), defaultFeatureMetadata().finalizedFeaturesEpoch().get(), false);
        }
    ...
    private static QuorumInfo defaultQuorumInfo(boolean emptyOptionals) {
        return new QuorumInfo(1, 1, 1L, singletonList(new QuorumInfo.ReplicaState(1, 100, emptyOptionals ? OptionalLong.empty() : OptionalLong.of(1000), emptyOptionals ? OptionalLong.empty() : OptionalLong.of(1000))), singletonList(new QuorumInfo.ReplicaState(1, 100, emptyOptionals ? OptionalLong.empty() : OptionalLong.of(1000), emptyOptionals ? OptionalLong.empty() : OptionalLong.of(1000))));
    }
    ...
    private static DescribeQuorumResponse prepareDescribeQuorumResponse(Errors topLevelError, Errors partitionLevelError, Boolean topicCountError, Boolean topicNameError, Boolean partitionCountError, Boolean partitionIndexError, Boolean emptyOptionals) {
        ...
        for (int i = 0; i < (partitionCountError ? 2 : 1); i++) {
            DescribeQuorumResponseData.ReplicaState replica = new DescribeQuorumResponseData.ReplicaState().setReplicaId(1).setLogEndOffset(100);
            replica.setLastFetchTimestamp(emptyOptionals ? -1 : 1000);
            replica.setLastCaughtUpTimestamp(emptyOptionals ? -1 : 1000);
            partitions.add(new DescribeQuorumResponseData.PartitionData().setPartitionIndex(partitionIndex).setLeaderId(1).setLeaderEpoch(1).setHighWatermark(1).setCurrentVoters(singletonList(replica)).setObservers(singletonList(replica)).setErrorCode(partitionLevelError.code()));
        }
    ...
    public void testTimeoutWithoutMetadata() throws Exception {
        try (final AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(Time.SYSTEM, mockBootstrapCluster(), newStrMap(AdminClientConfig.REQUEST_TIMEOUT_MS_CONFIG, "10"))) {
            ...
            KafkaFuture<Void> future = env.adminClient().createTopics(singleton(new NewTopic("myTopic", Collections.singletonMap(0, asList(0, 1, 2)))), new CreateTopicsOptions().timeoutMs(1000)).all();
    ...
    public void testCreateTopicsHandleNotControllerException() throws Exception {
        try (AdminClientUnitTestEnv env = mockClientEnv()) {
            ...
            env.kafkaClient().prepareResponse(RequestTestUtils.metadataResponse(env.cluster().nodes(), env.cluster().clusterResource().clusterId(), 1, Collections.emptyList()));
    ...
    public void testInvalidTopicNames() throws Exception {
        try (AdminClientUnitTestEnv env = mockClientEnv()) {
            ...
            Map<String, KafkaFuture<TopicDescription>> describeFutures = env.adminClient().describeTopics(sillyTopicNames).topicNameValues();
            for (String sillyTopicName : sillyTopicNames) {
    ...
    public void testMetadataRetries() throws Exception {
        ...
        try (final AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(Time.SYSTEM, bootstrapCluster, newStrMap(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9999", AdminClientConfig.DEFAULT_API_TIMEOUT_MS_CONFIG, "10000000", AdminClientConfig.RETRIES_CONFIG, "0"))) {
            ...
            env.kafkaClient().prepareResponse(RequestTestUtils.metadataResponse(initializedCluster.nodes(), initializedCluster.clusterResource().clusterId(), initializedCluster.controller().id(), Collections.emptyList()));
    ...
    private static DescribeLogDirsResponse prepareDescribeLogDirsResponse(Errors error, String logDir, TopicPartition tp, long partitionSize, long offsetLag, long totalBytes, long usableBytes) {
        return prepareDescribeLogDirsResponse(error, logDir, prepareDescribeLogDirsTopics(partitionSize, offsetLag, tp.topic(), tp.partition(), false), totalBytes, usableBytes);
    }
    ...
    private static DescribeLogDirsResponse prepareDescribeLogDirsResponse(Errors error, String logDir, List<DescribeLogDirsTopic> topics, long totalBytes, long usableBytes) {
        return new DescribeLogDirsResponse(new DescribeLogDirsResponseData().setResults(singletonList(new DescribeLogDirsResponseData.DescribeLogDirsResult().setErrorCode(error.code()).setLogDir(logDir).setTopics(topics).setTotalBytes(totalBytes).setUsableBytes(usableBytes))));
    }
    ...
    public void testDescribeLogDirsWithVolumeBytes() throws ExecutionException, InterruptedException {
        ...
        try (AdminClientUnitTestEnv env = mockClientEnv()) {
            env.kafkaClient().setNodeApiVersions(NodeApiVersions.create());
            env.kafkaClient().prepareResponseFrom(prepareDescribeLogDirsResponse(Errors.NONE, logDir, tp, partitionSize, offsetLag, totalBytes, usableBytes), env.cluster().nodeById(0));
    ...
    public void testDeleteRecordsTopicAuthorizationError() {
        ...
        try (AdminClientUnitTestEnv env = mockClientEnv()) {
            ...
            env.kafkaClient().prepareResponse(RequestTestUtils.metadataResponse(env.cluster().nodes(), env.cluster().clusterResource().clusterId(), env.cluster().controller().id(), topics));
    ...
    public void testDeleteRecordsMultipleSends() throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(time, mockCluster(3, 0))) {
            ...
            env.kafkaClient().prepareResponse(RequestTestUtils.metadataResponse(env.cluster().nodes(), env.cluster().clusterResource().clusterId(), env.cluster().controller().id(), topicMetadata));
    ...
    public void testDeleteRecords() throws Exception {
        ...
        Cluster cluster = new Cluster("mockClusterId", nodes.values(), partitionInfos, Collections.emptySet(), Collections.emptySet(), nodes.get(0));
    ...
    public void testDescribeCluster() throws Exception {
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(4, 0), AdminClientConfig.RETRIES_CONFIG, "2")) {
            ...
            env.kafkaClient().prepareResponse(prepareDescribeClusterResponse(0, env.cluster().nodes(), env.cluster().clusterResource().clusterId(), 2, MetadataResponse.AUTHORIZED_OPERATIONS_OMITTED));

            // Prepare the describe cluster response used for the second describe cluster
            env.kafkaClient().prepareResponse(prepareDescribeClusterResponse(0, env.cluster().nodes(), env.cluster().clusterResource().clusterId(), 3, 1 << AclOperation.DESCRIBE.code() | 1 << AclOperation.ALTER.code()));
    ...
    public void testDescribeClusterFailBack() throws Exception {
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(4, 0), AdminClientConfig.RETRIES_CONFIG, "2")) {
            ...
            env.kafkaClient().prepareResponse(RequestTestUtils.metadataResponse(0, env.cluster().nodes(), env.cluster().clusterResource().clusterId(), 2, Collections.emptyList(), MetadataResponse.AUTHORIZED_OPERATIONS_OMITTED, ApiKeys.METADATA.latestVersion()));
    ...
    private void verifyListConsumerGroupOffsetsOptions(boolean batchedApi) throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(time, cluster, AdminClientConfig.RETRIES_CONFIG, "0")) {
            ...
            final ListConsumerGroupOffsetsOptions options = new ListConsumerGroupOffsetsOptions().requireStable(true).timeoutMs(300);
            if (batchedApi) {
                final ListConsumerGroupOffsetsSpec groupSpec = new ListConsumerGroupOffsetsSpec().topicPartitions(partitions);
            ...
            assertEquals(Collections.singletonList(GROUP_ID), data.groups().stream().map(OffsetFetchRequestGroup::groupId).collect(Collectors.toList()));
            assertEquals(Collections.singletonList("A"), data.groups().get(0).topics().stream().map(OffsetFetchRequestTopics::name).collect(Collectors.toList()));
            assertEquals(Collections.singletonList(0), data.groups().get(0).topics().get(0).partitionIndexes());
        }
    }
    ...
    public void testListConsumerGroupOffsetsRetriableErrors() throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(1, 0))) {
            ...
            env.kafkaClient().prepareResponse(offsetFetchResponse(Errors.NONE, Collections.emptyMap()));
    ...
    public void testBatchedListConsumerGroupOffsetsWithNoFindCoordinatorBatching() throws Exception {
        ...
        ApiVersion offsetFetchV7 = new ApiVersion().setApiKey(ApiKeys.OFFSET_FETCH.id).setMinVersion((short) 0).setMaxVersion((short) 7);
    ...
    private void verifyListOffsetsForMultipleGroups(Map<String, ListConsumerGroupOffsetsSpec> groupSpecs, ListConsumerGroupOffsetsResult result) throws Exception {
        ...
        for (Map.Entry<String, ListConsumerGroupOffsetsSpec> entry : groupSpecs.entrySet()) {
            assertEquals(entry.getValue().topicPartitions(), result.partitionsToOffsetAndMetadata(entry.getKey()).get().keySet());
        }
    }
    ...
    public void testDeleteConsumerGroupOffsetsRetriableErrors() throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(1, 0))) {
            ...
            final DeleteConsumerGroupOffsetsResult errorResult1 = env.adminClient().deleteConsumerGroupOffsets(GROUP_ID, Stream.of(tp1).collect(Collectors.toSet()));
    ...
    public void testDeleteConsumerGroupOffsetsFindCoordinatorNonRetriableErrors() throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(1, 0))) {
            ...
            final DeleteConsumerGroupOffsetsResult errorResult = env.adminClient().deleteConsumerGroupOffsets(GROUP_ID, Stream.of(tp1).collect(Collectors.toSet()));
    ...
    public void testRemoveMembersFromGroupNonRetriableErrors() throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(1, 0))) {
            ...
            for (Errors error : nonRetriableErrors) {
                ...
                env.kafkaClient().prepareResponse(new LeaveGroupResponse(new LeaveGroupResponseData().setErrorCode(error.code())));
    ...
    public void testAlterPartitionReassignments() throws Exception {
        try (AdminClientUnitTestEnv env = mockClientEnv()) {
                            ...
                            .setResponses(Arrays.asList(
                                    new ReassignableTopicResponse()
                                            ...
                                            .setPartitions(Collections.singletonList(new ReassignablePartitionResponse()
                                                ...
                                                .setErrorCode(Errors.INVALID_REPLICA_ASSIGNMENT.code()))), new ReassignableTopicResponse().setName("B").setPartitions(Collections.singletonList(normalPartitionResponse))));
            ...
            AlterPartitionReassignmentsResponseData noErrResponseData = new AlterPartitionReassignmentsResponseData().setErrorCode(Errors.NONE.code()).setErrorMessage(Errors.NONE.message()).setResponses(Arrays.asList(
                                    new ReassignableTopicResponse()
                                            .setName("A")
    ...
    public void testAlterConsumerGroupOffsetsRetriableErrors() throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(1, 0))) {
            ...
            env.kafkaClient().prepareResponse(prepareOffsetCommitResponse(tp1, Errors.REBALANCE_IN_PROGRESS));

            env.kafkaClient().prepareResponse(prepareOffsetCommitResponse(tp1, Errors.NONE));

            Map<TopicPartition, OffsetAndMetadata> offsets = new HashMap<>();
            offsets.put(tp1, new OffsetAndMetadata(123L));
            final AlterConsumerGroupOffsetsResult result1 = env.adminClient().alterConsumerGroupOffsets(GROUP_ID, offsets);
    ...
    public void testAlterConsumerGroupOffsetsFindCoordinatorNonRetriableErrors() throws Exception {
        ...
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(mockCluster(1, 0))) {
            ...
            Map<TopicPartition, OffsetAndMetadata> offsets = new HashMap<>();
            offsets.put(tp1, new OffsetAndMetadata(123L));
            final AlterConsumerGroupOffsetsResult errorResult = env.adminClient().alterConsumerGroupOffsets(GROUP_ID, offsets);
    ...
    public void testListOffsets() throws Exception {
        ...
        final Cluster cluster = new Cluster("mockClusterId", Arrays.asList(node0), pInfos, Collections.emptySet(), Collections.emptySet(), node0);
    ...
    public void testListOffsetsRetriableErrors() throws Exception {
        ...
        final Cluster cluster = new Cluster("mockClusterId", nodes, pInfos, Collections.emptySet(), Collections.emptySet(), node0);
    ...
    public void testListOffsetsHandlesFulfillmentTimeouts() throws Exception {
        ...
        int numRetries = 2;
        try (AdminClientUnitTestEnv env = new AdminClientUnitTestEnv(cluster, AdminClientConfig.RETRIES_CONFIG, Integer.toString(numRetries))) {

            ListOffsetsTopicResponse tp0ErrorResponse = ListOffsetsResponse.singletonListOffsetsTopicResponse(tp0, Errors.REQUEST_TIMED_OUT, -1L, -1L, -1);
            ListOffsetsTopicResponse tp1Response = ListOffsetsResponse.singletonListOffsetsTopicResponse(tp1, Errors.NONE, -1L, 345L, 543);
            ...
            ListOffsetsTopicResponse tp0Response = ListOffsetsResponse.singletonListOffsetsTopicResponse(tp0, Errors.NONE, -1L, 789L, 987);
            ...
            for (int i = 0; i < numRetries + 1; i++) {
                env.kafkaClient().prepareResponseFrom(request -> request instanceof ListOffsetsRequest, new ListOffsetsResponse(responseDataWithError), node);
            }
            ListOffsetsResult result = env.adminClient().listOffsets(new HashMap<TopicPartition, OffsetSpec>() {
                {
            ...
            env.kafkaClient().prepareResponseFrom(request -> request instanceof ListOffsetsRequest, new ListOffsetsResponse(responseData), node);
            result = env.adminClient().listOffsets(new HashMap<TopicPartition, OffsetSpec>() {
                {
                    put(tp0, OffsetSpec.latest());
                    put(tp1, OffsetSpec.latest());
                }
    ...
    private Map<String, FeatureUpdate> makeTestFeatureUpdates() {
        return Utils.mkMap(Utils.mkEntry("test_feature_1", new FeatureUpdate((short) 2, FeatureUpdate.UpgradeType.UPGRADE)), Utils.mkEntry("test_feature_2", new FeatureUpdate((short) 3, FeatureUpdate.UpgradeType.SAFE_DOWNGRADE)));
    }
    ...
    public void testUpdateFeaturesHandleNotControllerException() throws Exception {
        try (final AdminClientUnitTestEnv env = mockClientEnv()) {
            ...
            env.kafkaClient().prepareResponse(RequestTestUtils.metadataResponse(env.cluster().nodes(), env.cluster().clusterResource().clusterId(), controllerId, Collections.emptyList()));
            ...
            final KafkaFuture<Void> future = env.adminClient().updateFeatures(Utils.mkMap(Utils.mkEntry("test_feature_1", new FeatureUpdate((short) 2, FeatureUpdate.UpgradeType.UPGRADE)), Utils.mkEntry("test_feature_2", new FeatureUpdate((short) 3, FeatureUpdate.UpgradeType.SAFE_DOWNGRADE))), new UpdateFeaturesOptions().timeoutMs(10000)).all();
            future.get();
        }
    }
    ...
    public void testUpdateFeaturesShouldFailRequestForInvalidFeatureName() {
        try (final AdminClientUnitTestEnv env = mockClientEnv()) {
            assertThrows(IllegalArgumentException.class, () -> env.adminClient().updateFeatures(Utils.mkMap(Utils.mkEntry("feature", new FeatureUpdate((short) 2, FeatureUpdate.UpgradeType.UPGRADE)), Utils.mkEntry("", new FeatureUpdate((short) 2, FeatureUpdate.UpgradeType.UPGRADE))), new UpdateFeaturesOptions()));
        }
    }

    @Test
    public void testUpdateFeaturesShouldFailRequestInClientWhenDowngradeFlagIsNotSetDuringDeletion() {
        assertThrows(IllegalArgumentException.class, () -> new FeatureUpdate((short) 0, FeatureUpdate.UpgradeType.UPGRADE));
    }
    ...
    public void testDescribeMetadataQuorumSuccess() throws Exception {
        try (final AdminClientUnitTestEnv env = mockClientEnv()) {
            env.kafkaClient().setNodeApiVersions(NodeApiVersions.create(ApiKeys.DESCRIBE_QUORUM.id, ApiKeys.DESCRIBE_QUORUM.oldestVersion(), ApiKeys.DESCRIBE_QUORUM.latestVersion()));

            // Test with optional fields set
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NONE, false, false, false, false, false));
            ...
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NONE, false, false, false, false, true));
    ...
    public void testDescribeMetadataQuorumRetriableError() throws Exception {
        try (final AdminClientUnitTestEnv env = mockClientEnv()) {
            ...
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NOT_LEADER_OR_FOLLOWER, false, false, false, false, false));
    ...
    public void testDescribeMetadataQuorumFailure() {
        try (final AdminClientUnitTestEnv env = mockClientEnv()) {
            ...
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.INVALID_REQUEST, Errors.NONE, false, false, false, false, false));
            KafkaFuture<QuorumInfo> future = env.adminClient().describeMetadataQuorum().quorumInfo();
            TestUtils.assertFutureThrows(future, InvalidRequestException.class);

            // Test incorrect topic count
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NONE, true, false, false, false, false));
            future = env.adminClient().describeMetadataQuorum().quorumInfo();
            TestUtils.assertFutureThrows(future, UnknownServerException.class);

            // Test incorrect topic name
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NONE, false, true, false, false, false));
            future = env.adminClient().describeMetadataQuorum().quorumInfo();
            TestUtils.assertFutureThrows(future, UnknownServerException.class);

            // Test incorrect partition count
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NONE, false, false, true, false, false));
            future = env.adminClient().describeMetadataQuorum().quorumInfo();
            TestUtils.assertFutureThrows(future, UnknownServerException.class);

            // Test incorrect partition index
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NONE, false, false, false, true, false));
            future = env.adminClient().describeMetadataQuorum().quorumInfo();
            TestUtils.assertFutureThrows(future, UnknownServerException.class);

            // Test partition level error
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.INVALID_REQUEST, false, false, false, false, false));
            future = env.adminClient().describeMetadataQuorum().quorumInfo();
            TestUtils.assertFutureThrows(future, InvalidRequestException.class);

            // Test all incorrect and no errors
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.NONE, Errors.NONE, true, true, true, true, false));
            future = env.adminClient().describeMetadataQuorum().quorumInfo();
            TestUtils.assertFutureThrows(future, UnknownServerException.class);

            // Test all incorrect and both errors
            env.kafkaClient().prepareResponse(body -> body instanceof DescribeQuorumRequest, prepareDescribeQuorumResponse(Errors.INVALID_REQUEST, Errors.INVALID_REQUEST, true, true, true, true, false));
            future = env.adminClient().describeMetadataQuorum().quorumInfo();
    ...
    private void testApiTimeout(int requestTimeoutMs, int defaultApiTimeoutMs, OptionalInt overrideApiTimeoutMs) throws Exception {
        ...
        assertEquals(2 * requestTimeoutMs, effectiveTimeoutMs, "This test expects the effective timeout to be twice the request timeout");
    ...
    public void testUnregisterBrokerSuccess() throws InterruptedException, ExecutionException {
        int nodeId = 1;
        try (final AdminClientUnitTestEnv env = mockClientEnv()) {
            env.kafkaClient().setNodeApiVersions(NodeApiVersions.create(ApiKeys.UNREGISTER_BROKER.id, (short) 0, (short) 0));
    ...
    public void testFenceProducers() throws Exception {
        try (AdminClientUnitTestEnv env = mockClientEnv()) {
            ...
            env.kafkaClient().prepareResponseFrom(request -> request instanceof InitProducerIdRequest, new InitProducerIdResponse(new InitProducerIdResponseData().setErrorCode(Errors.COORDINATOR_LOAD_IN_PROGRESS.code())), transactionCoordinator);
            // then find out that the coordinator has changed since then
            env.kafkaClient().prepareResponseFrom(request -> request instanceof InitProducerIdRequest, new InitProducerIdResponse(new InitProducerIdResponseData().setErrorCode(Errors.NOT_COORDINATOR.code())), transactionCoordinator);
            ...
            InitProducerIdResponseData initProducerIdResponseData = new InitProducerIdResponseData().setProducerId(4761).setProducerEpoch((short) 489);
            env.kafkaClient().prepareResponseFrom(request -> request instanceof InitProducerIdRequest, new InitProducerIdResponse(initProducerIdResponseData), transactionCoordinator);
    ...
    private OffsetFetchResponse offsetFetchResponse(Errors error, Map<TopicPartition, PartitionData> responseData) {
        return new OffsetFetchResponse(THROTTLE, Collections.singletonMap(GROUP_ID, error), Collections.singletonMap(GROUP_ID, responseData));
    }
