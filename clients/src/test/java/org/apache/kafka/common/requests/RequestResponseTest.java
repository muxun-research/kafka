...
public class RequestResponseTest {
    ...
    public void testBatchedFindCoordinatorRequestSerialization() {
        for (short version : FIND_COORDINATOR.allVersions()) {
            ...
            if (version < FindCoordinatorRequest.MIN_BATCHED_VERSION) {
                assertThrows(NoBatchedFindCoordinatorsException.class, () -> createBatchedFindCoordinatorRequest(asList("group1", "group2"), version));
            } else {
    ...
    public void testCreateTopicRequestV0FailsIfValidateOnly() {
        assertThrows(UnsupportedVersionException.class, () -> createCreateTopicRequest((short) 0, true));
    }
    ...
    public void testFetchRequestIsolationLevel() {
        ...
        FetchRequest deserialized = (FetchRequest) AbstractRequest.parseRequest(request.apiKey(), request.version(), request.serialize()).request;
        ...
        deserialized = (FetchRequest) AbstractRequest.parseRequest(request.apiKey(), request.version(), request.serialize()).request;
        assertEquals(request.isolationLevel(), deserialized.isolationLevel());
    }
    ...
    public void testFetchRequestWithMetadata() {
        FetchRequest request = createFetchRequest((short) 4, IsolationLevel.READ_COMMITTED);
        FetchRequest deserialized = (FetchRequest) AbstractRequest.parseRequest(FETCH, request.version(), request.serialize()).request;
        assertEquals(request.isolationLevel(), deserialized.isolationLevel());

        request = createFetchRequest((short) 4, IsolationLevel.READ_UNCOMMITTED);
        deserialized = (FetchRequest) AbstractRequest.parseRequest(FETCH, request.version(), request.serialize()).request;
        assertEquals(request.isolationLevel(), deserialized.isolationLevel());
    }
    ...
    public void testOffsetFetchRequestBuilderToStringV0ToV7() {
        ...
        for (Boolean requireStable : stableFlags) {
            ...
            assertTrue(allTopicPartitionsString.contains("groupId='someGroup', topics=null," + " groups=[], requireStable=" + requireStable));
            String string = new OffsetFetchRequest.Builder("group1", requireStable, singletonList(new TopicPartition("test11", 1)), false).toString();
    ...
    public void testOffsetFetchRequestBuilderToStringV8AndAbove() {
        ...
        for (Boolean requireStable : stableFlags) {
            ...
            assertTrue(allTopicPartitionsString.contains("groups=[OffsetFetchRequestGroup" + "(groupId='someGroup', topics=null)], requireStable=" + requireStable));

            String subsetTopicPartitionsString = new OffsetFetchRequest.Builder(Collections.singletonMap("group1", singletonList(new TopicPartition("test11", 1))), requireStable, false).toString();
    ...
    public void testValidApiVersionsRequest() {
        ...
        request = new ApiVersionsRequest(new ApiVersionsRequestData().setClientSoftwareName("apache-kafka.java").setClientSoftwareVersion("0.0.0-SNAPSHOT"), API_VERSIONS.latestVersion());
        assertTrue(request.isValid());
    }

    @Test
    public void testListGroupRequestV3FailsWithStates() {
        ListGroupsRequestData data = new ListGroupsRequestData().setStatesFilter(singletonList(ConsumerGroupState.STABLE.name()));
    ...
    public void testInitProducerIdRequestVersions() {
        ...
        final UnsupportedVersionException exception = assertThrows(UnsupportedVersionException.class, () -> bld.build((short) 2).serialize());
    ...
    private DescribeQuorumRequest createDescribeQuorumRequest(short version) {
        DescribeQuorumRequestData data = new DescribeQuorumRequestData().setTopics(singletonList(new DescribeQuorumRequestData.TopicData().setPartitions(singletonList(new DescribeQuorumRequestData.PartitionData().setPartitionIndex(0))).setTopicName("topic1")));
    ...
    private EndQuorumEpochRequest createEndQuorumEpochRequest(short version) {
        EndQuorumEpochRequestData data = new EndQuorumEpochRequestData().setClusterId("clusterId").setTopics(singletonList(new EndQuorumEpochRequestData.TopicData().setPartitions(singletonList(new EndQuorumEpochRequestData.PartitionData().setLeaderEpoch(0).setLeaderId(1).setPartitionIndex(2).setPreferredSuccessors(asList(0, 1, 2)))).setTopicName("topic1")));
        return new EndQuorumEpochRequest.Builder(data).build(version);
    }

    private EndQuorumEpochResponse createEndQuorumEpochResponse() {
        EndQuorumEpochResponseData data = new EndQuorumEpochResponseData().setErrorCode(Errors.NONE.code()).setTopics(singletonList(new EndQuorumEpochResponseData.TopicData().setPartitions(singletonList(new EndQuorumEpochResponseData.PartitionData().setErrorCode(Errors.NONE.code()).setLeaderEpoch(1))).setTopicName("topic1")));
        return new EndQuorumEpochResponse(data);
    }

    private BeginQuorumEpochRequest createBeginQuorumEpochRequest(short version) {
        BeginQuorumEpochRequestData data = new BeginQuorumEpochRequestData().setClusterId("clusterId").setTopics(singletonList(new BeginQuorumEpochRequestData.TopicData().setPartitions(singletonList(new BeginQuorumEpochRequestData.PartitionData().setLeaderEpoch(0).setLeaderId(1).setPartitionIndex(2)))));
    ...
    private BeginQuorumEpochResponse createBeginQuorumEpochResponse() {
        BeginQuorumEpochResponseData data = new BeginQuorumEpochResponseData().setErrorCode(Errors.NONE.code()).setTopics(singletonList(new BeginQuorumEpochResponseData.TopicData().setPartitions(singletonList(new BeginQuorumEpochResponseData.PartitionData().setErrorCode(Errors.NONE.code()).setLeaderEpoch(0).setLeaderId(1).setPartitionIndex(2)))));
        return new BeginQuorumEpochResponse(data);
    }

    private VoteRequest createVoteRequest(short version) {
        VoteRequestData data = new VoteRequestData().setClusterId("clusterId").setTopics(singletonList(new VoteRequestData.TopicData().setPartitions(singletonList(new VoteRequestData.PartitionData().setPartitionIndex(0).setCandidateEpoch(1).setCandidateId(2).setLastOffset(3L).setLastOffsetEpoch(4))).setTopicName("topic1")));
        return new VoteRequest.Builder(data).build(version);
    }

    private VoteResponse createVoteResponse() {
        VoteResponseData data = new VoteResponseData().setErrorCode(Errors.NONE.code()).setTopics(singletonList(new VoteResponseData.TopicData().setPartitions(singletonList(new VoteResponseData.PartitionData().setErrorCode(Errors.NONE.code()).setLeaderEpoch(0).setPartitionIndex(1).setLeaderId(2).setVoteGranted(false)))));
    ...
    private AlterUserScramCredentialsResponse createAlterUserScramCredentialsResponse() {
        AlterUserScramCredentialsResponseData data = new AlterUserScramCredentialsResponseData().setResults(singletonList(new AlterUserScramCredentialsResponseData.AlterUserScramCredentialsResult().setErrorCode(Errors.NONE.code()).setUser("user1").setErrorMessage("error message")));
    ...
    private DescribeUserScramCredentialsRequest createDescribeUserScramCredentialsRequest(short version) {
        DescribeUserScramCredentialsRequestData data = new DescribeUserScramCredentialsRequestData().setUsers(singletonList(new DescribeUserScramCredentialsRequestData.UserName().setName("user1")));
    ...
    private DescribeUserScramCredentialsResponse createDescribeUserScramCredentialsResponse() {
        DescribeUserScramCredentialsResponseData data = new DescribeUserScramCredentialsResponseData().setResults(singletonList(new DescribeUserScramCredentialsResponseData.DescribeUserScramCredentialsResult().setUser("user1").setErrorCode(Errors.NONE.code()).setErrorMessage("error message").setCredentialInfos(singletonList(new DescribeUserScramCredentialsResponseData.CredentialInfo().setIterations(1024).setMechanism((byte) 0))))).setErrorCode(Errors.NONE.code()).setErrorMessage("error message").setThrottleTimeMs(123);
    ...
    private AlterPartitionRequest createAlterPartitionRequest(short version) {
        AlterPartitionRequestData.PartitionData partitionData = new AlterPartitionRequestData.PartitionData().setPartitionIndex(1).setPartitionEpoch(2).setLeaderEpoch(3).setNewIsrWithEpochs(AlterPartitionRequest.newIsrToSimpleNewIsrWithBrokerEpochs(asList(1, 2)));
        ...
        AlterPartitionRequestData data = new AlterPartitionRequestData().setBrokerEpoch(123L).setBrokerId(1).setTopics(singletonList(new AlterPartitionRequestData.TopicData().setTopicName("topic1").setTopicId(Uuid.randomUuid()).setPartitions(singletonList(partitionData))));
    ...
    private AlterPartitionResponse createAlterPartitionResponse(int version) {
        AlterPartitionResponseData.PartitionData partitionData = new AlterPartitionResponseData.PartitionData().setPartitionEpoch(1).setIsr(asList(0, 1, 2)).setErrorCode(Errors.NONE.code()).setLeaderEpoch(2).setLeaderId(3);
        ...
        AlterPartitionResponseData data = new AlterPartitionResponseData().setErrorCode(Errors.NONE.code()).setThrottleTimeMs(123).setTopics(singletonList(new AlterPartitionResponseData.TopicData().setTopicName("topic1").setTopicId(Uuid.randomUuid()).setPartitions(singletonList(partitionData))));
    ...
    private DescribeLogDirsResponse createDescribeLogDirsResponse() {
        DescribeLogDirsResponseData data = new DescribeLogDirsResponseData().setResults(singletonList(new DescribeLogDirsResponseData.DescribeLogDirsResult().setErrorCode(Errors.NONE.code()).setLogDir("logdir").setTopics(singletonList(new DescribeLogDirsResponseData.DescribeLogDirsTopic().setName("topic1").setPartitions(singletonList(new DescribeLogDirsResponseData.DescribeLogDirsPartition().setPartitionIndex(0).setIsFutureKey(false).setOffsetLag(123L).setPartitionSize(234L))))))).setThrottleTimeMs(123);
        return new DescribeLogDirsResponse(data);
    }

    private DeleteRecordsRequest createDeleteRecordsRequest(short version) {
        DeleteRecordsRequestData.DeleteRecordsTopic topic = new DeleteRecordsRequestData.DeleteRecordsTopic().setName("topic1").setPartitions(singletonList(new DeleteRecordsRequestData.DeleteRecordsPartition().setPartitionIndex(1).setOffset(123L)));
        DeleteRecordsRequestData data = new DeleteRecordsRequestData().setTopics(singletonList(topic)).setTimeoutMs(123);
    ...
    private DeleteRecordsResponse createDeleteRecordsResponse() {
        ...
        partitions.add(new DeleteRecordsResponseData.DeleteRecordsPartitionResult().setErrorCode(Errors.NONE.code()).setLowWatermark(123L).setPartitionIndex(0));
        topics.add(new DeleteRecordsResponseData.DeleteRecordsTopicResult().setName("topic1").setPartitions(partitions));
    ...
    private DescribeClusterResponse createDescribeClusterResponse() {
        return new DescribeClusterResponse(new DescribeClusterResponseData().setBrokers(new DescribeClusterBrokerCollection(singletonList(new DescribeClusterBroker().setBrokerId(1).setHost("localhost").setPort(9092).setRack("rack1")).iterator())).setClusterId("clusterId").setControllerId(1).setClusterAuthorizedOperations(10));
    }
    ...
    private void checkDescribeConfigsResponseVersions() {
        for (short version : DESCRIBE_CONFIGS.allVersions()) {
            ...
            DescribeConfigsResponse deserialized = (DescribeConfigsResponse) AbstractResponse.parseResponse(DESCRIBE_CONFIGS, response.serialize(version), version);
    ...
    private FindCoordinatorRequest createFindCoordinatorRequest(short version) {
        return new FindCoordinatorRequest.Builder(new FindCoordinatorRequestData().setKeyType(CoordinatorType.GROUP.id()).setKey("test-group")).build(version);
    }

    private FindCoordinatorRequest createBatchedFindCoordinatorRequest(List<String> coordinatorKeys, short version) {
        return new FindCoordinatorRequest.Builder(new FindCoordinatorRequestData().setKeyType(CoordinatorType.GROUP.id()).setCoordinatorKeys(coordinatorKeys)).build(version);
    }
    ...
    private FetchRequest createFetchRequest(short version, FetchMetadata metadata, List<TopicIdPartition> toForget) {
        ...
        fetchData.put(new TopicPartition("test1", 0), new FetchRequest.PartitionData(Uuid.randomUuid(), 100, -1L, 1000000, Optional.empty()));
        fetchData.put(new TopicPartition("test2", 0), new FetchRequest.PartitionData(Uuid.randomUuid(), 200, -1L, 1000000, Optional.empty()));
        return FetchRequest.Builder.forConsumer(version, 100, 100000, fetchData).metadata(metadata).setMaxBytes(1000).removed(toForget).build(version);
    }
    ...
    private FetchRequest createFetchRequest(short version, IsolationLevel isolationLevel) {
        ...
        fetchData.put(new TopicPartition("test1", 0), new FetchRequest.PartitionData(Uuid.randomUuid(), 100, -1L, 1000000, Optional.empty()));
        fetchData.put(new TopicPartition("test2", 0), new FetchRequest.PartitionData(Uuid.randomUuid(), 200, -1L, 1000000, Optional.empty()));
        return FetchRequest.Builder.forConsumer(version, 100, 100000, fetchData).isolationLevel(isolationLevel).setMaxBytes(1000).build(version);
    }
    ...
    private FetchResponse createFetchResponse(Errors error, int sessionId) {
        return FetchResponse.parse(FetchResponse.of(error, 25, sessionId, new LinkedHashMap<>()).serialize(FETCH.latestVersion()), FETCH.latestVersion());
    }
    ...
    private JoinGroupResponse createJoinGroupResponse(short version) {
        ...
        for (int i = 0; i < 2; i++) {
            JoinGroupResponseMember member = new JoinGroupResponseData.JoinGroupResponseMember().setMemberId("consumer" + i).setMetadata(new byte[0]).setGroupInstanceId("instance" + i);
    ...
    private SyncGroupRequest createSyncGroupRequest(short version) {
        List<SyncGroupRequestAssignment> assignments = singletonList(new SyncGroupRequestAssignment().setMemberId("member").setAssignment(new byte[0]));
    ...
    private DescribeGroupsRequest createDescribeGroupRequest(short version) {
        return new DescribeGroupsRequest.Builder(new DescribeGroupsRequestData().setGroups(singletonList("test-group"))).build(version);
    }
    ...
    private DescribeGroupsResponse createDescribeGroupResponse() {
        ...
        DescribedGroup metadata = DescribeGroupsResponse.groupMetadata("test-group", Errors.NONE, "STABLE", "consumer", "roundrobin", singletonList(member), DescribeGroupsResponse.AUTHORIZED_OPERATIONS_OMITTED);
    ...
    private DeleteGroupsRequest createDeleteGroupsRequest(short version) {
        return new DeleteGroupsRequest.Builder(new DeleteGroupsRequestData().setGroupsNames(singletonList("test-group"))).build(version);
    }
    ...
    private ListOffsetsResponse createListOffsetResponse(short version) {
        if (version == 0) {
            ListOffsetsResponseData data = new ListOffsetsResponseData().setTopics(singletonList(new ListOffsetsTopicResponse().setName("test").setPartitions(singletonList(new ListOffsetsPartitionResponse().setPartitionIndex(0).setErrorCode(Errors.NONE.code()).setOldStyleOffsets(singletonList(100L))))));
            return new ListOffsetsResponse(data);
        } else if (version >= 1 && version <= LIST_OFFSETS.latestVersion()) {
            ListOffsetsPartitionResponse partition = new ListOffsetsPartitionResponse().setPartitionIndex(0).setErrorCode(Errors.NONE.code()).setTimestamp(10000L).setOffset(100L);
            if (version >= 4) {
                partition.setLeaderEpoch(27);
            }
            ListOffsetsResponseData data = new ListOffsetsResponseData().setTopics(singletonList(new ListOffsetsTopicResponse().setName("test").setPartitions(singletonList(partition))));
            return new ListOffsetsResponse(data);
        } else {
    ...
    private MetadataResponse createMetadataResponse() {
        ...
        allTopicMetadata.add(new MetadataResponse.TopicMetadata(Errors.NONE, "__consumer_offsets", true, singletonList(new MetadataResponse.PartitionMetadata(Errors.NONE, new TopicPartition("__consumer_offsets", 1), Optional.of(node.id()), Optional.of(5), replicas, isr, offlineReplicas))));
        allTopicMetadata.add(new MetadataResponse.TopicMetadata(Errors.LEADER_NOT_AVAILABLE, "topic2", false, emptyList()));
        allTopicMetadata.add(new MetadataResponse.TopicMetadata(Errors.NONE, "topic3", false, singletonList(new MetadataResponse.PartitionMetadata(Errors.LEADER_NOT_AVAILABLE, new TopicPartition("topic3", 0), Optional.empty(), Optional.empty(), replicas, isr, offlineReplicas))));
    ...
    private OffsetCommitRequest createOffsetCommitRequest(short version) {
        return new OffsetCommitRequest.Builder(new OffsetCommitRequestData().setGroupId("group1").setMemberId("consumer1").setGroupInstanceId(null).setGenerationId(100).setTopics(singletonList(new OffsetCommitRequestData.OffsetCommitRequestTopic().setName("test").setPartitions(asList(new OffsetCommitRequestData.OffsetCommitRequestPartition().setPartitionIndex(0).setCommittedOffset(100).setCommittedLeaderEpoch(RecordBatch.NO_PARTITION_LEADER_EPOCH).setCommittedMetadata(""), new OffsetCommitRequestData.OffsetCommitRequestPartition().setPartitionIndex(1).setCommittedOffset(200).setCommittedLeaderEpoch(RecordBatch.NO_PARTITION_LEADER_EPOCH).setCommittedMetadata(null)))))).build(version);
    }

    private OffsetCommitResponse createOffsetCommitResponse() {
        return new OffsetCommitResponse(new OffsetCommitResponseData().setTopics(singletonList(new OffsetCommitResponseData.OffsetCommitResponseTopic().setName("test").setPartitions(singletonList(new OffsetCommitResponseData.OffsetCommitResponsePartition().setPartitionIndex(0).setErrorCode(Errors.NONE.code()))))));
    }

    private OffsetFetchRequest createOffsetFetchRequest(short version, boolean requireStable) {
        if (version < 8) {
            return new OffsetFetchRequest.Builder("group1", requireStable, singletonList(new TopicPartition("test11", 1)), false).build(version);
        }
        return new OffsetFetchRequest.Builder(Collections.singletonMap("group1", singletonList(new TopicPartition("test11", 1))), requireStable, false).build(version);
    }
    ...
    private OffsetFetchRequest createOffsetFetchRequestWithMultipleGroups(short version, boolean requireStable) {
        ...
        List<TopicPartition> topic1 = singletonList(new TopicPartition("topic1", 0));
        List<TopicPartition> topic2 = asList(new TopicPartition("topic1", 0), new TopicPartition("topic2", 0), new TopicPartition("topic2", 1));
        List<TopicPartition> topic3 = asList(new TopicPartition("topic1", 0), new TopicPartition("topic2", 0), new TopicPartition("topic2", 1), new TopicPartition("topic3", 0), new TopicPartition("topic3", 1), new TopicPartition("topic3", 2));
    ...
    private ProduceResponse createProduceResponseWithErrorMessage() {
        Map<TopicPartition, ProduceResponse.PartitionResponse> responseData = new HashMap<>();
        responseData.put(new TopicPartition("test", 0), new ProduceResponse.PartitionResponse(Errors.NONE, 10000, RecordBatch.NO_TIMESTAMP, 100, singletonList(new ProduceResponse.RecordError(0, "error message")), "global error message"));
        return new ProduceResponse(responseData, 0);
    }
    ...
    private ControlledShutdownRequest createControlledShutdownRequest(short version) {
        ...
        return new ControlledShutdownRequest.Builder(data, CONTROLLED_SHUTDOWN.latestVersion()).build(version);
    }
    ...
    private LeaderAndIsrRequest createLeaderAndIsrRequest(short version) {
        ...
        partitionStates.add(new LeaderAndIsrPartitionState().setTopicName("topic5").setPartitionIndex(105).setControllerEpoch(0).setLeader(2).setLeaderEpoch(1).setIsr(isr).setPartitionEpoch(2).setReplicas(replicas).setIsNew(false));
        partitionStates.add(new LeaderAndIsrPartitionState().setTopicName("topic5").setPartitionIndex(1).setControllerEpoch(1).setLeader(1).setLeaderEpoch(1).setIsr(isr).setPartitionEpoch(2).setReplicas(replicas).setIsNew(false));
        partitionStates.add(new LeaderAndIsrPartitionState().setTopicName("topic20").setPartitionIndex(1).setControllerEpoch(1).setLeader(0).setLeaderEpoch(1).setIsr(isr).setPartitionEpoch(2).setReplicas(replicas).setIsNew(false));
    ...
    private SaslHandshakeResponse createSaslHandshakeResponse() {
        return new SaslHandshakeResponse(new SaslHandshakeResponseData().setErrorCode(Errors.NONE.code()).setMechanisms(singletonList("GSSAPI")));
    }
    ...
    private DeleteTopicsRequest createDeleteTopicsRequest(short version) {
        return new DeleteTopicsRequest.Builder(new DeleteTopicsRequestData().setTopicNames(asList("my_t1", "my_t2")).setTimeoutMs(1000)).build(version);
    }
    ...
    private AddPartitionsToTxnRequest createAddPartitionsToTxnRequest(short version) {
        if (version < 4) {
            return AddPartitionsToTxnRequest.Builder.forClient("tid", 21L, (short) 42, singletonList(new TopicPartition("topic", 73))).build(version);
        } else {
            AddPartitionsToTxnTransactionCollection transactions = new AddPartitionsToTxnTransactionCollection(singletonList(new AddPartitionsToTxnTransaction().setTransactionalId("tid").setProducerId(21L).setProducerEpoch((short) 42).setVerifyOnly(false).setTopics(new AddPartitionsToTxnTopicCollection(singletonList(new AddPartitionsToTxnTopic().setName("topic").setPartitions(Collections.singletonList(73))).iterator()))).iterator());
    ...
    private AddPartitionsToTxnResponse createAddPartitionsToTxnResponse(short version) {
        ...
        AddPartitionsToTxnResponseData.AddPartitionsToTxnResult result = AddPartitionsToTxnResponse.resultForTransaction(txnId, Collections.singletonMap(new TopicPartition("t", 0), Errors.NONE));
    ...
    private TxnOffsetCommitRequest createTxnOffsetCommitRequest(short version) {
        ...
        if (version < 3) {
            ...
            return new TxnOffsetCommitRequest.Builder("transactionalId", "groupId", 21L, (short) 42, offsets, "member", 2, Optional.of("instance")).build(version);
        }
    }
    ...
    private TxnOffsetCommitRequest createTxnOffsetCommitRequestWithAutoDowngrade() {
        ...
        return new TxnOffsetCommitRequest.Builder("transactionalId", "groupId", 21L, (short) 42, offsets, "member", 2, Optional.of("instance")).build();
    }
    ...
    private DescribeAclsResponse createDescribeAclsResponse() {
        DescribeAclsResponseData data = new DescribeAclsResponseData().setErrorCode(Errors.NONE.code()).setErrorMessage(Errors.NONE.message()).setThrottleTimeMs(0).setResources(singletonList(new DescribeAclsResource().setResourceType(ResourceType.TOPIC.code()).setResourceName("mytopic").setPatternType(PatternType.LITERAL.code()).setAcls(singletonList(new AclDescription().setHost("*").setOperation(AclOperation.WRITE.code()).setPermissionType(AclPermissionType.ALLOW.code()).setPrincipal("User:ANONYMOUS")))));
    ...
    private CreateAclsResponse createCreateAclsResponse() {
        return new CreateAclsResponse(new CreateAclsResponseData().setResults(asList(new CreateAclsResponseData.AclCreationResult(), new CreateAclsResponseData.AclCreationResult().setErrorCode(Errors.NONE.code()).setErrorMessage("Foo bar"))));
    }
    ...
    private AlterPartitionReassignmentsRequest createAlterPartitionReassignmentsRequest(short version) {
        ...
        data.topics().add(new AlterPartitionReassignmentsRequestData.ReassignableTopic().setName("topic").setPartitions(singletonList(new AlterPartitionReassignmentsRequestData.ReassignablePartition().setPartitionIndex(0).setReplicas(null))));
    ...
    private AlterPartitionReassignmentsResponse createAlterPartitionReassignmentsResponse() {
        ...
        data.responses().add(new AlterPartitionReassignmentsResponseData.ReassignableTopicResponse().setName("topic").setPartitions(singletonList(new AlterPartitionReassignmentsResponseData.ReassignablePartitionResponse().setPartitionIndex(0).setErrorCode(Errors.NONE.code()).setErrorMessage("No reassignment is in progress for topic topic partition 0"))));
    ...
    private DescribeClientQuotasResponse createDescribeClientQuotasResponse() {
        DescribeClientQuotasResponseData data = new DescribeClientQuotasResponseData().setEntries(singletonList(new DescribeClientQuotasResponseData.EntryData().setEntity(singletonList(new DescribeClientQuotasResponseData.EntityData().setEntityType(ClientQuotaEntity.USER).setEntityName("user"))).setValues(singletonList(new DescribeClientQuotasResponseData.ValueData().setKey("request_percentage").setValue(1.0)))));
    ...
    private AlterClientQuotasResponse createAlterClientQuotasResponse() {
        AlterClientQuotasResponseData data = new AlterClientQuotasResponseData().setEntries(singletonList(new AlterClientQuotasResponseData.EntryData().setEntity(singletonList(new AlterClientQuotasResponseData.EntityData().setEntityType(ClientQuotaEntity.USER).setEntityName("user")))));
    ...
    private BrokerHeartbeatRequest createBrokerHeartbeatRequest(short v) {
        BrokerHeartbeatRequestData data = new BrokerHeartbeatRequestData().setBrokerId(1).setBrokerEpoch(1).setCurrentMetadataOffset(1).setWantFence(false).setWantShutDown(false);
    ...
    private BrokerRegistrationResponse createBrokerRegistrationResponse() {
        BrokerRegistrationResponseData data = new BrokerRegistrationResponseData().setBrokerEpoch(1).setThrottleTimeMs(0);
    ...
    private DescribeTransactionsRequest createDescribeTransactionsRequest(short version) {
        DescribeTransactionsRequestData data = new DescribeTransactionsRequestData().setTransactionalIds(asList("t1", "t2", "t3"));
    ...
    private ListTransactionsRequest createListTransactionsRequest(short version) {
        return new ListTransactionsRequest.Builder(new ListTransactionsRequestData().setStateFilters(singletonList("Ongoing")).setProducerIdFilters(asList(1L, 2L, 15L))).build(version);
    }
    ...
    public void testInvalidSaslHandShakeRequest() {
        ...
        String msg = assertThrows(RuntimeException.class, () -> AbstractRequest.parseRequest(request.apiKey(), request.version(), serializedBytes)).getMessage();
    ...
    public void testInvalidSaslAuthenticateRequest() {
        ...
        byte[] b = new byte[]{0x11, 0x1f, 0x15, 0x2c, 0x5e, 0x2a, 0x20, 0x26, 0x6c, 0x39, 0x45, 0x1f, 0x25, 0x1c, 0x2d, 0x25, 0x43, 0x2a, 0x11, 0x76};
    ...
    public void testValidTaggedFieldsWithSaslAuthenticateRequest() {
        ...
        SaslAuthenticateRequest saslAuthenticateRequest = (SaslAuthenticateRequest) AbstractRequest.parseRequest(SASL_AUTHENTICATE, SASL_AUTHENTICATE.latestVersion(), accessor.buffer()).request;
    ...
    public void testInvalidTaggedFieldsWithSaslAuthenticateRequest() {
        ...
        String msg = assertThrows(RuntimeException.class, () -> AbstractRequest.parseRequest(SASL_AUTHENTICATE, SASL_AUTHENTICATE.latestVersion(), accessor.buffer())).getMessage();
