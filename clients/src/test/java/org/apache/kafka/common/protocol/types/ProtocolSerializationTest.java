...
public class ProtocolSerializationTest {
    ...
    public void testReadTaggedFieldsSizeTooLarge() {
        ...
        assertEquals("Error reading field of size " + Integer.MAX_VALUE + ", only " + expectedRemaining + " bytes available", e.getMessage());
    }
    ...
    public void testReadWhenOptionalDataMissingAtTheEndIsTolerated() {
        ...
        Schema newSchema = new Schema(
                ...
                new Field("field2", Type.NULLABLE_STRING, "", true, "default"), new Field("field3", Type.NULLABLE_STRING, "", true, null), new Field("field4", Type.NULLABLE_BYTES, "", true, ByteBuffer.allocate(0)), new Field("field5", Type.INT64, "doc", true, Long.MAX_VALUE));
        String value = "foo bar baz";
    ...
    public void testReadWithMissingNonOptionalExtraDataAtTheEnd() {
        Schema oldSchema = new Schema(new Field("field1", Type.NULLABLE_STRING));
        Schema newSchema = new Schema(
                true, new Field("field1", Type.NULLABLE_STRING), new Field("field2", Type.NULLABLE_STRING));
        String value = "foo bar baz";
        Struct oldFormat = new Struct(oldSchema).set("field1", value);
        ByteBuffer buffer = ByteBuffer.allocate(oldSchema.sizeOf(oldFormat));
        oldFormat.writeTo(buffer);
    ...
    public void testReadBytesBeyondItsSize() {
        Type[] types = new Type[]{Type.BYTES, Type.COMPACT_BYTES, Type.NULLABLE_BYTES, Type.COMPACT_NULLABLE_BYTES};
        for (Type type : types) {
