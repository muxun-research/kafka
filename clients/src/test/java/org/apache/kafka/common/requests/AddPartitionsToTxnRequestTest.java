...
public class AddPartitionsToTxnRequestTest {
    ...
    public void testBatchedRequests() {
        ...
        AddPartitionsToTxnResponse response = new AddPartitionsToTxnResponse(new AddPartitionsToTxnResponseData().setResultsByTransaction(results).setThrottleTimeMs(throttleTimeMs));
    ...
    private AddPartitionsToTxnTransactionCollection createTwoTransactionCollection() {
        ...
        topics0.add(new AddPartitionsToTxnTopic().setName(tp0.topic()).setPartitions(Collections.singletonList(tp0.partition())));
        AddPartitionsToTxnTopicCollection topics1 = new AddPartitionsToTxnTopicCollection();
        topics1.add(new AddPartitionsToTxnTopic().setName(tp1.topic()).setPartitions(Collections.singletonList(tp1.partition())));

        AddPartitionsToTxnTransactionCollection transactions = new AddPartitionsToTxnTransactionCollection();
        transactions.add(new AddPartitionsToTxnTransaction().setTransactionalId(transactionalId1).setProducerId(producerId).setProducerEpoch(producerEpoch).setVerifyOnly(true).setTopics(topics0));
        transactions.add(new AddPartitionsToTxnTransaction().setTransactionalId(transactionalId2).setProducerId(producerId + 1).setProducerEpoch((short) (producerEpoch + 1)).setVerifyOnly(false).setTopics(topics1));
        return transactions;
    }
}
