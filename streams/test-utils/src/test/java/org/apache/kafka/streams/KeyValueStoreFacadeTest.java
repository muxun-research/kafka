...
public class KeyValueStoreFacadeTest {
    ...
    public void shouldPutWithUnknownTimestamp() {
        ...
        verify(mockedKeyValueTimestampStore).put("key", ValueAndTimestamp.make("value", ConsumerRecord.NO_TIMESTAMP));
    }

    @Test
    public void shouldPutIfAbsentWithUnknownTimestamp() {
        doReturn(null, ValueAndTimestamp.make("oldValue", 42L)).when(mockedKeyValueTimestampStore).putIfAbsent("key", ValueAndTimestamp.make("value", ConsumerRecord.NO_TIMESTAMP));

        assertNull(keyValueStoreFacade.putIfAbsent("key", "value"));
        assertThat(keyValueStoreFacade.putIfAbsent("key", "value"), is("oldValue"));
        verify(mockedKeyValueTimestampStore, times(2)).putIfAbsent("key", ValueAndTimestamp.make("value", ConsumerRecord.NO_TIMESTAMP));
    }

    @Test
    public void shouldPutAllWithUnknownTimestamp() {
        keyValueStoreFacade.putAll(asList(KeyValue.pair("key1", "value1"), KeyValue.pair("key2", "value2")));
        verify(mockedKeyValueTimestampStore).put("key1", ValueAndTimestamp.make("value1", ConsumerRecord.NO_TIMESTAMP));
        verify(mockedKeyValueTimestampStore).put("key2", ValueAndTimestamp.make("value2", ConsumerRecord.NO_TIMESTAMP));
    }

    @Test
    public void shouldDeleteAndReturnPlainValue() {
        doReturn(null, ValueAndTimestamp.make("oldValue", 42L)).when(mockedKeyValueTimestampStore).delete("key");
    ...
    public void shouldReturnIsPersistent() {
        when(mockedKeyValueTimestampStore.persistent()).thenReturn(true, false);
    ...
    public void shouldReturnIsOpen() {
        when(mockedKeyValueTimestampStore.isOpen()).thenReturn(true, false);
