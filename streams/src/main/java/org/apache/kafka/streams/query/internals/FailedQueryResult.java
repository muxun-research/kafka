...
public final class FailedQueryResult<R> extends AbstractQueryResult<R> implements QueryResult<R> {
     ...
     */
    public R getResult() {
        throw new IllegalArgumentException("Cannot get result for failed query. Failure is " + failureReason.name() + ": " + failure);
    }

    @Override
    public String toString() {
        return "FailedQueryResult{" + "failureReason=" + failureReason + ", failure='" + failure + '\'' + ", executionInfo=" + getExecutionInfo() + ", position=" + getPosition() + '}';
    }
}
