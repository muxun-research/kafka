...
public interface QueryResult<R> {
     ...
     */
    static <R> QueryResult<R> forFailure(final FailureReason failureReason, final String failureMessage) {
     ...
     */
    static <R> QueryResult<R> forUnknownQueryType(final Query<R> query, final StateStore store) {

        return forFailure(FailureReason.UNKNOWN_QUERY_TYPE, "This store (" + store.getClass() + ") doesn't know how to execute " + "the given query (" + query + ")." + " Contact the store maintainer if you need support for a new query type.");
    }
     ...
     * Used by StateStores to respond to a {@link StateStore#query(Query, PositionBound, QueryConfig)}.
     */
    static <R> QueryResult<R> notUpToBound(final Position currentPosition, final PositionBound positionBound, final Integer partition) {
