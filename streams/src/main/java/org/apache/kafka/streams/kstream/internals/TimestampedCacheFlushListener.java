...
class TimestampedCacheFlushListener<KOut, VOut> implements CacheFlushListener<KOut, ValueAndTimestamp<VOut>> {
    ...
    public void apply(final Record<KOut, Change<ValueAndTimestamp<VOut>>> record) {
        ...
        try {
            context.forward(record.withValue(new Change<>(getValueOrNull(record.value().newValue), getValueOrNull(record.value().oldValue), record.value().isLatest)).withTimestamp(record.value().newValue != null ? record.value().newValue.timestamp() : record.timestamp()));
        } finally {
