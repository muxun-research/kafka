...
public abstract class AbstractTask implements Task {
    ...
    AbstractTask(final TaskId id, final ProcessorTopology topology, final StateDirectory stateDirectory, final ProcessorStateManager stateMgr, final Set<TopicPartition> inputPartitions, final TaskConfig config, final String taskType, final Class<? extends AbstractTask> clazz) {
        this.id = id;
    ...
    public void maybeInitTaskTimeoutOrThrow(final long currentWallClockMs, final Exception cause) {
        if (deadlineMs == NO_DEADLINE) {
            ...
            final String errorMessage = String.format("Task %s did not make progress within %d ms. Adjust `%s` if needed.", id, currentWallClockMs - deadlineMs + config.taskTimeoutMs, StreamsConfig.TASK_TIMEOUT_MS_CONFIG);
        ...
        if (cause != null) {
            log.debug(String.format("Task did not make progress. Remaining time to deadline %d; retrying.", deadlineMs - currentWallClockMs), cause);
        } else {
            log.debug("Task did not make progress. Remaining time to deadline {}; retrying.", deadlineMs - currentWallClockMs);
        }
