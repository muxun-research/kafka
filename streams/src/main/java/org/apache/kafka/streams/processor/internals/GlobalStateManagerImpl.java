...
public class GlobalStateManagerImpl implements GlobalStateManager {
    ...
    public GlobalStateManagerImpl(final LogContext logContext, final Time time, final ProcessorTopology topology, final Consumer<byte[], byte[]> globalConsumer, final StateDirectory stateDirectory, final StateRestoreListener stateRestoreListener, final StreamsConfig config) {
        this.time = time;
        ...
        final int requestTimeoutMs = new ClientUtils.QuietConsumerConfig(consumerProps).getInt(ConsumerConfig.REQUEST_TIMEOUT_MS_CONFIG);
        pollMsPlusRequestTimeout = Duration.ofMillis(config.getLong(StreamsConfig.POLL_MS_CONFIG) + requestTimeoutMs);
    ...
    public Set<String> initialize() {
        ...
        checkpointFileCache.keySet().forEach(tp -> {
            if (!changelogTopics.contains(tp.topic())) {
                log.error("Encountered a topic-partition in the global checkpoint file not associated with any global" + " state store, topic-partition: {}, checkpoint file: {}. If this topic-partition is no longer valid," + " an application reset and state store directory cleanup will be required.", tp.topic(), checkpointFile);
    ...
    @Override
    public void registerStore(final StateStore store, final StateRestoreCallback stateRestoreCallback, final CommitCallback ignored) {
        ...
        final Map<TopicPartition, Long> highWatermarks = retryUntilSuccessOrThrowOnTaskTimeout(() -> globalConsumer.endOffsets(topicPartitions), String.format("Failed to get offsets for partitions %s. The broker may be transiently unavailable at the moment.", topicPartitions));

        try {
            restoreState(stateRestoreCallback, topicPartitions, highWatermarks, store.name(), converterForStore(store));
        } finally {
    ...
    private List<TopicPartition> topicPartitionsForStore(final StateStore store) {
        ...
        final List<PartitionInfo> partitionInfos = retryUntilSuccessOrThrowOnTaskTimeout(() -> globalConsumer.partitionsFor(sourceTopic), String.format("Failed to get partitions for topic %s. The broker may be transiently unavailable at the moment.", sourceTopic));
    ...
    private void restoreState(final StateRestoreCallback stateRestoreCallback,
        ...
        for (final TopicPartition topicPartition : topicPartitions) {
            ...
            final RecordBatchingStateRestoreCallback stateRestoreAdapter = StateRestoreCallbackAdapter.adapt(stateRestoreCallback);
    ...
    private long getGlobalConsumerOffset(final TopicPartition topicPartition) {
        return retryUntilSuccessOrThrowOnTaskTimeout(() -> globalConsumer.position(topicPartition), String.format("Failed to get position for partition %s. The broker may be transiently unavailable at the moment.", topicPartition));
    }

    private <R> R retryUntilSuccessOrThrowOnTaskTimeout(final Supplier<R> supplier, final String errorMessage) {
        ...
        do {
            try {
                ...
                if (taskTimeoutMs == 0L) {
                    throw new StreamsException(String.format("Retrying is disabled. You can enable it by setting `%s` to a value larger than zero.", StreamsConfig.TASK_TIMEOUT_MS_CONFIG), retriableException);
                }
    ...
    private long maybeUpdateDeadlineOrThrow(final long currentDeadlineMs) {
        ...
        if (currentDeadlineMs == NO_DEADLINE) {
            ...
            throw new TimeoutException(String.format("Global task did not make progress to restore state within %d ms. Adjust `%s` if needed.", currentWallClockMs - currentDeadlineMs + taskTimeoutMs, StreamsConfig.TASK_TIMEOUT_MS_CONFIG));
        }
        ...
        for (final Map.Entry<String, Optional<StateStore>> entry : globalStores.entrySet()) {
            if (entry.getValue().isPresent()) {
                ...
                try {
                    ...
                    closeFailed.append("Failed to close global state store:").append(entry.getKey()).append(". Reason: ").append(e).append("\n");
                }
    ...
    public void checkpoint() {
        ...
        try {
            ...
            log.warn("Failed to write offset checkpoint file to {} for global stores: {}." + " This may occur if OS cleaned the state.dir in case when it is located in the (default) ${java.io.tmpdir}/kafka-streams directory." + " Changing the location of state.dir may resolve the problem", checkpointFile, e);
        }
    }
