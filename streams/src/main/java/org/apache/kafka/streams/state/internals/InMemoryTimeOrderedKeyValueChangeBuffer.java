...
import static org.apache.kafka.streams.state.internals.TimeOrderedKeyValueBufferChangelogDeserializationHelper.*;
...
public final class InMemoryTimeOrderedKeyValueChangeBuffer<K, V, T> implements TimeOrderedKeyValueBuffer<K, V, Change<V>> {
    ...
    static final RecordHeaders CHANGELOG_HEADERS = new RecordHeaders(new Header[]{new RecordHeader("v", V_3_CHANGELOG_HEADER_VALUE)});
    ...
    private InMemoryTimeOrderedKeyValueChangeBuffer(final String storeName, final boolean loggingEnabled, final Serde<K> keySerde, final Serde<V> valueSerde) {
        this.storeName = storeName;
    ...
    private void init(final StateStore root) {
        ...
        bufferSizeSensor = StateStoreMetrics.suppressionBufferSizeSensor(taskId, METRIC_SCOPE, storeName, streamsMetrics);
        bufferCountSensor = StateStoreMetrics.suppressionBufferCountSensor(taskId, METRIC_SCOPE, storeName, streamsMetrics);
    ...
    private void logValue(final Bytes key, final BufferKey bufferKey, final BufferValue value) {
        ...
        ((RecordCollector.Supplier) context).recordCollector().send(changelogTopic, key, array, CHANGELOG_HEADERS, partition, null, KEY_SERIALIZER, VALUE_SERIALIZER, null, null);
    }

    private void logTombstone(final Bytes key) {
        ((RecordCollector.Supplier) context).recordCollector().send(changelogTopic, key, null, null, partition, null, KEY_SERIALIZER, VALUE_SERIALIZER, null, null);
    }
    ...
    @Override
    public void evictWhile(final Supplier<Boolean> predicate, final Consumer<Eviction<K, Change<V>>> callback) {
        ...
        if (predicate.get()) {
            ...
            while (next != null && predicate.get()) {
                ...
                final Change<V> value = valueSerde.deserializeParts(changelogTopic, new Change<>(bufferValue.newValue(), bufferValue.oldValue()));
    ...
    @Override
    public boolean put(final long time, final Record<K, Change<V>> record, final ProcessorRecordContext recordContext) {
    ...
    public String toString() {
        return "InMemoryTimeOrderedKeyValueChangeBuffer{" + "storeName='" + storeName + '\'' + ", changelogTopic='" + changelogTopic + '\'' + ", open=" + open + ", loggingEnabled=" + loggingEnabled + ", minTimestamp=" + minTimestamp + ", memBufferSize=" + memBufferSize + ", \n\tdirtyKeys=" + dirtyKeys + ", \n\tindex=" + index + ", \n\tsortedMap=" + sortedMap + '}';
    }
}
