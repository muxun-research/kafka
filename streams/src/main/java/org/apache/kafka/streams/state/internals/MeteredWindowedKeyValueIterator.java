...
class MeteredWindowedKeyValueIterator<K, V> implements KeyValueIterator<Windowed<K>, V> {
    ...
    MeteredWindowedKeyValueIterator(final KeyValueIterator<Windowed<Bytes>, byte[]> iter, final Sensor sensor, final StreamsMetrics metrics, final Function<byte[], K> deserializeKey, final Function<byte[], V> deserializeValue, final Time time) {
        this.iter = iter;
        this.sensor = sensor;
