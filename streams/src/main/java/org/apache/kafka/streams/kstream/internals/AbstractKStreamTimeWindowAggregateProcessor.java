...
public abstract class AbstractKStreamTimeWindowAggregateProcessor<KIn, VIn, VAgg> extends ContextualProcessor<KIn, VIn, Windowed<KIn>, Change<VAgg>> {
    ...
    protected AbstractKStreamTimeWindowAggregateProcessor(final String storeName, final EmitStrategy emitStrategy, final boolean sendOldValues) {
    ...
    public void init(final ProcessorContext<Windowed<KIn>, Change<VAgg>> context) {
        ...
        if (emitStrategy.type() == StrategyType.ON_WINDOW_CLOSE) {
            ...
            tupleForwarder = new TimestampedTupleForwarder<>(windowStore, context, new TimestampedCacheFlushListener<>(context), sendOldValues);
        }
    }

    protected void maybeForwardUpdate(final Record<KIn, VIn> record, final Window window, final VAgg oldAgg, final VAgg newAgg, final long newTimestamp) {
        if (emitStrategy.type() == StrategyType.ON_WINDOW_CLOSE) {
            return;
        }

        tupleForwarder.maybeForward(record.withKey(new Windowed<>(record.key(), window)).withValue(new Change<>(newAgg, sendOldValues ? oldAgg : null)).withTimestamp(newTimestamp));
    }
    ...
    protected void logSkippedRecordForExpiredWindow(final Logger log, final long timestamp, final long windowExpire, final String window) {
        if (context().recordMetadata().isPresent()) {
            final RecordMetadata recordMetadata = context().recordMetadata().get();
            log.warn("Skipping record for expired window. " + "topic=[{}] " + "partition=[{}] " + "offset=[{}] " + "timestamp=[{}] " + "window={} " + "expiration=[{}] " + "streamTime=[{}]", recordMetadata.topic(), recordMetadata.partition(), recordMetadata.offset(), timestamp, window, windowExpire, observedStreamTime);
        } else {
            log.warn("Skipping record for expired window. Topic, partition, and offset not known. " + "timestamp=[{}] " + "window={} " + "expiration=[{}] " + "streamTime=[{}]", timestamp, window, windowExpire, observedStreamTime);
        }
    ...
    private void fetchAndEmit(final Record<KIn, VIn> record, final long windowCloseTime, final long emitRangeLowerBound, final long emitRangeUpperBound) {
        ...
        try (final KeyValueIterator<Windowed<KIn>, ValueAndTimestamp<VAgg>> windowToEmit = windowStore.fetchAll(emitRangeLowerBound, emitRangeUpperBound)) {
            ...
            while (windowToEmit.hasNext()) {
                ...
                tupleForwarder.maybeForward(record.withKey(kv.key).withValue(new Change<>(kv.value.value(), null)).withTimestamp(kv.value.timestamp()).withHeaders(record.headers()));
            }
