...
class MeteredWindowStoreIterator<V> implements WindowStoreIterator<V> {
    ...
    MeteredWindowStoreIterator(final WindowStoreIterator<byte[]> iter, final Sensor sensor, final StreamsMetrics metrics, final Function<byte[], V> valueFrom, final Time time) {
        this.iter = iter;
        this.sensor = sensor;
