...
public class ListValueStoreBuilder<K, V> extends AbstractStoreBuilder<K, V, KeyValueStore<K, V>> {
    ...
    public ListValueStoreBuilder(final KeyValueBytesStoreSupplier storeSupplier, final Serde<K> keySerde, final Serde<V> valueSerde, final Time time) {
    ...
    public KeyValueStore<K, V> build() {
        return new MeteredKeyValueStore<>(maybeWrapCaching(maybeWrapLogging(new ListValueStore(storeSupplier.get()))), storeSupplier.metricsScope(), time, keySerde, valueSerde);
    }
