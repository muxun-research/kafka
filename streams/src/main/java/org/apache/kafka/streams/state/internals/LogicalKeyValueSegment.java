...
class LogicalKeyValueSegment implements Comparable<LogicalKeyValueSegment>, Segment, VersionedStoreSegment {
    ...
    public synchronized void deleteRange(final Bytes keyFrom, final Bytes keyTo) {
        physicalStore.deleteRange(prefixKeyFormatter.addPrefix(keyFrom), prefixKeyFormatter.addPrefix(keyTo));
    }

    @Override
    public synchronized void put(final Bytes key, final byte[] value) {
        physicalStore.put(prefixKeyFormatter.addPrefix(key), value);
    }

    @Override
    public synchronized byte[] putIfAbsent(final Bytes key, final byte[] value) {
        return physicalStore.putIfAbsent(prefixKeyFormatter.addPrefix(key), value);
    }

    @Override
    public synchronized void putAll(final List<KeyValue<Bytes, byte[]>> entries) {
        physicalStore.putAll(entries.stream().map(kv -> new KeyValue<>(prefixKeyFormatter.addPrefix(kv.key), kv.value)).collect(Collectors.toList()));
    }
    ...
    public synchronized KeyValueIterator<Bytes, byte[]> range(final Bytes from, final Bytes to) {
        ...
        final Bytes fromBound = from == null ? prefixKeyFormatter.getPrefix() : prefixKeyFormatter.addPrefix(from);
        ...
        final Bytes toBound = to == null ? incrementWithoutOverflow(prefixKeyFormatter.getPrefix()) : prefixKeyFormatter.addPrefix(to);
        final KeyValueIterator<Bytes, byte[]> iteratorWithKeyPrefixes = physicalStore.range(fromBound, toBound, openIterators);
        return new StrippedPrefixKeyValueIteratorAdapter(iteratorWithKeyPrefixes, prefixKeyFormatter::removePrefix, prefixKeyFormatter::startsWithPrefix);
    }

    @Override
    public synchronized KeyValueIterator<Bytes, byte[]> all() {
        final KeyValueIterator<Bytes, byte[]> iteratorWithKeyPrefixes = physicalStore.prefixScan(prefixKeyFormatter.getPrefix(), new BytesSerializer(), openIterators);
        return new StrippedPrefixKeyValueIteratorAdapter(iteratorWithKeyPrefixes, prefixKeyFormatter::removePrefix);
    }
    ...
    public void addToBatch(final KeyValue<byte[], byte[]> record, final WriteBatch batch) throws RocksDBException {
        physicalStore.addToBatch(new KeyValue<>(prefixKeyFormatter.addPrefix(record.key), record.value), batch);
    }
    ...
    private static class StrippedPrefixKeyValueIteratorAdapter implements KeyValueIterator<Bytes, byte[]> {
        ...
        StrippedPrefixKeyValueIteratorAdapter(final KeyValueIterator<Bytes, byte[]> iteratorWithKeyPrefixes, final Function<Bytes, Bytes> prefixRemover) {
            this(iteratorWithKeyPrefixes, prefixRemover, bytes -> true);
        }

        StrippedPrefixKeyValueIteratorAdapter(final KeyValueIterator<Bytes, byte[]> iteratorWithKeyPrefixes, final Function<Bytes, Bytes> prefixRemover, final Function<Bytes, Boolean> prefixChecker) {
        ...
        public KeyValue<Bytes, byte[]> next() {
            ...
            final KeyValue<Bytes, byte[]> next = new KeyValue<>(prefixRemover.apply(nextWithKeyPrefix.key), nextWithKeyPrefix.value);
            pruneNonPrefixedElements();
            return next;
        }
        ...
        private void pruneNonPrefixedElements() {
            while (iteratorWithKeyPrefixes.hasNext() && !prefixChecker.apply(iteratorWithKeyPrefixes.peekNextKey())) {
