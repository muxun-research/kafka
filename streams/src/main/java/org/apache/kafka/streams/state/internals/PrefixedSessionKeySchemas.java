...
public class PrefixedSessionKeySchemas {
    ...
    public static class TimeFirstSessionKeySchema implements KeySchema {
        ...
        public Bytes upperRange(final Bytes key, final long to) {
            ...
            return Bytes.wrap(ByteBuffer.allocate(PREFIX_SIZE + 2 * TIMESTAMP_SIZE + key.get().length).put(TIME_FIRST_PREFIX)
                    // the end timestamp can be as large as possible as long as it's larger than start time
                    .putLong(Long.MAX_VALUE)
                    // this is the start timestamp
                    .putLong(to).put(key.get()).array());
        }
        ...
        public Bytes lowerRange(final Bytes key, final long from) {
            ...
            return Bytes.wrap(ByteBuffer.allocate(PREFIX_SIZE + 2 * TIMESTAMP_SIZE + key.get().length).put(TIME_FIRST_PREFIX).putLong(from).putLong(0L).put(key.get()).array());
        }
        ...
        @Override
        public HasNextCondition hasNextCondition(final Bytes binaryKeyFrom, final Bytes binaryKeyTo, final long earliestWindowEndTime, final long latestWindowStartTime, final boolean forward) {
            return iterator -> {
        ...
        @Override
        public <S extends Segment> List<S> segmentsToSearch(final Segments<S> segments, final long earliestWindowEndTime, final long latestWindowStartTime, final boolean forward) {
        ...
        public static void writeBinary(final ByteBuffer buf, final Bytes key, final long startTime, final long endTime) {
            buf.putLong(endTime);
        ...
        public static Bytes toBinary(final Bytes key, final long startTime, final long endTime) {
