...
public class Repartitioned<K, V> implements NamedOperation<Repartitioned<K, V>> {
    ...
    private Repartitioned(final String name, final Serde<K> keySerde, final Serde<V> valueSerde, final Integer numberOfPartitions, final StreamPartitioner<K, V> partitioner) {
        this.name = name;
     ...
     * @see KStream#repartition(Repartitioned)
     */
    public static <K, V> Repartitioned<K, V> with(final Serde<K> keySerde, final Serde<V> valueSerde) {
