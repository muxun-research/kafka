...
public class PrefixedWindowKeySchemas {
    ...
    public static class TimeFirstWindowKeySchema implements KeySchema {
        ...
        public Bytes upperRange(final Bytes key, final long to) {
            ...
            return Bytes.wrap(ByteBuffer.allocate(PREFIX_SIZE + TIMESTAMP_SIZE + maxKey.length + SEQNUM_SIZE).put(TIME_FIRST_PREFIX).putLong(to).put(maxKey).putInt(Integer.MAX_VALUE).array());
        }

        @Override
        public Bytes lowerRange(final Bytes key, final long from) {
            if (key == null) {
                return Bytes.wrap(ByteBuffer.allocate(PREFIX_SIZE + TIMESTAMP_SIZE).put(TIME_FIRST_PREFIX).putLong(from).array());
            }
             ...
             */
            return Bytes.wrap(ByteBuffer.allocate(PREFIX_SIZE + TIMESTAMP_SIZE + key.get().length).put(TIME_FIRST_PREFIX).putLong(from).put(key.get()).array());
        }
        ...
        public HasNextCondition hasNextCondition(final Bytes binaryKeyFrom, final Bytes binaryKeyTo, final long from, final long to, final boolean forward) {
            return iterator -> {
                while (iterator.hasNext()) {
                    ...
                    final Bytes keyBytes = Bytes.wrap(TimeFirstWindowKeySchema.extractStoreKeyBytes(bytes.get()));
        ...
        static Bytes toStoreKeyBinary(final byte[] serializedKey, final long timestamp, final int seqnum) {
            final ByteBuffer buf = ByteBuffer.allocate(PREFIX_SIZE + TIMESTAMP_SIZE + serializedKey.length + SEQNUM_SIZE);
        ...
        public static Windowed<Bytes> fromStoreBytesKey(final byte[] binaryKey, final long windowSize) {
            final Bytes key = Bytes.wrap(extractStoreKeyBytes(binaryKey));
            final Window window = extractStoreWindow(binaryKey, windowSize);
            return new Windowed<>(key, window);
        }

        static Window extractStoreWindow(final byte[] binaryKey, final long windowSize) {
    ...
    public static class KeyFirstWindowKeySchema implements KeySchema {
        ...
        public static Bytes toStoreKeyBinary(final byte[] serializedKey, final long timestamp, final int seqnum) {
        ...
        private static Bytes wrapPrefix(final Bytes noPrefixKey, final boolean upperRange) {
            ...
            if (noPrefixKey == null) {
                ...
                final byte[] ret = ByteBuffer.allocate(PREFIX_SIZE).put(prefix).array();
                return Bytes.wrap(ret);
            }
            final byte[] ret = ByteBuffer.allocate(PREFIX_SIZE + noPrefixKey.get().length).put(KEY_FIRST_PREFIX).put(noPrefixKey.get()).array();
            return Bytes.wrap(ret);
        }
