...
class WindowStoreIteratorWrapper {
    ...
    WindowStoreIteratorWrapper(final KeyValueIterator<Bytes, byte[]> bytesIterator, final long windowSize) {
        this(bytesIterator, windowSize, WindowKeySchema::extractStoreTimestamp, WindowKeySchema::fromStoreBytesKey);
    }

    WindowStoreIteratorWrapper(final KeyValueIterator<Bytes, byte[]> bytesIterator, final long windowSize, final Function<byte[], Long> timestampExtractor, final BiFunction<byte[], Long, Windowed<Bytes>> windowConstructor) {
    ...
    private static class WrappedWindowStoreIterator implements WindowStoreIterator<byte[]> {
        ...
        WrappedWindowStoreIterator(final KeyValueIterator<Bytes, byte[]> bytesIterator, final Function<byte[], Long> timestampExtractor) {
            this.bytesIterator = bytesIterator;
            this.timestampExtractor = timestampExtractor;
        }
    ...
    private static class WrappedKeyValueIterator implements KeyValueIterator<Windowed<Bytes>, byte[]> {
        ...
        WrappedKeyValueIterator(final KeyValueIterator<Bytes, byte[]> bytesIterator, final long windowSize, final BiFunction<byte[], Long, Windowed<Bytes>> windowConstructor) {
            this.bytesIterator = bytesIterator;
            this.windowSize = windowSize;
            this.windowConstructor = windowConstructor;
        }
