...
public abstract class AbstractRocksDBTimeOrderedSegmentedBytesStore extends AbstractDualSchemaRocksDBSegmentedBytesStore<KeyValueSegment> {
    ...
    AbstractRocksDBTimeOrderedSegmentedBytesStore(final String name, final String metricsScope, final long retention, final long segmentInterval, final KeySchema baseKeySchema, final Optional<KeySchema> indexKeySchema) {
        super(name, baseKeySchema, indexKeySchema, new KeyValueSegments(name, metricsScope, retention, segmentInterval), retention);
    }
    ...
    KeyValueIterator<Bytes, byte[]> fetch(final Bytes key, final long from, final long to, final boolean forward) {
        ...
        if (indexKeySchema.isPresent()) {
            final List<KeyValueSegment> searchSpace = indexKeySchema.get().segmentsToSearch(segments, actualFrom, to, forward);
            ...
            return getIndexToBaseStoreIterator(new SegmentIterator<>(searchSpace.iterator(), indexKeySchema.get().hasNextCondition(key, key, actualFrom, to, forward), binaryFrom, binaryTo, forward));
        }


        final List<KeyValueSegment> searchSpace = baseKeySchema.segmentsToSearch(segments, actualFrom, to, forward);
        ...
        return new SegmentIterator<>(searchSpace.iterator(), baseKeySchema.hasNextCondition(key, key, actualFrom, to, forward), binaryFrom, binaryTo, forward);
    }
    ...
    KeyValueIterator<Bytes, byte[]> fetch(final Bytes keyFrom, final Bytes keyTo, final long from, final long to, final boolean forward) {
        ...
        if (indexKeySchema.isPresent()) {
            ...
            return getIndexToBaseStoreIterator(new SegmentIterator<>(searchSpace.iterator(), indexKeySchema.get().hasNextCondition(keyFrom, keyTo, actualFrom, to, forward), binaryFrom, binaryTo, forward));
        }
        ...
        return new SegmentIterator<>(searchSpace.iterator(), baseKeySchema.hasNextCondition(keyFrom, keyTo, actualFrom, to, forward), binaryFrom, binaryTo, forward);
    }
    ...
    public KeyValueIterator<Bytes, byte[]> fetchAll(final long timeFrom, final long timeTo) {
        ...
        return new SegmentIterator<>(searchSpace.iterator(), baseKeySchema.hasNextCondition(null, null, actualFrom, timeTo, true), binaryFrom, binaryTo, true);
    }
    ...
    public KeyValueIterator<Bytes, byte[]> backwardFetchAll(final long timeFrom, final long timeTo) {
        ...
        return new SegmentIterator<>(searchSpace.iterator(), baseKeySchema.hasNextCondition(null, null, actualFrom, timeTo, false), binaryFrom, binaryTo, false);
    }
}