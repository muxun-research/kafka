...
public class QueryableStateIntegrationTest {
    ...
    private void verifyOffsetLagFetch(final List<KafkaStreams> streamsList, final Set<String> stores, final List<Integer> partitionsPerStreamsInstance) {
    ...
    private void assertNoKVKeyFailures(final String storeName, final long timeout, final List<String> noMetadataKeys, final List<String> nullStoreKeys, final List<String> nullValueKeys, final Map<String, Exception> exceptionalKeys) throws IOException {
        ...
        assertThat(reason.toString(), noMetadataKeys.isEmpty() && nullStoreKeys.isEmpty() && nullValueKeys.isEmpty() && exceptionalKeys.isEmpty());
    }
    ...
    public void shouldRejectNonExistentStoreName(final TestInfo testInfo) throws InterruptedException {
        ...
        final Properties properties = mkProperties(mkMap(mkEntry(StreamsConfig.APPLICATION_ID_CONFIG, safeUniqueTestName(getClass(), testInfo)), mkEntry(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, CLUSTER.bootstrapServers())));
    ...
    public void shouldBeAbleToQueryDuringRebalance() throws Exception {
        ...
        for (int i = 0; i < numThreads; i++) {
            ...
            final KafkaStreams streams = createCountStream(streamThree, outputTopicThree, outputTopicConcurrentWindowed, storeName, windowStoreName, props);
        ...
        try {
            ...
            for (int i = 0; i < streamsList.size(); i++) {
                verifyAllKVKeys(streamsList, streamsList.get(i), listeners.get(i), inputValuesKeys, storeName + "-" + streamThree, DEFAULT_TIMEOUT_MS, true);
                verifyAllWindowedKeys(streamsList, streamsList.get(i), listeners.get(i), inputValuesKeys, windowStoreName + "-" + streamThree, 0L, WINDOW_SIZE, DEFAULT_TIMEOUT_MS, true);
            }
            verifyOffsetLagFetch(streamsList, stores, Arrays.asList(4, 4));
            ...
            for (int i = 1; i < streamsList.size(); i++) {
                ...
                assertThat(String.format("Streams instance %s did not close in %d ms", i, closeTimeout.toMillis()), streamsList.get(i).close(closeTimeout));
            }
            ...
            verifyAllKVKeys(streamsList, streamsList.get(0), listeners.get(0), inputValuesKeys, storeName + "-" + streamThree, DEFAULT_TIMEOUT_MS, true);
            verifyAllWindowedKeys(streamsList, streamsList.get(0), listeners.get(0), inputValuesKeys, windowStoreName + "-" + streamThree, 0L, WINDOW_SIZE, DEFAULT_TIMEOUT_MS, true);
    ...
    public void shouldBeAbleQueryStandbyStateDuringRebalance() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < streamsList.size(); i++) {
                verifyAllKVKeys(streamsList, streamsList.get(i), listeners.get(i), inputValuesKeys, storeName + "-" + streamThree, DEFAULT_TIMEOUT_MS, false);
                verifyAllWindowedKeys(streamsList, streamsList.get(i), listeners.get(i), inputValuesKeys, windowStoreName + "-" + streamThree, 0L, WINDOW_SIZE, DEFAULT_TIMEOUT_MS, false);
            }
            verifyOffsetLagFetch(streamsList, stores, Arrays.asList(8, 8));
            ...
            verifyAllKVKeys(streamsList, streamsList.get(0), listeners.get(0), inputValuesKeys, storeName + "-" + streamThree, DEFAULT_TIMEOUT_MS, false);
            verifyAllWindowedKeys(streamsList, streamsList.get(0), listeners.get(0), inputValuesKeys, windowStoreName + "-" + streamThree, 0L, WINDOW_SIZE, DEFAULT_TIMEOUT_MS, false);
    ...
    public void shouldBeAbleToQueryMapValuesAfterFilterState() throws Exception {
        ...
        final StreamsBuilder builder = new StreamsBuilder();
        final String[] keys = {"hello", "goodbye", "welcome", "go", "kafka"};
        final Set<KeyValue<String, String>> batch1 = new HashSet<>(Arrays.asList(new KeyValue<>(keys[0], "1"),
                new KeyValue<>(keys[1], "1"),
                new KeyValue<>(keys[2], "3"),
                new KeyValue<>(keys[3], "5"),
                new KeyValue<>(keys[4], "2"))
        ...
        IntegrationTestUtils.produceKeyValuesSynchronously(
            streamOne,
            batch1,
            TestUtils.producerConfig(CLUSTER.bootstrapServers(), StringSerializer.class, StringSerializer.class, new Properties()), mockTime);
    ...
    private void verifyCanQueryState(final int cacheSizeBytes) throws Exception {
        ...
        s1.groupByKey().count(Materialized.as(storeName)).toStream().to(outputTopic, Produced.with(Serdes.String(), Serdes.Long()));

        final String windowStoreName = "windowed-count";
        s1.groupByKey().windowedBy(TimeWindows.of(ofMillis(WINDOW_SIZE))).count(Materialized.as(windowStoreName));
        kafkaStreams = new KafkaStreams(builder.build(), streamsConfiguration);
        startApplicationAndWaitUntilRunning(kafkaStreams);

        waitUntilAtLeastNumRecordProcessed(outputTopic, 1);

        final ReadOnlyKeyValueStore<String, Long> myCount = IntegrationTestUtils.getStore(storeName, kafkaStreams, keyValueStore());

        final ReadOnlyWindowStore<String, Long> windowStore = IntegrationTestUtils.getStore(windowStoreName, kafkaStreams, QueryableStoreTypes.windowStore());
    ...
    public void shouldAllowToQueryAfterThreadDied() throws Exception {
        ...
        final ReadOnlyKeyValueStore<String, String> store = IntegrationTestUtils.getStore(storeName, kafkaStreams, keyValueStore());

        TestUtils.waitForCondition(() -> "12".equals(store.get("a")) && "34".equals(store.get("b")), maxWaitMs, "wait for agg to be <a,12> and <b,34>");

        IntegrationTestUtils.produceKeyValuesSynchronously(streamOne, Collections.singleton(KeyValue.pair("a", "5")), TestUtils.producerConfig(
                CLUSTER.bootstrapServers(), StringSerializer.class, StringSerializer.class, new Properties()), mockTime);

        TestUtils.waitForCondition(failed::get, maxWaitMs, "wait for thread to fail");

        final ReadOnlyKeyValueStore<String, String> store2 = IntegrationTestUtils.getStore(storeName, kafkaStreams, keyValueStore());

        try {
            TestUtils.waitForCondition(() -> ("125".equals(store2.get("a")) || "1225".equals(store2.get("a")) || "12125".equals(store2.get("a"))) && ("34".equals(store2.get("b")) || "344".equals(store2.get("b")) || "3434".equals(store2.get("b"))),
                maxWaitMs,
    ...
    private void waitUntilAtLeastNumRecordProcessed(final String topic,
        ...
        IntegrationTestUtils.waitUntilMinValuesRecordsReceived(config, topic, numRecs, timeout);
    }

    private Set<KeyValue<String, Long>> fetch(final ReadOnlyWindowStore<String, Long> store,
                                              final String key) {
        try (final WindowStoreIterator<Long> fetch = store.fetch(key, ofEpochMilli(0), ofEpochMilli(System.currentTimeMillis()))) {
            if (fetch.hasNext()) {
