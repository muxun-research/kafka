...
class DefaultStateUpdaterTest {
    ...
    private DefaultStateUpdater stateUpdater = new DefaultStateUpdater("test-state-updater", metrics, config, changelogReader, topologyMetadata, time);
    ...
    public void shouldRestoreSingleActiveStatefulTask() throws Exception {
        final StreamTask task = statefulTask(TASK_0_0, mkSet(TOPIC_PARTITION_A_0, TOPIC_PARTITION_B_0)).inState(State.RESTORING).build();
        when(changelogReader.completedChangelogs()).thenReturn(Collections.emptySet()).thenReturn(mkSet(TOPIC_PARTITION_A_0)).thenReturn(mkSet(TOPIC_PARTITION_A_0, TOPIC_PARTITION_B_0));
        when(changelogReader.allChangelogsCompleted()).thenReturn(false).thenReturn(false).thenReturn(true);
        stateUpdater.start();
    ...
    public void shouldRestoreMultipleActiveStatefulTasks() throws Exception {
        ...
        when(changelogReader.completedChangelogs()).thenReturn(Collections.emptySet()).thenReturn(mkSet(TOPIC_PARTITION_C_0)).thenReturn(mkSet(TOPIC_PARTITION_C_0, TOPIC_PARTITION_A_0)).thenReturn(mkSet(TOPIC_PARTITION_C_0, TOPIC_PARTITION_A_0, TOPIC_PARTITION_B_0));
        when(changelogReader.allChangelogsCompleted()).thenReturn(false).thenReturn(false).thenReturn(false).thenReturn(true);
        stateUpdater.start();
    ...
    public void shouldReturnTrueForRestoreActiveTasksIfStandbyTask() throws Exception {
        final StandbyTask task = standbyTask(TASK_0_0, mkSet(TOPIC_PARTITION_A_0, TOPIC_PARTITION_B_0)).inState(State.RUNNING).build();
        when(changelogReader.completedChangelogs()).thenReturn(Collections.emptySet());
        when(changelogReader.allChangelogsCompleted()).thenReturn(false);
        stateUpdater.start();
        stateUpdater.add(task);
    ...
    public void shouldRestoreActiveStatefulTaskThenUpdateStandbyTaskAndAgainRestoreActiveStatefulTask() throws Exception {
        ...
        when(changelogReader.completedChangelogs()).thenReturn(Collections.emptySet()).thenReturn(mkSet(TOPIC_PARTITION_A_0)).thenReturn(mkSet(TOPIC_PARTITION_B_0));
        when(changelogReader.allChangelogsCompleted()).thenReturn(false);
        stateUpdater.start();
    ...
    public void shouldUpdateStandbyTaskAfterAllActiveStatefulTasksFailed() throws Exception {
        ...
        final TaskCorruptedException taskCorruptedException = new TaskCorruptedException(mkSet(activeTask1.id(), activeTask2.id()));
        final Map<TaskId, Task> updatingTasks1 = mkMap(mkEntry(activeTask1.id(), activeTask1), mkEntry(activeTask2.id(), activeTask2), mkEntry(standbyTask.id(), standbyTask));
        ...
        final ExceptionAndTasks expectedExceptionAndTasks = new ExceptionAndTasks(mkSet(activeTask1, activeTask2), taskCorruptedException);
    ...
    private void shouldNotRemoveTaskFromFailedTasks(final Task task) throws Exception {
        ...
        when(changelogReader.completedChangelogs()).thenReturn(Collections.emptySet());
        when(changelogReader.allChangelogsCompleted()).thenReturn(false);
        final Map<TaskId, Task> updatingTasks = mkMap(mkEntry(task.id(), task), mkEntry(controlTask.id(), controlTask));
        doThrow(streamsException).doReturn(0L).when(changelogReader).restore(updatingTasks);
        stateUpdater.start();
    ...
    public void shouldAddFailedTasksToQueueWhenRestoreThrowsStreamsExceptionWithTask() throws Exception {
        ...
        final Map<TaskId, Task> updatingTasksBeforeFirstThrow = mkMap(mkEntry(task1.id(), task1), mkEntry(task2.id(), task2), mkEntry(task3.id(), task3));
        final Map<TaskId, Task> updatingTasksBeforeSecondThrow = mkMap(mkEntry(task2.id(), task2), mkEntry(task3.id(), task3));
        doReturn(0L).doThrow(streamsException1).when(changelogReader).restore(updatingTasksBeforeFirstThrow);
        doReturn(0L).doThrow(streamsException2).when(changelogReader).restore(updatingTasksBeforeSecondThrow);
        stateUpdater.start();
    ...
    public void shouldAddFailedTasksToQueueWhenRestoreThrowsTaskCorruptedException() throws Exception {
        ...
        final Map<TaskId, Task> updatingTasks = mkMap(mkEntry(task1.id(), task1), mkEntry(task2.id(), task2), mkEntry(task3.id(), task3));
    ...
    public void shouldDrainFailedTasksAndExceptions() throws Exception {
        ...
        final String exceptionMessage = "The Streams were crossed!";
        final StreamsException streamsException1 = new StreamsException(exceptionMessage, task1.id());
        final Map<TaskId, Task> updatingTasks1 = mkMap(mkEntry(task1.id(), task1));
        doThrow(streamsException1).when(changelogReader).restore(updatingTasks1);
        ...
        final Map<TaskId, Task> updatingTasks2 = mkMap(mkEntry(task2.id(), task2), mkEntry(task3.id(), task3), mkEntry(task4.id(), task4));
        doThrow(streamsException2).when(changelogReader).restore(updatingTasks2);
        final Map<TaskId, Task> updatingTasks3 = mkMap(mkEntry(task3.id(), task3), mkEntry(task4.id(), task4));
        doThrow(streamsException3).when(changelogReader).restore(updatingTasks3);
        final Map<TaskId, Task> updatingTasks4 = mkMap(mkEntry(task4.id(), task4));
    ...
    public void shouldGetTasksFromExceptionsAndFailedTasks() throws Exception {
        ...
        final TaskCorruptedException taskCorruptedException = new TaskCorruptedException(mkSet(standbyTask1.id(), standbyTask2.id()));
        final StreamsException streamsException = new StreamsException("The Streams were crossed!", activeTask1.id());
        final Map<TaskId, Task> updatingTasks1 = mkMap(mkEntry(activeTask1.id(), activeTask1), mkEntry(standbyTask1.id(), standbyTask1), mkEntry(standbyTask2.id(), standbyTask2));
        doReturn(0L).doThrow(taskCorruptedException).doReturn(0L).when(changelogReader).restore(updatingTasks1);
        final Map<TaskId, Task> updatingTasks2 = mkMap(mkEntry(activeTask1.id(), activeTask1));
        doReturn(0L).doThrow(streamsException).doReturn(0L).when(changelogReader).restore(updatingTasks2);
        stateUpdater.start();
        stateUpdater.add(standbyTask1);
        stateUpdater.add(activeTask1);
        stateUpdater.add(standbyTask2);
        final ExceptionAndTasks expectedExceptionAndTasks1 = new ExceptionAndTasks(mkSet(standbyTask1, standbyTask2), taskCorruptedException);
    ...
    public void shouldRecordMetrics() throws Exception {
        ...
        final Map<TaskId, Task> tasks1234 = mkMap(mkEntry(activeTask1.id(), activeTask1), mkEntry(activeTask2.id(), activeTask2), mkEntry(standbyTask3.id(), standbyTask3), mkEntry(standbyTask4.id(), standbyTask4));
        final Map<TaskId, Task> tasks13 = mkMap(mkEntry(activeTask1.id(), activeTask1), mkEntry(standbyTask3.id(), standbyTask3));
    ...
    private static <T> void verifyMetric(final Metrics metrics, final MetricName metricName, final Matcher<T> matcher) {
    ...
    private void verifyGetTasks(final Set<StreamTask> expectedActiveTasks, final Set<StandbyTask> expectedStandbyTasks) {
    ...
    private void verifyRestoredActiveTasks(final StreamTask... tasks) throws Exception {
        if (tasks.length == 0) {
            waitForCondition(() -> stateUpdater.getRestoredActiveTasks().isEmpty(), VERIFICATION_TIMEOUT, "Did not get empty restored active task within the given timeout!");
        } else {
    ...
    private void verifyUpdatingTasks(final Task... tasks) throws Exception {
        if (tasks.length == 0) {
            waitForCondition(() -> stateUpdater.getUpdatingTasks().isEmpty(), VERIFICATION_TIMEOUT, "Did not get empty updating task within the given timeout!");
        } else {
    ...
    private void verifyRemovedTasks(final Task... tasks) throws Exception {
        if (tasks.length == 0) {
            waitForCondition(() -> stateUpdater.getRemovedTasks().isEmpty(), VERIFICATION_TIMEOUT, "Did not get empty removed task within the given timeout!");
        } else {
    ...
    private void verifyPausedTasks(final Task... tasks) throws Exception {
        if (tasks.length == 0) {
            waitForCondition(() -> stateUpdater.getPausedTasks().isEmpty(), VERIFICATION_TIMEOUT, "Did not get empty paused task within the given timeout!");
        } else {
    ...
    private void verifyDrainingRemovedTasks(final Task... tasks) throws Exception {
        ...
        waitForCondition(() -> {
            ...
            return removedTasks.containsAll(mkSet(tasks)) && removedTasks.size() == expectedRemovedTasks.size();
        }, VERIFICATION_TIMEOUT, "Did not get all restored active task within the given timeout!");
        assertFalse(stateUpdater.hasRemovedTasks());
        assertTrue(stateUpdater.drainRemovedTasks().isEmpty());
    }
