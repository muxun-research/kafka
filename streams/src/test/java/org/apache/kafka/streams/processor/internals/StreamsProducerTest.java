...
public class StreamsProducerTest {
    ...
    private final StreamsConfig nonEosConfig = new StreamsConfig(mkMap(mkEntry(StreamsConfig.APPLICATION_ID_CONFIG, "appId"), mkEntry(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "dummy:1234")));

    @SuppressWarnings("deprecation")
    private final StreamsConfig eosAlphaConfig = new StreamsConfig(mkMap(mkEntry(StreamsConfig.APPLICATION_ID_CONFIG, "appId"), mkEntry(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "dummy:1234"), mkEntry(StreamsConfig.PROCESSING_GUARANTEE_CONFIG, StreamsConfig.EXACTLY_ONCE)));

    private final StreamsConfig eosBetaConfig = new StreamsConfig(mkMap(mkEntry(StreamsConfig.APPLICATION_ID_CONFIG, "appId"), mkEntry(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "dummy:1234"), mkEntry(StreamsConfig.PROCESSING_GUARANTEE_CONFIG, StreamsConfig.EXACTLY_ONCE_V2)));
    ...
    final StreamsProducer streamsProducerWithMock = new StreamsProducer(nonEosConfig, "threadId", clientSupplier, null, null, logContext, mockTime);
    final StreamsProducer eosAlphaStreamsProducerWithMock = new StreamsProducer(eosAlphaConfig, "threadId", clientSupplier, new TaskId(0, 0), null, logContext, mockTime);
    ...
    private final ProducerRecord<byte[], byte[]> record = new ProducerRecord<>(topic, 0, 0L, new byte[0], new byte[0], new RecordHeaders());

    private final Map<TopicPartition, OffsetAndMetadata> offsetsAndMetadata = mkMap(mkEntry(new TopicPartition(topic, 0), new OffsetAndMetadata(0L, null)));

    @Before
    public void before() {
        mockClientSupplier.setCluster(cluster);
        nonEosStreamsProducer = new StreamsProducer(nonEosConfig, "threadId-StreamThread-0", mockClientSupplier, null, null, logContext, mockTime);
        ...
        eosAlphaStreamsProducer = new StreamsProducer(eosAlphaConfig, "threadId-StreamThread-0", eosAlphaMockClientSupplier, new TaskId(0, 0), null, logContext, mockTime);
        ...
        eosBetaStreamsProducer = new StreamsProducer(eosBetaConfig, "threadId-StreamThread-0", eosBetaMockClientSupplier, null, UUID.randomUUID(), logContext, mockTime);
    ...
    public void shouldFailIfStreamsConfigIsNull() {
        final NullPointerException thrown = assertThrows(NullPointerException.class, () -> new StreamsProducer(null, "threadId", mockClientSupplier, new TaskId(0, 0), UUID.randomUUID(), logContext, mockTime));
    ...
    public void shouldFailIfThreadIdIsNull() {
        final NullPointerException thrown = assertThrows(NullPointerException.class, () -> new StreamsProducer(nonEosConfig, null, mockClientSupplier, new TaskId(0, 0), UUID.randomUUID(), logContext, mockTime));
    ...
    public void shouldFailIfClientSupplierIsNull() {
        final NullPointerException thrown = assertThrows(NullPointerException.class, () -> new StreamsProducer(nonEosConfig, "threadId", null, new TaskId(0, 0), UUID.randomUUID(), logContext, mockTime));
    ...
    public void shouldFailIfLogContextIsNull() {
        final NullPointerException thrown = assertThrows(NullPointerException.class, () -> new StreamsProducer(nonEosConfig, "threadId", mockClientSupplier, new TaskId(0, 0), UUID.randomUUID(), null, mockTime));
    ...
    public void shouldFailOnResetProducerForAtLeastOnce() {
        final IllegalStateException thrown = assertThrows(IllegalStateException.class, () -> nonEosStreamsProducer.resetProducer());
    ...
    public void shouldFailOnResetProducerForExactlyOnceAlpha() {
        final IllegalStateException thrown = assertThrows(IllegalStateException.class, () -> eosAlphaStreamsProducer.resetProducer());
    ...
    public void shouldFailOnInitTxIfEosDisabled() {
        final IllegalStateException thrown = assertThrows(IllegalStateException.class, nonEosStreamsProducer::initTransaction);
    ...
    public void shouldThrowStreamsExceptionOnSendError() {
        ...
        final StreamsException thrown = assertThrows(StreamsException.class, () -> nonEosStreamsProducer.send(record, null));
    ...
    public void shouldFailOnSendFatal() {
        ...
        final RuntimeException thrown = assertThrows(RuntimeException.class, () -> nonEosStreamsProducer.send(record, null));
    ...
    public void shouldFailOnCommitIfEosDisabled() {
        final IllegalStateException thrown = assertThrows(IllegalStateException.class, () -> nonEosStreamsProducer.commitTransaction(null, new ConsumerGroupMetadata("appId")));
    ...
    public void shouldFailOnAbortIfEosDisabled() {
        final IllegalStateException thrown = assertThrows(IllegalStateException.class, nonEosStreamsProducer::abortTransaction);
    ...
    public void shouldSetTransactionIdUsingProcessIdIfEosV2Enabled() {
        ...
        new StreamsProducer(mockConfig, "threadId-StreamThread-0", eosAlphaMockClientSupplier, null, processId, logContext, mockTime);
    ...
    public void shouldFailIfTaskIdIsNullForEosAlpha() {
        final NullPointerException thrown = assertThrows(NullPointerException.class, () -> new StreamsProducer(eosAlphaConfig, "threadId", mockClientSupplier, null, UUID.randomUUID(), logContext, mockTime));
    ...
    public void shouldFailIfProcessIdNullForEosBeta() {
        final NullPointerException thrown = assertThrows(NullPointerException.class, () -> new StreamsProducer(eosBetaConfig, "threadId", mockClientSupplier, new TaskId(0, 0), null, logContext, mockTime));
    ...
    public void shouldFailOnMaybeBeginTransactionIfTransactionsNotInitializedForExactlyOnceAlpha() {
        final StreamsProducer streamsProducer = new StreamsProducer(eosAlphaConfig, "threadId", eosAlphaMockClientSupplier, new TaskId(0, 0), null, logContext, mockTime);

        final IllegalStateException thrown = assertThrows(IllegalStateException.class, () -> streamsProducer.send(record, null));
    ...
    public void shouldFailOnMaybeBeginTransactionIfTransactionsNotInitializedForExactlyOnceBeta() {
        final StreamsProducer streamsProducer = new StreamsProducer(eosBetaConfig, "threadId-StreamThread-0", eosBetaMockClientSupplier, null, UUID.randomUUID(), logContext, mockTime);
    ...
    public void shouldThrowTaskMigrateExceptionOnEosBeginTxnFenced() {
        ...
        final TaskMigratedException thrown = assertThrows(TaskMigratedException.class, () -> eosAlphaStreamsProducer.send(null, null));

        assertThat(thrown.getMessage(), is("Producer got fenced trying to begin a new transaction [test];" + " it means all tasks belonging to this thread should be migrated."));
    }
    ...
    public void shouldFailOnEosBeginTxnFatal() {
        ...
        // calling `send()` implicitly starts a new transaction
        final RuntimeException thrown = assertThrows(RuntimeException.class, () -> eosAlphaStreamsProducer.send(null, null));
    ...
    private void testThrowTaskMigratedExceptionOnEosSend(final RuntimeException exception) {
        ...
        final TaskMigratedException thrown = assertThrows(TaskMigratedException.class, () -> eosAlphaStreamsProducer.send(record, null));

        assertThat(thrown.getCause(), is(exception));
        assertThat(thrown.getMessage(), is("Producer got fenced trying to send a record [test];" + " it means all tasks belonging to this thread should be migrated."));
    }
    ...
    public void shouldFailOnEosCommitTxFatal() {
        ...
        final RuntimeException thrown = assertThrows(RuntimeException.class, () -> eosAlphaStreamsProducer.commitTransaction(offsetsAndMetadata, new ConsumerGroupMetadata("appId")));
    ...
    public void shouldThrowStreamsExceptionOnEosAbortTxError() {
        ...
        assertThat(thrown.getMessage(), is("Error encounter trying to abort a transaction [test]"));
    }
    ...
    public void shouldComputeTotalBlockedTime() {
        setProducerMetrics(nonEosMockProducer, BUFFER_POOL_WAIT_TIME, FLUSH_TME, TXN_INIT_TIME, TXN_BEGIN_TIME, TXN_SEND_OFFSETS_TIME, TXN_COMMIT_TIME, TXN_ABORT_TIME, METADATA_WAIT_TIME);

        final double expectedTotalBlocked = BUFFER_POOL_WAIT_TIME + FLUSH_TME + TXN_INIT_TIME + TXN_BEGIN_TIME + TXN_SEND_OFFSETS_TIME + TXN_COMMIT_TIME + TXN_ABORT_TIME + METADATA_WAIT_TIME;
    ...
    public void shouldComputeTotalBlockedTimeAfterReset() {
        setProducerMetrics(eosBetaMockProducer, BUFFER_POOL_WAIT_TIME, FLUSH_TME, TXN_INIT_TIME, TXN_BEGIN_TIME, TXN_SEND_OFFSETS_TIME, TXN_COMMIT_TIME, TXN_ABORT_TIME, METADATA_WAIT_TIME);
        final double expectedTotalBlocked = BUFFER_POOL_WAIT_TIME + FLUSH_TME + TXN_INIT_TIME + TXN_BEGIN_TIME + TXN_SEND_OFFSETS_TIME + TXN_COMMIT_TIME + TXN_ABORT_TIME + METADATA_WAIT_TIME;
        ...
        replay(mockTime);
        eosBetaStreamsProducer.resetProducer();
        setProducerMetrics(eosBetaMockClientSupplier.producers.get(1), BUFFER_POOL_WAIT_TIME, FLUSH_TME, TXN_INIT_TIME, TXN_BEGIN_TIME, TXN_SEND_OFFSETS_TIME, TXN_COMMIT_TIME, TXN_ABORT_TIME, METADATA_WAIT_TIME);

        assertThat(eosBetaStreamsProducer.totalBlockedTime(), closeTo(2 * expectedTotalBlocked + clodeDelay, 0.01));
    }
    ...
    private void setProducerMetrics(final MockProducer<?, ?> producer, final double bufferPoolWaitTime, final double flushTime, final double txnInitTime, final double txnBeginTime, final double txnSendOffsetsTime, final double txnCommitTime, final double txnAbortTime, final double metadataWaitTime) {
