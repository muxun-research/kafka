...
public class InternalTopicManagerTest {
    ...
    public void shouldCreateTopics() throws Exception {
        ...
        internalTopicManager.setup(mkMap(mkEntry(topic1, internalTopicConfig1), mkEntry(topic2, internalTopicConfig2)));
    ...
    public void shouldOnlyRetryNotSuccessfulFuturesDuringSetup() {
        ...
        when(admin.createTopics(mkSet(newTopic1, newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic1, createTopicSuccessfulFuture), mkEntry(topic2, createTopicFailFuture))));
        when(admin.createTopics(mkSet(newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic2, createTopicSuccessfulFuture))));
    ...
    private void shouldRetryCreateTopicWhenRetriableExceptionIsThrown(final Exception retriableException) {
        ...
        when(admin.createTopics(mkSet(newTopic))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic1, createTopicSuccessfulFuture)))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic2, createTopicSuccessfulFuture))));
    ...
    public void shouldThrowInformativeExceptionForOlderBrokers() {
        final AdminClient admin = new MockAdminClient() {
            ...
            public CreateTopicsResult createTopics(final Collection<NewTopic> newTopics, final CreateTopicsOptions options) {
                ...
                try {
                    new CreateTopicsRequest.Builder(new CreateTopicsRequestData().setTopics(topicsToBeCreated).setTimeoutMs(0).setValidateOnly(options.shouldValidateOnly())).build((short) 3); // pass in old unsupported request version for old brokers
    ...
    public void shouldThrowTimeoutExceptionInGetPartitionInfo() {
        ...
        final InternalTopicManager internalTopicManager = new InternalTopicManager(time, mockAdminClient, new StreamsConfig(config));

        final TimeoutException exception = assertThrows(TimeoutException.class, () -> internalTopicManager.getTopicPartitionInfo(Collections.singleton(topic1)));

        assertThat(exception.getMessage(), is("Could not create topics within 50 milliseconds. This can happen if the Kafka cluster is temporarily not available."));
    }
    ...
    public void shouldThrowTimeoutExceptionIfTopicExistsDuringSetup() {
        ...
        final TimeoutException exception = assertThrows(TimeoutException.class, () -> internalTopicManager.setup(Collections.singletonMap(topic1, internalTopicConfig)));

        assertThat(exception.getMessage(), is("Could not create internal topics within " + (Integer) config.get(StreamsConfig.consumerPrefix(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG)) / 2 + " milliseconds. This can happen if the Kafka cluster is temporarily not available or a topic is marked" + " for deletion and the broker did not complete its deletion within the timeout." + " The last errors seen per topic are:" + " {" + topic1 + "=org.apache.kafka.common.errors.TopicExistsException: Topic test_topic exists already.}"));
    }
    ...
    public void shouldThrowWhenCreateTopicsThrowsUnexpectedException() {
        ...
        final NewTopic newTopic = newTopic(topic1, internalTopicConfig, streamsConfig);
        when(admin.createTopics(mkSet(newTopic))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic1, createTopicFailFuture))));
    ...
    public void shouldThrowWhenCreateTopicsResultsDoNotContainTopic() {
        ...
        final InternalTopicConfig internalTopicConfig = setupRepartitionTopicConfig(topic1, 1);
        final NewTopic newTopic = newTopic(topic1, internalTopicConfig, streamsConfig);
        when(admin.createTopics(mkSet(newTopic))).thenAnswer(answer -> new MockCreateTopicsResult(Collections.singletonMap(topic2, new KafkaFutureImpl<>())));
    ...
    public void shouldThrowTimeoutExceptionWhenFuturesNeverCompleteDuringSetup() {
        ...
        final InternalTopicConfig internalTopicConfig = setupRepartitionTopicConfig(topic1, 1);
        final NewTopic newTopic = newTopic(topic1, internalTopicConfig, streamsConfig);
        when(admin.createTopics(mkSet(newTopic))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic1, createTopicFutureThatNeverCompletes))));
    ...
    public void shouldCleanUpWhenUnexpectedExceptionIsThrownDuringSetup() {
        ...
        when(admin.deleteTopics(mkSet(topic1))).thenAnswer(answer -> new MockDeleteTopicsResult(mkMap(mkEntry(topic1, deleteTopicSuccessfulFuture))));
    ...
    public void shouldCleanUpWhenCreateTopicsResultsDoNotContainTopic() {
        ...
        final NewTopic newTopic1 = newTopic(topic1, internalTopicConfig1, streamsConfig);
        final NewTopic newTopic2 = newTopic(topic2, internalTopicConfig2, streamsConfig);
        when(admin.createTopics(mkSet(newTopic1, newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic1, createTopicSuccessfulFuture), mkEntry(topic2, createTopicFailFuture1))));
        when(admin.createTopics(mkSet(newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic3, createTopicSuccessfulFuture))));
        final KafkaFutureImpl<Void> deleteTopicSuccessfulFuture = new KafkaFutureImpl<>();
        deleteTopicSuccessfulFuture.complete(null);
        when(admin.deleteTopics(mkSet(topic1))).thenAnswer(answer -> new MockDeleteTopicsResult(mkMap(mkEntry(topic1, deleteTopicSuccessfulFuture))));
    ...
    public void shouldCleanUpWhenCreateTopicsTimesOut() {
        ...
        when(admin.createTopics(mkSet(newTopic1, newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic1, createTopicSuccessfulFuture), mkEntry(topic2, createTopicFailFuture1))));
        final KafkaFutureImpl<TopicMetadataAndConfig> createTopicFutureThatNeverCompletes = new KafkaFutureImpl<>();
        when(admin.createTopics(mkSet(newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic2, createTopicFutureThatNeverCompletes))));
        final KafkaFutureImpl<Void> deleteTopicSuccessfulFuture = new KafkaFutureImpl<>();
        deleteTopicSuccessfulFuture.complete(null);
        when(admin.deleteTopics(mkSet(topic1))).thenAnswer(answer -> new MockDeleteTopicsResult(mkMap(mkEntry(topic1, deleteTopicSuccessfulFuture))));
    ...
    private void shouldRetryDeleteTopicWhenRetriableException(final Exception retriableException) {
        ...
        final KafkaFutureImpl<Void> deleteTopicSuccessfulFuture = new KafkaFutureImpl<>();
        deleteTopicSuccessfulFuture.complete(null);
        when(admin.deleteTopics(mkSet(topic1))).thenAnswer(answer -> new MockDeleteTopicsResult(mkMap(mkEntry(topic1, deleteTopicFailFuture)))).thenAnswer(answer -> new MockDeleteTopicsResult(mkMap(mkEntry(topic1, deleteTopicSuccessfulFuture))));
    ...
    public void shouldThrowTimeoutExceptionWhenFuturesNeverCompleteDuringCleanUp() {
        ...
        when(admin.deleteTopics(mkSet(topic1))).thenAnswer(answer -> new MockDeleteTopicsResult(mkMap(mkEntry(topic1, deleteTopicFutureThatNeverCompletes))));
    ...
    public void shouldThrowWhenDeleteTopicsThrowsUnexpectedException() {
        ...
        when(admin.deleteTopics(mkSet(topic1))).thenAnswer(answer -> new MockDeleteTopicsResult(mkMap(mkEntry(topic1, deleteTopicFailFuture))));
    ...
    private void setupCleanUpScenario(final AdminClient admin, final StreamsConfig streamsConfig, final InternalTopicConfig internalTopicConfig1, final InternalTopicConfig internalTopicConfig2) {
        ...
        final NewTopic newTopic2 = newTopic(topic2, internalTopicConfig2, streamsConfig);
        when(admin.createTopics(mkSet(newTopic1, newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic1, createTopicSuccessfulFuture), mkEntry(topic2, createTopicFailFuture1))));
        when(admin.createTopics(mkSet(newTopic2))).thenAnswer(answer -> new MockCreateTopicsResult(mkMap(mkEntry(topic2, createTopicFailFuture2))));
    }

    @Test
    public void shouldReturnCorrectPartitionCounts() {
        mockAdminClient.addTopic(false, topic1, Collections.singletonList(new TopicPartitionInfo(0, broker1, singleReplica, Collections.emptyList())), null);
        assertEquals(Collections.singletonMap(topic1, 1), internalTopicManager.getNumPartitions(Collections.singleton(topic1), Collections.emptySet()));
    }
    ...
    public void shouldReturnCorrectPartitionInfo() {
        ...
        mockAdminClient.addTopic(false, topic1, Collections.singletonList(topicPartitionInfo), null);
    ...
    public void shouldCreateRequiredTopics() throws Exception {
        ...
        assertEquals(new ConfigEntry(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_COMPACT), mockAdminClient.describeConfigs(Collections.singleton(resource4)).values().get(resource4).get().get(TopicConfig.CLEANUP_POLICY_CONFIG));
    }
    ...
    public void shouldCompleteTopicValidationOnRetry() {
        ...
        when(admin.describeTopics(mkSet(topic1, topic2))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic1, topicDescriptionSuccessFuture), mkEntry(topic2, topicDescriptionFailFuture))));
        when(admin.createTopics(Collections.singleton(new NewTopic(topic2, Optional.of(1), Optional.of((short) 1)).configs(mkMap(mkEntry(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_COMPACT), mkEntry(TopicConfig.MESSAGE_TIMESTAMP_TYPE_CONFIG, "CreateTime")))))).thenAnswer(answer -> new MockCreateTopicsResult(Collections.singletonMap(topic2, topicCreationFuture)));
        when(admin.describeTopics(Collections.singleton(topic2))).thenAnswer(answer -> new MockDescribeTopicsResult(Collections.singletonMap(topic2, topicDescriptionSuccessFuture)));
    ...
    public void shouldNotCreateTopicIfExistsWithDifferentPartitions() {
        mockAdminClient.addTopic(false, topic1, new ArrayList<TopicPartitionInfo>() {
            {
                add(new TopicPartitionInfo(0, broker1, singleReplica, Collections.emptyList()));
    ...
    public void shouldExhaustRetriesOnTimeoutExceptionForMakeReady() {
        ...
        final InternalTopicManager topicManager = new InternalTopicManager(new AutoAdvanceMockTime(time), mockAdminClient, new StreamsConfig(config));
        ...
        try {
            ...
            assertThat(expected.getMessage(), is("Could not create topics within 50 milliseconds. " + "This can happen if the Kafka cluster is temporarily not available."));
        }
    }
    ...
    public void shouldCreateTopicWhenTopicLeaderNotAvailableAndThenTopicNotFound() {
        ...
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(Collections.singletonMap(topic1, topicDescriptionLeaderNotAvailableFuture)));
        // we would not need to call create-topics for the first time
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(Collections.singletonMap(topic1, topicDescriptionUnknownTopicFuture)));
        when(admin.createTopics(Collections.singleton(new NewTopic(topic1, Optional.of(1), Optional.of((short) 1)).configs(mkMap(mkEntry(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_DELETE), mkEntry(TopicConfig.MESSAGE_TIMESTAMP_TYPE_CONFIG, "CreateTime"), mkEntry(TopicConfig.SEGMENT_BYTES_CONFIG, "52428800"), mkEntry(TopicConfig.RETENTION_MS_CONFIG, "-1")))))).thenAnswer(answer -> new MockCreateTopicsResult(Collections.singletonMap(topic1, topicCreationFuture)));
    ...
    public void shouldCompleteValidateWhenTopicLeaderNotAvailableAndThenDescribeSuccess() {
        ...
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(Collections.singletonMap(topic1, topicDescriptionFailFuture)));
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(Collections.singletonMap(topic1, topicDescriptionSuccessFuture)));
    ...
    public void shouldExhaustRetriesOnMarkedForDeletionTopic() {
        ...
        final TimeoutException exception = assertThrows(TimeoutException.class, () -> internalTopicManager.makeReady(Collections.singletonMap(topic1, internalTopicConfig)));
        assertNull(exception.getCause());
        assertThat(exception.getMessage(), equalTo("Could not create topics within 50 milliseconds." + " This can happen if the Kafka cluster is temporarily not available."));
    }
    ...
    public void shouldReportMisconfigurationsOfCleanupPolicyForUnwindowedUnversionedChangelogTopics() {
        ...
        unwindowedUnversionedChangelogConfigWithDeleteCleanupPolicy.put(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_DELETE);
        ...
        unwindowedUnversionedChangelogConfigWithDeleteCompactCleanupPolicy.put(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_COMPACT + "," + TopicConfig.CLEANUP_POLICY_DELETE);
    ...
    public void shouldReportMisconfigurationsOfCleanupPolicyForVersionedChangelogTopics() {
        ...
        final ValidationResult validationResult = internalTopicManager.validate(mkMap(mkEntry(topic1, internalTopicConfig1), mkEntry(topic2, internalTopicConfig2), mkEntry(topic3, internalTopicConfig3), mkEntry(topic4, internalTopicConfig4)));
        ...
        assertThat(misconfigurationsForTopics, hasKey(topic2));
        assertThat(misconfigurationsForTopics.get(topic2).size(), is(1));
        assertThat(misconfigurationsForTopics.get(topic2).get(0), is("Min compaction lag (" + TopicConfig.MIN_COMPACTION_LAG_MS_CONFIG + ") of existing internal topic " + topic2 + " is " + shorterCompactionLagMs + " but should be " + compactionLagMs + " or larger."));
        assertThat(misconfigurationsForTopics, hasKey(topic3));
        assertThat(misconfigurationsForTopics.get(topic3).size(), is(1));
        assertThat(misconfigurationsForTopics.get(topic3).get(0), is("Cleanup policy (" + TopicConfig.CLEANUP_POLICY_CONFIG + ") of existing internal topic " + topic3 + " should not contain \"" + TopicConfig.CLEANUP_POLICY_DELETE + "\"."));
        assertThat(misconfigurationsForTopics, hasKey(topic4));
        assertThat(misconfigurationsForTopics.get(topic4).size(), is(1));
        assertThat(misconfigurationsForTopics.get(topic4).get(0), is("Cleanup policy (" + TopicConfig.CLEANUP_POLICY_CONFIG + ") of existing internal topic " + topic4 + " should not contain \"" + TopicConfig.CLEANUP_POLICY_DELETE + "\"."));
        assertThat(misconfigurationsForTopics, not(hasKey(topic1)));
    }
    ...
    public void shouldThrowWhenPartitionCountUnknown() {
        ...
        assertThrows(IllegalStateException.class, () -> internalTopicManager.validate(Collections.singletonMap(topic1, internalTopicConfig)));
    }
    ...
    public void shouldOnlyRetryDescribeTopicsWhenDescribeTopicsThrowsLeaderNotAvailableExceptionDuringValidation() {
        ...
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic1, topicDescriptionFailFuture)))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic1, topicDescriptionSuccessfulFuture))));
        ...
        when(admin.describeConfigs(Collections.singleton(topicResource))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource, topicConfigSuccessfulFuture))));
        final InternalTopicConfig internalTopicConfig = setupRepartitionTopicConfig(topic1, 1);
    ...
    public void shouldOnlyRetryDescribeConfigsWhenDescribeConfigsThrowsLeaderNotAvailableExceptionDuringValidation() {
        ...
        final KafkaFutureImpl<TopicDescription> topicDescriptionSuccessfulFuture = new KafkaFutureImpl<>();
        topicDescriptionSuccessfulFuture.complete(new TopicDescription(topic1, false, Collections.singletonList(new TopicPartitionInfo(0, broker1, cluster, Collections.emptyList()))));
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic1, topicDescriptionSuccessfulFuture))));
        ...
        topicConfigSuccessfulFuture.complete(new Config(repartitionTopicConfig().entrySet().stream().map(entry -> new ConfigEntry(entry.getKey(), entry.getValue())).collect(Collectors.toSet())));
        final ConfigResource topicResource = new ConfigResource(Type.TOPIC, topic1);
        when(admin.describeConfigs(Collections.singleton(topicResource))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource, topicConfigsFailFuture)))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource, topicConfigSuccessfulFuture))));
        final InternalTopicConfig internalTopicConfig = setupRepartitionTopicConfig(topic1, 1);
    ...
    public void shouldOnlyRetryNotSuccessfulFuturesDuringValidation() {
        ...
        when(admin.describeTopics(mkSet(topic1, topic2))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic1, topicDescriptionSuccessfulFuture1), mkEntry(topic2, topicDescriptionFailFuture))));
        when(admin.describeTopics(mkSet(topic2))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic2, topicDescriptionSuccessfulFuture2))));
        final KafkaFutureImpl<Config> topicConfigSuccessfulFuture = new KafkaFutureImpl<>();
        topicConfigSuccessfulFuture.complete(new Config(repartitionTopicConfig().entrySet().stream().map(entry -> new ConfigEntry(entry.getKey(), entry.getValue())).collect(Collectors.toSet())));
        ...
        when(admin.describeConfigs(mkSet(topicResource1, topicResource2))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource1, topicConfigSuccessfulFuture), mkEntry(topicResource2, topicConfigSuccessfulFuture))));
        final InternalTopicConfig internalTopicConfig1 = setupRepartitionTopicConfig(topic1, 1);
        final InternalTopicConfig internalTopicConfig2 = setupRepartitionTopicConfig(topic2, 1);
    ...
    public void shouldThrowWhenDescribeTopicsThrowsUnexpectedExceptionDuringValidation() {
        ...
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic1, topicDescriptionFailFuture))));
        final InternalTopicConfig internalTopicConfig = setupRepartitionTopicConfig(topic1, 1);
    ...
    public void shouldThrowWhenDescribeConfigsThrowsUnexpectedExceptionDuringValidation() {
        final AdminClient admin = mock(AdminClient.class);
        final InternalTopicManager topicManager = new InternalTopicManager(time, admin, new StreamsConfig(config));
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap()));
        ...
        final ConfigResource topicResource = new ConfigResource(Type.TOPIC, topic1);
        when(admin.describeConfigs(Collections.singleton(topicResource))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource, configDescriptionFailFuture))));
        final InternalTopicConfig internalTopicConfig = setupRepartitionTopicConfig(topic1, 1);
    ...
    public void shouldThrowWhenTopicDescriptionsDoNotContainTopicDuringValidation() {
        ...
        final KafkaFutureImpl<TopicDescription> topicDescriptionSuccessfulFuture = new KafkaFutureImpl<>();
        topicDescriptionSuccessfulFuture.complete(new TopicDescription(topic1, false, Collections.singletonList(new TopicPartitionInfo(0, broker1, cluster, Collections.emptyList()))));
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic2, topicDescriptionSuccessfulFuture))));
        final KafkaFutureImpl<Config> topicConfigSuccessfulFuture = new KafkaFutureImpl<>();
    ...
    public void shouldThrowWhenConfigDescriptionsDoNotContainTopicDuringValidation() {
        ...
        final ConfigResource topicResource1 = new ConfigResource(Type.TOPIC, topic1);
        final ConfigResource topicResource2 = new ConfigResource(Type.TOPIC, topic2);
        when(admin.describeConfigs(Collections.singleton(topicResource1))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource2, topicConfigSuccessfulFuture))));
        final InternalTopicConfig internalTopicConfig = setupRepartitionTopicConfig(topic1, 1);
    ...
    public void shouldThrowWhenConfigDescriptionsDoNotCleanupPolicyForUnwindowedUnversionedConfigDuringValidation() {
        shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(setupUnwindowedUnversionedChangelogTopicConfig(topic1, 1), configWithoutKey(unwindowedUnversionedChangelogConfig(), TopicConfig.CLEANUP_POLICY_CONFIG));
    }

    @Test
    public void shouldThrowWhenConfigDescriptionsDoNotContainCleanupPolicyForWindowedConfigDuringValidation() {
        final long retentionMs = 1000;
        shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(setupWindowedChangelogTopicConfig(topic1, 1, retentionMs), configWithoutKey(windowedChangelogConfig(retentionMs), TopicConfig.CLEANUP_POLICY_CONFIG));
    }

    @Test
    public void shouldThrowWhenConfigDescriptionsDoNotContainRetentionMsForWindowedConfigDuringValidation() {
        final long retentionMs = 1000;
        shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(setupWindowedChangelogTopicConfig(topic1, 1, retentionMs), configWithoutKey(windowedChangelogConfig(retentionMs), TopicConfig.RETENTION_MS_CONFIG));
    }

    @Test
    public void shouldThrowWhenConfigDescriptionsDoNotContainRetentionBytesForWindowedConfigDuringValidation() {
        final long retentionMs = 1000;
        shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(setupWindowedChangelogTopicConfig(topic1, 1, retentionMs), configWithoutKey(windowedChangelogConfig(retentionMs), TopicConfig.RETENTION_BYTES_CONFIG));
    }

    @Test
    public void shouldThrowWhenConfigDescriptionsDoNotContainCleanupPolicyForRepartitionConfigDuringValidation() {
        shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(setupRepartitionTopicConfig(topic1, 1), configWithoutKey(repartitionTopicConfig(), TopicConfig.CLEANUP_POLICY_CONFIG));
    }

    @Test
    public void shouldThrowWhenConfigDescriptionsDoNotContainRetentionMsForRepartitionConfigDuringValidation() {
        shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(setupRepartitionTopicConfig(topic1, 1), configWithoutKey(repartitionTopicConfig(), TopicConfig.RETENTION_MS_CONFIG));
    }

    @Test
    public void shouldThrowWhenConfigDescriptionsDoNotContainRetentionBytesForRepartitionConfigDuringValidation() {
        shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(setupRepartitionTopicConfig(topic1, 1), configWithoutKey(repartitionTopicConfig(), TopicConfig.RETENTION_BYTES_CONFIG));
    }

    private Config configWithoutKey(final Map<String, String> config, final String key) {
        return new Config(config.entrySet().stream().filter(entry -> !entry.getKey().equals(key)).map(entry -> new ConfigEntry(entry.getKey(), entry.getValue())).collect(Collectors.toSet()));
    }
    ...
    private void shouldThrowWhenConfigDescriptionsDoNotContainConfigDuringValidation(final InternalTopicConfig streamsSideTopicConfig, final Config brokerSideTopicConfig) {
        ...
        final ConfigResource topicResource1 = new ConfigResource(Type.TOPIC, topic1);
        when(admin.describeConfigs(Collections.singleton(topicResource1))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource1, topicConfigSuccessfulFuture))));
    ...
    public void shouldThrowTimeoutExceptionWhenFuturesNeverCompleteDuringValidation() {
        ...
        when(admin.describeTopics(Collections.singleton(topic1))).thenAnswer(answer -> new MockDescribeTopicsResult(mkMap(mkEntry(topic1, topicDescriptionFutureThatNeverCompletes))));
        final KafkaFutureImpl<Config> topicConfigSuccessfulFuture = new KafkaFutureImpl<>();
        topicConfigSuccessfulFuture.complete(new Config(repartitionTopicConfig().entrySet().stream().map(entry -> new ConfigEntry(entry.getKey(), entry.getValue())).collect(Collectors.toSet())));
        final ConfigResource topicResource = new ConfigResource(Type.TOPIC, topic1);
        when(admin.describeConfigs(Collections.singleton(topicResource))).thenAnswer(answer -> new MockDescribeConfigsResult(mkMap(mkEntry(topicResource, topicConfigSuccessfulFuture))));
    ...
    private NewTopic newTopic(final String topicName, final InternalTopicConfig topicConfig, final StreamsConfig streamsConfig) {
        return new NewTopic(topicName, topicConfig.numberOfPartitions(), Optional.of(streamsConfig.getInt(StreamsConfig.REPLICATION_FACTOR_CONFIG).shortValue())).configs(topicConfig.getProperties(Collections.emptyMap(), streamsConfig.getLong(StreamsConfig.WINDOW_STORE_CHANGE_LOG_ADDITIONAL_RETENTION_MS_CONFIG)));
    }

    private Map<String, String> repartitionTopicConfig() {
        return mkMap(mkEntry(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_DELETE), mkEntry(TopicConfig.RETENTION_MS_CONFIG, "-1"), mkEntry(TopicConfig.RETENTION_BYTES_CONFIG, null));
    }

    private Map<String, String> unwindowedUnversionedChangelogConfig() {
        return mkMap(mkEntry(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_COMPACT));
    }

    private Map<String, String> windowedChangelogConfig(final long retentionMs) {
        return mkMap(mkEntry(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_COMPACT + "," + TopicConfig.CLEANUP_POLICY_DELETE), mkEntry(TopicConfig.RETENTION_MS_CONFIG, String.valueOf(retentionMs)), mkEntry(TopicConfig.RETENTION_BYTES_CONFIG, null));
    }

    private Map<String, String> versionedChangelogConfig(final long compactionLagMs) {
        return mkMap(mkEntry(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_COMPACT), mkEntry(TopicConfig.MIN_COMPACTION_LAG_MS_CONFIG, String.valueOf(compactionLagMs)));
    }

    private void setupTopicInMockAdminClient(final String topic, final Map<String, String> topicConfig) {
        mockAdminClient.addTopic(false, topic, Collections.singletonList(new TopicPartitionInfo(0, broker1, cluster, Collections.emptyList())), topicConfig);
    }

    private InternalTopicConfig setupUnwindowedUnversionedChangelogTopicConfig(final String topicName, final int partitionCount) {
        final InternalTopicConfig internalTopicConfig = new UnwindowedUnversionedChangelogTopicConfig(topicName, Collections.emptyMap());
        internalTopicConfig.setNumberOfPartitions(partitionCount);
        return internalTopicConfig;
    }

    private InternalTopicConfig setupWindowedChangelogTopicConfig(final String topicName, final int partitionCount, final long retentionMs) {
        final InternalTopicConfig internalTopicConfig = new WindowedChangelogTopicConfig(topicName, mkMap(mkEntry(TopicConfig.RETENTION_MS_CONFIG, String.valueOf(retentionMs))), 10);
        internalTopicConfig.setNumberOfPartitions(partitionCount);
        return internalTopicConfig;
    }

    private InternalTopicConfig setupVersionedChangelogTopicConfig(final String topicName, final int partitionCount, final long compactionLagMs) {
        final InternalTopicConfig internalTopicConfig = new VersionedChangelogTopicConfig(topicName, mkMap(mkEntry(TopicConfig.MIN_COMPACTION_LAG_MS_CONFIG, String.valueOf(compactionLagMs))), 12);
        internalTopicConfig.setNumberOfPartitions(partitionCount);
        return internalTopicConfig;
    }

    private InternalTopicConfig setupRepartitionTopicConfig(final String topicName, final int partitionCount) {
