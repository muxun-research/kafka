...
public class RangeQueryIntegrationTest {
    ...
    private void writeInputData() {
        IntegrationTestUtils.produceKeyValuesSynchronously(inputStream, records, TestUtils.producerConfig(CLUSTER.bootstrapServers(), StringSerializer.class, StringSerializer.class), CLUSTER.time);
    }
    ...
    private void testRange(final String name, final ReadOnlyKeyValueStore<String, String> store, final String from, final String to, final boolean forward) {
        try (final KeyValueIterator<String, String> resultIterator = forward ? store.range(from, to) : store.reverseRange(from, to); final KeyValueIterator<String, String> expectedIterator = forward ? store.all() : store.reverseAll()) {
