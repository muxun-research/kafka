...
public class StickyTaskAssignorTest {
    ...
    public void shouldAssignTasksEvenlyWithUnequalTopicGroupSizes() {
        ...
        assertThat((client1Tasks.size() == 3 && client2Tasks.size() == 4) || (client1Tasks.size() == 4 && client2Tasks.size() == 3), is(true));
    ...
    public void shouldAssignMoreTasksToClientWithMoreCapacity() {
        ...
        final boolean probingRebalanceNeeded = assign(TASK_0_0, TASK_0_1, TASK_0_2, new TaskId(1, 0), new TaskId(1, 1), new TaskId(1, 2), new TaskId(2, 0), new TaskId(2, 1), new TaskId(2, 2), new TaskId(3, 0), new TaskId(3, 1), new TaskId(3, 2));
        ...
        for (final UUID uuid : allUUIDs) {
            ...
            for (final UUID otherUUID : allUUIDs) {
                if (!uuid.equals(otherUUID)) {
                    assertThat("clients shouldn't have same task assignment", clients.get(otherUUID).assignedTasks(), not(equalTo(taskIds)));
                }
            }
    ...
    public void shouldViolateBalanceToPreserveActiveTaskStickiness() {
        ...
        Collections.shuffle(taskIds);
        final boolean probingRebalanceNeeded = new StickyTaskAssignor(true).assign(clients, new HashSet<>(taskIds), new HashSet<>(taskIds), new AssignorConfiguration.AssignmentConfigs(0L, 1, 0, 60_000L, EMPTY_RACK_AWARE_ASSIGNMENT_TAGS));
        assertThat(probingRebalanceNeeded, is(false));
    ...
    private boolean assign(final int numStandbys, final TaskId... tasks) {
        ...
        Collections.shuffle(taskIds);
        return new StickyTaskAssignor().assign(clients, new HashSet<>(taskIds), new HashSet<>(taskIds), new AssignorConfiguration.AssignmentConfigs(0L, 1, numStandbys, 60_000L, EMPTY_RACK_AWARE_ASSIGNMENT_TAGS));
    }
