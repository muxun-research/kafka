...
public class KStreamKStreamJoinTest {
    ...
    public void shouldLogAndMeterOnSkippedRecordsWithNullValueWithBuiltInMetricsVersionLatest() {
        ...
        try (final LogCaptureAppender appender = LogCaptureAppender.createAndRegister(KStreamKStreamJoin.class); final TopologyTestDriver driver = new TopologyTestDriver(builder.build(), props)) {
    ...
    public void shouldThrottleEmitNonJoinedOuterRecordsEvenWhenClockDrift() {
        ...
        final KStreamKStreamJoin<String, String, String, String> join = new KStreamKStreamJoin<>(false, "other", new JoinWindowsInternal(JoinWindows.ofTimeDifferenceWithNoGrace(ofMillis(1000))), (key, v1, v2) -> v1 + v2, true, Optional.of("outer"), tracker);
        ...
        final WindowStore<String, String> otherStore = new WindowStoreBuilder<>(new InMemoryWindowBytesStoreSupplier("other", 1000L, 100, false), Serdes.String(), Serdes.String(), new MockTime()).build();

        final KeyValueStore<TimestampedKeyAndJoinSide<String>, LeftOrRightValue<String, String>> outerStore = Mockito.spy(new KeyValueStoreBuilder<>(new InMemoryKeyValueBytesStoreSupplier("outer"), new TimestampedKeyAndJoinSideSerde<>(Serdes.String()), new LeftOrRightValueSerde<>(Serdes.String(), Serdes.String()), new MockTime()).build());
    ...
    public void testAsymmetricWindowingAfter() {
        ...
        joined = stream1.join(stream2, MockValueJoiner.TOSTRING_JOINER, JoinWindows.ofTimeDifferenceWithNoGrace(ofMillis(0)).after(ofMillis(100)), StreamJoined.with(Serdes.Integer(), Serdes.String(), Serdes.String()));
        joined.process(supplier);
    ...
    private void buildStreamsJoinThatShouldThrow(final StreamJoined<String, Integer, Integer> streamJoined, final JoinWindows joinWindows, final String expectedExceptionMessagePrefix) {

        final StreamsBuilder builder = new StreamsBuilder();
        final KStream<String, Integer> left = builder.stream("left", Consumed.with(Serdes.String(), Serdes.Integer()));
        final KStream<String, Integer> right = builder.stream("right", Consumed.with(Serdes.String(), Serdes.Integer()));

        final StreamsException streamsException = assertThrows(StreamsException.class, () -> left.join(right, Integer::sum, joinWindows, streamJoined));

        assertTrue(streamsException.getMessage().startsWith(expectedExceptionMessagePrefix));
    }

    private WindowBytesStoreSupplier buildWindowBytesStoreSupplier(final String name, final long retentionPeriod, final long windowSize, final boolean retainDuplicates) {
        return Stores.inMemoryWindowStore(name, Duration.ofMillis(retentionPeriod), Duration.ofMillis(windowSize), retainDuplicates);
    }
