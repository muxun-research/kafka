...
public class KStreamImplValueJoinerWithKeyTest {
    ...
    public void shouldIncludeKeyInStreamSteamJoinResults() {
        leftStream.join(rightStream, valueJoinerWithKey, joinWindows, streamJoined).to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
        // Left KV A, 3, Right KV A, 5
        runJoinTopology(builder, Collections.singletonList(KeyValue.pair("A", "A:5")), false, rightTopic);
    }

    @Test
    public void shouldIncludeKeyInStreamLeftJoinResults() {
        leftStream.leftJoin(rightStream, valueJoinerWithKey, joinWindows, streamJoined).to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
        // Left KV A, 3, Right KV A, 5
        ...
        runJoinTopology(builder, expectedResults, false, rightTopic);
    }

    @Test
    public void shouldIncludeKeyInStreamOuterJoinResults() {
        leftStream.outerJoin(rightStream, valueJoinerWithKey, joinWindows, streamJoined).to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
    ...
    public void shouldIncludeKeyInStreamTableJoinResults() {
        leftStream.join(ktable, valueJoinerWithKey, joined).to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
        // Left KV A, 3, Table KV A, 5
        runJoinTopology(builder, Collections.singletonList(KeyValue.pair("A", "A:5")), true, ktableTopic);
    }

    @Test
    public void shouldIncludeKeyInStreamTableLeftJoinResults() {
        leftStream.leftJoin(ktable, valueJoinerWithKey, joined).to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
        // Left KV A, 3, Table KV A, 5
        runJoinTopology(builder, Collections.singletonList(KeyValue.pair("A", "A:5")), true, ktableTopic);
    }
    ...
    public void shouldIncludeKeyInStreamGlobalTableJoinResults() {
        leftStream.join(globalKTable, keyValueMapper, valueJoinerWithKey).to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
        // Left KV A, 3, GlobalTable KV A, 5
        runJoinTopology(builder, Collections.singletonList(KeyValue.pair("A", "A:5")), true, globalTopic);
    }

    @Test
    public void shouldIncludeKeyInStreamGlobalTableLeftJoinResults() {
        leftStream.leftJoin(globalKTable, keyValueMapper, valueJoinerWithKey).to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
        // Left KV A, 3, GlobalTable KV A, 5
        runJoinTopology(builder, Collections.singletonList(KeyValue.pair("A", "A:5")), true, globalTopic);
    }
