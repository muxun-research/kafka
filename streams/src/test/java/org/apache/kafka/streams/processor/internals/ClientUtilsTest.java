...
public class ClientUtilsTest {
    ...
    private static final Headers HEADERS = new RecordHeaders(asList(new RecordHeader("h1", "headerVal1".getBytes()),   // 2 + 10 --> 12 bytes
            new RecordHeader("h2", "headerVal2".getBytes())    // 2 + 10 --> 12 bytes
    ));
    private static final int HEADERS_BYTES = 24;

    // 20 bytes
    private static final int RECORD_METADATA_BYTES = 8 + // timestamp
            8 + // offset
            4;  // partition

    // 57 bytes
    private static final long SIZE_IN_BYTES = KEY_BYTES + VALUE_BYTES + TOPIC_BYTES + HEADERS_BYTES + RECORD_METADATA_BYTES;

    // 54 bytes
    private static final long NULL_KEY_SIZE_IN_BYTES = VALUE_BYTES + TOPIC_BYTES + HEADERS_BYTES + RECORD_METADATA_BYTES;

    // 52 bytes
    private static final long TOMBSTONE_SIZE_IN_BYTES = KEY_BYTES + TOPIC_BYTES + HEADERS_BYTES + RECORD_METADATA_BYTES;

    private static final Set<TopicPartition> PARTITIONS = mkSet(new TopicPartition(TOPIC, 1), new TopicPartition(TOPIC, 2));
    ...
    public void shouldComputeSizeInBytesForConsumerRecord() {
        final ConsumerRecord<byte[], byte[]> record = new ConsumerRecord<>(TOPIC, 1, 0L, 0L, TimestampType.CREATE_TIME, KEY_BYTES, VALUE_BYTES, KEY, VALUE, HEADERS, Optional.empty());
    ...
    public void shouldComputeSizeInBytesForProducerRecord() {
        final ProducerRecord<byte[], byte[]> record = new ProducerRecord<>(TOPIC, 1, 0L, KEY, VALUE, HEADERS);
    ...
    public void shouldComputeSizeInBytesForConsumerRecordWithNullKey() {
        final ConsumerRecord<byte[], byte[]> record = new ConsumerRecord<>(TOPIC, 1, 0, 0L, TimestampType.CREATE_TIME, 0, 5, null, VALUE, HEADERS, Optional.empty());
    ...
    public void shouldComputeSizeInBytesForProducerRecordWithNullKey() {
        final ProducerRecord<byte[], byte[]> record = new ProducerRecord<>(TOPIC, 1, 0L, null, VALUE, HEADERS);
    ...
    public void shouldComputeSizeInBytesForConsumerRecordWithNullValue() {
        final ConsumerRecord<byte[], byte[]> record = new ConsumerRecord<>(TOPIC, 1, 0, 0L, TimestampType.CREATE_TIME, KEY_BYTES, 0, KEY, null, HEADERS, Optional.empty());
    ...
    public void shouldComputeSizeInBytesForProducerRecordWithNullValue() {
        final ProducerRecord<byte[], byte[]> record = new ProducerRecord<>(TOPIC, 1, 0L, KEY, null, HEADERS);
