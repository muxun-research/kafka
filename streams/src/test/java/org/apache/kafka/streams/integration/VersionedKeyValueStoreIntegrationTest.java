...
public class VersionedKeyValueStoreIntegrationTest {
    ...
    public void shouldPutGetAndDelete() throws Exception {
        ...
        streamsBuilder.addStateStore(Stores.versionedKeyValueStoreBuilder(Stores.persistentVersionedKeyValueStore(STORE_NAME, Duration.ofMillis(HISTORY_RETENTION)), Serdes.Integer(), Serdes.String())).stream(inputStream, Consumed.with(Serdes.Integer(), Serdes.String())).process(() -> new VersionedStoreContentCheckerProcessor(true), STORE_NAME).to(outputStream, Produced.with(Serdes.Integer(), Serdes.Integer()));
        ...
        final List<KeyValue<Integer, Integer>> receivedRecords = IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), IntegerDeserializer.class, IntegerDeserializer.class), outputStream, numRecordsProduced);
    ...
    public void shouldSetChangelogTopicProperties() throws Exception {
        ...
        streamsBuilder.addStateStore(Stores.versionedKeyValueStoreBuilder(Stores.persistentVersionedKeyValueStore(STORE_NAME, Duration.ofMillis(HISTORY_RETENTION)), Serdes.Integer(), Serdes.String())).stream(inputStream, Consumed.with(Serdes.Integer(), Serdes.String())).process(() -> new VersionedStoreContentCheckerProcessor(false), STORE_NAME).to(outputStream, Produced.with(Serdes.Integer(), Serdes.Integer()));
        ...
        IntegrationTestUtils.waitUntilMinRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), IntegerDeserializer.class, IntegerDeserializer.class), outputStream, 1);
    ...
    public void shouldRestore() throws Exception {
        ...
        // wait for output
        IntegrationTestUtils.waitUntilMinRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), IntegerDeserializer.class, IntegerDeserializer.class), outputStream, initialRecordsProduced);
        ...
        streamsBuilder.addStateStore(Stores.versionedKeyValueStoreBuilder(Stores.persistentVersionedKeyValueStore(STORE_NAME, Duration.ofMillis(HISTORY_RETENTION)), Serdes.Integer(), Serdes.String())).stream(inputStream, Consumed.with(Serdes.Integer(), Serdes.String())).process(() -> new VersionedStoreContentCheckerProcessor(true, data), STORE_NAME).to(outputStream, Produced.with(Serdes.Integer(), Serdes.Integer()));
        ...
        // wait for output and verify
        final List<KeyValue<Integer, Integer>> receivedRecords = IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), IntegerDeserializer.class, IntegerDeserializer.class), outputStream, initialRecordsProduced + additionalRecordsProduced);
    ...
    public void shouldAllowCustomIQv2ForCustomStoreImplementations() {
        ...
        streamsBuilder.addStateStore(Stores.versionedKeyValueStoreBuilder(new CustomIQv2VersionedStoreSupplier(), Serdes.Integer(), Serdes.String())).stream(inputStream, Consumed.with(Serdes.Integer(), Serdes.String())).process(() -> new VersionedStoreContentCheckerProcessor(false), STORE_NAME);
        ...
        final StateQueryRequest<String> request = StateQueryRequest.inStore(STORE_NAME).withQuery(new TestQuery()).withPartitions(Collections.singleton(0));
        final StateQueryResult<String> result = IntegrationTestUtils.iqv2WaitForResult(kafkaStreams, request);
    ...
    public void shouldManualUpgradeFromNonVersionedTimestampedToVersioned() throws Exception {
        ...
        streamsBuilder.addStateStore(Stores.timestampedKeyValueStoreBuilder(Stores.persistentTimestampedKeyValueStore(STORE_NAME), Serdes.Integer(), Serdes.String())).stream(inputStream, Consumed.with(Serdes.Integer(), Serdes.String())).process(TimestampedStoreContentCheckerProcessor::new, STORE_NAME).to(outputStream, Produced.with(Serdes.Integer(), Serdes.Integer()));
    ...
    private void shouldManualUpgradeFromNonVersionedToVersioned(final Topology originalTopology) throws Exception {
        ...
        // wait for output and verify
        List<KeyValue<Integer, Integer>> receivedRecords = IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), IntegerDeserializer.class, IntegerDeserializer.class), outputStream, initialRecordsProduced);
        ...
        // wait for output and verify
        receivedRecords = IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), IntegerDeserializer.class, IntegerDeserializer.class), outputStream, initialRecordsProduced + additionalRecordsProduced);
    ...
    @SafeVarargs
    private final int produceDataToTopic(final String topic, final long timestamp, final KeyValue<Integer, String>... keyValues) {
        IntegrationTestUtils.produceKeyValuesSynchronouslyWithTimestamp(topic, Arrays.asList(keyValues), TestUtils.producerConfig(CLUSTER.bootstrapServers(), IntegerSerializer.class, StringSerializer.class), timestamp);
    ...
    @SuppressWarnings("varargs")
    @SafeVarargs
    private final int produceDataToTopic(final String topic, final DataTracker dataTracker, final long timestamp, final KeyValue<Integer, String>... keyValues) {
    ...
    private static class VersionedStoreContentCheckerProcessor implements Processor<Integer, String, Integer, Integer> {
         ...
         */
        VersionedStoreContentCheckerProcessor(final boolean writeToStore, final DataTracker initialData) {
        ...
        private static boolean contentsMatch(final VersionedRecord<String> versionedRecord, final String expectedValue, final long expectedTimestamp) {
