...
public class EosV2UpgradeIntegrationTest {
    ...
    private static final List<KeyValue<KafkaStreams.State, KafkaStreams.State>> CRASH = Collections.unmodifiableList(Collections.singletonList(KeyValue.pair(State.PENDING_ERROR, State.ERROR)));
    ...
    private final static String UNEXPECTED_EXCEPTION_MSG = "Fail the test since we got an unexpected exception, or " + "there are too many exceptions thrown, please check standard error log for more info.";
    ...
    public void createTopics() throws Exception {
        ...
        CLUSTER.deleteTopicsAndWait(MULTI_PARTITION_INPUT_TOPIC, MULTI_PARTITION_OUTPUT_TOPIC, applicationId + "-" + storeName + "-changelog");
    ...
    private void waitForRunning(final List<KeyValue<KafkaStreams.State, KafkaStreams.State>> observed) throws Exception {
        waitForCondition(() -> !observed.isEmpty() && observed.get(observed.size() - 1).value.equals(State.RUNNING), MAX_WAIT_TIME_MS, () -> "Client did not startup on time. Observers transitions: " + observed);
    }

    private void waitForStateTransition(final List<KeyValue<KafkaStreams.State, KafkaStreams.State>> observed, final List<KeyValue<KafkaStreams.State, KafkaStreams.State>> expected) throws Exception {

        waitForCondition(() -> observed.equals(expected), MAX_WAIT_TIME_MS, () -> "Client did not have the expected state transition on time. Observers transitions: " + observed + "Expected transitions: " + expected);
    }

    private void waitForStateTransitionContains(final List<KeyValue<KafkaStreams.State, KafkaStreams.State>> observed, final List<KeyValue<KafkaStreams.State, KafkaStreams.State>> expected) throws Exception {

        waitForCondition(() -> observed.containsAll(expected), MAX_WAIT_TIME_MS, () -> "Client did not have the expected state transition on time. Observers transitions: " + observed + "Expected transitions: " + expected);
    }
    ...
    private void writeInputData(final List<KeyValue<Long, Long>> records) {
        final Properties config = TestUtils.producerConfig(CLUSTER.bootstrapServers(), LongSerializer.class, LongSerializer.class);
        config.setProperty(ProducerConfig.PARTITIONER_CLASS_CONFIG, KeyPartitioner.class.getName());
        IntegrationTestUtils.produceKeyValuesSynchronously(MULTI_PARTITION_INPUT_TOPIC, records, config, CLUSTER.time);
    }
    ...
    private List<KeyValue<Long, Long>> readResult(final int numberOfRecords, final boolean readCommitted) throws Exception {
        if (readCommitted) {
            return IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), CONSUMER_GROUP_ID, LongDeserializer.class, LongDeserializer.class, Utils.mkProperties(Collections.singletonMap(ConsumerConfig.ISOLATION_LEVEL_CONFIG, IsolationLevel.READ_COMMITTED.name().toLowerCase(Locale.ROOT)))), MULTI_PARTITION_OUTPUT_TOPIC, numberOfRecords, MAX_WAIT_TIME_MS);
        }

        // read uncommitted
        return IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), LongDeserializer.class, LongDeserializer.class), MULTI_PARTITION_OUTPUT_TOPIC, numberOfRecords);
    }
    ...
    private List<KeyValue<Long, Long>> computeExpectedResult(final List<KeyValue<Long, Long>> input, final Map<Long, Long> currentState) {
    ...
    private Set<Long> keysFromInstance(final KafkaStreams streams) throws Exception {
        ...
        waitForCondition(() -> {
            final ReadOnlyKeyValueStore<Long, Long> store = streams.store(StoreQueryParameters.fromNameAndType(storeName, QueryableStoreTypes.keyValueStore()));
            ...
            return true;
        }, MAX_WAIT_TIME_MS, "Could not get keys from store: " + storeName);
