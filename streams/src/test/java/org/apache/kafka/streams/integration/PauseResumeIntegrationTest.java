...
public class PauseResumeIntegrationTest {
    ...
    private static final List<KeyValue<String, Long>> COUNT_OUTPUT_DATA = asList(pair("A", 1L), pair("B", 1L), pair("A", 2L), pair("C", 1L), pair("C", 2L));
    private static final List<KeyValue<String, Long>> COUNT_OUTPUT_DATA2 = asList(pair("A", 3L), pair("B", 2L), pair("A", 4L), pair("C", 3L), pair("C", 4L));
    ...
    private void assertStreamsLocalStoreLagStaysConstant(final KafkaStreams streams) throws InterruptedException {
        waitForCondition(() -> !streams.allLocalStorePartitionLags().isEmpty(), "Lags for local store partitions were not found within the timeout!");
    ...
    private void awaitOutput(final String topicName, final int count, final List<KeyValue<String, Long>> output) throws Exception {
