...
public class SessionCacheFlushListenerTest {
    ...
    public void shouldForwardKeyNewValueOldValueAndTimestamp() {
        ...
        context.setCurrentNode(null);
        context.forward(new Record<>(new Windowed<>("key", new SessionWindow(21L, 73L)), new Change<>("newValue", "oldValue"), 73L));
        expectLastCall();
        replay(context);

        new SessionCacheFlushListener<>(context).apply(new Record<>(new Windowed<>("key", new SessionWindow(21L, 73L)), new Change<>("newValue", "oldValue"), 42L));
