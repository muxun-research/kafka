...
public class StreamTaskTest {
    ...
    private static ProcessorTopology withRepartitionTopics(final List<ProcessorNode<?, ?, ?, ?>> processorNodes, final Map<String, SourceNode<?, ?>> sourcesByTopic, final Set<String> repartitionTopics) {
        return new ProcessorTopology(processorNodes, sourcesByTopic, emptyMap(), emptyList(), emptyList(), emptyMap(), repartitionTopics);
    }

    private static ProcessorTopology withSources(final List<ProcessorNode<?, ?, ?, ?>> processorNodes, final Map<String, SourceNode<?, ?>> sourcesByTopic) {
        return new ProcessorTopology(processorNodes, sourcesByTopic, emptyMap(), emptyList(), emptyList(), emptyMap(), Collections.emptySet());
    }
    ...
    private static StreamsConfig createConfig(final String eosConfig, final String enforcedProcessingValue, final String deserializationExceptionHandler) {
        ...
        return new StreamsConfig(mkProperties(mkMap(mkEntry(StreamsConfig.APPLICATION_ID_CONFIG, APPLICATION_ID), mkEntry(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:2171"), mkEntry(StreamsConfig.BUFFERED_RECORDS_PER_PARTITION_CONFIG, "3"), mkEntry(StreamsConfig.STATE_DIR_CONFIG, canonicalPath), mkEntry(StreamsConfig.METRICS_RECORDING_LEVEL_CONFIG, DEBUG.name), mkEntry(StreamsConfig.DEFAULT_TIMESTAMP_EXTRACTOR_CLASS_CONFIG, MockTimestampExtractor.class.getName()), mkEntry(StreamsConfig.PROCESSING_GUARANTEE_CONFIG, eosConfig), mkEntry(StreamsConfig.MAX_TASK_IDLE_MS_CONFIG, enforcedProcessingValue), mkEntry(StreamsConfig.DEFAULT_DESERIALIZATION_EXCEPTION_HANDLER_CLASS_CONFIG, deserializationExceptionHandler))));
    }
    ...
    public void shouldResetOffsetsToLastCommittedForSpecifiedPartitions() {
        ...
        final java.util.function.Consumer<Set<TopicPartition>> resetter = EasyMock.mock(java.util.function.Consumer.class);
    ...
    public void shouldReadCommittedStreamTimeAndProcessorMetadataOnInitialize() {
        ...
        final ProcessorMetadata processorMetadata = new ProcessorMetadata(mkMap(mkEntry("key1", 1L), mkEntry("key2", 2L)));

        consumer.commitSync(partitions.stream().collect(Collectors.toMap(Function.identity(), tp -> new OffsetAndMetadata(0L, new TopicPartitionMetadata(10L, processorMetadata).encode()))));
    ...
    public void shouldReadCommittedStreamTimeAndMergeProcessorMetadataOnInitialize() {
        ...
        final ProcessorMetadata processorMetadata1 = new ProcessorMetadata(mkMap(mkEntry("key1", 1L), mkEntry("key2", 2L)));

        final Map<TopicPartition, OffsetAndMetadata> meta1 = mkMap(mkEntry(partition1, new OffsetAndMetadata(0L, new TopicPartitionMetadata(10L, processorMetadata1).encode())));

        final ProcessorMetadata processorMetadata2 = new ProcessorMetadata(mkMap(mkEntry("key1", 10L), mkEntry("key3", 30L)));

        final Map<TopicPartition, OffsetAndMetadata> meta2 = mkMap(mkEntry(partition2, new OffsetAndMetadata(0L, new TopicPartitionMetadata(20L, processorMetadata2).encode())));
    ...
    public void shouldProcessInOrder() {
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 10, 101), getConsumerRecordWithOffsetAsTimestamp(partition1, 20, 102), getConsumerRecordWithOffsetAsTimestamp(partition1, 30, 103)));

        task.addRecords(partition2, asList(getConsumerRecordWithOffsetAsTimestamp(partition2, 25, 201), getConsumerRecordWithOffsetAsTimestamp(partition2, 35, 202), getConsumerRecordWithOffsetAsTimestamp(partition2, 45, 203)));
    ...
    public void shouldProcessRecordsAfterPrepareCommitWhenEosDisabled() {
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 10), getConsumerRecordWithOffsetAsTimestamp(partition1, 20), getConsumerRecordWithOffsetAsTimestamp(partition1, 30)));
    ...
    public void shouldRecordBufferedRecords() {
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 10), getConsumerRecordWithOffsetAsTimestamp(partition1, 20)));
    ...
    public void shouldThrowOnTimeoutExceptionAndBufferRecordForRetryIfEosDisabled() {
        ...
        final TimeoutException exception = assertThrows(TimeoutException.class, () -> task.process(0));
        ...
        final TimeoutException nextException = assertThrows(TimeoutException.class, () -> task.process(0));
    ...
    public void shouldThrowTaskCorruptedExceptionOnTimeoutExceptionIfEosEnabled() {
        ...
        assertThrows(TaskCorruptedException.class, () -> task.process(0));
    }
    ...
    private Metric getProcessorMetric(final String operation, final String nameFormat, final String taskId, final String processorNodeId, final String builtInMetricsVersion) {

        return getMetricByNameFilterByTags(metrics.metrics(), String.format(nameFormat, operation), "stream-processor-node-metrics", mkMap(mkEntry("task-id", taskId), mkEntry("processor-node-id", processorNodeId), mkEntry(THREAD_ID_TAG, Thread.currentThread().getName())));
    }
    ...
    public void shouldPauseAndResumeBasedOnBufferedRecords() {
        ...
        task.addRecords(partition2, asList(getConsumerRecordWithOffsetAsTimestamp(partition2, 35), getConsumerRecordWithOffsetAsTimestamp(partition2, 45), getConsumerRecordWithOffsetAsTimestamp(partition2, 55), getConsumerRecordWithOffsetAsTimestamp(partition2, 65)));
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 30), getConsumerRecordWithOffsetAsTimestamp(partition1, 40), getConsumerRecordWithOffsetAsTimestamp(partition1, 50)));
    ...
    public void shouldPunctuateOnceStreamTimeAfterGap() {
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 20), getConsumerRecordWithOffsetAsTimestamp(partition1, 142), getConsumerRecordWithOffsetAsTimestamp(partition1, 155), getConsumerRecordWithOffsetAsTimestamp(partition1, 160)));

        task.addRecords(partition2, asList(getConsumerRecordWithOffsetAsTimestamp(partition2, 25), getConsumerRecordWithOffsetAsTimestamp(partition2, 145), getConsumerRecordWithOffsetAsTimestamp(partition2, 159), getConsumerRecordWithOffsetAsTimestamp(partition2, 161)));
    ...
    public void shouldRespectPunctuateCancellationStreamTime() {
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 20), getConsumerRecordWithOffsetAsTimestamp(partition1, 30), getConsumerRecordWithOffsetAsTimestamp(partition1, 40)));

        task.addRecords(partition2, asList(getConsumerRecordWithOffsetAsTimestamp(partition2, 25), getConsumerRecordWithOffsetAsTimestamp(partition2, 35), getConsumerRecordWithOffsetAsTimestamp(partition2, 45)));
    ...
    public void shouldCommitNextOffsetAndProcessorMetadataFromQueueIfAvailable() {
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 0L), getConsumerRecordWithOffsetAsTimestamp(partition1, 3L), getConsumerRecordWithOffsetAsTimestamp(partition1, 5L)));
        ...
        final TopicPartitionMetadata expected = new TopicPartitionMetadata(3L, new ProcessorMetadata(mkMap(mkEntry("key1", 100L), mkEntry("key2", 200L))));
    ...
    public void shouldCommitConsumerPositionIfRecordQueueIsEmpty() {
        ...
        assertTrue(task.commitNeeded());
        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(3L, metadata.encode())))));
        task.postCommit(false);
        ...
        assertTrue(task.commitNeeded());
        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(3L, metadata.encode())), mkEntry(partition2, new OffsetAndMetadata(1L, metadata.encode())))));
        task.postCommit(false);
    ...
    public void shouldCommitOldProcessorMetadataWhenNotDirty() {
        ...
        final TopicPartitionMetadata expectedMetadata1 = new TopicPartitionMetadata(0L, new ProcessorMetadata(mkMap(mkEntry("key1", 100L))));

        final TopicPartitionMetadata expectedMetadata2 = new TopicPartitionMetadata(RecordQueue.UNKNOWN, new ProcessorMetadata(mkMap(mkEntry("key1", 100L))));

        assertTrue(task.commitNeeded());

        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(1L, expectedMetadata1.encode())), mkEntry(partition2, new OffsetAndMetadata(2L, expectedMetadata2.encode())))));
        task.postCommit(false);
        ...
        final TopicPartitionMetadata expectedMetadata3 = new TopicPartitionMetadata(1L, new ProcessorMetadata(mkMap(mkEntry("key1", 100L))));
        assertTrue(task.commitNeeded());

        // Processor metadata not updated, we just need to commit to partition1 again with new offset
        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(2L, expectedMetadata3.encode())))));
        task.postCommit(false);
    ...
    public void shouldFailOnCommitIfTaskIsClosed() {
        ...
        final IllegalStateException thrown = assertThrows(IllegalStateException.class, task::prepareCommit);
    ...
    public void shouldNotShareHeadersBetweenPunctuateIterations() {
        ...
        task.punctuate(processorSystemTime, 1L, PunctuationType.WALL_CLOCK_TIME, timestamp -> task.processorContext().headers().add("dummy", null));
        task.punctuate(processorSystemTime, 1L, PunctuationType.WALL_CLOCK_TIME, timestamp -> assertFalse(task.processorContext().headers().iterator().hasNext()));
    }
    ...
    public void shouldWrapKafkaExceptionWithStreamsExceptionWhenProcess() {
        ...
        task.addRecords(partition1, asList(getConsumerRecordWithOffsetAsTimestamp(partition1, 10), getConsumerRecordWithOffsetAsTimestamp(partition1, 20), getConsumerRecordWithOffsetAsTimestamp(partition1, 30)));
        task.addRecords(partition2, asList(getConsumerRecordWithOffsetAsTimestamp(partition2, 5),  // this is the first record to process
                getConsumerRecordWithOffsetAsTimestamp(partition2, 35), getConsumerRecordWithOffsetAsTimestamp(partition2, 45)));

        assertThat("Map did not contain the partitions", task.highWaterMark().containsKey(partition1) && task.highWaterMark().containsKey(partition2));
    ...
    public void shouldReInitializeTopologyWhenResuming() throws IOException {
        stateDirectory = EasyMock.createNiceMock(StateDirectory.class);
        EasyMock.expect(stateDirectory.lock(taskId)).andReturn(true);
        EasyMock.expect(recordCollector.offsets()).andReturn(Collections.emptyMap()).andThrow(new AssertionError("Should not try to read offsets")).anyTimes();
    ...
    public void shouldNotCheckpointOffsetsAgainOnCommitIfSnapshotNotChangedMuch() {
        ...
        EasyMock.expect(stateManager.changelogOffsets()).andReturn(singletonMap(changelogPartition, 10L)) // restoration checkpoint
                .andReturn(singletonMap(changelogPartition, 10L)).andReturn(singletonMap(changelogPartition, 20L));
        EasyMock.expectLastCall();
        EasyMock.replay(stateManager, recordCollector);
    ...
    public void shouldCheckpointOffsetsOnCommitIfSnapshotMuchChanged() {
        ...
        EasyMock.expect(stateManager.changelogPartitions()).andReturn(singleton(changelogPartition));
        EasyMock.expect(stateManager.changelogOffsets()).andReturn(singletonMap(changelogPartition, 0L)).andReturn(singletonMap(changelogPartition, 10L)).andReturn(singletonMap(changelogPartition, 12000L));
        stateManager.registerStore(stateStore, stateStore.stateRestoreCallback, null);
        EasyMock.expectLastCall();
        EasyMock.replay(stateManager, recordCollector);
    ...
    public void shouldNotCheckpointOffsetsOnCommitIfEosIsEnabled() {
        ...
        task.prepareCommit();
        task.postCommit(false);
        final File checkpointFile = new File(stateDirectory.getOrCreateDirectoryForTask(taskId), StateManagerUtil.CHECKPOINT_FILE_NAME);
    ...
    public void shouldCheckpointForSuspendedTask() {
        stateManager.checkpoint();
        EasyMock.expectLastCall().once();
        EasyMock.expect(stateManager.changelogOffsets()).andReturn(singletonMap(partition1, 1L));
        EasyMock.expect(recordCollector.offsets()).andReturn(emptyMap()).anyTimes();
        EasyMock.replay(stateManager, recordCollector);
    ...
    public void shouldNotCheckpointForSuspendedRunningTaskWithSmallProgress() {
        EasyMock.expect(stateManager.changelogOffsets()).andReturn(singletonMap(partition1, 0L)) // restoration checkpoint
                .andReturn(singletonMap(partition1, 1L)).andReturn(singletonMap(partition1, 2L));
    ...
    public void shouldCheckpointForSuspendedRunningTaskWithLargeProgress() {
        ...
        EasyMock.expect(stateManager.changelogOffsets()).andReturn(singletonMap(partition1, 0L)).andReturn(singletonMap(partition1, 12000L)).andReturn(singletonMap(partition1, 24000L));
        stateManager.checkpoint();
        EasyMock.expectLastCall().times(2);
        EasyMock.replay(stateManager, recordCollector);
    ...
    public void shouldOnlyCheckpointStateWithBigAdvanceIfNotEnforced() {
        ...
        stateManager.checkpoint();
        EasyMock.expectLastCall().once();
        EasyMock.expect(stateManager.changelogOffsets()).andReturn(Collections.singletonMap(partition1, 50L)).andReturn(Collections.singletonMap(partition1, 11000L)).andReturn(Collections.singletonMap(partition1, 12000L));
        EasyMock.replay(stateManager);
    ...
    public void shouldCheckpointOffsetsOnPostCommit() {
        ...
        EasyMock.expectLastCall().once();
        EasyMock.expect(stateManager.changelogPartitions()).andReturn(Collections.emptySet()).anyTimes();
        EasyMock.expect(stateManager.changelogOffsets()).andReturn(singletonMap(partition1, offset + 10000L)) // restoration checkpoint
    ...
    public void shouldUpdatePartitions() {
        ...
        task.updateInputPartitions(newPartitions, mkMap(mkEntry(source1.name(), asList(topic1, "newTopic")), mkEntry(source2.name(), singletonList(topic2))));
    ...
    public void shouldThrowTopologyExceptionIfTaskCreatedForUnknownTopic() {
        ...
        assertThat(exception.getMessage(), equalTo("Invalid topology: " + "Topic " + topic1 + " is unknown to the topology. This may happen if different KafkaStreams instances of the same " + "application execute different Topologies. Note that Topologies are only identical if all operators " + "are added in the same order."));
    }
    ...
    public void shouldUpdateOffsetIfAllRecordsAreCorrupted() {
        task = createStatelessTask(createConfig(AT_LEAST_ONCE, "0", LogAndContinueExceptionHandler.class.getName()));
        task.initializeIfNeeded();
        task.completeRestoration(noOpResetter -> {
        });

        long offset = -1;
        final List<ConsumerRecord<byte[], byte[]>> records = asList(getCorruptedConsumerRecordWithOffsetAsTimestamp(++offset), getCorruptedConsumerRecordWithOffsetAsTimestamp(++offset));
        ...
        assertTrue(task.commitNeeded());
        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(offset + 1, new TopicPartitionMetadata(RecordQueue.UNKNOWN, new ProcessorMetadata()).encode())))));
    }
    ...
    public void shouldUpdateOffsetIfValidRecordFollowsCorrupted() {
        ...
        final List<ConsumerRecord<byte[], byte[]>> records = asList(getCorruptedConsumerRecordWithOffsetAsTimestamp(++offset), getConsumerRecordWithOffsetAsTimestamp(partition1, ++offset));
        consumer.addRecord(records.get(0));
        consumer.addRecord(records.get(1));
        consumer.poll(Duration.ZERO);
        task.addRecords(partition1, records);
        ...
        assertTrue(task.process(offset));
        assertTrue(task.commitNeeded());
        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(offset + 1, new TopicPartitionMetadata(offset, new ProcessorMetadata()).encode())))));
    }
    ...
    public void shouldUpdateOffsetIfCorruptedRecordFollowsValid() {
        ...
        final List<ConsumerRecord<byte[], byte[]>> records = asList(getConsumerRecordWithOffsetAsTimestamp(partition1, ++offset), getCorruptedConsumerRecordWithOffsetAsTimestamp(++offset));
        consumer.addRecord(records.get(0));
        consumer.addRecord(records.get(1));
        consumer.poll(Duration.ZERO);
        task.addRecords(partition1, records);
        ...
        assertTrue(task.process(offset));
        assertTrue(task.commitNeeded());
        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(1, new TopicPartitionMetadata(0, new ProcessorMetadata()).encode())))));

        assertTrue(task.process(offset));
        assertTrue(task.commitNeeded());
        assertThat(task.prepareCommit(), equalTo(mkMap(mkEntry(partition1, new OffsetAndMetadata(2, new TopicPartitionMetadata(0, new ProcessorMetadata()).encode())))));
    }
    ...
    private ConsumerRecord<byte[], byte[]> getConsumerRecordWithOffsetAsTimestamp(final TopicPartition topicPartition, final long offset, final int value) {
        return new ConsumerRecord<>(topicPartition.topic(), topicPartition.partition(), offset, offset, // use the offset as the timestamp
                TimestampType.CREATE_TIME, 0, 0, recordKey, intSerializer.serialize(null, value), new RecordHeaders(), Optional.empty());
    }

    private ConsumerRecord<byte[], byte[]> getConsumerRecordWithOffsetAsTimestamp(final TopicPartition topicPartition, final long offset) {
        return new ConsumerRecord<>(topicPartition.topic(), topicPartition.partition(), offset, offset, // use the offset as the timestamp
                TimestampType.CREATE_TIME, 0, 0, recordKey, recordValue, new RecordHeaders(), Optional.empty());
    }

    private ConsumerRecord<byte[], byte[]> getConsumerRecordWithOffsetAsTimestamp(final Integer key, final long offset) {
        return new ConsumerRecord<>(topic1, 1, offset, offset, // use the offset as the timestamp
                TimestampType.CREATE_TIME, 0, 0, new IntegerSerializer().serialize(topic1, key), recordValue, new RecordHeaders(), Optional.empty());
    }

    private ConsumerRecord<byte[], byte[]> getCorruptedConsumerRecordWithOffsetAsTimestamp(final long offset) {
        return new ConsumerRecord<>(topic1, 1, offset, offset, // use the offset as the timestamp
                TimestampType.CREATE_TIME, -1, -1, new byte[0], "I am not an integer.".getBytes(), new RecordHeaders(), Optional.empty());
    }
