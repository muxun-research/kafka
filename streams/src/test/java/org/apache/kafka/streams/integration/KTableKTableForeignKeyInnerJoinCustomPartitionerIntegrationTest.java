...
public class KTableKTableForeignKeyInnerJoinCustomPartitionerIntegrationTest {
    ...
    public static void startCluster() throws IOException, InterruptedException {
        ...
        final List<KeyValue<String, String>> table1 = asList(new KeyValue<>("ID123-1", "ID123-A1"), new KeyValue<>("ID123-2", "ID123-A2"), new KeyValue<>("ID123-3", "ID123-A3"), new KeyValue<>("ID123-4", "ID123-A4"));

        final List<KeyValue<String, String>> table2 = asList(new KeyValue<>("ID123", "BBB"));
    ...
    private void verifyKTableKTableJoin(final Set<KeyValue<String, String>> expectedResult) throws Exception {
        ...
        final Set<KeyValue<String, String>> result = new HashSet<>(waitUntilMinKeyValueRecordsReceived(CONSUMER_CONFIG, OUTPUT, expectedResult.size()));
    ...
    private static KafkaStreams prepareTopologyWithNonSingletonPartitions(final String queryableName, final Properties streamsConfig) {
        ...
        final TableJoined<String, String> tableJoined = TableJoined.with(new MultiPartitioner(), (topic, key, value, numPartitions) -> Math.abs(key.hashCode()) % numPartitions);
    ...
    private static Repartitioned<String, String> repartitionA() {
        ...
        return repartitioned.withKeySerde(Serdes.String()).withValueSerde(Serdes.String()).withStreamPartitioner((topic, key, value, numPartitions) -> Math.abs(getKeyB(key).hashCode()) % numPartitions).withNumberOfPartitions(4);
    }
