...
public class KStreamRepartitionIntegrationTest {
    ...
    public void whenShuttingDown() throws IOException {
        kafkaStreamsInstances.stream().filter(Objects::nonNull).forEach(KafkaStreams::close);
    ...
    public void shouldRepartitionToMultiplePartitions() throws Exception {
        ...
        final List<KeyValue<Integer, String>> expectedRecordsOnRepartition = Arrays.asList(new KeyValue<>(1, "A"), new KeyValue<>(1, "A"), new KeyValue<>(1, "A"), new KeyValue<>(1, "A"), new KeyValue<>(2, "B"), new KeyValue<>(2, "B"), new KeyValue<>(2, "B"), new KeyValue<>(2, "B"));
        ...
        final Repartitioned<Integer, String> repartitioned = Repartitioned.<Integer, String>as(repartitionName).withStreamPartitioner(new BroadcastingPartitioner());

        builder.stream(inputTopic, Consumed.with(Serdes.Integer(), Serdes.String())).repartition(repartitioned).to(broadcastingOutputTopic);
        ...
        validateReceivedMessages(new IntegerDeserializer(), new StringDeserializer(), expectedRecordsOnRepartition, topic);


        validateReceivedMessages(new IntegerDeserializer(), new StringDeserializer(), expectedRecordsOnRepartition, broadcastingOutputTopic);
    ...
    private int getNumberOfPartitionsForTopic(final String topic) throws Exception {
        try (final AdminClient adminClient = createAdminClient()) {
            final TopicDescription topicDescription = adminClient.describeTopics(Collections.singleton(topic)).topicNameValues().get(topic).get();
    ...
    private static int countOccurrencesInTopology(final String topologyString, final String searchPattern) {
    ...
    private void sendEvents(final long timestamp, final List<KeyValue<Integer, String>> events) throws Exception {
        sendEvents(inputTopic, timestamp, events);
    }

    private void sendEvents(final String topic, final long timestamp, final List<KeyValue<Integer, String>> events) throws Exception {
    ...
    private KafkaStreams startStreams(final StreamsBuilder builder, final State expectedOldState, final State expectedNewState, final Thread.UncaughtExceptionHandler uncaughtExceptionHandler) throws InterruptedException {
        return startStreams(builder, expectedOldState, expectedNewState, streamsConfiguration, uncaughtExceptionHandler);
    }

    private KafkaStreams startStreams(final StreamsBuilder builder, final State expectedOldState, final State expectedNewState, final Properties streamsConfiguration, final Thread.UncaughtExceptionHandler uncaughtExceptionHandler) throws InterruptedException {
    ...
    private <K, V> void validateReceivedMessages(final Deserializer<K> keySerializer, final Deserializer<V> valueSerializer, final List<KeyValue<K, V>> expectedRecords) throws Exception {

        validateReceivedMessages(keySerializer, valueSerializer, expectedRecords, outputTopic);
    }

    private <K, V> void validateReceivedMessages(final Deserializer<K> keySerializer, final Deserializer<V> valueSerializer, final List<KeyValue<K, V>> expectedRecords, final String outputTopic) throws Exception {
