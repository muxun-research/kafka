...
public class ConsistencyVectorIntegrationTest {
    ...
    public void shouldHaveSamePositionBoundActiveAndStandBy() throws Exception {
        ...
        try {
            ...
            final StateQueryRequest<ValueAndTimestamp<Integer>> request = StateQueryRequest.inStore(TABLE_NAME).withQuery(KeyQuery.<Integer, ValueAndTimestamp<Integer>>withKey(key)).withPositionBound(PositionBound.unbounded());
    ...
    private void checkPosition(final int batch1NumMessages, final StateQueryRequest<ValueAndTimestamp<Integer>> request, final KafkaStreams kafkaStreams1) throws InterruptedException {
        final long maxWaitMs = TestUtils.DEFAULT_MAX_WAIT_MS;
        final long expectedEnd = System.currentTimeMillis() + maxWaitMs;

        while (true) {
            final StateQueryResult<ValueAndTimestamp<Integer>> stateQueryResult = IntegrationTestUtils.iqv2WaitForResult(kafkaStreams1, request);
            final QueryResult<ValueAndTimestamp<Integer>> queryResult = stateQueryResult.getPartitionResults().get(0);
            if (queryResult.isSuccess() && queryResult.getResult() != null) {
                // invariant: each value is also at the equivalent offset
                assertThat("Result:" + queryResult, queryResult.getPosition(), is(Position.emptyPosition().withComponent(INPUT_TOPIC_NAME, 0, queryResult.getResult().value())));
                ...
                if (expectedEnd <= System.currentTimeMillis()) {
                    throw new RuntimeException("Test timed out in " + maxWaitMs);
                }
            }
