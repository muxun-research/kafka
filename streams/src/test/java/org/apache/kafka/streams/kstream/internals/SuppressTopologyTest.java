...
public class SuppressTopologyTest {
    ...
    public void shouldApplyNameToSuppressionNode() {
        final StreamsBuilder namedNodeBuilder = new StreamsBuilder();
        namedNodeBuilder.stream("input", Consumed.with(STRING_SERDE, STRING_SERDE)).groupByKey().count().suppress(untilTimeLimit(Duration.ofSeconds(1), unbounded()).withName("asdf")).toStream().to("output", Produced.with(STRING_SERDE, Serdes.Long()));
        final String namedNodeTopology = namedNodeBuilder.build().describe().toString();
    ...
    public void shouldThrowOnSuppressForMaterializedVersionedTable() {
        ...
        final KTable<Object, Object> versionedTable = builder.table("input", Materialized.as(Stores.persistentVersionedKeyValueStore("store", Duration.ZERO)));

        assertThrows(TopologyException.class, () -> versionedTable.suppress(Suppressed.untilTimeLimit(Duration.ZERO, Suppressed.BufferConfig.unbounded())));
    }

    @Test
    public void shouldThrowOnSuppressForNonMaterializedVersionedTable() {
        final StreamsBuilder builder = new StreamsBuilder();
        builder.table("input", Materialized.as(Stores.persistentVersionedKeyValueStore("store", Duration.ZERO))).filter((k, v) -> true).suppress(Suppressed.untilTimeLimit(Duration.ZERO, Suppressed.BufferConfig.unbounded()));
