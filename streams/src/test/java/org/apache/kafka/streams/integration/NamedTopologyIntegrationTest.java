...
import org.apache.kafka.streams.processor.internals.namedtopology.*;
...
public class NamedTopologyIntegrationTest {
    ...
    private static final List<KeyValue<String, Long>> STANDARD_INPUT_DATA = asList(pair("A", 100L), pair("B", 200L), pair("A", 300L), pair("C", 400L), pair("C", -50L));
    private static final List<KeyValue<String, Long>> COUNT_OUTPUT_DATA = asList(pair("B", 1L), pair("A", 2L), pair("C", 2L)); // output of count operation with caching
    private static final List<KeyValue<String, Long>> SUM_OUTPUT_DATA = asList(pair("B", 200L), pair("A", 400L), pair("C", 350L)); // output of summation with caching
    ...
    public void shouldProcessSingleNamedTopologyAndPrefixInternalTopics() throws Exception {
        topology1Builder.stream(INPUT_STREAM_1).selectKey((k, v) -> k).groupByKey().count(ROCKSDB_STORE).toStream().to(OUTPUT_STREAM_1);
    ...
    public void shouldWaitForMissingInputTopicsToBeCreated() throws Exception {
        ...
        try {
            ...
            final List<KeyValue<String, Integer>> output = waitUntilMinKeyValueRecordsReceived(consumerConfig, OUTPUT_STREAM_1, 3);
    ...
    public void shouldBackOffTaskAndEmitDataWithinSameTopology() throws Exception {
        ...
        try {
            ...
            IntegrationTestUtils.startApplicationAndWaitUntilRunning(streams);
            IntegrationTestUtils.produceKeyValuesSynchronouslyWithTimestamp(DELAYED_INPUT_STREAM_2, singletonList(new KeyValue<>(1, "A")), TestUtils.producerConfig(CLUSTER.bootstrapServers(), IntegerSerializer.class, StringSerializer.class, new Properties()), 0L);
    ...
    private void verifyPartitionsAndStoresForTopology(final String topologyName, final StreamsMetadataImpl metadata) {
        ...
        assertThat(streams1SourceTopicsForTopology.containsAll(metadata.topicPartitions().stream().map(TopicPartition::topic).collect(Collectors.toList())), is(true));
        assertThat(streams2SourceTopicsForTopology.containsAll(metadata.topicPartitions().stream().map(TopicPartition::topic).collect(Collectors.toList())), is(true));
    ...
    private static boolean verifyEquivalentMetadataForHost(final StreamsMetadataImpl left, final StreamsMetadataImpl right) {
        return left.hostInfo().equals(right.hostInfo()) && left.stateStoreNames().equals(right.stateStoreNames()) && left.topicPartitions().equals(right.topicPartitions()) && left.standbyStateStoreNames().equals(right.standbyStateStoreNames()) && left.standbyTopicPartitions().equals(right.standbyTopicPartitions());
    }
    ...
    private static class TrackingExceptionHandler implements StreamsUncaughtExceptionHandler {
        ...
        public synchronized StreamThreadExceptionResponse handle(final Throwable exception) {
            final String topologyName = exception instanceof StreamsException && ((StreamsException) exception).taskId().isPresent() ? ((StreamsException) exception).taskId().get().topologyName() : null;
        ...
        public synchronized Throwable nextError(final String topologyName) {
            return newErrorsByTopology.containsKey(topologyName) ? newErrorsByTopology.get(topologyName).poll() : null;
        }
    }
}
