...
public class ListValueStoreTest {
    ...
    <K, V> KeyValueStore<K, V> buildStore(final Serde<K> keySerde, final Serde<V> valueSerde) {
        return new ListValueStoreBuilder<>(storeType == StoreType.RocksDB ? Stores.persistentKeyValueStore("rocksDB list store") : Stores.inMemoryKeyValueStore("in-memory list store"), keySerde, valueSerde, Time.SYSTEM).build();
    }
    ...
    public void shouldGetAll() {
        ...
        assertEquals(asList(zero, zeroAgain, one, two), toList(listStore.all()));
    }
    ...
    public void shouldGetAllNonDeletedRecords() {
        ...
        assertEquals(asList(zero, two, four), toList(listStore.all()));
    }
    ...
    public void shouldGetAllReturnTimestampOrderedRecords() {
        ...
        assertEquals(asList(zero, one, two1, two2, three, four), toList(listStore.all()));
    }
