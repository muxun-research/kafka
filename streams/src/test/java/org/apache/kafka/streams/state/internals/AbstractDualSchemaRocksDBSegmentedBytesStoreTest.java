...
public abstract class AbstractDualSchemaRocksDBSegmentedBytesStoreTest<S extends Segment> {
    ...
    public void shouldPutAndFetchEdgeSingleKey() {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyA.getBytes()), startEdgeTime, endEdgeTime)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L));

            assertEquals(expected, toList(values));
        }

        // Can fetch start/end edge for single key
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyB.getBytes()), startEdgeTime, endEdgeTime)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L));

            assertEquals(expected, toList(values));
        }

        // Can fetch from 0 to max for single key
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyA.getBytes()), 0, Long.MAX_VALUE)) {
        ...
        // Can fetch from 0 to max for single key
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyB.getBytes()), 0, Long.MAX_VALUE)) {
    ...
    public void shouldPutAndFetchEdgeKeyRange() {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyA.getBytes()), Bytes.wrap(keyB.getBytes()), startEdgeTime, endEdgeTime)) {

            final List<KeyValue<Windowed<String>, Long>> expected = getIndexSchema() == null ? asList(KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L)) : asList(KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L));
            assertEquals(expected, toList(values));
        }

        // Can fetch from 0 to max for key range
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyA.getBytes()), Bytes.wrap(keyB.getBytes()), 0L, Long.MAX_VALUE)) {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(null, Bytes.wrap(keyA.getBytes()), startEdgeTime, endEdgeTime - 1L)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L));

            assertEquals(expected, toList(values));
        }

        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyB.getBytes()), null, startEdgeTime + 1, endEdgeTime)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L));

            assertEquals(expected, toList(values));
        }

        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(null, null, 0, Long.MAX_VALUE)) {

            final List<KeyValue<Windowed<String>, Long>> expected = getIndexSchema() == null ? asList(KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L)) : asList(KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L));
            assertEquals(expected, toList(values));
        }

        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(null, null, startEdgeTime, endEdgeTime)) {
    ...
    public void shouldPutAndBackwardFetchEdgeSingleKey() {
        ...
        // Can fetch start/end edge for single key
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyA.getBytes()), startEdgeTime, endEdgeTime)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L));

            assertEquals(expected, toList(values));
        }

        // Can fetch start/end edge for single key
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyB.getBytes()), startEdgeTime, endEdgeTime)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L));

            assertEquals(expected, toList(values));
        }

        // Can fetch from 0 to max for single key
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyA.getBytes()), 0, Long.MAX_VALUE)) {
        ...
        // Can fetch from 0 to max for single key
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyB.getBytes()), 0, Long.MAX_VALUE)) {
    ...
    public void shouldPutAndBackwardFetchEdgeKeyRange() {
        ...
        // Can fetch from start/end for key range
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyA.getBytes()), Bytes.wrap(keyB.getBytes()), startEdgeTime, endEdgeTime)) {

            final List<KeyValue<Windowed<String>, Long>> expected = getIndexSchema() == null ? asList(KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L)) : asList(KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L));
            assertEquals(expected, toList(values));
        }

        // Can fetch from 0 to max for key range
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyA.getBytes()), Bytes.wrap(keyB.getBytes()), 0L, Long.MAX_VALUE)) {
        ...
        // KeyB should be ignored and KeyA should be included even in storage
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(null, Bytes.wrap(keyA.getBytes()), startEdgeTime, endEdgeTime - 1L)) {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyB.getBytes()), null, startEdgeTime + 1, endEdgeTime)) {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(null, null, 0, Long.MAX_VALUE)) {

            final List<KeyValue<Windowed<String>, Long>> expected = getIndexSchema() == null ? asList(KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L)) : asList(KeyValue.pair(new Windowed<>(keyB, endEdgeWindow), 150L), KeyValue.pair(new Windowed<>(keyB, startEdgeWindow), 100L), KeyValue.pair(new Windowed<>(keyA, endEdgeWindow), 50L), KeyValue.pair(new Windowed<>(keyA, startEdgeWindow), 10L));
            assertEquals(expected, toList(values));
        }

        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(null, null, startEdgeTime, endEdgeTime)) {
    ...
    public void shouldPutAndFetchWithPrefixKey() {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyA.getBytes()), 0, Long.MAX_VALUE)) {
            ...
            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, maxWindow), 10L));

            assertEquals(expected, toList(values));
        }

        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyA.getBytes()), Bytes.wrap(keyB.getBytes()), 0, Long.MAX_VALUE)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyB, maxWindow), 50L), KeyValue.pair(new Windowed<>(keyA, maxWindow), 10L));
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(null, Bytes.wrap(keyB.getBytes()), 0, Long.MAX_VALUE)) {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(Bytes.wrap(keyB.getBytes()), null, 0, Long.MAX_VALUE)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyC, maxWindow), 100L), KeyValue.pair(new Windowed<>(keyB, maxWindow), 50L));
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.fetch(null, null, 0, Long.MAX_VALUE)) {
            ...
            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyC, maxWindow), 100L), KeyValue.pair(new Windowed<>(keyB, maxWindow), 50L), KeyValue.pair(new Windowed<>(keyA, maxWindow), 10L));
    ...
    public void shouldPutAndBackwardFetchWithPrefix() {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyA.getBytes()), Bytes.wrap(keyB.getBytes()), 0, Long.MAX_VALUE)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, maxWindow), 10L), KeyValue.pair(new Windowed<>(keyB, maxWindow), 50L));

            assertEquals(expected, toList(values));
        }

        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(null, Bytes.wrap(keyB.getBytes()), 0, Long.MAX_VALUE)) {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(Bytes.wrap(keyB.getBytes()), null, 0, Long.MAX_VALUE)) {

            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyB, maxWindow), 50L), KeyValue.pair(new Windowed<>(keyC, maxWindow), 100L));
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = bytesStore.backwardFetch(null, null, 0, Long.MAX_VALUE)) {
            ...
            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, maxWindow), 10L), KeyValue.pair(new Windowed<>(keyB, maxWindow), 50L), KeyValue.pair(new Windowed<>(keyC, maxWindow), 100L));
    ...
    public void shouldFetchSessionForSingleKey() {
        ...
        final byte[] value1 = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSession(key1, windows[0].start(), windows[0].end());
        assertNull(value1);

        // Record expired as timestampFromRawKey = 1000 while observedStreamTime = 60,000 and retention = 1000.
        final byte[] value2 = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSession(key1, windows[1].start(), windows[1].end());
        assertNull(value2);

        // expired record
        // timestampFromRawKey = 1500 while observedStreamTime = 60,000 and retention = 1000.
        final byte[] value3 = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSession(key2, windows[2].start(), windows[2].end());
        assertNull(value3);

        // only non-expired record
        // timestampFromRawKey = 60,000 while observedStreamTime = 60,000 and retention = 1000.
        final byte[] value4 = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSession(key3, windows[3].start(), windows[3].end());
        assertEquals(Bytes.wrap(value4), Bytes.wrap(expectedValue4));

        final byte[] noValue = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSession(key3, 2000, 3000);
        assertNull(noValue);
    }
    ...
    public void shouldFetchSessionForTimeRange() {
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSessions(100L, 100L)) {
            ...
            final List<KeyValue<Windowed<String>, Long>> expected = Collections.singletonList(KeyValue.pair(new Windowed<>(keyA, sessionWindows[0]), 10L));
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSessions(100L, 200L)) {
            ...
            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, sessionWindows[0]), 10L), KeyValue.pair(new Windowed<>(keyB, sessionWindows[1]), 100L));
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSessions(100L, 300L)) {
            ...
            final List<KeyValue<Windowed<String>, Long>> expected = asList(KeyValue.pair(new Windowed<>(keyA, sessionWindows[0]), 10L), KeyValue.pair(new Windowed<>(keyB, sessionWindows[1]), 100L), KeyValue.pair(new Windowed<>(keyC, sessionWindows[2]), 200L));
        ...
        try (final KeyValueIterator<Bytes, byte[]> values = ((RocksDBTimeOrderedSessionSegmentedBytesStore) bytesStore).fetchSessions(101L, 299L)) {
            ...
            final List<KeyValue<Windowed<String>, Long>> expected = Collections.singletonList(KeyValue.pair(new Windowed<>(keyB, sessionWindows[1]), 100L));
    ...
    public void shouldSkipAndRemoveDanglingIndex() {
        ...
        if (getIndexSchema() == null) {
            assertThrows(IllegalStateException.class, () -> bytesStore.putIndex(Bytes.wrap(keyA.getBytes()), new byte[0]));
        } else {
            // Only put to index
            ...
            try (final KeyValueIterator<Bytes, byte[]> results = bytesStore.fetch(Bytes.wrap(keyA.getBytes()), Bytes.wrap(keyB.getBytes()), 1, 2000)) {
                ...
                if (getBaseSchema() instanceof TimeFirstWindowKeySchema) {
                    ...
                    expected = asList(KeyValue.pair(new Windowed<>(keyA, windows[0]), 10L), KeyValue.pair(new Windowed<>(keyB, windows[2]), 20L));
                } else {
    ...
    public void shouldFindValuesWithinRange() {
        ...
        try (final KeyValueIterator<Bytes, byte[]> results = bytesStore.fetch(Bytes.wrap(key.getBytes()), 1, 999)) {
            ...
            if (getBaseSchema() instanceof TimeFirstWindowKeySchema) {
                ...
                // and search happens in the range 1-2000
                expected = asList(KeyValue.pair(new Windowed<>(key, windows[0]), 10L), KeyValue.pair(new Windowed<>(key, windows[1]), 50L));
            } else {
                // For session key schema, actual from is 501
    ...
    public void shouldRollSegments() {
        ...
        assertEquals(Collections.singletonList(KeyValue.pair(new Windowed<>(key, windows[3]), 1000L)), results1);
    ...
    public void shouldGetAllSegments() {
        ...
        assertEquals(Collections.singletonList(KeyValue.pair(new Windowed<>(keyB, windows[3]), 100L)), results);
    ...
    public void shouldLoadSegmentsWithOldStyleDateFormattedName() {
        ...
        assertThat(results, equalTo(asList(KeyValue.pair(new Windowed<>(key, windows[0]), 50L), KeyValue.pair(new Windowed<>(key, windows[3]), 100L))));
    ...
    private List<ConsumerRecord<byte[], byte[]>> getChangelogRecords() {
        ...
        records.add(new ConsumerRecord<>("", 0, 0L, RecordBatch.NO_TIMESTAMP, TimestampType.NO_TIMESTAMP_TYPE, -1, -1, serializeKey(new Windowed<>("a", windows[0]), true).get(), serializeValue(50L), headers, Optional.empty()));

        headers.remove(ChangelogRecordDeserializationHelper.CHANGELOG_POSITION_HEADER_KEY);
        position1 = position1.withComponent("", 0, 2);
        headers.add(new RecordHeader(ChangelogRecordDeserializationHelper.CHANGELOG_POSITION_HEADER_KEY, PositionSerde.serialize(position1).array()));
        records.add(new ConsumerRecord<>("", 0, 0L, RecordBatch.NO_TIMESTAMP, TimestampType.NO_TIMESTAMP_TYPE, -1, -1, serializeKey(new Windowed<>("a", windows[2]), true).get(), serializeValue(100L), headers, Optional.empty()));

        headers.remove(ChangelogRecordDeserializationHelper.CHANGELOG_POSITION_HEADER_KEY);
        position1 = position1.withComponent("", 0, 3);
        headers.add(new RecordHeader(ChangelogRecordDeserializationHelper.CHANGELOG_POSITION_HEADER_KEY, PositionSerde.serialize(position1).array()));
        records.add(new ConsumerRecord<>("", 0, 0L, RecordBatch.NO_TIMESTAMP, TimestampType.NO_TIMESTAMP_TYPE, -1, -1, serializeKey(new Windowed<>("a", windows[3]), true).get(), serializeValue(200L), headers, Optional.empty()));
    ...
    private List<ConsumerRecord<byte[], byte[]>> getChangelogRecordsWithTombstones() {
        ...
        headers.add(ChangelogRecordDeserializationHelper.CHANGELOG_VERSION_HEADER_RECORD_CONSISTENCY);
        headers.add(new RecordHeader(ChangelogRecordDeserializationHelper.CHANGELOG_POSITION_HEADER_KEY, PositionSerde.serialize(position).array()));
        records.add(new ConsumerRecord<>("", 0, 0L, RecordBatch.NO_TIMESTAMP, TimestampType.NO_TIMESTAMP_TYPE, -1, -1, serializeKey(new Windowed<>("a", windows[2]), true).get(), null, headers, Optional.empty()));
    ...
    private List<KeyValue<Windowed<String>, Long>> toList(final KeyValueIterator<Bytes, byte[]> iterator) {
        ...
        while (iterator.hasNext()) {
            ...
            if (getBaseSchema() instanceof TimeFirstWindowKeySchema) {
                final KeyValue<Windowed<String>, Long> deserialized = KeyValue.pair(TimeFirstWindowKeySchema.fromStoreKey(next.key.get(), windowSizeForTimeWindow, stateSerdes.keyDeserializer(), stateSerdes.topic()), stateSerdes.valueDeserializer().deserialize("dummy", next.value));
                results.add(deserialized);
            } else if (getBaseSchema() instanceof TimeFirstSessionKeySchema) {
                final KeyValue<Windowed<String>, Long> deserialized = KeyValue.pair(TimeFirstSessionKeySchema.from(next.key.get(), stateSerdes.keyDeserializer(), "dummy"), stateSerdes.valueDeserializer().deserialize("dummy", next.value));
                results.add(deserialized);
            } else {
                throw new IllegalStateException("Unrecognized serde schema");
            }
