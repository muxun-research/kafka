...
public class TaskAssignorConvergenceTest {
    private static final class Harness {
        ...
        private static Harness initializeCluster(final int numStatelessTasks, final int numStatefulTasks, final int numNodes, final Supplier<Integer> partitionCountSupplier) {
            int subtopology = 0;
        ...
        private void recordConfig(final AssignmentConfigs configuration) {
            history.append("Creating assignor with configuration: ").append(configuration).append('\n');
        }
    ...
    public void staticAssignmentShouldConvergeWithTheFirstAssignment() {
        final AssignmentConfigs configs = new AssignmentConfigs(100L, 2, 0, 60_000L, EMPTY_RACK_AWARE_ASSIGNMENT_TAGS);
    ...
    public void assignmentShouldConvergeAfterAddingNode() {
        ...
        final AssignmentConfigs configs = new AssignmentConfigs(100L, maxWarmupReplicas, numStandbyReplicas, 60_000L, EMPTY_RACK_AWARE_ASSIGNMENT_TAGS);
    ...
    private static void verifyBalancedAssignment(final Harness harness) {
        ...
        if (taskSkewReport.totalSkewedTasks() > 0) {
            fail(new StringBuilder().append("Expected a balanced task assignment, but was: ").append(taskSkewReport).append('\n').append(failureContext).toString());
        }
    }
