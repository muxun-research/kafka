...
public class RocksDBWindowStoreTest extends AbstractWindowBytesStoreTest {
    ...
    private final KeyValueSegments segments = new KeyValueSegments(STORE_NAME, METRICS_SCOPE, RETENTION_PERIOD, SEGMENT_INTERVAL);

    enum StoreType {
        RocksDBWindowStore, RocksDBTimeOrderedWindowStoreWithIndex, RocksDBTimeOrderedWindowStoreWithoutIndex
    }
    ...
    public static Collection<Object[]> getKeySchema() {
        return asList(new Object[][]{{StoreType.RocksDBWindowStore}, {StoreType.RocksDBTimeOrderedWindowStoreWithIndex}, {StoreType.RocksDBTimeOrderedWindowStoreWithoutIndex}});
    }

    @Override
    <K, V> WindowStore<K, V> buildWindowStore(final long retentionPeriod, final long windowSize, final boolean retainDuplicates, final Serde<K> keySerde, final Serde<V> valueSerde) {

        switch (storeType) {
            case RocksDBWindowStore: {
                return Stores.windowStoreBuilder(Stores.persistentWindowStore(STORE_NAME, ofMillis(retentionPeriod), ofMillis(windowSize), retainDuplicates), keySerde, valueSerde).build();
            }
            case RocksDBTimeOrderedWindowStoreWithIndex: {
                final long defaultSegmentInterval = Math.max(retentionPeriod / 2, 60_000L);
                return Stores.windowStoreBuilder(new RocksDbIndexedTimeOrderedWindowBytesStoreSupplier(STORE_NAME, retentionPeriod, defaultSegmentInterval, windowSize, retainDuplicates, true), keySerde, valueSerde).build();
            }
            case RocksDBTimeOrderedWindowStoreWithoutIndex: {
                final long defaultSegmentInterval = Math.max(retentionPeriod / 2, 60_000L);
                return Stores.windowStoreBuilder(new RocksDbIndexedTimeOrderedWindowBytesStoreSupplier(STORE_NAME, retentionPeriod, defaultSegmentInterval, windowSize, retainDuplicates, false), keySerde, valueSerde).build();
            }
            default:
    ...
    public void testRolling() {
        ...
        if (storeType == StoreType.RocksDBWindowStore) {
            assertEquals(
                    // expired record
                    new HashSet<>(Collections.emptyList()), valuesToSet(windowStore.fetch(2, ofEpochMilli(startTime + increment * 2 - WINDOW_SIZE), ofEpochMilli(startTime + increment * 2 + WINDOW_SIZE))));
        } else {
            assertEquals(
                    // expired record
                    new HashSet<>(Collections.singletonList("two")), valuesToSet(windowStore.fetch(2, ofEpochMilli(startTime + increment * 2 - WINDOW_SIZE), ofEpochMilli(startTime + increment * 2 + WINDOW_SIZE))));
        }
        assertEquals(new HashSet<>(Collections.emptyList()), valuesToSet(windowStore.fetch(3, ofEpochMilli(startTime + increment * 3 - WINDOW_SIZE), ofEpochMilli(startTime + increment * 3 + WINDOW_SIZE))));
        assertEquals(new HashSet<>(Collections.singletonList("four")), valuesToSet(windowStore.fetch(4, ofEpochMilli(startTime + increment * 4 - WINDOW_SIZE), ofEpochMilli(startTime + increment * 4 + WINDOW_SIZE))));
        assertEquals(new HashSet<>(Collections.singletonList("five")), valuesToSet(windowStore.fetch(5, ofEpochMilli(startTime + increment * 5 - WINDOW_SIZE), ofEpochMilli(startTime + increment * 5 + WINDOW_SIZE))));
        ...
        if (storeType == StoreType.RocksDBWindowStore) {
            assertEquals(
                    // expired record
                    new HashSet<>(Collections.emptyList()), valuesToSet(windowStore.fetch(4, ofEpochMilli(startTime + increment * 4 - WINDOW_SIZE), ofEpochMilli(startTime + increment * 4 + WINDOW_SIZE))));
        } else {
            assertEquals(
                    // expired record
                    new HashSet<>(Collections.singletonList("four")), valuesToSet(windowStore.fetch(4, ofEpochMilli(startTime + increment * 4 - WINDOW_SIZE), ofEpochMilli(startTime + increment * 4 + WINDOW_SIZE))));
