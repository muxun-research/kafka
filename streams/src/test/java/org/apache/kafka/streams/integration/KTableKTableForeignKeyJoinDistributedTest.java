...
public class KTableKTableForeignKeyJoinDistributedTest {
    ...
    public void setupTopics() throws InterruptedException {
        ...
        final List<KeyValue<String, String>> leftTable = Arrays.asList(new KeyValue<>("lhsValue1", "lhsValue1|rhs1"), new KeyValue<>("lhsValue2", "lhsValue2|rhs2"), new KeyValue<>("lhsValue3", "lhsValue3|rhs3"), new KeyValue<>("lhsValue4", "lhsValue4|rhs4"));
        final List<KeyValue<String, String>> rightTable = Arrays.asList(new KeyValue<>("rhs1", "rhsValue1"), new KeyValue<>("rhs2", "rhsValue2"), new KeyValue<>("rhs3", "rhsValue3"));
    ...
    public void shouldBeInitializedWithDefaultSerde(final TestInfo testInfo) throws Exception {
        ...
        createClients(builder1.build(streamsConfiguration1), streamsConfiguration1, builder2.build(streamsConfiguration2), streamsConfiguration2);
        ...
        final Set<KeyValue<String, String>> result = new HashSet<>(IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(CONSUMER_CONFIG, OUTPUT, expectedResult.size()));
    ...
    private void createClients(final Topology topology1, final Properties streamsConfiguration1, final Topology topology2, final Properties streamsConfiguration2) {
    ...
    private void setStateListenersForVerification(final Predicate<ThreadMetadata> taskCondition) {
        client1.setStateListener((newState, oldState) -> {
            if (newState == KafkaStreams.State.RUNNING && client1.metadataForLocalThreads().stream().allMatch(taskCondition)) {
                client1IsOk = true;
            }
        });
        client2.setStateListener((newState, oldState) -> {
            if (newState == KafkaStreams.State.RUNNING && client2.metadataForLocalThreads().stream().allMatch(taskCondition)) {
                client2IsOk = true;
            }
        });
    }
