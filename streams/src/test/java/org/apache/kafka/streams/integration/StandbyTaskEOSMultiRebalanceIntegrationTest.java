...
public class StandbyTaskEOSMultiRebalanceIntegrationTest {
    ...
    public void shouldHonorEOSWhenUsingCachingAndStandbyReplicas() throws Exception {
        ...
        IntegrationTestUtils.produceKeyValuesSynchronouslyWithTimestamp(inputTopic, IntStream.range(0, initialBulk).boxed().map(i -> new KeyValue<>(i, i)).collect(Collectors.toList()), TestUtils.producerConfig(CLUSTER.bootstrapServers(), IntegerSerializer.class, IntegerSerializer.class, new Properties()), 10L + time);
        ...
        IntegrationTestUtils.waitUntilMinRecordsReceived(TestUtils.consumerConfig(CLUSTER.bootstrapServers(), UUID.randomUUID().toString(), IntegerDeserializer.class, IntegerDeserializer.class, readCommitted), outputTopic, initialBulk);
        ...
        waitForCondition(() -> streamInstanceTwo.store(StoreQueryParameters.fromNameAndType(storeName, QueryableStoreTypes.<Integer, Integer>keyValueStore()).enableStaleStores()).get(0) != null, TWO_MINUTE_TIMEOUT, "Could not get key from standby store");
        LOG.info("Second stream have some data in the state store");


        LOG.info("Produce the second bulk");
        IntegrationTestUtils.produceKeyValuesSynchronouslyWithTimestamp(inputTopic, IntStream.range(initialBulk, initialBulk + secondBulk).boxed().map(i -> new KeyValue<>(i, i)).collect(Collectors.toList()), TestUtils.producerConfig(CLUSTER.bootstrapServers(), IntegerSerializer.class, IntegerSerializer.class, new Properties()), 1000L + time);
    ...
    private KafkaStreams buildWithUniqueIdAssignmentTopology(final String stateDirPath) {
        ...
        builder.addStateStore(Stores.keyValueStoreBuilder(Stores.persistentKeyValueStore(counterName), Serdes.Integer(), Serdes.Integer()).withCachingEnabled());
        builder.<Integer, Integer>stream(inputTopic).process(() -> new Processor<Integer, Integer, Integer, Integer>() {
        ...
        }, storeName, counterName).to(outputTopic);
