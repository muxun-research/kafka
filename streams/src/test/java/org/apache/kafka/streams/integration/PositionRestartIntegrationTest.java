...
public class PositionRestartIntegrationTest {
    ...
    public PositionRestartIntegrationTest(final boolean cache, final boolean log, final String storeToTest, final String kind) {
        this.cache = cache;
        this.log = log;
        this.storeToTest = StoresToTest.valueOf(storeToTest);
        this.kind = kind;
        this.streamsConfig = streamsConfiguration(cache, log, storeToTest, kind);
    }
    ...
    public void verifyStore() {
        ...
        if (storeToTest.keyValue()) {
            ...
            query = WindowKeyQuery.withKeyAndWindowStartRange(2, Instant.ofEpochMilli(WINDOW_START), Instant.ofEpochMilli(WINDOW_START));
    ...
    private void shouldReachExpectedPosition(final Query<?> query) {
        final StateQueryRequest<?> request = inStore(STORE_NAME).withQuery(query).withPartitions(mkSet(0, 1)).withPositionBound(PositionBound.at(INPUT_POSITION));
    ...
    private static Properties streamsConfiguration(final boolean cache, final boolean log, final String supplier, final String kind) {
        final String safeTestName = PositionRestartIntegrationTest.class.getName() + "-" + cache + "-" + log + "-" + supplier + "-" + kind;
