...
public class KStreamKTableJoinTest {
    ...
    private void pushToTableNonRandom(final int messageCount, final String valuePrefix) {
        for (int i = 0; i < messageCount; i++) {
            inputTableTopic.pipeInput(expectedKeys[i], valuePrefix + expectedKeys[i], 0);
        }
    }
    ...
    private void makeJoin(final Duration grace) {
        ...
        final Consumed<Integer, String> consumed = Consumed.with(Serdes.Integer(), Serdes.String());
        stream = builder.stream(streamTopic, consumed);
        table = builder.table("tableTopic2", consumed, Materialized.as(Stores.persistentVersionedKeyValueStore("V-grace", Duration.ofMinutes(5))));
        stream.join(table, MockValueJoiner.TOSTRING_JOINER, Joined.with(Serdes.Integer(), Serdes.String(), Serdes.String(), "Grace", grace)).process(supplier);
        final Properties props = StreamsTestUtils.getStreamsConfig(Serdes.Integer(), Serdes.String());
        driver = new TopologyTestDriver(builder.build(), props);
        inputStreamTopic = driver.createInputTopic(streamTopic, new IntegerSerializer(), new StringSerializer(), Instant.ofEpochMilli(0L), Duration.ZERO);
    ...
    public void shouldFailIfTableIsNotVersioned() {
        ...
        final IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> streamA.join(tableB, (value1, value2) -> value1 + value2, Joined.with(Serdes.String(), Serdes.String(), Serdes.String(), "first-join", Duration.ofMillis(6))).to("out-one"));
        assertThat(exception.getMessage(), is("KTable must be versioned to use a grace period in a stream table join."));
    }
    ...
    public void shouldDelayJoinByGracePeriod() {
        ...
        pushToStream(4, "X");
        processor.checkAndClearProcessResult(new KeyValueTimestamp<>(0, "X0+Y0", 0), new KeyValueTimestamp<>(1, "X1+Y1", 1));
        ...
        // push all four items to the primary stream. this should produce two items.
        pushToStream(4, "X");
        processor.checkAndClearProcessResult(new KeyValueTimestamp<>(0, "X0+YY0", 0), new KeyValueTimestamp<>(1, "X1+YY1", 1));

        inputStreamTopic.pipeInput(5, "test", 7);

        processor.checkAndClearProcessResult(new KeyValueTimestamp<>(2, "X2+YY2", 2), new KeyValueTimestamp<>(2, "X2+YY2", 2), new KeyValueTimestamp<>(3, "X3+YY3", 3), new KeyValueTimestamp<>(3, "X3+YY3", 3));
    ...
    public void shouldClearTableEntryOnNullValueUpdates() {
        ...
        // push all four items to the primary stream. this should produce four items.
        pushToStream(4, "X");
        processor.checkAndClearProcessResult(new KeyValueTimestamp<>(0, "X0+Y0", 0), new KeyValueTimestamp<>(1, "X1+Y1", 1), new KeyValueTimestamp<>(2, "X2+Y2", 2), new KeyValueTimestamp<>(3, "X3+Y3", 3));
        ...
        // push all four items to the primary stream. this should produce two items.
        pushToStream(4, "XX");
        processor.checkAndClearProcessResult(new KeyValueTimestamp<>(2, "XX2+Y2", 2), new KeyValueTimestamp<>(3, "XX3+Y3", 3));
    }
    ...
    public void shouldLogAndMeterWhenSkippingNullLeftKey() {
        try (final LogCaptureAppender appender = LogCaptureAppender.createAndRegister(KStreamKTableJoin.class)) {
            final TestInputTopic<Integer, String> inputTopic = driver.createInputTopic(streamTopic, new IntegerSerializer(), new StringSerializer());
            inputTopic.pipeInput(null, "A");

            assertThat(appender.getMessages(), hasItem("Skipping record due to null join key or value. topic=[streamTopic] partition=[0] " + "offset=[0]"));
        }
    }
