...
public class PurgeRepartitionTopicIntegrationTest {
    ...
    private class RepartitionTopicVerified implements TestCondition {
        ...
        public final boolean conditionMet() {
            ...
            try {
                final Collection<LogDirDescription> logDirInfo = adminClient.describeLogDirs(Collections.singleton(0)).descriptions().get(0).get().values();

                for (final LogDirDescription partitionInfo : logDirInfo) {
                    final ReplicaInfo replicaInfo = partitionInfo.replicaInfos().get(new TopicPartition(REPARTITION_TOPIC, 0));
    ...
    public void shouldRestoreState() throws Exception {
        ...
        TestUtils.waitForCondition(new RepartitionTopicVerified(currentSize -> currentSize > PURGE_SEGMENT_BYTES), 60000, "Repartition topic " + REPARTITION_TOPIC + " not received more than " + PURGE_SEGMENT_BYTES + "B of data after 60000 ms.");
