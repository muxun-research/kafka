...
public class IQv2IntegrationTest {
    ...
    public static void before() throws InterruptedException, IOException, ExecutionException, TimeoutException {
        ...
        try (final Producer<Integer, Integer> producer = new KafkaProducer<>(producerProps)) {
            for (int i = 0; i < 3; i++) {
                final Future<RecordMetadata> send = producer.send(new ProducerRecord<>(INPUT_TOPIC_NAME, i % partitions, Time.SYSTEM.milliseconds(), i, i, null));
                futures.add(send);
        ...
        assertThat(INPUT_POSITION, equalTo(Position.emptyPosition().withComponent(INPUT_TOPIC_NAME, 0, 1L).withComponent(INPUT_TOPIC_NAME, 1, 0L)));
    }
    ...
    public void beforeTest(final TestInfo testInfo) {
        ...
        builder.table(INPUT_TOPIC_NAME, Consumed.with(Serdes.Integer(), Serdes.Integer()), Materialized.as(STORE_NAME));
    ...
    public void shouldFailUnknownStore() {
        ...
        final StateQueryRequest<ValueAndTimestamp<Integer>> request = inStore("unknown-store").withQuery(query);
    ...
    public void shouldFetchExplicitlyFromAllPartitions() {
        final KeyQuery<Integer, ValueAndTimestamp<Integer>> query = KeyQuery.withKey(1);
        final Set<Integer> partitions = mkSet(0, 1);
        final StateQueryRequest<ValueAndTimestamp<Integer>> request = inStore(STORE_NAME).withQuery(query).withAllPartitions();
