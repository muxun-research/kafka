...
import org.apache.kafka.coordinator.group.consumer.*;
...
public class GroupMetadataManager {
    ...
    public static class Builder {
        ...
        GroupMetadataManager build() {
            ...
            return new GroupMetadataManager(snapshotRegistry, logContext, time, timer, assignors, metadataImage, consumerGroupMaxSize, consumerGroupSessionTimeoutMs, consumerGroupHeartbeatIntervalMs, consumerGroupMetadataRefreshIntervalMs);
        }
    }
    ...
    private GroupMetadataManager(SnapshotRegistry snapshotRegistry, LogContext logContext, Time time, CoordinatorTimer<Record> timer, List<PartitionAssignor> assignors, MetadataImage metadataImage, int consumerGroupMaxSize, int consumerGroupSessionTimeoutMs, int consumerGroupHeartbeatIntervalMs, int consumerGroupMetadataRefreshIntervalMs) {
    ...
    private void throwIfConsumerGroupHeartbeatRequestIsInvalid(ConsumerGroupHeartbeatRequestData request) throws InvalidRequestException, UnsupportedAssignorException {
        ...
        if (request.serverAssignor() != null && !assignors.containsKey(request.serverAssignor())) {
            throw new UnsupportedAssignorException("ServerAssignor " + request.serverAssignor() + " is not supported. Supported assignors: " + String.join(", ", assignors.keySet()) + ".");
        }
    }
    ...
    private void throwIfConsumerGroupIsFull(ConsumerGroup group, String memberId) throws GroupMaxSizeReachedException {
        ...
        if (group.numMembers() >= consumerGroupMaxSize && (memberId.isEmpty() || !group.hasMember(memberId))) {
            throw new GroupMaxSizeReachedException("The consumer group has reached its maximum capacity of " + consumerGroupMaxSize + " members.");
        }
    }
    ...
    private ConsumerGroupHeartbeatResponseData.Assignment createResponseAssignment(ConsumerGroupMember member) {
        ConsumerGroupHeartbeatResponseData.Assignment assignment = new ConsumerGroupHeartbeatResponseData.Assignment().setAssignedTopicPartitions(fromAssignmentMap(member.assignedPartitions()));
    ...
    private List<ConsumerGroupHeartbeatResponseData.TopicPartitions> fromAssignmentMap(Map<Uuid, Set<Integer>> assignment) {
        return assignment.entrySet().stream().map(keyValue -> new ConsumerGroupHeartbeatResponseData.TopicPartitions().setTopicId(keyValue.getKey()).setPartitions(new ArrayList<>(keyValue.getValue()))).collect(Collectors.toList());
    }
    ...
    private CoordinatorResult<ConsumerGroupHeartbeatResponseData, Record> consumerGroupHeartbeat(String groupId, String memberId, int memberEpoch, String instanceId, String rackId, int rebalanceTimeoutMs, String clientId, String clientHost, List<String> subscribedTopicNames, String subscribedTopicRegex, String assignorName, List<ConsumerGroupHeartbeatRequestData.TopicPartitions> ownedTopicPartitions) throws ApiException {
        ...
        if (!updatedMember.equals(member)) {
            ...
            if (!updatedMember.subscribedTopicNames().equals(member.subscribedTopicNames())) {
                log.info("[GroupId {}] Member {} updated its subscribed topics to: {}.", groupId, memberId, updatedMember.subscribedTopicNames());
                bumpGroupEpoch = true;
            }

            if (!updatedMember.subscribedTopicRegex().equals(member.subscribedTopicRegex())) {
                log.info("[GroupId {}] Member {} updated its subscribed regex to: {}.", groupId, memberId, updatedMember.subscribedTopicRegex());
                bumpGroupEpoch = true;
            }
        }
        ...
        if (bumpGroupEpoch || group.hasMetadataExpired(currentTimeMs)) {
            ...
            subscriptionMetadata = group.computeSubscriptionMetadata(member, updatedMember, metadataImage.topics());

            if (!subscriptionMetadata.equals(group.subscriptionMetadata())) {
                log.info("[GroupId {}] Computed new subscription metadata: {}.", groupId, subscriptionMetadata);
                bumpGroupEpoch = true;
        ...
        if (groupEpoch > targetAssignmentEpoch) {
            String preferredServerAssignor = group.computePreferredServerAssignor(member, updatedMember).orElse(defaultAssignor.name());
            ...
            try {
                ...
                log.info("[GroupId {}] Computed a new target assignment for epoch {}: {}.", groupId, groupEpoch, assignmentResult.targetAssignment());
        ...
        if (updatedMember.state() != ConsumerGroupMember.MemberState.STABLE || updatedMember.targetMemberEpoch() != targetAssignmentEpoch) {
            ...
            updatedMember = new CurrentAssignmentBuilder(updatedMember).withTargetAssignment(targetAssignmentEpoch, targetAssignment).withCurrentPartitionEpoch(group::currentPartitionEpoch).withOwnedTopicPartitions(ownedTopicPartitions).build();
            ...
            if (updatedMember != prevMember) {
                ...
                log.info("[GroupId {}] Member {} transitioned from {} to {}.", groupId, memberId, member.currentAssignmentSummary(), updatedMember.currentAssignmentSummary());

                if (updatedMember.state() == ConsumerGroupMember.MemberState.REVOKING) {
                    scheduleConsumerGroupRevocationTimeout(groupId, memberId, updatedMember.rebalanceTimeoutMs(), updatedMember.memberEpoch());
                } else {
        ...
        ConsumerGroupHeartbeatResponseData response = new ConsumerGroupHeartbeatResponseData().setMemberId(updatedMember.memberId()).setMemberEpoch(updatedMember.memberEpoch()).setHeartbeatIntervalMs(consumerGroupHeartbeatIntervalMs);
    ...
    private CoordinatorResult<ConsumerGroupHeartbeatResponseData, Record> consumerGroupLeave(String groupId, String memberId) throws ApiException {
        ...
        return new CoordinatorResult<>(records, new ConsumerGroupHeartbeatResponseData().setMemberId(memberId).setMemberEpoch(-1));
    }
    ...
    private List<Record> consumerGroupFenceMember(ConsumerGroup group, ConsumerGroupMember member) {
        ...
        Map<String, TopicMetadata> subscriptionMetadata = group.computeSubscriptionMetadata(member, null, metadataImage.topics());

        if (!subscriptionMetadata.equals(group.subscriptionMetadata())) {
            log.info("[GroupId {}] Computed new subscription metadata: {}.", group.groupId(), subscriptionMetadata);
    ...
    private void scheduleConsumerGroupSessionTimeout(String groupId, String memberId) {
        ...
        timer.schedule(key, consumerGroupSessionTimeoutMs, TimeUnit.MILLISECONDS, true, () -> {
            try {
                ConsumerGroup group = getOrMaybeCreateConsumerGroup(groupId, false);
                ConsumerGroupMember member = group.getOrMaybeCreateMember(memberId, false);
                log.info("[GroupId {}] Member {} fenced from the group because its session expired.", groupId, memberId);
                return consumerGroupFenceMember(group, member);
            } catch (GroupIdNotFoundException ex) {
                log.debug("[GroupId {}] Could not fence {} because the group does not exist.", groupId, memberId);
            } catch (UnknownMemberIdException ex) {
                log.debug("[GroupId {}] Could not fence {} because the member does not exist.", groupId, memberId);
            }
        ...
        timer.schedule(key, revocationTimeoutMs, TimeUnit.MILLISECONDS, true, () -> {
            try {
                ...
                if (member.state() != ConsumerGroupMember.MemberState.REVOKING || member.memberEpoch() != expectedMemberEpoch) {
                    log.debug("[GroupId {}] Ignoring revocation timeout for {} because the member " + "state does not match the expected state.", groupId, memberId);
                    return Collections.emptyList();
                }

                log.info("[GroupId {}] Member {} fenced from the group because " + "it failed to revoke partitions within {}ms.", groupId, memberId, revocationTimeoutMs);
                return consumerGroupFenceMember(group, member);
            } catch (GroupIdNotFoundException ex) {
                log.debug("[GroupId {}] Could not fence {}} because the group does not exist.", groupId, memberId);
            } catch (UnknownMemberIdException ex) {
    ...
    public CoordinatorResult<ConsumerGroupHeartbeatResponseData, Record> consumerGroupHeartbeat(RequestContext context, ConsumerGroupHeartbeatRequestData request) throws ApiException {
        ...
        if (request.memberEpoch() == -1) {
            ...
            return consumerGroupLeave(request.groupId(), request.memberId());
        } else {
            // Otherwise, it is a regular heartbeat.
            return consumerGroupHeartbeat(request.groupId(), request.memberId(), request.memberEpoch(), request.instanceId(), request.rackId(), request.rebalanceTimeoutMs(), context.clientId(), context.clientAddress.toString(), request.subscribedTopicNames(), request.subscribedTopicRegex(), request.serverAssignor(), request.topicPartitions());
        }
    }
    ...
    private void subscribeGroupToTopic(String groupId, String topicName) {
        groupsByTopics.computeIfAbsent(topicName, __ -> new TimelineHashSet<>(snapshotRegistry, 1)).add(groupId);
    }
    ...
    private void updateGroupsByTopics(String groupId, Set<String> oldSubscribedTopics, Set<String> newSubscribedTopics) {
        if (oldSubscribedTopics.isEmpty()) {
            newSubscribedTopics.forEach(topicName -> subscribeGroupToTopic(groupId, topicName));
        } else if (newSubscribedTopics.isEmpty()) {
            oldSubscribedTopics.forEach(topicName -> unsubscribeGroupFromTopic(groupId, topicName));
        } else {
                ...
                case CONSUMER:
                    ...
                    consumerGroup.members().forEach((memberId, member) -> {
                        ...
                        if (member.state() == ConsumerGroupMember.MemberState.REVOKING) {
                            scheduleConsumerGroupRevocationTimeout(groupId, memberId, member.rebalanceTimeoutMs(), member.memberEpoch());
                        }
                    });
                    break;
