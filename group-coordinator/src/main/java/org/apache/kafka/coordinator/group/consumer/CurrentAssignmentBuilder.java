...
public class CurrentAssignmentBuilder {
    ...
    private ConsumerGroupMember transitionToNewTargetAssignmentState() {
        ...
        for (Uuid topicId : allTopicIds) {
            ...
            Set<Integer> currentAssignedPartitions = member.assignedPartitions().getOrDefault(topicId, Collections.emptySet());
            Set<Integer> currentRevokingPartitions = member.partitionsPendingRevocation().getOrDefault(topicId, Collections.emptySet());
        ...
        if (!newPartitionsPendingRevocation.isEmpty()) {
            ...
            return new ConsumerGroupMember.Builder(member).setAssignedPartitions(newAssignedPartitions).setPartitionsPendingRevocation(newPartitionsPendingRevocation).setPartitionsPendingAssignment(newPartitionsPendingAssignment).setTargetMemberEpoch(targetAssignmentEpoch).build();
        } else {
    ...
    private boolean matchesAssignedPartitions(List<ConsumerGroupHeartbeatRequestData.TopicPartitions> ownedTopicPartitions) {
        if (ownedTopicPartitions == null)
            return false;
        if (ownedTopicPartitions.size() != member.assignedPartitions().size())
            return false;
