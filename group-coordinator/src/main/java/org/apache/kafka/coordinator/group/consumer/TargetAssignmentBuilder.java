...
public class TargetAssignmentBuilder {
    ...
    public static class TargetAssignmentResult {
        ...
        TargetAssignmentResult(List<org.apache.kafka.coordinator.group.Record> records, Map<String, Assignment> targetAssignment) {
    ...
    public TargetAssignmentResult build() throws PartitionAssignorException {
        ...
        members.forEach((memberId, member) -> memberSpecs.put(memberId, createAssignmentMemberSpec(member, targetAssignment.getOrDefault(memberId, Assignment.EMPTY), subscriptionMetadata)));
        ...
        subscriptionMetadata.forEach((topicName, topicMetadata) -> topics.put(topicMetadata.id(), new AssignmentTopicMetadata(topicMetadata.numPartitions())));

        // Compute the assignment.
        GroupAssignment newGroupAssignment = assignor.assign(new AssignmentSpec(Collections.unmodifiableMap(memberSpecs), Collections.unmodifiableMap(topics)));
        ...
        memberSpecs.keySet().forEach(memberId -> {
            ...
            if (oldMemberAssignment == null) {
                ...
                records.add(newTargetAssignmentRecord(groupId, memberId, newMemberAssignment.partitions()));
            } else {
