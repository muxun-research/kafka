...
public class GenericGroup implements Group {
    ...
    public boolean usesConsumerGroupProtocol() {
        return protocolType.map(type -> type.equals(ConsumerProtocol.PROTOCOL_TYPE)).orElse(false);
    }
    ...
    public void add(GenericGroupMember member, CompletableFuture<JoinGroupResponseData> future) {
        member.groupInstanceId().ifPresent(instanceId -> {
            if (staticMembers.containsKey(instanceId)) {
                throw new IllegalStateException("Static member with groupInstanceId=" + instanceId + " cannot be added to group " + groupId + " since" + " it is already a member.");
            }
        ...
        if (leaderId.isPresent()) {
            ...
            if (!currentLeader.isAwaitingJoin()) {
                for (GenericGroupMember member : members.values()) {
                    if (member.isAwaitingJoin()) {
                        leaderId = Optional.of(member.memberId());
                        log.info("Group leader [memberId: {}, groupInstanceId: {}] " + "failed to join before the rebalance timeout. Member {} " + "was elected as the new leader.", currentLeader.memberId(), currentLeader.groupInstanceId().orElse("None"), member);
                        return true;
                    }
                }
                log.info("Group leader [memberId: {}, groupInstanceId: {}] " + "failed to join before the rebalance timeout and the " + "group couldn't proceed to the next generation because " + "no member joined.", currentLeader.memberId(), currentLeader.groupInstanceId().orElse("None"));
                return false;
            }
            return true;
        }
    ...
    public GenericGroupMember replaceStaticMember(String groupInstanceId, String oldMemberId, String newMemberId) {
        ...
        JoinGroupResponseData joinGroupResponse = new JoinGroupResponseData().setMembers(Collections.emptyList()).setMemberId(oldMemberId).setGenerationId(NO_GENERATION).setProtocolName(null).setProtocolType(null).setLeader(NO_LEADER).setSkipAssignment(false).setErrorCode(Errors.FENCED_INSTANCE_ID.code());
    ...
    public String selectProtocol() {
        ...
        allMembers().stream().map(member -> member.vote(candidates)).forEach(protocolName -> {
    ...
    private Set<String> candidateProtocols() {
        ...
        return supportedProtocols.entrySet().stream().filter(protocol -> protocol.getValue() == members.size()).map(Map.Entry::getKey).collect(Collectors.toSet());
    }
    ...
    public boolean supportsProtocols(GenericGroupMember member) {
        return supportsProtocols(member.protocolType(), GenericGroupMember.plainProtocolSet(member.supportedProtocols()));
    }
    ...
    public boolean supportsProtocols(String memberProtocolType, Set<String> memberProtocols) {
        if (isInState(EMPTY)) {
            ...
            return protocolType.map(type -> type.equals(memberProtocolType)).orElse(false) && memberProtocols.stream().anyMatch(name -> supportedProtocols.getOrDefault(name, 0) == members.size());
        }
    }
        ...
        if (protocolName.isPresent()) {
            try {
                ...
                members.values().forEach(member -> {
                    ...
                    allSubscribedTopics.addAll(new HashSet<>(ConsumerProtocol.deserializeSubscription(buffer, (short) 0).topics()));
                });
                return Optional.of(allSubscribedTopics);
            } catch (SchemaException e) {
                log.warn("Failed to parse Consumer Protocol " + ConsumerProtocol.PROTOCOL_TYPE + ":" + protocolName.get() + " of group " + groupId + ". " + "Consumer group coordinator is not aware of the subscribed topics.", e);
            }
        }
    ...
    public List<JoinGroupResponseMember> currentGenericGroupMembers() {
        if (isInState(DEAD) || isInState(PREPARING_REBALANCE)) {
            throw new IllegalStateException("Cannot obtain generic member metadata for group " + groupId + " in state " + state);
        }

        return members.values().stream().map(member -> new JoinGroupResponseMember().setMemberId(member.memberId()).setGroupInstanceId(member.groupInstanceId().orElse(null)).setMetadata(member.metadata(protocolName.orElse(null)))).collect(Collectors.toList());
    }
    ...
    public ListGroupsResponseData.ListedGroup asListedGroup() {
        return new ListGroupsResponseData.ListedGroup().setGroupId(groupId).setProtocolType(protocolType.orElse("")).setGroupState(state.toString());
    }
    ...
    private void assertValidTransition(GenericGroupState targetState) {
        if (!targetState.validPreviousStates().contains(state)) {
            throw new IllegalStateException("Group " + groupId + " should be in one of " + targetState.validPreviousStates() + " states before moving to " + targetState + " state. Instead it is in " + state + " state.");
        }
    }

    @Override
    public String toString() {
        return "GenericGroupMetadata(" + "groupId=" + groupId + ", " + "generation=" + generationId + ", " + "protocolType=" + protocolType + ", " + "currentState=" + currentState() + ", " + "members=" + members + ")";
    }
}
