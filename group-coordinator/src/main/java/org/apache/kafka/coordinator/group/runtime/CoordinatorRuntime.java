...
public class CoordinatorRuntime<S extends Coordinator<U>, U> implements AutoCloseable {
    ...
    public static class Builder<S extends Coordinator<U>, U> {
        ...
        public CoordinatorRuntime<S, U> build() {
            ...
            return new CoordinatorRuntime<>(logPrefix, logContext, eventProcessor, partitionWriter, loader, coordinatorBuilderSupplier, time, timer);
        }
    }
    ...
    class EventBasedCoordinatorTimer implements CoordinatorTimer<U> {
        ...
        @Override
        public void schedule(String key, long delay, TimeUnit unit, boolean retry, TimeoutOperation<U> operation) {
                ...
                public void run() {
                    ...
                    event.future.exceptionally(ex -> {
                        if (ex instanceof RejectedExecutionException) {
                            log.debug("The write event {} for the timer {} was not executed because it was " + "cancelled or overridden.", event.name, key);
                            return null;
                        }

                        if (ex instanceof NotCoordinatorException || ex instanceof CoordinatorLoadInProgressException) {
                            log.debug("The write event {} for the timer {} failed due to {}. Ignoring it because " + "the coordinator is not active.", event.name, key, ex.getMessage());
                            return null;
                        }

                        if (retry) {
                            log.info("The write event {} for the timer {} failed due to {}. Rescheduling it. ", event.name, key, ex.getMessage());
                            schedule(key, 500, TimeUnit.MILLISECONDS, retry, operation);
                        } else {
                            log.error("The write event {} for the timer {} failed due to {}. Ignoring it. ", event.name, key, ex.getMessage());
                        }
                    ...
                    try {
                        ...
                        log.info("Failed to enqueue write event {} for timer {} because the runtime is closed. Ignoring it.", event.name, key);
                    }
                }
            };
    ...
    class CoordinatorContext {
        ...
        private CoordinatorContext(TopicPartition tp) {
            this.tp = tp;
            this.logContext = new LogContext(String.format("[%s topic=%s partition=%d] ", logPrefix, tp.topic(), tp.partition()));
        ...
        private void updateLastWrittenOffset(long offset) {
            if (offset <= lastWrittenOffset) {
                throw new IllegalStateException("New last written offset " + offset + " of " + tp + " must be larger than " + lastWrittenOffset + ".");
            }
        ...
        private void revertLastWrittenOffset(long offset) {
            if (offset > lastWrittenOffset) {
                throw new IllegalStateException("New offset " + offset + " of " + tp + " must be smaller than " + lastWrittenOffset + ".");
            }
        ...
        private void updateLastCommittedOffset(long offset) {
            if (offset <= lastCommittedOffset) {
                throw new IllegalStateException("New committed offset " + offset + " of " + tp + " must be larger than " + lastCommittedOffset + ".");
            }
        ...
        private void transitionTo(CoordinatorState newState) {
            ...
            switch (newState) {
                case LOADING:
                    ...
                    coordinator = coordinatorBuilderSupplier.get().withLogContext(logContext).withSnapshotRegistry(snapshotRegistry).withTime(time).withTimer(timer).build();
                    break;
    ...
    class CoordinatorReadEvent<T> implements CoordinatorEvent {
        ...
        public void run() {
            try {
                ...
                response = op.generateResponse(context.coordinator, context.lastCommittedOffset);
     ...
     */
    private CoordinatorRuntime(String logPrefix, LogContext logContext, CoordinatorEventProcessor processor, PartitionWriter<U> partitionWriter, CoordinatorLoader<U> loader, CoordinatorBuilderSupplier<S, U> coordinatorBuilderSupplier, Time time, Timer timer) {
        this.logPrefix = logPrefix;
        this.logContext = logContext;
    ...
    public void scheduleLoadOperation(TopicPartition tp, int partitionEpoch) {
        ...
        scheduleInternalOperation("Load(tp=" + tp + ", epoch=" + partitionEpoch + ")", tp, () -> {
            ...
            if (context.epoch < partitionEpoch) {
                        ...
                        loader.load(tp, context.coordinator).whenComplete((state, exception) -> {
                            scheduleInternalOperation("CompleteLoad(tp=" + tp + ", epoch=" + partitionEpoch + ")", tp, () -> {
                                ...
                                if (ctx.state != CoordinatorState.LOADING) {
                                    log.info("Ignoring load completion from {} because context is in {} state.", ctx.tp, ctx.state);
                                    return;
                                }
                                try {
                                    if (exception != null)
                                    ...
                                    log.error("Failed to load metadata from {} with epoch {} due to {}.", tp, partitionEpoch, ex.toString());
                ...
                log.info("Ignoring loading metadata from {} since current epoch {} is larger than or equals to {}.", context.tp, context.epoch, partitionEpoch);
            }
        });
    }
    ...
    public void scheduleUnloadOperation(TopicPartition tp, int partitionEpoch) {
        ...
        scheduleInternalOperation("UnloadCoordinator(tp=" + tp + ", epoch=" + partitionEpoch + ")", tp, () -> {
            ...
            if (context.epoch < partitionEpoch) {
                ...
                log.info("Ignored unloading metadata for {} in epoch {} since current epoch is {}.", tp, partitionEpoch, context.epoch);
            }
        });
    }
        ...
        coordinators.keySet().forEach(tp -> {
            scheduleInternalOperation("UpdateImage(tp=" + tp + ", offset=" + newImage.offset() + ")", tp, () -> {
                ...
                if (context.state == CoordinatorState.ACTIVE) {
                    ...
                    log.debug("Ignoring new metadata image with offset {} for {} because the coordinator is not active.", newImage.offset(), tp);
                }
            });
        });
    }
