...
public class Assignment {
    ...
    public Assignment(Map<Uuid, Set<Integer>> partitions) {
        this((byte) 0, partitions, VersionedMetadata.EMPTY);
    }
    ...
    public String toString() {
        return "Assignment(" + "error=" + error + ", partitions=" + partitions + ", metadata=" + metadata + ')';
    }
    ...
    public static Assignment fromRecord(ConsumerGroupTargetAssignmentMemberValue record) {
        return new Assignment(record.error(), record.topicPartitions().stream().collect(Collectors.toMap(ConsumerGroupTargetAssignmentMemberValue.TopicPartition::topicId, topicPartitions -> new HashSet<>(topicPartitions.partitions()))), new VersionedMetadata(record.metadataVersion(), ByteBuffer.wrap(record.metadataBytes())));
    }
}
