...
public class ConsumerGroupTest {
    ...
    public void testGetOrCreateMember() {
        ...
        assertThrows(UnknownMemberIdException.class, () -> consumerGroup.getOrMaybeCreateMember("does-not-exist", false));
    }
    ...
    public void testUpdatingMemberUpdatesPartitionEpoch() {
        ...
        member = new ConsumerGroupMember.Builder("member").setMemberEpoch(10).setAssignedPartitions(mkAssignment(mkTopicAssignment(fooTopicId, 1, 2, 3))).setPartitionsPendingRevocation(mkAssignment(mkTopicAssignment(barTopicId, 4, 5, 6))).setPartitionsPendingAssignment(mkAssignment(mkTopicAssignment(zarTopicId, 7, 8, 9))).build();
        ...
        member = new ConsumerGroupMember.Builder(member).setMemberEpoch(11).setAssignedPartitions(mkAssignment(mkTopicAssignment(barTopicId, 1, 2, 3))).setPartitionsPendingRevocation(mkAssignment(mkTopicAssignment(zarTopicId, 4, 5, 6))).setPartitionsPendingAssignment(mkAssignment(mkTopicAssignment(fooTopicId, 7, 8, 9))).build();
    ...
    public void testGroupState() {
        ...
        member2 = new ConsumerGroupMember.Builder(member2).setMemberEpoch(2).setPreviousMemberEpoch(1).setTargetMemberEpoch(2).setPartitionsPendingAssignment(mkAssignment(mkTopicAssignment(fooTopicId, 0))).build();
        ...
        member2 = new ConsumerGroupMember.Builder(member2).setMemberEpoch(2).setPreviousMemberEpoch(1).setTargetMemberEpoch(2).setAssignedPartitions(mkAssignment(mkTopicAssignment(fooTopicId, 0))).setPartitionsPendingAssignment(Collections.emptyMap()).build();
    ...
    public void testPreferredServerAssignor() {
        ...
        assertEquals(Optional.of("range"), consumerGroup.computePreferredServerAssignor(null, member1));
        ...
        assertEquals(Optional.empty(), consumerGroup.computePreferredServerAssignor(member1, null));

        // Member 2 has got an updated assignor but this is not reflected in the group yet so
        // we pass the updated member. The assignor should be range.
        assertEquals(Optional.of("range"), consumerGroup.computePreferredServerAssignor(null, member2));
        ...
        // we pass the updated member. The assignor should be range or uniform.
        assertEquals(Optional.of("uniform"), consumerGroup.computePreferredServerAssignor(member2, updatedMember2));
        ...
        assertEquals(Optional.empty(), consumerGroup.computePreferredServerAssignor(member3, updatedMember3));
    ...
    public void testUpdateSubscriptionMetadata() {
        ...
        MetadataImage image = new GroupMetadataManagerTest.MetadataImageBuilder().addTopic(fooTopicId, "foo", 1).addTopic(barTopicId, "bar", 2).addTopic(zarTopicId, "zar", 3).build();

        ConsumerGroupMember member1 = new ConsumerGroupMember.Builder("member1").setSubscribedTopicNames(Arrays.asList("foo")).build();
        ConsumerGroupMember member2 = new ConsumerGroupMember.Builder("member2").setSubscribedTopicNames(Arrays.asList("bar")).build();
        ConsumerGroupMember member3 = new ConsumerGroupMember.Builder("member3").setSubscribedTopicNames(Arrays.asList("zar")).build();

        ConsumerGroup consumerGroup = createConsumerGroup("group-foo");

        // It should be empty by default.
        assertEquals(Collections.emptyMap(), consumerGroup.computeSubscriptionMetadata(null, null, image.topics()));

        // Compute while taking into account member 1.
        assertEquals(mkMap(mkEntry("foo", new TopicMetadata(fooTopicId, "foo", 1))), consumerGroup.computeSubscriptionMetadata(null, member1, image.topics()));

        // Updating the group with member1.
        consumerGroup.updateMember(member1);

        // It should return foo now.
        assertEquals(mkMap(mkEntry("foo", new TopicMetadata(fooTopicId, "foo", 1))), consumerGroup.computeSubscriptionMetadata(null, null, image.topics()));

        // Compute while taking into account removal of member 1.
        assertEquals(Collections.emptyMap(), consumerGroup.computeSubscriptionMetadata(member1, null, image.topics()));

        // Compute while taking into account member 2.
        assertEquals(mkMap(mkEntry("foo", new TopicMetadata(fooTopicId, "foo", 1)), mkEntry("bar", new TopicMetadata(barTopicId, "bar", 2))), consumerGroup.computeSubscriptionMetadata(null, member2, image.topics()));

        // Updating the group with member2.
        consumerGroup.updateMember(member2);

        // It should return foo and bar.
        assertEquals(mkMap(mkEntry("foo", new TopicMetadata(fooTopicId, "foo", 1)), mkEntry("bar", new TopicMetadata(barTopicId, "bar", 2))), consumerGroup.computeSubscriptionMetadata(null, null, image.topics()));

        // Compute while taking into account removal of member 2.
        assertEquals(mkMap(mkEntry("foo", new TopicMetadata(fooTopicId, "foo", 1))), consumerGroup.computeSubscriptionMetadata(member2, null, image.topics()));

        // Removing member1 results in returning bar.
        assertEquals(mkMap(mkEntry("bar", new TopicMetadata(barTopicId, "bar", 2))), consumerGroup.computeSubscriptionMetadata(member1, null, image.topics()));

        // Compute while taking into account member 3.
        assertEquals(mkMap(mkEntry("foo", new TopicMetadata(fooTopicId, "foo", 1)), mkEntry("bar", new TopicMetadata(barTopicId, "bar", 2)), mkEntry("zar", new TopicMetadata(zarTopicId, "zar", 3))), consumerGroup.computeSubscriptionMetadata(null, member3, image.topics()));

        // Updating group with member3.
        consumerGroup.updateMember(member3);

        // It should return foo, bar and zar.
        assertEquals(mkMap(mkEntry("foo", new TopicMetadata(fooTopicId, "foo", 1)), mkEntry("bar", new TopicMetadata(barTopicId, "bar", 2)), mkEntry("zar", new TopicMetadata(zarTopicId, "zar", 3))), consumerGroup.computeSubscriptionMetadata(null, null, image.topics()));
    }
