...
public class ConsumerGroupMemberTest {
    ...
    public void testNewMember() {
        ...
        ConsumerGroupMember member = new ConsumerGroupMember.Builder("member-id").setMemberEpoch(10).setPreviousMemberEpoch(9).setTargetMemberEpoch(11).setInstanceId("instance-id").setRackId("rack-id").setRebalanceTimeoutMs(5000).setClientId("client-id").setClientHost("hostname").setSubscribedTopicNames(Arrays.asList("foo", "bar")).setSubscribedTopicRegex("regex").setServerAssignorName("range").setClientAssignors(Collections.singletonList(new ClientAssignor("assignor", (byte) 0, (byte) 0, (byte) 1, new VersionedMetadata((byte) 1, ByteBuffer.allocate(0))))).setAssignedPartitions(mkAssignment(mkTopicAssignment(topicId1, 1, 2, 3))).setPartitionsPendingRevocation(mkAssignment(mkTopicAssignment(topicId2, 4, 5, 6))).setPartitionsPendingAssignment(mkAssignment(mkTopicAssignment(topicId3, 7, 8, 9))).build();
        ...
        assertEquals(Collections.singletonList(new ClientAssignor("assignor", (byte) 0, (byte) 0, (byte) 1, new VersionedMetadata((byte) 1, ByteBuffer.allocate(0)))), member.clientAssignors());
    ...
    public void testUpdateMember() {
        ...
        updatedMember = new ConsumerGroupMember.Builder(member).maybeUpdateRackId(Optional.of("new-rack-id")).maybeUpdateInstanceId(Optional.of("new-instance-id")).maybeUpdateServerAssignorName(Optional.of("new-assignor")).maybeUpdateSubscribedTopicNames(Optional.of(Arrays.asList("zar"))).maybeUpdateSubscribedTopicRegex(Optional.of("new-regex")).maybeUpdateRebalanceTimeoutMs(OptionalInt.of(6000)).maybeUpdateClientAssignors(Optional.of(Collections.emptyList())).build();
    ...
    public void testUpdateWithConsumerGroupMemberMetadataValue() {
        ...
        assertEquals("regex", member.subscribedTopicRegex());
        assertEquals("range", member.serverAssignorName().get());
        assertEquals(Collections.singletonList(new ClientAssignor("client", (byte) 0, (byte) 0, (byte) 2, new VersionedMetadata((byte) 1, ByteBuffer.allocate(0)))), member.clientAssignors());
    }
