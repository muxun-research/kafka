...
public class MultiThreadedEventProcessorTest {
    ...
    public void testCreateAndClose() throws Exception {
        CoordinatorEventProcessor eventProcessor = new MultiThreadedEventProcessor(new LogContext(), "event-processor-", 2);
        eventProcessor.close();
    }

    @Test
    public void testEventsAreProcessed() throws Exception {
        try (CoordinatorEventProcessor eventProcessor = new MultiThreadedEventProcessor(new LogContext(), "event-processor-", 2)) {
            AtomicInteger numEventsExecuted = new AtomicInteger(0);

            List<FutureEvent<Integer>> events = Arrays.asList(new FutureEvent<>(new TopicPartition("foo", 0), numEventsExecuted::incrementAndGet), new FutureEvent<>(new TopicPartition("foo", 1), numEventsExecuted::incrementAndGet), new FutureEvent<>(new TopicPartition("foo", 2), numEventsExecuted::incrementAndGet), new FutureEvent<>(new TopicPartition("foo", 0), numEventsExecuted::incrementAndGet), new FutureEvent<>(new TopicPartition("foo", 1), numEventsExecuted::incrementAndGet), new FutureEvent<>(new TopicPartition("foo", 2), numEventsExecuted::incrementAndGet));

            events.forEach(eventProcessor::enqueue);

            CompletableFuture.allOf(events.stream().map(FutureEvent::future).toArray(CompletableFuture[]::new)).get(10, TimeUnit.SECONDS);
    ...
    public void testProcessingGuarantees() throws Exception {
        try (CoordinatorEventProcessor eventProcessor = new MultiThreadedEventProcessor(new LogContext(), "event-processor-", 2)) {
            ...
            List<FutureEvent<Integer>> events = Arrays.asList(new FutureEvent<>(new TopicPartition("foo", 0), numEventsExecuted::incrementAndGet, true), // Event 0
    ...
    public void testEventsAreRejectedWhenClosed() throws Exception {
        ...
        assertThrows(RejectedExecutionException.class, () -> eventProcessor.enqueue(new FutureEvent<>(new TopicPartition("foo", 0), () -> 0)));
    }

    @Test
    public void testEventsAreDrainedWhenClosed() throws Exception {
        try (MultiThreadedEventProcessor eventProcessor = new MultiThreadedEventProcessor(new LogContext(), "event-processor-", 1 // Use a single thread to block event in the processor.
        )) {
            AtomicInteger numEventsExecuted = new AtomicInteger(0);
            ...
            waitForCondition(() -> numEventsExecuted.get() > 0, "Blocking event not executed.");
            ...
            assertThrows(RejectedExecutionException.class, () -> eventProcessor.enqueue(blockingEvent));
            ...
            events.forEach(event -> {
                Throwable t = assertThrows(ExecutionException.class, () -> event.future.get(DEFAULT_MAX_WAIT_MS, TimeUnit.SECONDS));
