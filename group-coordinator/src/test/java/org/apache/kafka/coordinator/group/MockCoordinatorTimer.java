...
public class MockCoordinatorTimer<T> implements CoordinatorTimer<T> {
    ...
    public static class ScheduledTimeout<T> {
        ...
        ScheduledTimeout(String key, long deadlineMs, TimeoutOperation<T> operation) {
            this.key = key;
    ...
    public static class ExpiredTimeout<T> {
        ...
        ExpiredTimeout(String key, List<T> records) {
            this.key = key;
    ...
    private final PriorityQueue<ScheduledTimeout<T>> timeoutQueue = new PriorityQueue<>(Comparator.comparingLong(entry -> entry.deadlineMs));
     ...
     */
    @Override
    public void schedule(String key, long delay, TimeUnit unit, boolean retry, TimeoutOperation<T> operation) {
        cancel(key);
