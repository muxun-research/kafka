...
public class MirrorSourceConnector extends SourceConnector {
    ...
    MirrorSourceConnector(SourceAndTarget sourceAndTarget, ReplicationPolicy replicationPolicy, MirrorSourceConfig config, ConfigPropertyFilter configPropertyFilter, Admin targetAdmin) {
        this.sourceAndTarget = sourceAndTarget;
        this.replicationPolicy = replicationPolicy;
        this.configPropertyFilter = configPropertyFilter;
        this.config = config;
    ...
    public void start(Map<String, String> props) {
        ...
        scheduler.scheduleRepeatingDelayed(this::refreshTopicPartitions, config.refreshTopicsInterval(), "refreshing topics");
    ...
    public List<Map<String, String>> taskConfigs(int maxTasks) {
        ...
        return IntStream.range(0, numTasks).mapToObj(i -> config.taskConfigForTopicPartitions(roundRobinByTask.get(i), i)).collect(Collectors.toList());
    }
    ...
    public org.apache.kafka.common.config.Config validate(Map<String, String> props) {
        ...
        if ("required".equals(props.get(EXACTLY_ONCE_SUPPORT_CONFIG))) {
            if (!consumerUsesReadCommitted(props)) {
                ...
                exactlyOnceSupport.addErrorMessage("MirrorSourceConnector can only provide exactly-once guarantees when its source consumer is configured with " + ConsumerConfig.ISOLATION_LEVEL_CONFIG + " set to '" + READ_COMMITTED + "'; " + "otherwise, records from aborted and uncommitted transactions will be replicated from the " + "source cluster to the target cluster.");
            }
        }
    ...
    public ExactlyOnceSupport exactlyOnceSupport(Map<String, String> props) {
        return consumerUsesReadCommitted(props) ? ExactlyOnceSupport.SUPPORTED : ExactlyOnceSupport.UNSUPPORTED;
    }

    private boolean consumerUsesReadCommitted(Map<String, String> props) {
        Object consumerIsolationLevel = MirrorSourceConfig.sourceConsumerConfig(props).get(ConsumerConfig.ISOLATION_LEVEL_CONFIG);
    ...
    List<TopicPartition> findSourceTopicPartitions() throws InterruptedException, ExecutionException {
        Set<String> topics = listTopics(sourceAdminClient).stream().filter(this::shouldReplicateTopic).collect(Collectors.toSet());
        return describeTopics(sourceAdminClient, topics).stream().flatMap(MirrorSourceConnector::expandTopicDescription).collect(Collectors.toList());
    }

    // visible for testing
    List<TopicPartition> findTargetTopicPartitions() throws InterruptedException, ExecutionException {
        Set<String> topics = listTopics(targetAdminClient).stream().filter(t -> sourceAndTarget.source().equals(replicationPolicy.topicSource(t))).filter(t -> !t.equals(config.checkpointsTopic())).collect(Collectors.toSet());
        return describeTopics(targetAdminClient, topics).stream().flatMap(MirrorSourceConnector::expandTopicDescription).collect(Collectors.toList());
    }
    ...
    void refreshTopicPartitions() throws InterruptedException, ExecutionException {
        ...
        Set<TopicPartition> upstreamTargetTopicPartitions = targetTopicPartitions.stream().map(x -> new TopicPartition(replicationPolicy.upstreamTopic(x.topic()), x.partition())).collect(Collectors.toSet());
        ...
        if (!knownSourceTopicPartitionsSet.equals(sourceTopicPartitionsSet) || !missingInTarget.isEmpty()) {
            ...
            log.info("Found {} new topic-partitions on {}. " + "Found {} deleted topic-partitions on {}. " + "Found {} topic-partitions missing on {}.", newTopicPartitions.size(), sourceAndTarget.source(), deletedTopicPartitions.size(), sourceAndTarget.source(), missingInTarget.size(), sourceAndTarget.target());
    ...
    private Set<String> topicsBeingReplicated() {
        ...
        return knownSourceTopicPartitions.stream().map(TopicPartition::topic).distinct().filter(x -> knownTargetTopics.contains(formatRemoteTopic(x))).collect(Collectors.toSet());
    }
    ...
    void syncTopicAcls() throws InterruptedException, ExecutionException {
        ...
        if (!rawBindings.isPresent())
            return;
        List<AclBinding> filteredBindings = rawBindings.get().stream().filter(x -> x.pattern().resourceType() == ResourceType.TOPIC).filter(x -> x.pattern().patternType() == PatternType.LITERAL).filter(this::shouldReplicateAcl).filter(x -> shouldReplicateTopic(x.pattern().name())).map(this::targetAclBinding).collect(Collectors.toList());
    ...
    void syncTopicConfigs() throws InterruptedException, ExecutionException {
        ...
        Map<String, Config> targetConfigs = sourceConfigs.entrySet().stream().collect(Collectors.toMap(x -> formatRemoteTopic(x.getKey()), x -> targetConfig(x.getValue(), incremental)));
        if (incremental) {
    ...
    private void createOffsetSyncsTopic() {
        MirrorUtils.createSinglePartitionCompactedTopic(config.offsetSyncsTopic(), config.offsetSyncsTopicReplicationFactor(), offsetSyncsAdminClient);
    }
    ...
    void createNewTopics(Set<String> newSourceTopics, Map<String, Long> sourceTopicToPartitionCounts) throws ExecutionException, InterruptedException {
        ...
        Map<String, NewTopic> newTopics = newSourceTopics.stream().map(sourceTopic -> {
            ...
            return new NewTopic(remoteTopic, partitionCount, (short) replicationFactor).configs(configs);
    ...
    void deprecatedAlterConfigs(Map<String, Config> topicConfigs) {
        Map<ConfigResource, Config> configs = topicConfigs.entrySet().stream().collect(Collectors.toMap(x -> new ConfigResource(ConfigResource.Type.TOPIC, x.getKey()), Entry::getValue));
    ...
    void incrementalAlterConfigs(Map<String, Config> topicConfigs) {
        ...
        targetAdminClient.incrementalAlterConfigs(configOps).values().forEach((k, v) -> v.whenComplete((x, e) -> {
            if (e != null) {
                if (config.useIncrementalAlterConfigs().equals(MirrorSourceConfig.REQUEST_INCREMENTAL_ALTER_CONFIGS) && e instanceof UnsupportedVersionException && !encounteredError.get()) {
                    //Fallback logic
                    log.warn("The target cluster {} is not compatible with IncrementalAlterConfigs API. " + "Therefore using deprecated AlterConfigs API for syncing configs for topic {}", sourceAndTarget.target(), k.name(), e);
                    encounteredError.set(true);
                    useIncrementalAlterConfigs = false;
                } else if (config.useIncrementalAlterConfigs().equals(MirrorSourceConfig.REQUIRE_INCREMENTAL_ALTER_CONFIGS) && e instanceof UnsupportedVersionException && !encounteredError.get()) {
                    log.error("Failed to sync configs for topic {} on cluster {} with IncrementalAlterConfigs API", k.name(), sourceAndTarget.target(), e);
                    encounteredError.set(true);
                    context.raiseError(new ConnectException("use.incremental.alter.configs was set to \"required\", but the target cluster '" + sourceAndTarget.target() + "' is not compatible with IncrementalAlterConfigs API", e));
                } else {
                    log.warn("Could not alter configuration of topic {}.", k.name(), e);
                }
            }
    ...
    Map<String, Config> describeTopicConfigs(Set<String> topics) throws InterruptedException, ExecutionException {
        Set<ConfigResource> resources = topics.stream().map(x -> new ConfigResource(ConfigResource.Type.TOPIC, x)).collect(Collectors.toSet());
        return sourceAdminClient.describeConfigs(resources).all().get().entrySet().stream().collect(Collectors.toMap(x -> x.getKey().name(), Entry::getValue));
    }
    ...
    boolean shouldReplicateTopic(String topic) {
        return (topicFilter.shouldReplicateTopic(topic) || replicationPolicy.isHeartbeatsTopic(topic)) && !replicationPolicy.isInternalTopic(topic) && !isCycle(topic);
    }
