...
public class DedicatedMirrorIntegrationTest {
    ...
    public void teardown() throws Throwable {
        ...
        mirrorMakers.forEach((name, mirrorMaker) -> Utils.closeQuietly(mirrorMaker::stop, "MirrorMaker worker '" + name + "'", shutdownFailure));
        mirrorMakers.forEach((name, mirrorMaker) -> mirrorMaker.awaitStop());
        kafkaClusters.forEach((name, kafkaCluster) -> Utils.closeQuietly(kafkaCluster::stop, "Embedded Kafka cluster '" + name + "'", shutdownFailure));
    ...
    private void awaitTopicCreation(String clusterName, Admin admin, String topic) throws Exception {
        ...
        }, TOPIC_CREATION_TIMEOUT_MS, "topic " + topic + " was not created on cluster " + clusterName + " in time");
    }
    ...
    private void awaitMirrorMakerStart(final MirrorMaker mm, final SourceAndTarget sourceAndTarget) throws InterruptedException {
        waitForCondition(() -> {
            try {
                return CONNECTOR_CLASSES.stream().allMatch(connectorClazz -> isConnectorRunningForMirrorMaker(connectorClazz, mm, sourceAndTarget));
            } catch (Exception ex) {
    ...
    private void awaitTopicContent(EmbeddedKafkaCluster cluster, String clusterName, String topic, int numMessages) throws Exception {
        try (Consumer<?, ?> consumer = cluster.createConsumer(Collections.singletonMap(AUTO_OFFSET_RESET_CONFIG, "earliest"))) {
            ...
            }, TOPIC_REPLICATION_TIMEOUT_MS, () -> "could not read " + numMessages + " from topic " + topic + " on cluster " + clusterName + " in time; only read " + messagesRead.get());
        }
    }
    ...
    private boolean isConnectorRunningForMirrorMaker(final Class<?> connectorClazz, final MirrorMaker mm, final SourceAndTarget sourceAndTarget) {
        ...
        try {
            ...
            if (connectorStatus.connector().state().equals(AbstractStatus.State.FAILED.toString())) {
                throw new NoRetryException(new AssertionError(String.format("Connector %s is in FAILED state for MirrorMaker %s and source->target=%s", connectorClazz, mm, sourceAndTarget)));
            }
