...
public class TopicAdmin implements AutoCloseable {
    ...
    private static final List<Class<? extends Exception>> CAUSES_TO_RETRY_TOPIC_CREATION = Arrays.asList(InvalidReplicationFactorException.class, TimeoutException.class);
    ...
    public static class NewTopicBuilder {
        ...
        public NewTopic build() {
            return new NewTopic(name, Optional.of(numPartitions), Optional.of(replicationFactor)).configs(configs);
        }
    }
        ...
        do {
            try {
                ...
                if (timer.notExpired() && retryableTopicCreationException(e)) {
                    log.info("'{}' topic creation failed due to '{}', retrying, {}ms remaining", topicDescription.name(), e.getMessage(), timer.remainingMs());
                } else {
                    throw e;
                }
            }
    ...
    public boolean verifyTopicCleanupPolicyOnlyCompact(String topic, String workerTopicConfig, String topicPurpose) {
        ...
        if (cleanupPolicies.isEmpty()) {
            log.info("Unable to use admin client to verify the cleanup policy of '{}' " + "topic is '{}', either because the broker is an older " + "version or because the Kafka principal used for Connect " + "internal topics does not have the required permission to " + "describe topic configurations.", topic, TopicConfig.CLEANUP_POLICY_COMPACT);
            return false;
        }
    ...
    public Map<TopicPartition, Long> retryEndOffsets(Set<TopicPartition> partitions, Duration timeoutDuration, long retryBackoffMs) {
        ...
        try {
            return RetryUtil.retryUntilTimeout(() -> endOffsets(partitions), () -> "list offsets for topic partitions", timeoutDuration, retryBackoffMs);
