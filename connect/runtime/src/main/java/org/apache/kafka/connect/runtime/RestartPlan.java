...
public class RestartPlan {
    ...
    public RestartPlan(RestartRequest request, ConnectorStateInfo restartStateInfo) {
        ...
        this.idsToRestart = Collections.unmodifiableList(stateInfo.tasks().stream().filter(this::isRestarting).map(taskState -> new ConnectorTaskId(request.connectorName(), taskState.id())).collect(Collectors.toList()));
    }
    ...
    public String toString() {
        return shouldRestartConnector() ? String.format("plan to restart connector and %d of %d tasks for %s", restartTaskCount(), totalTaskCount(), request) : String.format("plan to restart %d of %d tasks for %s", restartTaskCount(), totalTaskCount(), request);
    }
}
