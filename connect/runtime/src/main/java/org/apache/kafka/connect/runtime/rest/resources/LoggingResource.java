...
public class LoggingResource implements ConnectResource {
    ...
    public Response listLoggers() {
        ...
        Collections.list(enumeration).stream().filter(logger -> logger.getLevel() != null).forEach(logger -> loggers.put(logger.getName(), levelToMap(logger)));
    ...
    public Response setLevel(final @PathParam("logger") String namedLogger, final Map<String, String> levelMap) {
