...
public class ConnectRestServerTest {
    ...
    public void testValidCustomizedHttpResponseHeaders() throws IOException {
        String headerConfig = "add X-XSS-Protection: 1; mode=block, \"add Cache-Control: no-cache, no-store, must-revalidate\"";
    ...
    private void expectEmptyRestExtensions() {
        doReturn(Collections.emptyList()).when(plugins).newPlugins(eq(Collections.emptyList()), any(AbstractConfig.class), eq(ConnectRestExtension.class));
    }
}
