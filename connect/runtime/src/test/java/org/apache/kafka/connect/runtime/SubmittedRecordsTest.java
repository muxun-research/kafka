...
public class SubmittedRecordsTest {
    ...
    public void testAwaitMessagesAfterAllRemoved() {
        ...
        assertFalse("Await should fail since neither of the in-flight records has been removed so far", submittedRecords.awaitAllMessages(0, TimeUnit.MILLISECONDS));

        recordToRemove1.drop();
        assertFalse("Await should fail since only one of the two submitted records has been removed so far", submittedRecords.awaitAllMessages(0, TimeUnit.MILLISECONDS));

        recordToRemove1.drop();
        assertFalse("Await should fail since only one of the two submitted records has been removed so far, " + "even though that record has been removed twice", submittedRecords.awaitAllMessages(0, TimeUnit.MILLISECONDS));

        recordToRemove2.drop();
        assertTrue("Await should succeed since both submitted records have now been removed", submittedRecords.awaitAllMessages(0, TimeUnit.MILLISECONDS));
    }
    ...
    public void testAwaitMessagesReturnsAfterAsynchronousAck() throws Exception {
        ...
        assertTrue("Should not have finished awaiting message delivery before either in-flight record was acknowledged", awaitComplete.getCount() > 0);

        inFlightRecord1.ack();
        assertTrue("Should not have finished awaiting message delivery before one in-flight record was acknowledged", awaitComplete.getCount() > 0);

        inFlightRecord1.ack();
        assertTrue("Should not have finished awaiting message delivery before one in-flight record was acknowledged, " + "even though the other record has been acknowledged twice", awaitComplete.getCount() > 0);

        inFlightRecord2.ack();
        assertTrue("Should have finished awaiting message delivery after both in-flight records were acknowledged", awaitComplete.await(1, TimeUnit.SECONDS));
        assertTrue("Await of in-flight messages should have succeeded", awaitResult.get());
    }
    ...
    private void assertNoEmptyDeques() {
        submittedRecords.records.forEach((partition, deque) -> assertFalse("Empty deque for partition " + partition + " should have been cleaned up from internal records map", deque.isEmpty()));
    }
    ...
    private final void assertMetadata(CommittableOffsets committableOffsets, int committableMessages, int uncommittableMessages, int numDeques, int largestDequeSize, Map<String, Object>... largestDequePartitions) {
        assertEquals(committableMessages, committableOffsets.numCommittableMessages());
