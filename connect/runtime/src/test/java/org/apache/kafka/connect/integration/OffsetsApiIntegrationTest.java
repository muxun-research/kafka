...
public class OffsetsApiIntegrationTest {
    ...
    public void setup() {
        ...
        connectBuilder = new EmbeddedConnectCluster.Builder().name("connect-cluster").numWorkers(NUM_WORKERS).brokerProps(brokerProps).workerProps(workerProps);
    }
    ...
    public void testGetNonExistentConnectorOffsets() {
        ...
        connect.start();
        ConnectRestException e = assertThrows(ConnectRestException.class, () -> connect.connectorOffsets("non-existent-connector"));
    ...
    public void testGetSinkConnectorOffsetsOverriddenConsumerGroupId() throws Exception {
        ...
        connectorConfigs.put(ConnectorConfig.CONNECTOR_CLIENT_CONSUMER_OVERRIDES_PREFIX + CommonClientConfigs.GROUP_ID_CONFIG, "overridden-group-id");
    ...
    public void testGetSinkConnectorOffsetsDifferentKafkaClusterTargeted() throws Exception {
        ...
        try (AutoCloseable ignored = kafkaCluster::stop) {
            ...
            Map<String, String> connectorConfigs = baseSinkConnectorConfigs();
            connectorConfigs.put(ConnectorConfig.CONNECTOR_CLIENT_CONSUMER_OVERRIDES_PREFIX + CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, kafkaCluster.bootstrapServers());
            connectorConfigs.put(ConnectorConfig.CONNECTOR_CLIENT_ADMIN_OVERRIDES_PREFIX + CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, kafkaCluster.bootstrapServers());
    ...
    private void getAndVerifySinkConnectorOffsets(Map<String, String> connectorConfigs, EmbeddedKafkaCluster kafkaCluster) throws Exception {
        ...
        verifyExpectedSinkConnectorOffsets(CONNECTOR_NAME, TOPIC, 5, 2 * NUM_RECORDS_PER_PARTITION, "Sink connector consumer group offsets should catch up to the topic end offsets");
    }
    ...
    public void testGetSourceConnectorOffsetsDifferentKafkaClusterTargeted() throws Exception {
        ...
        try (AutoCloseable ignored = kafkaCluster::stop) {
            ...
            Map<String, String> connectorConfigs = baseSourceConnectorConfigs();
            connectorConfigs.put(ConnectorConfig.CONNECTOR_CLIENT_PRODUCER_OVERRIDES_PREFIX + CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, kafkaCluster.bootstrapServers());
            connectorConfigs.put(ConnectorConfig.CONNECTOR_CLIENT_ADMIN_OVERRIDES_PREFIX + CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, kafkaCluster.bootstrapServers());
    ...
    private void getAndVerifySourceConnectorOffsets(Map<String, String> connectorConfigs) throws Exception {
        ...
        verifyExpectedSourceConnectorOffsets(CONNECTOR_NAME, NUM_TASKS, 2 * NUM_RECORDS_PER_PARTITION, "Source connector offsets should reflect the expected number of records produced");
    }
    ...
    public void testAlterOffsetsNonExistentConnector() throws Exception {
        connect = connectBuilder.build();
        connect.start();
        ConnectRestException e = assertThrows(ConnectRestException.class, () -> connect.alterConnectorOffsets("non-existent-connector", new ConnectorOffsets(Collections.singletonList(new ConnectorOffset(Collections.emptyMap(), Collections.emptyMap())))));
        assertEquals(404, e.errorCode());
    }
    ...
    public void testAlterOffsetsNonStoppedConnector() throws Exception {
        ...
        for (int i = 0; i < NUM_TASKS; i++) {
            offsets.add(new ConnectorOffset(Collections.singletonMap("task.id", CONNECTOR_NAME + "-" + i), Collections.singletonMap("saved", 5)));
        }

        // Try altering offsets for a running connector
        ConnectRestException e = assertThrows(ConnectRestException.class, () -> connect.alterConnectorOffsets(CONNECTOR_NAME, new ConnectorOffsets(offsets)));
        assertEquals(400, e.errorCode());

        connect.pauseConnector(CONNECTOR_NAME);
        connect.assertions().assertConnectorAndExactlyNumTasksArePaused(CONNECTOR_NAME, NUM_TASKS, "Connector did not pause in time");

        // Try altering offsets for a paused (not stopped) connector
        e = assertThrows(ConnectRestException.class, () -> connect.alterConnectorOffsets(CONNECTOR_NAME, new ConnectorOffsets(offsets)));
        assertEquals(400, e.errorCode());
    }
    ...
    private void alterAndVerifySinkConnectorOffsets(Map<String, String> connectorConfigs, EmbeddedKafkaCluster kafkaCluster) throws Exception {
        ...
        verifyExpectedSinkConnectorOffsets(CONNECTOR_NAME, TOPIC, numPartitions, NUM_RECORDS_PER_PARTITION, "Sink connector consumer group offsets should catch up to the topic end offsets");
        ...
        assertThat(response, containsString("The Connect framework-managed offsets for this connector have been altered successfully. " + "However, if this connector manages offsets externally, they will need to be manually altered in the system that the connector uses."));

        verifyExpectedSinkConnectorOffsets(CONNECTOR_NAME, TOPIC, numPartitions - 1, 5, "Sink connector consumer group offsets should reflect the altered offsets");
        ...
        verifyExpectedSinkConnectorOffsets(CONNECTOR_NAME, TOPIC, numPartitions - 1, 3, "Sink connector consumer group offsets should reflect the altered offsets");

        // Resume the connector and expect its offsets to catch up to the latest offsets
        connect.resumeConnector(CONNECTOR_NAME);
        connect.assertions().assertConnectorAndExactlyNumTasksAreRunning(CONNECTOR_NAME, NUM_TASKS, "Connector tasks did not resume in time");
        verifyExpectedSinkConnectorOffsets(CONNECTOR_NAME, TOPIC, numPartitions, NUM_RECORDS_PER_PARTITION, "Sink connector consumer group offsets should catch up to the topic end offsets");
    }
    ...
    public void testAlterSinkConnectorOffsetsZombieSinkTasks() throws Exception {
        ...
        connect.configureConnector(CONNECTOR_NAME, connectorConfigs);
        connect.assertions().assertConnectorAndAtLeastNumTasksAreRunning(CONNECTOR_NAME, 1, "Connector tasks did not start in time.");
        ...
        ConnectRestException e = assertThrows(ConnectRestException.class, () -> connect.alterConnectorOffsets(CONNECTOR_NAME, new ConnectorOffsets(offsetsToAlter)));
    ...
    public void alterAndVerifySourceConnectorOffsets(Map<String, String> connectorConfigs) throws Exception {
        ...
        // The MonitorableSourceConnector has a source partition per task
        for (int i = 0; i < NUM_TASKS; i++) {
            offsetsToAlter.add(new ConnectorOffset(Collections.singletonMap("task.id", CONNECTOR_NAME + "-" + i), Collections.singletonMap("saved", 5)));
        }
        ...
        // The MonitorableSourceConnector has a source partition per task
        for (int i = 0; i < NUM_TASKS; i++) {
            offsetsToAlter.add(new ConnectorOffset(Collections.singletonMap("task.id", CONNECTOR_NAME + "-" + i), Collections.singletonMap("saved", 7)));
        }
    ...
    private void resetAndVerifySinkConnectorOffsets(Map<String, String> connectorConfigs, EmbeddedKafkaCluster kafkaCluster) throws Exception {
        ...
        assertThat(response, containsString("The Connect framework-managed offsets for this connector have been reset successfully. " + "However, if this connector manages offsets externally, they will need to be manually reset in the system that the connector uses."));
     ...
     */
    private void verifyExpectedSinkConnectorOffsets(String connectorName, String expectedTopic, int expectedPartitions, int expectedOffset, String conditionDetails) throws InterruptedException {
     ...
     * @throws InterruptedException if the thread is interrupted while waiting for the actual offsets to match the expected offsets
     */
    private void verifyExpectedSourceConnectorOffsets(String connectorName, int numTasks, int expectedOffset, String conditionDetails) throws InterruptedException {
        TestUtils.waitForCondition(() -> {
            ConnectorOffsets offsets = connect.connectorOffsets(connectorName);
            // The MonitorableSourceConnector has a source partition per task
