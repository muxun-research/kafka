...
public class DistributedHerderTest {
    ...
    private static final ConfigInfos CONN1_CONFIG_INFOS = new ConfigInfos(CONN1, 0, Collections.emptyList(), Collections.emptyList());
    ...
    private static final ConfigInfos CONN2_CONFIG_INFOS = new ConfigInfos(CONN2, 0, Collections.emptyList(), Collections.emptyList());
    private static final ConfigInfos CONN2_INVALID_CONFIG_INFOS = new ConfigInfos(CONN2, 1, Collections.emptyList(), Collections.emptyList());
    ...
    private static final ClusterConfigState SNAPSHOT_STOPPED_CONN1 = new ClusterConfigState(1, null, Collections.singletonMap(CONN1, 0), Collections.singletonMap(CONN1, CONN1_CONFIG), Collections.singletonMap(CONN1, TargetState.STOPPED), Collections.emptyMap(), // Stopped connectors should have an empty set of task configs
            Collections.singletonMap(CONN1, 3), Collections.singletonMap(CONN1, 10), Collections.singleton(CONN1), Collections.emptySet());

    private static final ClusterConfigState SNAPSHOT_STOPPED_CONN1_FENCED = new ClusterConfigState(1, null, Collections.singletonMap(CONN1, 0), Collections.singletonMap(CONN1, CONN1_CONFIG), Collections.singletonMap(CONN1, TargetState.STOPPED), Collections.emptyMap(), Collections.singletonMap(CONN1, 0), Collections.singletonMap(CONN1, 11), Collections.emptySet(), Collections.emptySet());
    private static final ClusterConfigState SNAPSHOT_UPDATED_CONN1_CONFIG = new ClusterConfigState(1, null, Collections.singletonMap(CONN1, 3), Collections.singletonMap(CONN1, CONN1_CONFIG_UPDATED), Collections.singletonMap(CONN1, TargetState.STARTED), TASK_CONFIGS_MAP, Collections.emptyMap(), Collections.emptyMap(), Collections.emptySet(), Collections.emptySet());
    ...
    public void testIncrementalCooperativeRebalanceForNewMember() throws Exception {
        ...
        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.NO_ERROR, 1, Arrays.asList(CONN1), Arrays.asList(TASK1), 0);
    ...
    public void testIncrementalCooperativeRebalanceForExistingMember() {
        ...
        EasyMock.expect(member.memberId()).andStubReturn("member");
        EasyMock.expect(member.currentProtocolVersion()).andStubReturn(CONNECT_PROTOCOL_V1);
        expectRebalance(Arrays.asList(CONN1), Arrays.asList(TASK1), ConnectProtocol.Assignment.NO_ERROR, 1, Collections.emptyList(), Collections.emptyList(), 0);
    ...
    public void testIncrementalCooperativeRebalanceWithDelay() throws Exception {
        ...
        EasyMock.expect(member.memberId()).andStubReturn("member");
        EasyMock.expect(member.currentProtocolVersion()).andStubReturn(CONNECT_PROTOCOL_V1);
        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.NO_ERROR, 1, Collections.emptyList(), Arrays.asList(TASK2), rebalanceDelay);
        expectConfigRefreshAndSnapshot(SNAPSHOT);

        worker.startSourceTask(EasyMock.eq(TASK2), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.eq(herder), EasyMock.eq(TargetState.STARTED));
        PowerMock.expectLastCall().andReturn(true);
        member.poll(EasyMock.anyInt());
        PowerMock.expectLastCall().andAnswer(() -> {
            time.sleep(9900L);
    ...
    public void revokeAndReassign(boolean incompleteRebalance) throws TimeoutException {
        ...
        expectRebalance(Arrays.asList(CONN1), Arrays.asList(), ConnectProtocol.Assignment.NO_ERROR, configOffset, Arrays.asList(), Arrays.asList());
    ...
    public void testConnectorNameConflictsWithWorkerGroupId() {
        ...
        Map<String, ConfigValue> validatedConfigs = herder.validateSinkConnectorConfig(connectorMock, SinkConnectorConfig.configDef(), config);

        ConfigValue nameConfig = validatedConfigs.get(ConnectorConfig.NAME_CONFIG);
        assertEquals(Collections.singletonList("Consumer group for sink connector named test-group conflicts with Connect worker group connect-test-group"), nameConfig.errorMessages());
    ...
    public void testExactlyOnceSourceSupportValidation() {
        ...
        EasyMock.expect(connectorMock.exactlyOnceSupport(EasyMock.eq(config))).andReturn(ExactlyOnceSupport.SUPPORTED);

        PowerMock.replayAll(connectorMock);

        Map<String, ConfigValue> validatedConfigs = herder.validateSourceConnectorConfig(connectorMock, SourceConnectorConfig.configDef(), config);
    ...
    public void testExactlyOnceSourceSupportValidationOnUnsupportedConnector() {
        ...
        SourceConnector connectorMock = PowerMock.createMock(SourceConnector.class);
        EasyMock.expect(connectorMock.exactlyOnceSupport(EasyMock.eq(config))).andReturn(ExactlyOnceSupport.UNSUPPORTED);
        ...
        List<String> errors = validatedConfigs.get(SourceConnectorConfig.EXACTLY_ONCE_SUPPORT_CONFIG).errorMessages();
        assertEquals(Collections.singletonList("The connector does not support exactly-once semantics with the provided configuration."), errors);
    ...
    public void testExactlyOnceSourceSupportValidationOnUnknownConnector() {
        ...
        SourceConnector connectorMock = PowerMock.createMock(SourceConnector.class);
        EasyMock.expect(connectorMock.exactlyOnceSupport(EasyMock.eq(config))).andReturn(null);
        ...
        assertTrue("Error message did not contain expected text: " + errors.get(0), errors.get(0).contains("The connector does not implement the API required for preflight validation of exactly-once source support."));
    ...
    public void testExactlyOnceSourceSupportValidationHandlesConnectorErrorsGracefully() {
        ...
        EasyMock.expect(connectorMock.exactlyOnceSupport(EasyMock.eq(config))).andThrow(new NullPointerException(errorMessage));
        ...
        List<String> errors = validatedConfigs.get(SourceConnectorConfig.EXACTLY_ONCE_SUPPORT_CONFIG).errorMessages();
        assertFalse(errors.isEmpty());
        assertTrue("Error message did not contain expected text: " + errors.get(0), errors.get(0).contains(errorMessage));
        assertEquals(1, errors.size());
    ...
    public void testExactlyOnceSourceSupportValidationWhenExactlyOnceNotEnabledOnWorker() {
        ...
        List<String> errors = validatedConfigs.get(SourceConnectorConfig.EXACTLY_ONCE_SUPPORT_CONFIG).errorMessages();
        assertEquals(Collections.singletonList("This worker does not have exactly-once source support enabled."), errors);
    ...
    public void testExactlyOnceSourceSupportValidationHandlesInvalidValuesGracefully() {
        ...
        List<String> errors = validatedConfigs.get(SourceConnectorConfig.EXACTLY_ONCE_SUPPORT_CONFIG).errorMessages();
        assertFalse(errors.isEmpty());
        assertTrue("Error message did not contain expected text: " + errors.get(0), errors.get(0).contains("String must be one of (case insensitive): "));
        assertEquals(1, errors.size());
    ...
    public void testConnectorTransactionBoundaryValidation() {
        ...
        SourceConnector connectorMock = PowerMock.createMock(SourceConnector.class);
        EasyMock.expect(connectorMock.canDefineTransactionBoundaries(EasyMock.eq(config))).andReturn(ConnectorTransactionBoundaries.SUPPORTED);
    ...
    public void testConnectorTransactionBoundaryValidationOnUnsupportedConnector() {
        ...
        SourceConnector connectorMock = PowerMock.createMock(SourceConnector.class);
        EasyMock.expect(connectorMock.canDefineTransactionBoundaries(EasyMock.eq(config))).andReturn(ConnectorTransactionBoundaries.UNSUPPORTED);
        ...
        List<String> errors = validatedConfigs.get(SourceConnectorConfig.TRANSACTION_BOUNDARY_CONFIG).errorMessages();
        assertFalse(errors.isEmpty());
        assertTrue("Error message did not contain expected text: " + errors.get(0), errors.get(0).contains("The connector does not support connector-defined transaction boundaries with the given configuration."));
        assertEquals(1, errors.size());
    ...
    public void testConnectorTransactionBoundaryValidationHandlesConnectorErrorsGracefully() {
        ...
        EasyMock.expect(connectorMock.canDefineTransactionBoundaries(EasyMock.eq(config))).andThrow(new ConnectException(errorMessage));
    ...
    public void testDestroyConnector() throws Exception {
        ...
        PowerMock.expectLastCall().times(2);
        expectRebalance(Arrays.asList(CONN1), Arrays.asList(TASK1), ConnectProtocol.Assignment.NO_ERROR, 2, "leader", "leaderUrl", Collections.emptyList(), Collections.emptyList(), 0, true);
    ...
    public void testDoRestartConnectorAndTasksOnlyTasks() {
        ...
        worker.startSourceTask(EasyMock.eq(TASK0), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.eq(herder), EasyMock.anyObject(TargetState.class));
        PowerMock.expectLastCall().andReturn(true);
    ...
    public void testRestartTask() throws Exception {
        ...
        member.poll(EasyMock.anyInt());
        PowerMock.expectLastCall();
        worker.startSourceTask(EasyMock.eq(TASK0), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.eq(herder), EasyMock.eq(TargetState.STARTED));
        PowerMock.expectLastCall().andReturn(true);
    ...
    public void testConnectorConfigAdded() {
        ...
        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.NO_ERROR, 1, Arrays.asList(CONN1), Collections.emptyList());
        EasyMock.expect(member.currentProtocolVersion()).andStubReturn(CONNECT_PROTOCOL_V0);
        Capture<Callback<TargetState>> onStart = newCapture();
        worker.startConnector(EasyMock.eq(CONN1), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.eq(herder), EasyMock.eq(TargetState.STARTED), capture(onStart));
        PowerMock.expectLastCall().andAnswer(() -> {
    ...
    public void testConnectorConfigUpdateFailedTransformation() throws Exception {
        ...
        EasyMock.expect(configTransformer.transform(EasyMock.eq(CONN1), EasyMock.anyObject())).andThrow(new ConfigException("Simulated exception thrown during config transformation"));
        worker.stopAndAwaitConnector(CONN1);
        PowerMock.expectLastCall();
    ...
    public void testConnectorResumed() throws Exception {
        ...
        EasyMock.expect(member.currentProtocolVersion()).andStubReturn(CONNECT_PROTOCOL_V0);
        Capture<Callback<TargetState>> onStart = newCapture();
        worker.startConnector(EasyMock.eq(CONN1), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.eq(herder), EasyMock.eq(TargetState.PAUSED), capture(onStart));
        PowerMock.expectLastCall().andAnswer(() -> {
            onStart.getValue().onCompletion(null, TargetState.PAUSED);
            return true;
        });
    ...
    public void testStopConnectorNotLeader() throws Exception {
        ...
        assertTrue("Callback should already have been invoked by herder", cb.isDone());
        ExecutionException e = assertThrows("Should not be able to handle request to stop connector when not leader", ExecutionException.class, () -> cb.get(0, TimeUnit.SECONDS));
        assertTrue(e.getCause() instanceof NotLeaderException);
    ...
    public void testConnectorResumedRunningTaskOnly() throws Exception {
        ...
        expectRebalance(1, Collections.emptyList(), singletonList(TASK0));
        expectConfigRefreshAndSnapshot(SNAPSHOT_PAUSED_CONN1);
        worker.startSourceTask(EasyMock.eq(TASK0), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.eq(herder), EasyMock.eq(TargetState.PAUSED));
        PowerMock.expectLastCall().andReturn(true);
        member.poll(EasyMock.anyInt());
        PowerMock.expectLastCall();
    ...
    public void testTaskConfigAdded() {
        ...
        PowerMock.expectLastCall();
        // Performs rebalance and gets new assignment
        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.NO_ERROR, 1, Collections.emptyList(), Arrays.asList(TASK0));
        worker.startSourceTask(EasyMock.eq(TASK0), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.eq(herder), EasyMock.eq(TargetState.STARTED));
        PowerMock.expectLastCall().andReturn(true);
        member.poll(EasyMock.anyInt());
        PowerMock.expectLastCall();
    ...
    public void testJoinLeaderCatchUpFails() throws Exception {
        ...
        EasyMock.expect(member.memberId()).andStubReturn("leader");
        EasyMock.expect(member.currentProtocolVersion()).andStubReturn(CONNECT_PROTOCOL_V0);
        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.CONFIG_MISMATCH, 1, "leader", "leaderUrl", Collections.emptyList(), Collections.emptyList(), 0, true);
    ...
    public void testJoinLeaderCatchUpRetriesForIncrementalCooperative() throws Exception {
        ...
        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.NO_ERROR, 1, "leader", "leaderUrl", Arrays.asList(CONN1), Arrays.asList(TASK1), 0, true);
        ...
        for (int i = maxRetries; i >= maxRetries - retries; --i) {
            before = time.milliseconds();
            int workerUnsyncBackoffMs = DistributedConfig.SCHEDULED_REBALANCE_MAX_DELAY_MS_DEFAULT / 10 / i;
            herder.tick();
            assertEquals(before + coordinatorDiscoveryTimeoutMs + workerUnsyncBackoffMs, time.milliseconds());
    ...
    public void testKeyRotationWhenWorkerBecomesLeader() throws Exception {
        ...
        ClusterConfigState snapshotWithKey = new ClusterConfigState(2, initialKey, Collections.singletonMap(CONN1, 3), Collections.singletonMap(CONN1, CONN1_CONFIG), Collections.singletonMap(CONN1, TargetState.STARTED), TASK_CONFIGS_MAP, Collections.emptyMap(), Collections.emptyMap(), Collections.emptySet(), Collections.emptySet());
    ...
    public void testKeyRotationDisabledWhenWorkerBecomesFollower() throws Exception {
        ...
        ClusterConfigState snapshotWithKey = new ClusterConfigState(1, initialKey, Collections.singletonMap(CONN1, 3), Collections.singletonMap(CONN1, CONN1_CONFIG), Collections.singletonMap(CONN1, TargetState.STARTED), TASK_CONFIGS_MAP, Collections.emptyMap(), Collections.emptyMap(), Collections.emptySet(), Collections.emptySet());
        expectConfigRefreshAndSnapshot(snapshotWithKey);
    ...
    public void testFailedToReadBackNewlyWrittenSessionKey() throws Exception {
        ...
        ClusterConfigState snapshotWithSessionKey = new ClusterConfigState(1, sessionKey, Collections.singletonMap(CONN1, 3), Collections.singletonMap(CONN1, CONN1_CONFIG), Collections.singletonMap(CONN1, TargetState.STARTED), TASK_CONFIGS_MAP, Collections.emptyMap(), Collections.emptyMap(), Collections.emptySet(), Collections.emptySet());
    ...
    private void testTaskRequestedZombieFencingForwardingToLeader(boolean succeed) throws Exception {
        ...
        org.easymock.IExpectationSetters<RestClient.HttpResponse<Object>> expectRequest = EasyMock.expect(restClient.httpRequest(anyObject(), EasyMock.eq("PUT"), EasyMock.isNull(), EasyMock.isNull(), EasyMock.isNull(), anyObject(), anyObject()));
        if (succeed) {
        ...
        if (!succeed) {
            ExecutionException fencingException = assertThrows(ExecutionException.class, () -> fencing.get(10, TimeUnit.SECONDS));
    ...
    public void testExternalZombieFencingRequestForAlreadyFencedConnector() throws Exception {
        ClusterConfigState configState = exactlyOnceSnapshot(expectNewSessionKey(), TASK_CONFIGS_MAP, Collections.singletonMap(CONN1, 12), Collections.singletonMap(CONN1, 5), Collections.emptySet());
    ...
    public void testExternalZombieFencingRequestForSingleTaskConnector() throws Exception {
        ClusterConfigState configState = exactlyOnceSnapshot(expectNewSessionKey(), Collections.singletonMap(TASK1, TASK_CONFIG), Collections.singletonMap(CONN1, 1), Collections.singletonMap(CONN1, 5), Collections.singleton(CONN1));
    ...
    public void testExternalZombieFencingRequestForFreshConnector() throws Exception {
        ClusterConfigState configState = exactlyOnceSnapshot(expectNewSessionKey(), TASK_CONFIGS_MAP, Collections.emptyMap(), Collections.singletonMap(CONN1, 5), Collections.singleton(CONN1));
        testExternalZombieFencingRequestThatRequiresNoPhysicalFencing(configState, true);
    }

    private void testExternalZombieFencingRequestThatRequiresNoPhysicalFencing(ClusterConfigState configState, boolean expectTaskCountRecord) throws Exception {
        expectHerderStartup();
    ...
    public void testExternalZombieFencingRequestImmediateCompletion() throws Exception {
        ...
        ClusterConfigState configState = exactlyOnceSnapshot(sessionKey, TASK_CONFIGS_MAP, Collections.singletonMap(CONN1, 2), Collections.singletonMap(CONN1, 5), Collections.singleton(CONN1));
        expectConfigRefreshAndSnapshot(configState);
        ...
        EasyMock.expect(worker.fenceZombies(EasyMock.eq(CONN1), EasyMock.eq(2), EasyMock.eq(CONN1_CONFIG))).andReturn(workerFencingFuture);
    ...
    public void testExternalZombieFencingRequestSynchronousFailure() throws Exception {
        ...
        EasyMock.expect(worker.fenceZombies(EasyMock.eq(CONN1), EasyMock.eq(2), EasyMock.eq(CONN1_CONFIG))).andThrow(fencingException);
    ...
    public void testExternalZombieFencingRequestDelayedCompletion() throws Exception {
        ...
        ClusterConfigState configState = exactlyOnceSnapshot(sessionKey, TASK_CONFIGS_MAP, taskCountRecords, taskConfigGenerations, pendingFencing, tasksPerConnector);
        ...
        tasksPerConnector.forEach((connector, numStackedRequests) -> {
            ...
            EasyMock.expect(herderFencingFuture.whenComplete(EasyMock.capture(herderFencingCallback))).andReturn(null).times(numStackedRequests + 1);
            ...
            EasyMock.expect(worker.fenceZombies(EasyMock.eq(connector), EasyMock.eq(taskCountRecords.get(connector)), EasyMock.anyObject())).andReturn(workerFencingFuture);
        ...
        tasksPerConnector.forEach((connector, numStackedRequests) -> {
            List<FutureCallback<Void>> connectorFencingRequests = IntStream.range(0, numStackedRequests).mapToObj(i -> new FutureCallback<Void>()).collect(Collectors.toList());

            connectorFencingRequests.forEach(fencing -> herder.fenceZombieSourceTasks(connector, fencing));
    ...
    public void testVerifyTaskGeneration() {
        Map<String, Integer> taskConfigGenerations = new HashMap<>();
        herder.configState = new ClusterConfigState(1, null, Collections.singletonMap(CONN1, 3), Collections.singletonMap(CONN1, CONN1_CONFIG), Collections.singletonMap(CONN1, TargetState.STARTED), TASK_CONFIGS_MAP, Collections.emptyMap(), taskConfigGenerations, Collections.emptySet(), Collections.emptySet());
        ...
        herder.assignment = new ExtendedAssignment((short) 2, (short) 0, "leader", "leaderUrl", 0, Collections.emptySet(), Collections.singleton(TASK1), Collections.emptySet(), Collections.emptySet(), 0);
    ...
    public void testKeyExceptionDetection() {
        assertFalse(herder.isPossibleExpiredKeyException(time.milliseconds(), new RuntimeException()));
        assertFalse(herder.isPossibleExpiredKeyException(time.milliseconds(), new BadRequestException("")));
        assertFalse(herder.isPossibleExpiredKeyException(time.milliseconds() - TimeUnit.MINUTES.toMillis(2), new ConnectRestException(FORBIDDEN.getStatusCode(), "")));
        assertTrue(herder.isPossibleExpiredKeyException(time.milliseconds(), new ConnectRestException(FORBIDDEN.getStatusCode(), "")));
    }
    ...
    public void testThreadNames() {
        assertTrue(Whitebox.<ThreadPoolExecutor>getInternalState(herder, "herderExecutor").getThreadFactory().newThread(EMPTY_RUNNABLE).getName().startsWith(DistributedHerder.class.getSimpleName()));

        assertTrue(Whitebox.<ThreadPoolExecutor>getInternalState(herder, "forwardRequestExecutor").getThreadFactory().newThread(EMPTY_RUNNABLE).getName().startsWith("ForwardRequestExecutor"));

        assertTrue(Whitebox.<ThreadPoolExecutor>getInternalState(herder, "startAndStopExecutor").getThreadFactory().newThread(EMPTY_RUNNABLE).getName().startsWith("StartAndStopExecutor"));
    }
    ...
    public void testTaskReconfigurationRetriesWithConnectorTaskConfigsException() {
        ...
        EasyMock.expect(worker.connectorTaskConfigs(CONN1, sinkConnectorConfig)).andThrow(new ConnectException("Failed to generate task configs")).times(2);
    ...
    public void testTaskReconfigurationRetriesWithLeaderRequestForwardingException() {
        ...
        EasyMock.expect(restClient.httpRequest(EasyMock.anyString(), EasyMock.eq("POST"), EasyMock.anyObject(HttpHeaders.class), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(SecretKey.class), EasyMock.anyString())).andThrow(new ConnectException("Request to leader to reconfigure connector tasks failed")).times(2);

        EasyMock.expect(restClient.httpRequest(EasyMock.anyString(), EasyMock.eq("POST"), EasyMock.anyObject(HttpHeaders.class), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(SecretKey.class), EasyMock.anyString())).andReturn(null);
    ...
    public void testConnectorOffsets() throws Exception {
        ...
        EasyMock.expect(configBackingStore.snapshot()).andReturn(SNAPSHOT);
        ConnectorOffsets offsets = new ConnectorOffsets(Collections.singletonList(new ConnectorOffset(Collections.singletonMap("partitionKey", "partitionValue"), Collections.singletonMap("offsetKey", "offsetValue"))));
    ...
    public void testModifyOffsetsSinkConnector() throws Exception {
        ...
        Map<Map<String, ?>, Map<String, ?>> offsets = Collections.singletonMap(Collections.singletonMap("partitionKey", "partitionValue"), Collections.singletonMap("offsetKey", "offsetValue"));
        member.wakeup();
        PowerMock.expectLastCall();
        member.ensureActive();
        PowerMock.expectLastCall();
    ...
    public void testModifyOffsetsSourceConnectorExactlyOnceEnabledZombieFencingFailure() throws Exception {
        ...
        assertEquals("Failed to perform zombie fencing for source connector prior to modifying offsets", e.getCause().getMessage());
    ...
    private void expectRebalance(final long offset, final List<String> assignedConnectors, final List<ConnectorTaskId> assignedTasks, final boolean isLeader) {

        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.NO_ERROR, offset, "leader", "leaderUrl", assignedConnectors, assignedTasks, 0, isLeader);
    }

    private void expectRebalance(final long offset, final List<String> assignedConnectors, final List<ConnectorTaskId> assignedTasks, String leader, String leaderUrl, boolean isLeader) {
        expectRebalance(Collections.emptyList(), Collections.emptyList(), ConnectProtocol.Assignment.NO_ERROR, offset, leader, leaderUrl, assignedConnectors, assignedTasks, 0, isLeader);
    }

    // Handles common initial part of rebalance callback. Does not handle instantiation of connectors and tasks.
    private void expectRebalance(final Collection<String> revokedConnectors, final List<ConnectorTaskId> revokedTasks, final short error, final long offset, final List<String> assignedConnectors, final List<ConnectorTaskId> assignedTasks) {
        expectRebalance(revokedConnectors, revokedTasks, error, offset, assignedConnectors, assignedTasks, 0);
    }

    // Handles common initial part of rebalance callback. Does not handle instantiation of connectors and tasks.
    private void expectRebalance(final Collection<String> revokedConnectors, final List<ConnectorTaskId> revokedTasks, final short error, final long offset, final List<String> assignedConnectors, final List<ConnectorTaskId> assignedTasks, int delay) {
    ...
    // Handles common initial part of rebalance callback. Does not handle instantiation of connectors and tasks.
    private void expectRebalance(final Collection<String> revokedConnectors, final List<ConnectorTaskId> revokedTasks, final short error, final long offset, String leader, String leaderUrl, final List<String> assignedConnectors, final List<ConnectorTaskId> assignedTasks, int delay, boolean isLeader) {
        member.ensureActive();
        PowerMock.expectLastCall().andAnswer(() -> {
            ...
            if (connectProtocolVersion == CONNECT_PROTOCOL_V0) {
                assignment = new ExtendedAssignment(connectProtocolVersion, error, leader, leaderUrl, offset, assignedConnectors, assignedTasks, Collections.emptyList(), Collections.emptyList(), 0);
            } else {
                assignment = new ExtendedAssignment(connectProtocolVersion, error, leader, leaderUrl, offset, assignedConnectors, assignedTasks, new ArrayList<>(revokedConnectors), new ArrayList<>(revokedTasks), delay);
            }
    ...
    private ClusterConfigState exactlyOnceSnapshot(SessionKey sessionKey, Map<ConnectorTaskId, Map<String, String>> taskConfigs, Map<String, Integer> taskCountRecords, Map<String, Integer> taskConfigGenerations, Set<String> pendingFencing) {
        ...
        Map<String, Integer> taskCounts = connectors.stream().collect(Collectors.toMap(Function.identity(), c -> 1));

        return exactlyOnceSnapshot(sessionKey, taskConfigs, taskCountRecords, taskConfigGenerations, pendingFencing, taskCounts);
    }

    private ClusterConfigState exactlyOnceSnapshot(SessionKey sessionKey, Map<ConnectorTaskId, Map<String, String>> taskConfigs, Map<String, Integer> taskCountRecords, Map<String, Integer> taskConfigGenerations, Set<String> pendingFencing, Map<String, Integer> taskCounts) {
        ...
        Map<String, Map<String, String>> connectorConfigs = connectors.stream().collect(Collectors.toMap(Function.identity(), c -> CONN1_CONFIG));

        return new ClusterConfigState(1, sessionKey, taskCounts, connectorConfigs, Collections.singletonMap(CONN1, TargetState.STARTED), taskConfigs, taskCountRecords, taskConfigGenerations, pendingFencing, Collections.emptySet());
    }
