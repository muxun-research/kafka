...
public class MonitorableSourceConnector extends SampleSourceConnector {
    ...
    public static class MonitorableSourceTask extends SourceTask {
        ...
        public void start(Map<String, String> props) {
            ...
            Map<String, Object> offset = Optional.ofNullable(context.offsetStorageReader().offset(sourcePartition(taskId))).orElse(Collections.emptyMap());
        ...
        public List<SourceRecord> poll() {
            if (!stopped) {
                ...
                return LongStream.range(0, currentBatchSize).mapToObj(i -> {
                    seqno++;
                    SourceRecord record = new SourceRecord(sourcePartition(taskId), sourceOffset(seqno), topicName, null, Schema.STRING_SCHEMA, "key-" + taskId + "-" + seqno, Schema.STRING_SCHEMA, "value-" + taskId + "-" + seqno, null, new ConnectHeaders().addLong("header-" + seqno, seqno));
