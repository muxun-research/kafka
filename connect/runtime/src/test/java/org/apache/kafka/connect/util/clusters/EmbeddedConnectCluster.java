...
public class EmbeddedConnectCluster {
    ...
    private EmbeddedConnectCluster(String name, Map<String, String> workerProps, int numWorkers, int numBrokers, Properties brokerProps, boolean maskExitProcedures, Map<String, String> additionalKafkaClusterClientConfigs) {
    ...
    public String toString() {
        return String.format("EmbeddedConnectCluster(name= %s, numBrokers= %d, numInitialWorkers= %d, workerProps= %s)", connectClusterName, numBrokers, numInitialWorkers, workerProps);
    }
    ...
    public Set<WorkerHandle> activeWorkers() {
        ...
        return connectCluster.stream().filter(w -> {
            try {
                mapper.readerFor(ServerInfo.class).readValue(responseToString(requestGet(w.url().toString())));
                return true;
    ...
    public void deleteConnector(String connName) {
        ...
        if (response.getStatus() >= Response.Status.BAD_REQUEST.getStatusCode()) {
            throw new ConnectRestException(response.getStatus(), "Could not execute DELETE request. Error response: " + responseToString(response));
        }
    }
    ...
    public void restartConnector(String connName) {
        ...
        if (response.getStatus() >= Response.Status.BAD_REQUEST.getStatusCode()) {
            throw new ConnectRestException(response.getStatus(), "Could not execute POST request. Error response: " + responseToString(response));
        }
    }
    ...
    public ConnectorInfo connectorInfo(String connectorName) {
        ...
        try {
            ...
            log.error("Could not read connector info from response: {}", responseToString(response), e);
            throw new ConnectException("Could not not parse connector info", e);
        }
        throw new ConnectRestException(response.getStatus(), "Could not read connector info. Error response: " + responseToString(response));
    }
    ...
    public Map<String, Map<String, String>> taskConfigs(String connectorName) {
        ...
        try {
            ...
            log.error("Could not read task configs from response: {}", responseToString(response), e);
            throw new ConnectException("Could not not parse task configs", e);
        }
        throw new ConnectRestException(response.getStatus(), "Could not read task configs. Error response: " + responseToString(response));
    }
    ...
    public void resetConnectorTopics(String connectorName) {
        ...
        if (response.getStatus() >= Response.Status.BAD_REQUEST.getStatusCode()) {
            throw new ConnectRestException(response.getStatus(), "Resetting active topics for connector " + connectorName + " failed. " + "Error response: " + responseToString(response));
        }
    }
    ...
    public ConnectorOffsets connectorOffsets(String connectorName) {
        ...
        throw new ConnectRestException(response.getStatus(), "Could not fetch connector offsets. Error response: " + responseToString(response));
    }
    ...
    public String alterConnectorOffsets(String connectorName, ConnectorOffsets offsets) {
        ...
        if (response.getStatus() < Response.Status.BAD_REQUEST.getStatusCode()) {
            ...
            throw new ConnectRestException(response.getStatus(), "Could not alter connector offsets. Error response: " + responseToString(response));
        }
    }
    ...
    public String resetConnectorOffsets(String connectorName) {
        ...
        if (response.getStatus() < Response.Status.BAD_REQUEST.getStatusCode()) {
            ...
            throw new ConnectRestException(response.getStatus(), "Could not reset connector offsets. Error response: " + responseToString(response));
        }
    }
    ...
    public String adminEndpoint(String resource) {
        String url = connectCluster.stream().map(WorkerHandle::adminUrl).filter(Objects::nonNull).findFirst().orElseThrow(() -> new ConnectException("Admin endpoint is disabled.")).toString();
        return url + resource;
    }
    ...
    public String endpointForResource(String resource) {
        String url = connectCluster.stream().map(WorkerHandle::url).filter(Objects::nonNull).findFirst().orElseThrow(() -> new ConnectException("Connect workers have not been provisioned")).toString();
        return url + resource;
    }
    ...
    protected Response requestHttpMethod(String url, String body, Map<String, String> headers, String httpMethod) {
        ...
        try {
            ...
            log.info("{} response for URL={} is {}", httpMethod, url, res.getContentAsString().isEmpty() ? "empty" : res.getContentAsString());
            return Response.status(Response.Status.fromStatusCode(res.getStatus())).entity(res.getContentAsString()).build();
        } catch (Exception e) {
