...
public class AbstractHerderTest {
    ...
    private static final ClusterConfigState SNAPSHOT_NO_TASKS = new ClusterConfigState(1, null, Collections.singletonMap(CONN1, 3), Collections.singletonMap(CONN1, CONN1_CONFIG), Collections.singletonMap(CONN1, TargetState.STARTED), Collections.emptyMap(), Collections.emptyMap(), Collections.emptyMap(), Collections.emptySet(), Collections.emptySet());
    ...
    public void testConnectors() {
        AbstractHerder herder = mock(AbstractHerder.class, withSettings().useConstructor(worker, workerId, kafkaClusterId, statusStore, configStore, noneConnectorClientConfigOverridePolicy).defaultAnswer(CALLS_REAL_METHODS));
    ...
    public void testConnectorStatus() {
        ...
        when(herder.rawConfig(connectorName)).thenReturn(Collections.singletonMap(ConnectorConfig.CONNECTOR_CLASS_CONFIG, SampleSourceConnector.class.getName()));

        when(statusStore.get(connectorName)).thenReturn(new ConnectorStatus(connectorName, AbstractStatus.State.RUNNING, workerId, generation));

        when(statusStore.getAll(connectorName)).thenReturn(Collections.singletonList(new TaskStatus(taskId, AbstractStatus.State.UNASSIGNED, workerId, generation)));
    ...
    public void testConfigValidationMissingName() {
        ...
        assertEquals(Arrays.asList(ConnectorConfig.COMMON_GROUP, ConnectorConfig.TRANSFORMS_GROUP, ConnectorConfig.PREDICATES_GROUP, ConnectorConfig.ERROR_GROUP, SourceConnectorConfig.TOPIC_CREATION_GROUP, SourceConnectorConfig.EXACTLY_ONCE_SUPPORT_GROUP, SourceConnectorConfig.OFFSETS_TOPIC_GROUP), result.groups());
    ...
    public void testConfigValidationPrincipalOnlyOverride() {
        ...
        // Each transform also gets its own group
        List<String> expectedGroups = Arrays.asList(ConnectorConfig.COMMON_GROUP, ConnectorConfig.TRANSFORMS_GROUP, ConnectorConfig.PREDICATES_GROUP, ConnectorConfig.ERROR_GROUP, SourceConnectorConfig.TOPIC_CREATION_GROUP, SourceConnectorConfig.EXACTLY_ONCE_SUPPORT_GROUP, SourceConnectorConfig.OFFSETS_TOPIC_GROUP);
        assertEquals(expectedGroups, result.groups());
    ...
    private void assertErrorForKey(ConfigInfos configInfos, String testKey) {
        final List<String> errorsForKey = configInfos.values().stream().map(ConfigInfo::configValue).filter(configValue -> configValue.name().equals(testKey)).map(ConfigValueInfo::errors).flatMap(Collection::stream).collect(Collectors.toList());
    ...
    private <T> void testConnectorPluginConfig(String pluginName, Supplier<T> newPluginInstance, Function<T, ConfigDef> pluginConfig, Optional<ConfigDef> baseConfig) throws ClassNotFoundException {
        ...
        int expectedConfigSize = baseConfig.map(config -> config.names().size()).orElse(0) + expectedConfig.names().size();
    ...
    public void testConnectorOffsets() throws Exception {
        ...
        @SuppressWarnings("unchecked") ArgumentCaptor<Callback<ConnectorOffsets>> workerCallback = ArgumentCaptor.forClass(Callback.class);
        doAnswer(invocation -> {
    ...
    protected void addConfigKey(Map<String, ConfigDef.ConfigKey> keys, String name, String group) {
        keys.put(name, new ConfigDef.ConfigKey(name, ConfigDef.Type.STRING, null, null, ConfigDef.Importance.HIGH, "doc", group, 10, ConfigDef.Width.MEDIUM, "display name", Collections.emptyList(), null, false));
    }
    ...
    protected ConfigInfo findInfo(ConfigInfos infos, String name) {
        return infos.values().stream().filter(i -> i.configValue().name().equals(name)).findFirst().orElse(null);
    }
    ...
    private AbstractHerder createConfigValidationHerder(Class<? extends Connector> connectorClass, ConnectorClientConfigOverridePolicy connectorClientConfigOverridePolicy) {
    ...
    private AbstractHerder createConfigValidationHerder(Class<? extends Connector> connectorClass, ConnectorClientConfigOverridePolicy connectorClientConfigOverridePolicy, int countOfCallingNewConnector) {
        ...
        AbstractHerder herder = mock(AbstractHerder.class, withSettings().useConstructor(worker, workerId, kafkaClusterId, statusStore, configStore, connectorClientConfigOverridePolicy).defaultAnswer(CALLS_REAL_METHODS));
