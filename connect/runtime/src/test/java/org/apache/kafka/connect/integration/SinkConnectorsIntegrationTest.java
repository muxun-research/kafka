...
public class SinkConnectorsIntegrationTest {
    ...
    public void setup() throws Exception {
        ...
        connect = new EmbeddedConnectCluster.Builder().name("connect-cluster").numWorkers(NUM_WORKERS).workerProps(workerProps).brokerProps(brokerProps).build();
        connect.start();
    ...
    public void testEagerConsumerPartitionAssignment() throws Exception {
        ...
        connectorProps.put(CONNECTOR_CLIENT_CONSUMER_OVERRIDES_PREFIX + PARTITION_ASSIGNMENT_STRATEGY_CONFIG, RoundRobinAssignor.class.getName());
        ...
        connectorProps.put(CONNECTOR_CLIENT_CONSUMER_OVERRIDES_PREFIX + DEFAULT_API_TIMEOUT_MS_CONFIG, "5000");
        ...
        waitForCondition(() -> expectedRecordValues.equals(consumedRecordValues), TASK_CONSUME_TIMEOUT_MS, "Task did not receive records in time");
        ...
        waitForCondition(() -> expectedRecordValues.equals(consumedRecordValues) && expectedAssignment.equals(task.assignment()), TASK_CONSUME_TIMEOUT_MS, "Timed out while waiting for task to receive records and updated topic partition assignment");
        assertEquals(3, task.timesAssigned(tp1));
    ...
    public void testCooperativeConsumerPartitionAssignment() throws Exception {
        ...
        Map<String, String> connectorProps = baseSinkConnectorProps(String.join(",", topics));
        // Need an eager assignor here; round robin is as good as any
        connectorProps.put(CONNECTOR_CLIENT_CONSUMER_OVERRIDES_PREFIX + PARTITION_ASSIGNMENT_STRATEGY_CONFIG, CooperativeStickyAssignor.class.getName());
        // After deleting a topic, offset commits will fail for it; reduce the timeout here so that the test doesn't take forever to proceed past that point
        connectorProps.put(CONNECTOR_CLIENT_CONSUMER_OVERRIDES_PREFIX + DEFAULT_API_TIMEOUT_MS_CONFIG, "5000");
