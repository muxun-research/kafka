...
public class AbstractWorkerSourceTaskTest {
    ...
    public void testSendRecordsConvertsData() {
        ...
        List<SourceRecord> records = Collections.singletonList(new SourceRecord(PARTITION, OFFSET, "topic", null, KEY_SCHEMA, KEY, RECORD_SCHEMA, RECORD));
    ...
    public void testSendRecordsPropagatesTimestamp() {
        ...
        workerTask.toSend = Collections.singletonList(new SourceRecord(PARTITION, OFFSET, "topic", null, KEY_SCHEMA, KEY, RECORD_SCHEMA, RECORD, timestamp));
        workerTask.sendRecords();
    ...
    public void testHeaders() {
        Headers headers = new RecordHeaders().add("header_key", "header_value".getBytes());

        org.apache.kafka.connect.header.Headers connectHeaders = new ConnectHeaders().add("header_key", new SchemaAndValue(Schema.STRING_SCHEMA, "header_value"));
        ...
        workerTask.toSend = Collections.singletonList(new SourceRecord(PARTITION, OFFSET, TOPIC, null, KEY_SCHEMA, KEY, RECORD_SCHEMA, RECORD, null, connectHeaders));
        workerTask.sendRecords();
    ...
    public void testHeadersWithCustomConverter() throws Exception {
        ...
        org.apache.kafka.connect.header.Headers headersA = new ConnectHeaders().addString("encoding", encodingA);
        org.apache.kafka.connect.header.Headers headersB = new ConnectHeaders().addString("encoding", encodingB);

        workerTask.toSend = Arrays.asList(new SourceRecord(PARTITION, OFFSET, "topic", null, Schema.STRING_SCHEMA, "a", Schema.STRING_SCHEMA, stringA, null, headersA), new SourceRecord(PARTITION, OFFSET, "topic", null, Schema.STRING_SCHEMA, "b", Schema.STRING_SCHEMA, stringB, null, headersB));
        workerTask.sendRecords();
        ...
        assertEquals(ByteBuffer.wrap(stringA.getBytes(encodingA)), ByteBuffer.wrap(sentRecordA.value()));
        ...
        assertEquals(ByteBuffer.wrap(stringB.getBytes(encodingB)), ByteBuffer.wrap(sentRecordB.value()));
    ...
    public void testSendRecordsTopicDescribeRetries() {
        ...
        when(admin.describeTopics(TOPIC)).thenThrow(new RetriableException(new TimeoutException("timeout"))).thenReturn(Collections.emptyMap());
    ...
    public void testSendRecordsTopicDescribeRetriesMidway() {
        ...
        when(admin.describeTopics(anyString())).thenReturn(Collections.emptyMap()).thenThrow(new RetriableException(new TimeoutException("timeout"))).thenReturn(Collections.emptyMap());
        when(admin.createOrFindTopics(any(NewTopic.class))).thenAnswer((Answer<TopicAdmin.TopicCreationResponse>) invocation -> {
    ...
    public void testSendRecordsTopicCreateRetriesMidway() {
        ...
        when(admin.createOrFindTopics(any(NewTopic.class))).thenReturn(createdTopic(TOPIC)).thenThrow(new RetriableException(new TimeoutException("timeout"))).thenReturn(createdTopic(OTHER_TOPIC));
    ...
    public void testTopicDescribeFails() {
        ...
        expectPreliminaryCalls(TOPIC);
        when(admin.describeTopics(TOPIC)).thenThrow(new ConnectException(new TopicAuthorizationException("unauthorized")));
    ...
    public void testTopicCreateFails() {
        ...
        expectPreliminaryCalls(TOPIC);
        when(admin.describeTopics(TOPIC)).thenReturn(Collections.emptyMap());
        when(admin.createOrFindTopics(any(NewTopic.class))).thenThrow(new ConnectException(new TopicAuthorizationException("unauthorized")));
    ...
    private void createWorkerTask(Converter keyConverter, Converter valueConverter, HeaderConverter headerConverter) {
        workerTask = new AbstractWorkerSourceTask(taskId, sourceTask, statusListener, TargetState.STARTED, keyConverter, valueConverter, headerConverter, transformationChain, sourceTaskContext, producer, admin, TopicCreationGroup.configuredGroups(sourceConfig), offsetReader, offsetWriter, offsetStore, config, metrics, errorHandlingMetrics, plugins.delegatingLoader(), Time.SYSTEM, RetryWithToleranceOperatorTest.NOOP_OPERATOR, statusBackingStore, Runnable::run) {
            @Override
