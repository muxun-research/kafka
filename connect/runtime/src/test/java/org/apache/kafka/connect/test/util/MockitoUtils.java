...
public class MockitoUtils {
    ...
    public static long countInvocations(Object mock, String methodName, Class<?>... parameters) {
        return mockingDetails(mock).getInvocations().stream().map(InvocationOnMock::getMethod).filter(m -> methodName.equals(m.getName())).filter(m -> Arrays.equals(parameters, m.getParameterTypes())).count();
    }
