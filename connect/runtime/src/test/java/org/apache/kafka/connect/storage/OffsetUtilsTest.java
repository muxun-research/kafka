...
public class OffsetUtilsTest {
    ...
    public void testProcessPartitionKeyNotList() {
        try (LogCaptureAppender logCaptureAppender = LogCaptureAppender.createAndRegister(OffsetUtils.class)) {
            ...
            assertThat(logCaptureAppender.getMessages().get(0), containsString("Ignoring offset partition key with an unexpected format"));
        }
    }
    ...
    public void testProcessPartitionKeyListWithOneElement() {
        try (LogCaptureAppender logCaptureAppender = LogCaptureAppender.createAndRegister(OffsetUtils.class)) {
            ...
            assertEquals(0, connectorPartitions.size());
            assertEquals(1, logCaptureAppender.getMessages().size());
            assertThat(logCaptureAppender.getMessages().get(0), containsString("Ignoring offset partition key with an unexpected number of elements"));
        }
    }
    ...
    public void testProcessPartitionKeyListWithElementsOfWrongType() {
        try (LogCaptureAppender logCaptureAppender = LogCaptureAppender.createAndRegister(OffsetUtils.class)) {
            ...
            assertEquals(0, connectorPartitions.size());
            assertEquals(1, logCaptureAppender.getMessages().size());
            assertThat(logCaptureAppender.getMessages().get(0), containsString("Ignoring offset partition key with an unexpected format for the first element in the partition key list"));
            ...
            assertThat(logCaptureAppender.getMessages().get(1), containsString("Ignoring offset partition key with an unexpected format for the second element in the partition key list"));
        }
    }
