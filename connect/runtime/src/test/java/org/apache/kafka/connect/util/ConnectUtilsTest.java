...
public class ConnectUtilsTest {
    ...
    public void testNoOverrideWarning() {
        ...
        assertEquals(Optional.empty(), ConnectUtils.ensurePropertyAndGetWarning(props, "key", "value", "because i say so", true));
        assertEquals("value", props.get("key"));

        props.clear();
        assertEquals(Optional.empty(), ConnectUtils.ensurePropertyAndGetWarning(props, "key", "value", "because i say so", false));
        assertEquals("value", props.get("key"));
    ...
    public void testOverrideWarning() {
        ...
        String expectedWarning = "The value 'little brother' for the '\u1984' property will be ignored as it cannot be overridden " + "thanks to newly-introduced federal legislation. " + "The value 'big brother' will be used instead.";
        assertEquals(Optional.of(expectedWarning), ConnectUtils.ensurePropertyAndGetWarning(props, "\u1984", "big brother", "thanks to newly-introduced federal legislation", false));
        assertEquals(Collections.singletonMap("\u1984", "big brother"), props);

        props.clear();
        props.put("\u1984", "BIG BROTHER");
        expectedWarning = "The value 'BIG BROTHER' for the '\u1984' property will be ignored as it cannot be overridden " + "thanks to newly-introduced federal legislation. " + "The value 'big brother' will be used instead.";
        assertEquals(Optional.of(expectedWarning), ConnectUtils.ensurePropertyAndGetWarning(props, "\u1984", "big brother", "thanks to newly-introduced federal legislation", true));
        assertEquals(Collections.singletonMap("\u1984", "big brother"), props);
    }
