...
public class RetryWithToleranceOperatorTest {
    ...
    public static final RetryWithToleranceOperator NOOP_OPERATOR = new RetryWithToleranceOperator(ERRORS_RETRY_TIMEOUT_DEFAULT, ERRORS_RETRY_MAX_DELAY_DEFAULT, NONE, SYSTEM, new ErrorHandlingMetrics(new ConnectorTaskId("noop-connector", -1), new ConnectMetrics("noop-worker", new TestableWorkerConfig(PROPERTIES), new SystemTime(), "test-cluster")));
    public static final RetryWithToleranceOperator ALL_OPERATOR = new RetryWithToleranceOperator(ERRORS_RETRY_TIMEOUT_DEFAULT, ERRORS_RETRY_MAX_DELAY_DEFAULT, ALL, SYSTEM, new ErrorHandlingMetrics(new ConnectorTaskId("errors-all-tolerate-connector", -1), new ConnectMetrics("errors-all-tolerate-worker", new TestableWorkerConfig(PROPERTIES), new SystemTime(), "test-cluster")));
    ...
    public void testExecuteFailed() {
        ...
        retryWithToleranceOperator.executeFailed(Stage.TASK_PUT, SinkTask.class, consumerRecord, new Throwable());
    }

    @Test
    public void testExecuteFailedNoTolerance() {
        RetryWithToleranceOperator retryWithToleranceOperator = new RetryWithToleranceOperator(0, ERRORS_RETRY_MAX_DELAY_DEFAULT, NONE, SYSTEM, errorHandlingMetrics);

        assertThrows(ConnectException.class, () -> retryWithToleranceOperator.executeFailed(Stage.TASK_PUT, SinkTask.class, consumerRecord, new Throwable()));
    }
    ...
    public void testThreadSafety() throws Throwable {
        ...
        RetryWithToleranceOperator retryWithToleranceOperator = new RetryWithToleranceOperator(0, ERRORS_RETRY_MAX_DELAY_DEFAULT, ALL, SYSTEM, errorHandlingMetrics, new ProcessingContext() {
            ...
            public void attempt(int attempt) {
                if (!this.attempt.compareAndSet(attempt - 1, attempt)) {
                    failed.compareAndSet(null, new AssertionError("Concurrent call to attempt(): Attempts should increase monotonically " + "within the scope of a given currentContext()"));
                }
        ...
        List<? extends Future<?>> futures = IntStream.range(0, numThreads).boxed().map(id -> pool.submit(() -> {
            ...
            while (true) {
                ...
                try {
                    if (id < numThreads / 2) {
                        retryWithToleranceOperator.executeFailed(Stage.TASK_PUT, SinkTask.class, consumerRecord, new Throwable()).get();
                    } else {
                        retryWithToleranceOperator.execute(() -> null, Stage.TRANSFORMATION, SinkTask.class);
                    }
                } catch (Exception e) {
