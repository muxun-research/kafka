...
public final class ServerTopicConfigSynonyms {
     ...
     */
    public static final Map<String, String> TOPIC_CONFIG_SYNONYMS = Collections.unmodifiableMap(ALL_TOPIC_CONFIG_SYNONYMS.entrySet().stream().collect(Collectors.toMap(Entry::getKey, e -> e.getValue().get(0).name())));
    ...
    private static Entry<String, List<ConfigSynonym>> listWithLogPrefix(String topicConfigName, ConfigSynonym... synonyms) {
        List<ConfigSynonym> synonymsWithPrefix = Arrays.stream(synonyms).map(s -> new ConfigSynonym(LOG_PREFIX + s.name(), s.converter())).collect(Collectors.toList());
