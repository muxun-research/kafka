...
public class FutureUtilsTest {
    ...
    public void testWaitWithLogging() throws Throwable {
        ...
        assertEquals(123, FutureUtils.waitWithLogging(log, "[FutureUtilsTest] ", "the future to be completed", future, Deadline.fromDelay(Time.SYSTEM, 30, TimeUnit.SECONDS), Time.SYSTEM));
    ...
    public void testWaitWithLoggingTimeout(boolean immediateTimeout) throws Throwable {
        ...
        assertThrows(TimeoutException.class, () -> {
            FutureUtils.waitWithLogging(log, "[FutureUtilsTest] ", "the future to be completed", future, immediateTimeout ? Deadline.fromDelay(Time.SYSTEM, 0, TimeUnit.SECONDS) : Deadline.fromDelay(Time.SYSTEM, 1, TimeUnit.MILLISECONDS), Time.SYSTEM);
        });
        executorService.shutdownNow();
        executorService.awaitTermination(1, TimeUnit.MINUTES);
    }
    ...
    public void testWaitWithLoggingError() throws Throwable {
        ...
        assertEquals("Received a fatal error while waiting for the future to be completed", assertThrows(RuntimeException.class, () -> {
            FutureUtils.waitWithLogging(log, "[FutureUtilsTest] ", "the future to be completed", future, Deadline.fromDelay(Time.SYSTEM, 30, TimeUnit.SECONDS), Time.SYSTEM);
        }).getMessage());
    ...
    public void testChainFutureExceptionally() throws Throwable {
        ...
        Throwable cause = assertThrows(ExecutionException.class, () -> destinationFuture.get()).getCause();
