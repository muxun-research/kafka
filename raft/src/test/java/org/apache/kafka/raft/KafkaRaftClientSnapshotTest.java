...
final public class KafkaRaftClientSnapshotTest {
    ...
    public void testLatestSnapshotId() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).appendToLog(snapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(snapshotId.epoch(), Arrays.asList("d", "e", "f")).withEmptySnapshot(snapshotId).withElectedLeader(epoch, leaderId).build();
    ...
    public void testLatestSnapshotIdMissing() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).appendToLog(snapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(snapshotId.epoch(), Arrays.asList("d", "e", "f")).withElectedLeader(epoch, leaderId).build();
    ...
    public void testLeaderListenerNotified(boolean entireLog) throws Exception {
        ...
        RaftClientTestContext.Builder contextBuilder = new RaftClientTestContext.Builder(localId, voters).appendToLog(snapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(snapshotId.epoch(), Arrays.asList("d", "e", "f")).withEmptySnapshot(snapshotId);
    ...
    public void testFollowerListenerNotified(boolean entireLog) throws Exception {
        ...
        RaftClientTestContext.Builder contextBuilder = new RaftClientTestContext.Builder(localId, voters).appendToLog(snapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(snapshotId.epoch(), Arrays.asList("d", "e", "f")).withEmptySnapshot(snapshotId).withElectedLeader(epoch, leaderId);
        ...
        context.deliverResponse(fetchRequest.correlationId, fetchRequest.destinationId(), context.fetchResponse(epoch, leaderId, MemoryRecords.EMPTY, localLogEndOffset, Errors.NONE));
    ...
    public void testListenerRenotified() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).appendToLog(snapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(snapshotId.epoch(), Arrays.asList("d", "e", "f")).appendToLog(snapshotId.epoch(), Arrays.asList("g", "h", "i")).withEmptySnapshot(snapshotId).deleteBeforeSnapshot(snapshotId).build();
    ...
    public void testFetchRequestOffsetLessThanLogStart() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).withAppendLingerMs(1).build();
        ...
        long localLogEndOffset = context.log.endOffset().offset;
        assertTrue(appendRecords.size() <= localLogEndOffset, String.format("Record length = %s, log end offset = %s", appendRecords.size(), localLogEndOffset));
    ...
    public void testFetchRequestWithLargerLastFetchedEpoch() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).appendToLog(oldestSnapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(oldestSnapshotId.epoch(), Arrays.asList("d", "e", "f")).withAppendLingerMs(1).build();
    ...
    public void testFetchRequestTruncateToLogStart() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).appendToLog(oldestSnapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(oldestSnapshotId.epoch() + 2, Arrays.asList("d", "e", "f")).withAppendLingerMs(1).build();
        ...
        context.deliverRequest(context.fetchRequest(epoch, otherNodeId, oldestSnapshotId.offset() + 1, oldestSnapshotId.epoch() + 1, 0));
        context.pollUntilResponse();
        FetchResponseData.PartitionData partitionResponse = context.assertSentFetchPartitionResponse();
        assertEquals(Errors.NONE, Errors.forCode(partitionResponse.errorCode()));
        assertEquals(epoch, partitionResponse.currentLeader().leaderEpoch());
    ...
    public void testFetchRequestAtLogStartOffsetWithValidEpoch() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).appendToLog(oldestSnapshotId.epoch(), Arrays.asList("a", "b", "c")).appendToLog(oldestSnapshotId.epoch(), Arrays.asList("d", "e", "f")).appendToLog(oldestSnapshotId.epoch() + 2, Arrays.asList("g", "h", "i")).withAppendLingerMs(1).build();
    ...
    public void testFetchRequestAtLogStartOffsetWithInvalidEpoch() throws Exception {
        ...
        context.deliverRequest(context.fetchRequest(epoch, otherNodeId, oldestSnapshotId.offset(), oldestSnapshotId.epoch() + 1, 0));
        context.pollUntilResponse();
        FetchResponseData.PartitionData partitionResponse = context.assertSentFetchPartitionResponse();
        assertEquals(Errors.NONE, Errors.forCode(partitionResponse.errorCode()));
        assertEquals(epoch, partitionResponse.currentLeader().leaderEpoch());
    ...
    public void testFetchRequestWithLastFetchedEpochLessThanOldestSnapshot() throws Exception {
        ...
        context.deliverRequest(context.fetchRequest(epoch, otherNodeId, context.log.endOffset().offset, oldestSnapshotId.epoch() - 1, 0));
        context.pollUntilResponse();
        FetchResponseData.PartitionData partitionResponse = context.assertSentFetchPartitionResponse();
        assertEquals(Errors.NONE, Errors.forCode(partitionResponse.errorCode()));
        assertEquals(epoch, partitionResponse.currentLeader().leaderEpoch());
    ...
    public void testFetchSnapshotRequestWithOlderEpoch() throws Exception {
        ...
        context.deliverRequest(fetchSnapshotRequest(context.metadataPartition, epoch - 1, snapshotId, Integer.MAX_VALUE, 0));
    ...
    public void testFetchResponseWithInvalidSnapshotId() throws Exception {
        ...
        context.deliverResponse(fetchRequest.correlationId, fetchRequest.destinationId(), snapshotFetchResponse(context.metadataPartition, context.metadataTopicId, epoch, leaderId, invalidEpoch, 200L));
        ...
        context.deliverResponse(fetchRequest.correlationId, fetchRequest.destinationId(), snapshotFetchResponse(context.metadataPartition, context.metadataTopicId, epoch, leaderId, invalidEndOffset, 200L));
    ...
    public void testFetchSnapshotResponseMissingSnapshot() throws Exception {
        ...
        context.deliverResponse(snapshotRequest.correlationId, snapshotRequest.destinationId(), FetchSnapshotResponse.singleton(context.metadataPartition, responsePartitionSnapshot -> {
    ...
    public void testFetchSnapshotResponseFromNewerEpochNotLeader() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).withElectedLeader(epoch, firstLeaderId).build();

        context.pollUntilRequest();
        RaftRequest.Outbound fetchRequest = context.assertSentFetchRequest();
        context.assertFetchRequestData(fetchRequest, epoch, 0L, 0);

        context.deliverResponse(fetchRequest.correlationId, fetchRequest.destinationId(), snapshotFetchResponse(context.metadataPartition, context.metadataTopicId, epoch, firstLeaderId, snapshotId, 200L));
    ...
    public void testFetchSnapshotResponseWithInvalidId() throws Exception {
        ...
        context.deliverResponse(snapshotRequest.correlationId, snapshotRequest.destinationId(), FetchSnapshotResponse.singleton(context.metadataPartition, responsePartitionSnapshot -> {
            ...
            responsePartitionSnapshot.snapshotId().setEndOffset(-1).setEpoch(snapshotId.epoch());
    ...
    public void testFetchSnapshotRequestClusterIdValidation() throws Exception {
        ...
        context.deliverRequest(fetchSnapshotRequest(context.clusterId.toString(), context.metadataPartition, epoch, new OffsetAndEpoch(0, 0), Integer.MAX_VALUE, 0));
        context.pollUntilResponse();
        context.assertSentFetchSnapshotResponse(context.metadataPartition);

        // null cluster id is accepted
        context.deliverRequest(fetchSnapshotRequest(null, context.metadataPartition, epoch, new OffsetAndEpoch(0, 0), Integer.MAX_VALUE, 0));
        context.pollUntilResponse();
        context.assertSentFetchSnapshotResponse(context.metadataPartition);
        ...
        context.deliverRequest(fetchSnapshotRequest("invalid-uuid", context.metadataPartition, epoch, new OffsetAndEpoch(0, 0), Integer.MAX_VALUE, 0));
        context.pollUntilResponse();
        context.assertSentFetchSnapshotResponse(Errors.INCONSISTENT_CLUSTER_ID);
    }
    ...
    public void testCreateSnapshotAsLeaderWithInvalidSnapshotId() throws Exception {
        ...
        RaftClientTestContext context = new RaftClientTestContext.Builder(localId, voters).appendToLog(epoch, appendRecords).withAppendLingerMs(1).build();
    ...
    private static FetchSnapshotResponseData fetchSnapshotResponse(TopicPartition topicPartition, int leaderEpoch, int leaderId, OffsetAndEpoch snapshotId, long size, long position, ByteBuffer buffer) {
        return FetchSnapshotResponse.singleton(topicPartition, partitionSnapshot -> {
            ...
            return partitionSnapshot.setSize(size).setPosition(position).setUnalignedRecords(MemoryRecords.readableRecords(buffer.slice()));
        });
    }
    ...
    private static SnapshotWriter<String> snapshotWriter(RaftClientTestContext context, RawSnapshotWriter snapshot) {
        return RecordsSnapshotWriter.createWithHeader(snapshot, 4 * 1024, MemoryPool.NONE, context.time, 0, CompressionType.NONE, new StringSerde());
    }
