...
class BatchAccumulatorTest {
    ...
    private BatchAccumulator<String> buildAccumulator(int leaderEpoch, long baseOffset, int lingerMs, int maxBatchSize) {
        return new BatchAccumulator<>(leaderEpoch, baseOffset, lingerMs, maxBatchSize, memoryPool, time, CompressionType.NONE, serde);
    }
    ...
    public void testCompletedBatchReleaseBuffer() {
        ...
        Mockito.when(memoryPool.tryAllocate(maxBatchSize)).thenReturn(buffer);
    ...
    public void testRecordsAreSplit() {
        ...
        int batchHeaderSize = AbstractRecords.recordBatchHeaderSizeInBytes(RecordBatch.MAGIC_VALUE_V2, CompressionType.NONE);
        ...
        List<String> records = Stream.generate(() -> record).limit(numberOfRecords).collect(Collectors.toList());
    ...
    public void testDrainDoesNotBlockWithConcurrentAppend() throws Exception {
        ...
        BatchAccumulator<String> acc = new BatchAccumulator<>(leaderEpoch, baseOffset, lingerMs, maxBatchSize, memoryPool, time, CompressionType.NONE, serde);
        ...
        Mockito.doAnswer(invocation -> {
            ...
            return null;
        }).when(serde).write(Mockito.eq("b"), Mockito.any(ObjectSerializationCache.class), Mockito.any(Writable.class));
