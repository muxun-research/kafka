...
public class ReplicatedCounter implements RaftClient.Listener<Integer> {
    ...
    public synchronized void increment() {
        ...
        try {
            ...
            log.debug("Scheduled append of record {} with epoch {} at offset {}", uncommitted, epoch, offset);
    ...
    public synchronized void handleCommit(BatchReader<Integer> reader) {
        try {
            ...
            while (reader.hasNext()) {
                ...
                for (Integer nextCommitted : batch.records()) {
                    if (nextCommitted != committed + 1) {
                        throw new AssertionError(String.format("Expected next committed value to be %d, but instead found %d on node %d", committed + 1, nextCommitted, nodeId));
                    }
            ...
            if (lastOffsetSnapshotted + snapshotDelayInRecords < lastCommittedOffset) {
                log.debug("Generating new snapshot with committed offset {} and epoch {} since the previous snapshot includes {}", lastCommittedOffset, lastCommittedEpoch, lastOffsetSnapshotted);
                Optional<SnapshotWriter<Integer>> snapshot = client.createSnapshot(new OffsetAndEpoch(lastCommittedOffset + 1, lastCommittedEpoch), lastCommittedTimestamp);
        ...
        try {
            ...
            while (reader.hasNext()) {
                ...
                if (batch.records().size() != 1) {
                    throw new AssertionError(String.format("Expected the snapshot at %s to only contain one record %s", reader.snapshotId(), batch.records()));
                }
    ...
    public synchronized void handleLeaderChange(LeaderAndEpoch newLeader) {
        if (newLeader.isLeader(nodeId)) {
            log.debug("Counter uncommitted value initialized to {} after claiming leadership in epoch {}", committed, newLeader);
