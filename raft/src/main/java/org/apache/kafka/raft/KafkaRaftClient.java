...
import org.apache.kafka.raft.internals.*;
...
public class KafkaRaftClient<T> implements RaftClient<T> {
    ...
    public KafkaRaftClient(RecordSerde<T> serde, NetworkChannel channel, ReplicatedLog log, QuorumStateStore quorumStateStore, Time time, Metrics metrics, ExpirationService expirationService, LogContext logContext, String clusterId, OptionalInt nodeId, RaftConfig raftConfig) {
        this(serde, channel, new BlockingMessageQueue(), log, quorumStateStore, new BatchMemoryPool(5, MAX_BATCH_SIZE_BYTES), time, metrics, expirationService, MAX_FETCH_WAIT_MS, clusterId, nodeId, logContext, new Random(), raftConfig);
    }
    ...
    KafkaRaftClient(RecordSerde<T> serde, NetworkChannel channel, RaftMessageQueue messageQueue, ReplicatedLog log, QuorumStateStore quorumStateStore, MemoryPool memoryPool, Time time, Metrics metrics, ExpirationService expirationService, int fetchMaxWaitMs, String clusterId, OptionalInt nodeId, LogContext logContext, Random random, RaftConfig raftConfig) {
        ...
        voterAddresses.entrySet().stream().filter(e -> e.getValue() instanceof RaftConfig.InetAddressSpec).forEach(e -> this.channel.updateEndpoint(e.getKey(), (RaftConfig.InetAddressSpec) e.getValue()));
    }
    ...
    private void updateListenersProgress(long highWatermark) {
        for (ListenerContext listenerContext : listenerContexts.values()) {
            listenerContext.nextExpectedOffset().ifPresent(nextExpectedOffset -> {
                ...
                if (nextExpectedOffset < highWatermark && ((nextExpectedOffset == 0 && latestSnapshot().isPresent()) || nextExpectedOffset < log.startOffset())) {
                    SnapshotReader<T> snapshot = latestSnapshot().orElseThrow(() -> new IllegalStateException(String.format("Snapshot expected since next offset of %s is %d, log start offset is %d and high-watermark is %d", listenerContext.listenerName(), nextExpectedOffset, log.startOffset(), highWatermark)));
    ...
    private Optional<SnapshotReader<T>> latestSnapshot() {
        return log.latestSnapshot().map(reader -> RecordsSnapshotReader.of(reader, serde, BufferSupplier.create(), MAX_BATCH_SIZE_BYTES, true /* Validate batch CRC*/));
    }
    ...
    public void initialize() {
        ...
        if (quorum.isVoter() && quorum.remoteVoters().isEmpty() && !quorum.isCandidate()) {
    ...
    private void onBecomeLeader(long currentTimeMs) {
        ...
        BatchAccumulator<T> accumulator = new BatchAccumulator<>(quorum.epoch(), endOffset, raftConfig.appendLingerMs(), MAX_BATCH_SIZE_BYTES, memoryPool, time, CompressionType.NONE, serde);
    ...
    private void transitionToResigned(List<Integer> preferredSuccessors) {
        fetchPurgatory.completeAllExceptionally(Errors.NOT_LEADER_OR_FOLLOWER.exception("Not handling request since this node is resigning"));
    ...
    private void transitionToFollower(int epoch, int leaderId, long currentTimeMs) {
    ...
    private VoteResponseData buildVoteResponse(Errors partitionLevelError, boolean voteGranted) {
        return VoteResponse.singletonResponse(Errors.NONE, log.topicPartition(), partitionLevelError, quorum.epoch(), quorum.leaderIdOrSentinel(), voteGranted);
    }
    ...
    private int binaryExponentialElectionBackoffMs(int retries) {
        ...
        return Math.min(RETRY_BACKOFF_BASE_MS * random.nextInt(2 << Math.min(20, retries - 1)), raftConfig.electionBackoffMaxMs());
    }
    ...
    private BeginQuorumEpochResponseData buildBeginQuorumEpochResponse(Errors partitionLevelError) {
        return BeginQuorumEpochResponse.singletonResponse(Errors.NONE, log.topicPartition(), partitionLevelError, quorum.epoch(), quorum.leaderIdOrSentinel());
    }
    ...
    private EndQuorumEpochResponseData buildEndQuorumEpochResponse(Errors partitionLevelError) {
        return EndQuorumEpochResponse.singletonResponse(Errors.NONE, log.topicPartition(), partitionLevelError, quorum.epoch(), quorum.leaderIdOrSentinel());
    }
    ...
    private FetchResponseData buildFetchResponse(Errors error, Records records, ValidOffsetAndEpoch validOffsetAndEpoch, Optional<LogOffsetMetadata> highWatermark) {
        return RaftUtil.singletonFetchResponse(log.topicPartition(), log.topicId(), Errors.NONE, partitionData -> {
            partitionData.setRecords(records).setErrorCode(error.code()).setLogStartOffset(log.startOffset()).setHighWatermark(highWatermark.map(offsetMetadata -> offsetMetadata.offset).orElse(-1L));

            partitionData.currentLeader().setLeaderEpoch(quorum.epoch()).setLeaderId(quorum.leaderIdOrSentinel());
    ...
    private FetchResponseData buildEmptyFetchResponse(Errors error, Optional<LogOffsetMetadata> highWatermark) {
        return buildFetchResponse(error, MemoryRecords.EMPTY, ValidOffsetAndEpoch.valid(), highWatermark);
    }
        ...
        return future.handle((completionTimeMs, exception) -> {
            if (exception != null) {
                ...
                if (error != Errors.REQUEST_TIMED_OUT) {
                    logger.info("Failed to handle fetch from {} at {} due to {}", replicaId, fetchPartition.fetchOffset(), error);
                    return buildEmptyFetchResponse(error, Optional.empty());
                }
            }

            // FIXME: `completionTimeMs`, which can be null
            logger.trace("Completing delayed fetch from {} starting at offset {} at {}", replicaId, fetchPartition.fetchOffset(), completionTimeMs);
        ...
        if (error == Errors.NONE) {
            ...
            if (divergingEpoch.epoch() >= 0) {
                ...
                if (partitionResponse.snapshotId().epoch() < 0) {
                    ...
                    logger.info("Fetching snapshot {} from Fetch response from leader {}", snapshotId, quorum.leaderIdOrSentinel());
                }
            } else {
    ...
    private DescribeQuorumResponseData handleDescribeQuorumRequest(RaftRequest.Inbound requestMetadata, long currentTimeMs) {
        ...
        if (!quorum.isLeader()) {
            return DescribeQuorumResponse.singletonErrorResponse(log.topicPartition(), Errors.NOT_LEADER_OR_FOLLOWER);
        }

        LeaderState<T> leaderState = quorum.leaderStateOrThrow();
        return DescribeQuorumResponse.singletonResponse(log.topicPartition(), leaderState.describeQuorum(currentTimeMs));
    }
    ...
    private boolean handleFetchSnapshotResponse(RaftResponse.Inbound responseMetadata, long currentTimeMs) {
        ...
        if (Errors.forCode(partitionSnapshot.errorCode()) == Errors.SNAPSHOT_NOT_FOUND || partitionSnapshot.snapshotId().endOffset() < 0 || partitionSnapshot.snapshotId().epoch() < 0) {
             ...
             */
            logger.info("Leader doesn't know about snapshot id {}, returned error {} and snapshot id {}", state.fetchingSnapshot(), partitionSnapshot.errorCode(), partitionSnapshot.snapshotId());
        ...
        if (snapshot.sizeInBytes() == partitionSnapshot.size()) {
            ...
            if (log.truncateToLatestSnapshot()) {
                logger.info("Fully truncated the log at ({}, {}) after downloading snapshot {} from leader {}", log.endOffset(), log.lastFetchedEpoch(), snapshot.snapshotId(), quorum.leaderIdOrSentinel());

                updateFollowerHighWatermark(state, OptionalLong.of(log.highWatermark().offset));
            } else {
                throw new IllegalStateException(String.format("Full log truncation expected but didn't happen. Snapshot of %s, log end offset %s, last fetched %d", snapshot.snapshotId(), log.endOffset(), log.lastFetchedEpoch()));
            }
        }
    ...
    private boolean hasConsistentLeader(int epoch, OptionalInt leaderId) {
        ...
        if (leaderId.isPresent() && leaderId.getAsInt() == quorum.localIdOrSentinel()) {
            ...
            return epoch != quorum.epoch() || !leaderId.isPresent() || !quorum.leaderId().isPresent() || leaderId.equals(quorum.leaderId());
        }
    }
    ...
    private Optional<Boolean> maybeHandleCommonResponse(Errors error, OptionalInt leaderId, int epoch, long currentTimeMs) {
        if (epoch < quorum.epoch() || error == Errors.UNKNOWN_LEADER_EPOCH) {
        ...
        } else if (epoch > quorum.epoch() || error == Errors.FENCED_LEADER_EPOCH || error == Errors.NOT_LEADER_OR_FOLLOWER) {
            ...
            return Optional.of(true);
        } else if (epoch == quorum.epoch() && leaderId.isPresent() && !quorum.hasLeader()) {
    ...
    private void maybeTransition(OptionalInt leaderId, int epoch, long currentTimeMs) {
        if (!hasConsistentLeader(epoch, leaderId)) {
            throw new IllegalStateException("Received request or response with leader " + leaderId + " and epoch " + epoch + " which is inconsistent with current leader " + quorum.leaderId() + " and epoch " + quorum.epoch());
    ...
    private boolean handleUnexpectedError(Errors error, RaftResponse.Inbound response) {
        logger.error("Unexpected error {} in {} response: {}", error, ApiKeys.forId(response.data.apiKey()), response);
        return false;
    }
    ...
    private EndQuorumEpochRequestData buildEndQuorumEpochRequest(ResignedState state) {
        return EndQuorumEpochRequest.singletonRequest(log.topicPartition(), clusterId, quorum.epoch(), quorum.localIdOrThrow(), state.preferredSuccessors());
    }
    ...
    private BeginQuorumEpochRequestData buildBeginQuorumEpochRequest() {
        return BeginQuorumEpochRequest.singletonRequest(log.topicPartition(), clusterId, quorum.epoch(), quorum.localIdOrThrow());
    }

    private VoteRequestData buildVoteRequest() {
        OffsetAndEpoch endOffset = endOffset();
        return VoteRequest.singletonRequest(log.topicPartition(), clusterId, quorum.epoch(), quorum.localIdOrThrow(), endOffset.epoch(), endOffset.offset());
    }
    ...
    private FetchRequestData buildFetchRequest() {
        FetchRequestData request = RaftUtil.singletonFetchRequest(log.topicPartition(), log.topicId(), fetchPartition -> {
            fetchPartition.setCurrentLeaderEpoch(quorum.epoch()).setLastFetchedEpoch(log.lastFetchedEpoch()).setFetchOffset(log.endOffset().offset);
        });
        return request.setMaxBytes(MAX_FETCH_SIZE_BYTES).setMaxWaitMs(fetchMaxWaitMs).setClusterId(clusterId).setReplicaState(new FetchRequestData.ReplicaState().setReplicaId(quorum.localIdOrSentinel()));
    }
    ...
    private long maybeSendAnyVoterFetch(long currentTimeMs) {
        ...
        if (readyVoterIdOpt.isPresent()) {
            return maybeSendRequest(currentTimeMs, readyVoterIdOpt.getAsInt(), this::buildFetchRequest);
        } else {
    ...
    private FetchSnapshotResponseData.PartitionSnapshot addQuorumLeader(FetchSnapshotResponseData.PartitionSnapshot partitionSnapshot) {
        partitionSnapshot.currentLeader().setLeaderEpoch(quorum.epoch()).setLeaderId(quorum.leaderIdOrSentinel());
    ...
    private void appendBatch(LeaderState<T> state, BatchAccumulator.CompletedBatch<T> batch, long appendTimeMs) {
        try {
            ...
            CompletableFuture<Long> future = appendPurgatory.await(offsetAndEpoch.offset() + 1, Integer.MAX_VALUE);
    ...
    private long pollResigned(long currentTimeMs) {
        ...
        long endQuorumBackoffMs = maybeSendRequests(currentTimeMs, state.unackedVoters(), () -> buildEndQuorumEpochRequest(state));
    ...
    private long pollLeader(long currentTimeMs) {
        ...
        long timeUntilSend = maybeSendRequests(currentTimeMs, state.nonAcknowledgingVoters(), this::buildBeginQuorumEpochRequest);
    ...
    private long maybeSendVoteRequests(CandidateState state, long currentTimeMs) {
        ...
        if (!state.isVoteRejected()) {
            return maybeSendRequests(currentTimeMs, state.unrecordedVoters(), this::buildVoteRequest);
        }
        return Long.MAX_VALUE;
    }
    ...
    private long pollCandidate(long currentTimeMs) {
        ...
        if (shutdown != null) {
            ...
            logger.info("Election has timed out, backing off for {}ms before becoming a candidate again", backoffDurationMs);
    ...
    private boolean maybeCompleteShutdown(long currentTimeMs) {
        ...
        if (quorum.isObserver() || quorum.remoteVoters().isEmpty() || quorum.hasRemoteLeader()) {
    ...
    public void resign(int epoch) {
        ...
        if (epoch > currentEpoch) {
            ...
            logger.debug("Ignoring call to resign from epoch {} since it is smaller than the " + "current epoch {}", epoch, currentEpoch);
            return;
            ...
            if (!leaderStateOpt.isPresent()) {
                logger.debug("Ignoring call to resign from epoch {} since this node is " + "no longer the leader", epoch);
                return;
            }

            LeaderState<Object> leaderState = leaderStateOpt.get();
            if (leaderState.epoch() != epoch) {
                logger.debug("Ignoring call to resign from epoch {} since it is smaller than the " + "current epoch {}", epoch, leaderState.epoch());
            } else {
    ...
    @Override
    public Optional<SnapshotWriter<T>> createSnapshot(OffsetAndEpoch snapshotId, long lastContainedLogTime) {
        return RecordsSnapshotWriter.createWithHeader(() -> log.createNewSnapshot(snapshotId), MAX_BATCH_SIZE_BYTES, memoryPool, time, lastContainedLogTime, CompressionType.NONE, serde);
    }
    ...
    private class GracefulShutdown {
        ...
        public void failWithTimeout() {
            ...
            completeFuture.completeExceptionally(new TimeoutException("Timeout expired before graceful shutdown completed"));
        }
    ...
    private final class ListenerContext implements CloseListener<BatchReader<T>> {
        ...
        private void fireHandleCommit(long baseOffset, Records records) {
            fireHandleCommit(RecordsBatchReader.of(baseOffset, records, serde, BufferSupplier.create(), MAX_BATCH_SIZE_BYTES, this, true /* Validate batch CRC*/));
        }
        ...
        private void fireHandleCommit(BatchReader<T> reader) {
            ...
            logger.debug("Notifying listener {} of batch for baseOffset {} and lastOffset {}", listenerName(), reader.baseOffset(), reader.lastOffset());
