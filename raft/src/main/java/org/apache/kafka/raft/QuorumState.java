...
public class QuorumState {
    ...
    public void initialize(OffsetAndEpoch logEndOffsetAndEpoch) throws IllegalStateException {
        ...
        try {
            ...
            log.warn("Clearing local quorum state store after error loading state {}", store, e);
            store.clear();
            election = ElectionState.withUnknownLeader(0, voters);
        }
        ...
        if (!election.voters().isEmpty() && !voters.equals(election.voters())) {
            throw new IllegalStateException("Configured voter set: " + voters + " is different from the voter set read from the state file: " + election.voters() + ". Check if the quorum configuration is up to date, " + "or wipe out the local state file if necessary");
        } else if (election.hasVoted() && !isVoter()) {
            String localIdDescription = localId.isPresent() ? localId.getAsInt() + " is not a voter" : "is undefined";
            ...
            log.warn("Epoch from quorum-state file is {}, which is " + "smaller than last written epoch {} in the log", election.epoch, logEndOffsetAndEpoch.epoch());
            initialState = new UnattachedState(time, logEndOffsetAndEpoch.epoch(), voters, Optional.empty(), randomElectionTimeoutMs(), logContext);
            ...
            initialState = new ResignedState(time, localId.getAsInt(), election.epoch, voters, randomElectionTimeoutMs(), Collections.emptyList(), logContext);
        } else if (localId.isPresent() && election.isVotedCandidate(localId.getAsInt())) {
            initialState = new CandidateState(time, localId.getAsInt(), election.epoch, voters, Optional.empty(), 1, randomElectionTimeoutMs(), logContext);
        } else if (election.hasVoted()) {
            initialState = new VotedState(time, election.epoch, election.votedId(), voters, Optional.empty(), randomElectionTimeoutMs(), logContext);
        } else if (election.hasLeader()) {
            initialState = new FollowerState(time, election.epoch, election.leaderId(), voters, Optional.empty(), fetchTimeoutMs, logContext);
        } else {
            initialState = new UnattachedState(time, election.epoch, voters, Optional.empty(), randomElectionTimeoutMs(), logContext);
        }
    ...
    public void transitionToResigned(List<Integer> preferredSuccessors) {
        ...
        memoryTransitionTo(new ResignedState(time, localIdOrThrow(), epoch, voters, randomElectionTimeoutMs(), preferredSuccessors, logContext));
    }
    ...
    public void transitionToUnattached(int epoch) {
        ...
        if (epoch <= currentEpoch) {
            throw new IllegalStateException("Cannot transition to Unattached with epoch= " + epoch + " from current state " + state);
        }
        ...
        durableTransitionTo(new UnattachedState(time, epoch, voters, state.highWatermark(), electionTimeoutMs, logContext));
    }
    ...
    public void transitionToVoted(int epoch, int candidateId) {
        if (localId.isPresent() && candidateId == localId.getAsInt()) {
            ...
            throw new IllegalStateException("Cannot transition to Voted with votedId=" + candidateId + " and epoch=" + epoch + " since the local broker.id=" + localId + " is not a voter");
        } else if (!isVoter(candidateId)) {
            throw new IllegalStateException("Cannot transition to Voted with voterId=" + candidateId + " and epoch=" + epoch + " since it is not one of the voters " + voters);
        }
        ...
        if (epoch < currentEpoch) {
            ...
            throw new IllegalStateException("Cannot transition to Voted with votedId=" + candidateId + " and epoch=" + epoch + " from the current state " + state);
        }
        ...
        durableTransitionTo(new VotedState(time, epoch, candidateId, voters, state.highWatermark(), randomElectionTimeoutMs(), logContext));
    }
    ...
    public void transitionToFollower(int epoch, int leaderId) {
        if (localId.isPresent() && leaderId == localId.getAsInt()) {
            throw new IllegalStateException("Cannot transition to Follower with leaderId=" + leaderId + " and epoch=" + epoch + " since it matches the local broker.id=" + localId);
        } else if (!isVoter(leaderId)) {
            throw new IllegalStateException("Cannot transition to Follower with leaderId=" + leaderId + " and epoch=" + epoch + " since it is not one of the voters " + voters);
        }
        ...
        durableTransitionTo(new FollowerState(time, epoch, leaderId, voters, state.highWatermark(), fetchTimeoutMs, logContext));
    }

    public void transitionToCandidate() {
        if (isObserver()) {
            throw new IllegalStateException("Cannot transition to Candidate since the local broker.id=" + localId + " is not one of the voters " + voters);
        } else if (isLeader()) {
            throw new IllegalStateException("Cannot transition to Candidate since the local broker.id=" + localId + " since this node is already a Leader with state " + state);
        }
        ...
        durableTransitionTo(new CandidateState(time, localIdOrThrow(), newEpoch, voters, state.highWatermark(), retries, electionTimeoutMs, logContext));
    }

    public <T> LeaderState<T> transitionToLeader(long epochStartOffset, BatchAccumulator<T> accumulator) {
        if (isObserver()) {
            throw new IllegalStateException("Cannot transition to Leader since the local broker.id=" + localId + " is not one of the voters " + voters);
        } else if (!isCandidate()) {
        ...
        LeaderState<T> state = new LeaderState<>(localIdOrThrow(), epoch(), epochStartOffset, voters, candidateState.grantingVoters(), accumulator, logContext);
